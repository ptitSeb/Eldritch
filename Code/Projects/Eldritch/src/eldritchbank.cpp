...
EldritchBank::EldritchBank() { RegisterForEvents(); }

EldritchBank::~EldritchBank() {
  // I don't unregister for events here because world has already been
...
void EldritchBank::TryBankTransaction(const int Amount) {
  ...
  int ClampedAmount =
      CurrentPlayerMoney -
      Clamp(CurrentPlayerMoney - Amount, 0, Limit);  // Clamp to player limits
  ClampedAmount = Clamp(CurrentBankMoney + ClampedAmount, 0, Limit) -
                  CurrentBankMoney;  // Clamp to bank limits
...
uint EldritchBank::GetBankMoney() const {
  EldritchPersistence* const pPersistence =
      EldritchFramework::GetInstance()->GetGame()->GetPersistence();
