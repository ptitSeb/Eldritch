...
void EldritchSound3DListener::Initialize() {
  ...
  m_VerticalScalar =
      ConfigManager::GetFloat(sVerticalScalar, 1.0f, sAudioSystem);

  STATICHASH(OccludedFalloffRadius);
  m_OccludedFalloffRadius =
      ConfigManager::GetFloat(sOccludedFalloffRadius, 0.0f, sAudioSystem);
}

/*virtual*/ void EldritchSound3DListener::ModifyAttenuation(
  ...
  const bool Occluded =
      m_World->LineCheck(m_Location, SoundLocation, FromListenerInfo) &&
      m_World->LineCheck(SoundLocation, m_Location, FromSoundInfo);

  if (Occluded) {
    // Use the ratio between the distances to sound source and to occlusion as a
    ...
    const float DistanceRatio =
        (DistanceToOcclusionNear + DistanceToOcclusionFar) / DistanceToSound;

    // And attenuate occluded sounds more if they're more distant.
    const float OcclusionAttenuation =
        Attenuate(DistanceToSound, m_OccludedFalloffRadius);
