...
void EldritchGame::SetCurrentLevelName(const SimpleString& LevelName) {
  ...
  STATICHASH(Name);
  const SimpleString LevelFriendlyName =
      ConfigManager::GetString(sName, "", sm_CurrentLevelName);
...
/*virtual*/ void EldritchGame::HandleEvent(const WBEvent& Event) {
  ...
  if (EventName == sReturnToHub) {
    ...
    // HACK: Tick world once to pump the event queue. Fixes title screen bugs.
    // (Hack because this assumes we only ever use this when returning to title
    // screen.)
...
void EldritchGame::Initialize() {
  if (GetSaveLoad()->TryLoadMaster()) {
    // We're good! Don't flush the master file; if the game crashes, we still
    // want it around!
  } else {
    // There was no master file, or the master file didn't have a world state
    // (was saved
    // when the player was dead, etc.). Create a new world, but don't mess with
    // persistence.

    STATICHASH(EldritchWorld);
    STATICHASH(InitialLevel);
    SetCurrentLevelName(
        ConfigManager::GetString(sInitialLevel, "", sEldritchWorld));

    MAKEHASH(m_CurrentLevelName);

    STATICHASH(WorldDef);
    const HashedString InitialWorldDef = ConfigManager::GetHash(
        sWorldDef, HashedString::NullString, sm_CurrentLevelName);
...
void EldritchGame::ClearTravelPersistence() { m_TravelPersistence.Clear(); }

// Restart means we're going to flush all the dungeon world files and travel
// back to the old hub (e.g., "Return to Hub" from menu).
// FlushHub means we're also flushing the hub, and not traveling (e.g.,
// restarting after death).
// If neither of these is set, then we're traveling back and keeping dungeon
// world files (e.g., returning after finishing a world).
void EldritchGame::RequestReturnToHub(const bool Restart, const bool FlushHub) {
  ...
  STATICHASH(WorldDef);
  const HashedString InitialWorldDef = ConfigManager::GetHash(
      sWorldDef, HashedString::NullString, sInitialLevelName);
...
void EldritchGame::RequestGoToNextLevel() {
  ...
  const SimpleString NextLevelName =
      ConfigManager::GetString(sNextLevel, "", sm_CurrentLevelName);

  MAKEHASH(NextLevelName);

  STATICHASH(WorldDef);
  const HashedString NextWorldDef = ConfigManager::GetHash(
      sWorldDef, HashedString::NullString, sNextLevelName);
...
void EldritchGame::RequestGoToPrevLevel() {
  ...
  const SimpleString PrevLevelName =
      ConfigManager::GetString(sPrevLevel, "", sm_CurrentLevelName);

  MAKEHASH(PrevLevelName);

  STATICHASH(WorldDef);
  const HashedString PrevWorldDef = ConfigManager::GetHash(
      sWorldDef, HashedString::NullString, sPrevLevelName);

  RequestGoToLevel(PrevLevelName, PrevWorldDef, false);
}

void EldritchGame::RequestGoToLevel(const SimpleString& NextLevel,
...
SimpleString EldritchGame::DecorateWorldFileName(
    const SimpleString& LevelName) const {
  XTRACE_FUNCTION;
  const SimpleString SaveLoadPath =
      EldritchFramework::GetInstance()->GetSaveLoadPath();
  const SimpleString DecoratedFileName = SimpleString::PrintF(
      "%s%s.eldritchworldsave", SaveLoadPath.CStr(), LevelName.CStr());
...
void EldritchGame::GoToLevel() {
  ...
  if (IsRestarting) {
    if (FlushHubOnRestart) {
      ...
      // Don't save the current world, but wait to flush until we're done
      // traveling.
    }
  } else {
  ...
  WB_SET_AUTO(PostLevelTransition, Bool, RestoreSpawnPoint,
              m_RestoreSpawnPoint);
  ...
  // Make a checkpoint save, for crash protection
  // (Queued because we want to pump the event queue once to initialize the
  // world.)
  WB_MAKE_EVENT(Checkpoint, NULL);
  WB_LOG_EVENT(Checkpoint);
  WB_QUEUE_EVENT(pEventManager, Checkpoint, this);

  // Clear travel persistence now that we've successfully traveled and don't
  // need it.
  ClearTravelPersistence();
...
void EldritchGame::SetColorGradingTexture(const SimpleString& TextureFilename) {
  ...
  if (m_PostQuad) {
    ...
    TextureManager* const pTextureManager =
        pFramework->GetRenderer()->GetTextureManager();
    ASSERT(pTextureManager);

    m_PostQuad->SetTexture(1,
                           pTextureManager->GetTexture(TextureFilename.CStr()));
  }
}

void EldritchGame::SetFogParams(const float FogNear, const float FogFar,
...
const SimpleString EldritchGame::GetRIPDamage() {
  ...
  MAKEHASH(DamageDesc);
  const SimpleString RIPDamage =
      ConfigManager::GetLocalizedString(sDamageDesc, "");
...
const SimpleString EldritchGame::GetRIPLevel() {
  ...
  const SimpleString DefaultRIPName =
      ConfigManager::GetString(sDefaultRIPName, "", sEldritchWorld);

  MAKEHASH(m_CurrentLevelName);
  STATICHASH(RIPName);
  const SimpleString RIPLevelName = ConfigManager::GetString(
      sRIPName, DefaultRIPName.CStr(), sm_CurrentLevelName);

  MAKEHASH(RIPLevelName);
  const SimpleString RIPLevel =
      ConfigManager::GetLocalizedString(sRIPLevelName, "");
...
#elif BUILD_MAC
  const SimpleString Command =
      SimpleString::PrintF("open %s", WebSiteURL.CStr());
  system(Command.CStr());
#elif BUILD_LINUX
  const SimpleString Command =
      SimpleString::PrintF("xdg-open %s", WebSiteURL.CStr());
  system(Command.CStr());
#endif
}
...
void EldritchGame::LaunchRIPTweet() {
  ...
  const SimpleString RIPFormat =
      ConfigManager::GetLocalizedString(sRIPFormat, "");

  STATICHASH(RIPURL);
  const SimpleString RIPURL = ConfigManager::GetLocalizedString(sRIPURL, "");

  STATICHASH(RIPHashtags);
  const SimpleString RIPHashtags =
      ConfigManager::GetLocalizedString(sRIPHashtags, "");

  STATICHASH(RIPTwitter);
  const SimpleString RIPTwitter =
      ConfigManager::GetLocalizedString(sRIPTwitter, "");

  const SimpleString RIPDamage = GetRIPDamage();
  const SimpleString RIPLevel = GetRIPLevel();

  const SimpleString RIPFormatted =
      SimpleString::PrintF(RIPFormat.CStr(), RIPDamage.CStr(), RIPLevel.CStr());

  const SimpleString RIPEncoded = RIPFormatted.URLEncodeUTF8();
  const SimpleString RIPTweet = SimpleString::PrintF(
      RIPTwitter.CStr(), RIPEncoded.CStr(), RIPHashtags.CStr(), RIPURL.CStr());
...
#elif BUILD_LINUX
  const SimpleString Command =
      SimpleString::PrintF("xdg-open %s", RIPTweet.CStr());
  system(Command.CStr());
#endif
}
...
/*static*/ Vector EldritchGame::GetPlayerLocation() {
  ...
  WBCompEldTransform* const pTransform =
      pPlayer->GetTransformComponent<WBCompEldTransform>();
...
/*static*/ UIScreenEldMirror* EldritchGame::GetMirrorScreen() {
  ...
  UIScreenEldMirror* const pMirrorScreen =
      pUIManager->GetScreen<UIScreenEldMirror>(sMirrorScreen);
  ASSERT(pMirrorScreen);
...
/*static*/ UIWidgetImage* EldritchGame::GetMinimapImage() {
  ...
  UIWidgetImage* const pMinimap =
      pHUDScreen->GetWidget<UIWidgetImage>(sMinimap);
  return pMinimap;
}