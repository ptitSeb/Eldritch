...
void UIScreenEldBindInputs::InitializeFromDefinition(
  ...
  for (m_ExposedInputIndex = 0; m_ExposedInputIndex < NumExposedInputs;
       ++m_ExposedInputIndex) {
...
void UIScreenEldBindInputs::CreateRulesDefinition() {
  ...
  m_ControllerArchetypeName =
      ConfigManager::GetString(sControllerArchetype, "", sUsingRules);
...
void UIScreenEldBindInputs::CreateLabelWidgetDefinition() {
  m_LabelWidgetDefinitionName =
      SimpleString::PrintF("_BindLabel%d", m_ExposedInputIndex);
  const SimpleString LabelString =
      SimpleString::PrintF("Bind%s", m_ExposedInput.CStr());
  ...
  STATICHASH(Archetype);
  ConfigManager::SetString(sArchetype, m_ArchetypeName.CStr(),
                           sm_LabelWidgetDefinitionName);

  STATICHASH(Parent);
  ConfigManager::SetString(sParent, m_Parent.CStr(),
                           sm_LabelWidgetDefinitionName);

  STATICHASH(String);
  ConfigManager::SetString(sString, LabelString.CStr(),
                           sm_LabelWidgetDefinitionName);
...
void UIScreenEldBindInputs::CreateKeyboardWidgetDefinition() {
  m_KeyboardWidgetDefinitionName =
      SimpleString::PrintF("_BindKeyboard%d", m_ExposedInputIndex);
  const SimpleString InputString =
      SimpleString::PrintF("#{s:EldritchKeyboard:%s}", m_ExposedInput.CStr());

  MAKEHASH(m_KeyboardWidgetDefinitionName);

  STATICHASH(UIWidgetType);
  ConfigManager::SetString(sUIWidgetType, "Text",
                           sm_KeyboardWidgetDefinitionName);
  ...
  ConfigManager::SetString(sDynamicString, InputString.CStr(),
                           sm_KeyboardWidgetDefinitionName);
...
void UIScreenEldBindInputs::CreateMouseWidgetDefinition() {
  m_MouseWidgetDefinitionName =
      SimpleString::PrintF("_BindMouse%d", m_ExposedInputIndex);
  const SimpleString InputString =
      SimpleString::PrintF("#{l:EldritchMouse:%s}", m_ExposedInput.CStr());
...
void UIScreenEldBindInputs::CreateControllerWidgetDefinition() {
  m_ControllerWidgetDefinitionName =
      SimpleString::PrintF("_BindController%d", m_ExposedInputIndex);
  const SimpleString InputString = SimpleString::PrintF(
      "#{l:EldritchController:%s}",
      m_ExposedInput.CStr());  // Using localization to map name to glyph

  MAKEHASH(m_ControllerWidgetDefinitionName);

  STATICHASH(UIWidgetType);
  ConfigManager::SetString(sUIWidgetType, "Text",
                           sm_ControllerWidgetDefinitionName);

  STATICHASH(Archetype);
  ConfigManager::SetString(sArchetype, m_ControllerArchetypeName.CStr(),
                           sm_ControllerWidgetDefinitionName);
  ...
  STATICHASH(PixelX);
  ConfigManager::SetFloat(sPixelX, m_Column3X,
                          sm_ControllerWidgetDefinitionName);
...
void UIScreenEldBindInputs::CreateBindingActionDefinition() {
  m_BindActionDefinitionName =
      SimpleString::PrintF("_BindAction%d", m_ExposedInputIndex);

  MAKEHASH(m_BindActionDefinitionName);

  STATICHASH(ActionType);
  ConfigManager::SetString(sActionType, "EldBindInput",
                           sm_BindActionDefinitionName);

  STATICHASH(Input);
  ConfigManager::SetString(sInput, m_ExposedInput.CStr(),
                           sm_BindActionDefinitionName);
}

void UIScreenEldBindInputs::CreateCompositeWidgetDefinition() {
  m_CompositeWidgetDefinitionName =
      SimpleString::PrintF("_BindComposite%d", m_ExposedInputIndex);

  MAKEHASH(m_CompositeWidgetDefinitionName);

  STATICHASH(UIWidgetType);
  ConfigManager::SetString(sUIWidgetType, "Composite",
                           sm_CompositeWidgetDefinitionName);
  ...
  STATICHASH(Child0);
  ConfigManager::SetString(sChild0, m_LabelWidgetDefinitionName.CStr(),
                           sm_CompositeWidgetDefinitionName);

  STATICHASH(Child1);
  ConfigManager::SetString(sChild1, m_KeyboardWidgetDefinitionName.CStr(),
                           sm_CompositeWidgetDefinitionName);

  STATICHASH(Child2);
  ConfigManager::SetString(sChild2, m_MouseWidgetDefinitionName.CStr(),
                           sm_CompositeWidgetDefinitionName);

  STATICHASH(Child3);
  ConfigManager::SetString(sChild3, m_ControllerWidgetDefinitionName.CStr(),
                           sm_CompositeWidgetDefinitionName);
  ...
  STATICHASH(Action0);
  ConfigManager::SetString(sAction0, m_BindActionDefinitionName.CStr(),
                           sm_CompositeWidgetDefinitionName);
}

void UIScreenEldBindInputs::CreateCompositeWidget() {
  UIWidget* const pCompositeWidget =
      UIFactory::CreateWidget(m_CompositeWidgetDefinitionName, this);
