...
/*virtual*/ void UIScreenEldMirror::InitializeFromDefinition(
  ...
  const SimpleString MirrorRigMesh =
      ConfigManager::GetString(sMirrorRigMesh, "", sDefinitionName);
  SetRigMesh(MirrorRigMesh);

  STATICHASH(MirrorAnimation);
  m_MirrorAnimation = ConfigManager::GetHash(
      sMirrorAnimation, HashedString::NullString, sDefinitionName);
  ...
  m_MirrorYaw = DEGREES_TO_RADIANS(
      ConfigManager::GetFloat(sMirrorYaw, 0.0f, sDefinitionName));

  STATICHASH(MirrorViewFOV);
  m_MirrorViewFOV =
      ConfigManager::GetFloat(sMirrorViewFOV, 0.0f, sDefinitionName);

  STATICHASH(MirrorViewDistance);
  m_MirrorViewDistance =
      ConfigManager::GetFloat(sMirrorViewDistance, 0.0f, sDefinitionName);

  STATICHASH(MirrorViewHeight);
  m_MirrorViewHeight =
      ConfigManager::GetFloat(sMirrorViewHeight, 0.0f, sDefinitionName);

  STATICHASH(MirrorViewNearClip);
  m_MirrorViewNearClip =
      ConfigManager::GetFloat(sMirrorViewNearClip, 0.0f, sDefinitionName);

  STATICHASH(MirrorViewFarClip);
  m_MirrorViewFarClip =
      ConfigManager::GetFloat(sMirrorViewFarClip, 0.0f, sDefinitionName);

  STATICHASH(MirrorBackdropTile);
  m_MirrorBackdropTile =
      ConfigManager::GetInt(sMirrorBackdropTile, 0, sDefinitionName);

  STATICHASH(MirrorBackdropR);
  m_MirrorBackdropColor.r =
      ConfigManager::GetFloat(sMirrorBackdropR, 0.0f, sDefinitionName);

  STATICHASH(MirrorBackdropG);
  m_MirrorBackdropColor.g =
      ConfigManager::GetFloat(sMirrorBackdropG, 0.0f, sDefinitionName);

  STATICHASH(MirrorBackdropB);
  m_MirrorBackdropColor.b =
      ConfigManager::GetFloat(sMirrorBackdropB, 0.0f, sDefinitionName);

  m_MirrorBackdropColor.a = 1.0f;

  STATICHASH(MirrorBackdropDist);
  m_MirrorBackdropDistance =
      ConfigManager::GetFloat(sMirrorBackdropDist, 0.0f, sDefinitionName);

  STATICHASH(MirrorBackdropSize);
  m_MirrorBackdropExtents =
      0.5f *
      ConfigManager::GetFloat(sMirrorBackdropSize, 0.0f, sDefinitionName);

  STATICHASH(MirrorLightR);
  const float MirrorLightR =
      ConfigManager::GetFloat(sMirrorLightR, 0.0f, sDefinitionName);

  STATICHASH(MirrorLightG);
  const float MirrorLightG =
      ConfigManager::GetFloat(sMirrorLightG, 0.0f, sDefinitionName);

  STATICHASH(MirrorLightB);
  const float MirrorLightB =
      ConfigManager::GetFloat(sMirrorLightB, 0.0f, sDefinitionName);

  STATICHASH(MirrorLightX);
  const float MirrorLightX =
      ConfigManager::GetFloat(sMirrorLightX, 0.0f, sDefinitionName);

  STATICHASH(MirrorLightY);
  const float MirrorLightY =
      ConfigManager::GetFloat(sMirrorLightY, 0.0f, sDefinitionName);

  STATICHASH(MirrorLightZ);
  const float MirrorLightZ =
      ConfigManager::GetFloat(sMirrorLightZ, 0.0f, sDefinitionName);

  const Vector4 MirrorLightColor =
      Vector4(MirrorLightR, MirrorLightG, MirrorLightB, 1.0f);
  const Vector MirrorLightDir =
      Vector(MirrorLightX, MirrorLightY, MirrorLightZ).GetNormalized();

  m_MirrorIrradiance.m_Light[IRRDIR_Right] =
      MirrorLightColor *
      Saturate(MirrorLightDir.Dot(Vector(-1.0f, 0.0f, 0.0f)));
  m_MirrorIrradiance.m_Light[IRRDIR_Left] =
      MirrorLightColor * Saturate(MirrorLightDir.Dot(Vector(1.0f, 0.0f, 0.0f)));
  m_MirrorIrradiance.m_Light[IRRDIR_Front] =
      MirrorLightColor *
      Saturate(MirrorLightDir.Dot(Vector(0.0f, -1.0f, 0.0f)));
  m_MirrorIrradiance.m_Light[IRRDIR_Back] =
      MirrorLightColor * Saturate(MirrorLightDir.Dot(Vector(0.0f, 1.0f, 0.0f)));
  m_MirrorIrradiance.m_Light[IRRDIR_Up] =
      MirrorLightColor *
      Saturate(MirrorLightDir.Dot(Vector(0.0f, 0.0f, -1.0f)));
  m_MirrorIrradiance.m_Light[IRRDIR_Down] =
      MirrorLightColor * Saturate(MirrorLightDir.Dot(Vector(0.0f, 0.0f, 1.0f)));
}
...
void UIScreenEldMirror::SetHeadMesh(const SimpleString& MeshName,
                                    const SimpleString& TextureName) {
  SafeDelete(m_HeadMesh);
...
void UIScreenEldMirror::SetBodyMesh(const SimpleString& MeshName,
                                    const SimpleString& TextureName) {
  SafeDelete(m_BodyMesh);
...
Mesh* UIScreenEldMirror::CreateMesh(const SimpleString& MeshName,
                                    const SimpleString& TextureName) {
  EldritchFramework* const pFramework = EldritchFramework::GetInstance();
  IRenderer* const pRenderer = pFramework->GetRenderer();
  ...
  // Orient everything in the mirror off axis to get better beauty lighting from
  // cube lights
...
void UIScreenEldMirror::CreateBackdropMesh() {
  ...
  Positions.PushBack(MirrorRotation *
                     Vector(m_MirrorBackdropExtents, -m_MirrorBackdropDistance,
                            -m_MirrorBackdropExtents + m_MirrorViewHeight));
  Positions.PushBack(MirrorRotation *
                     Vector(-m_MirrorBackdropExtents, -m_MirrorBackdropDistance,
                            -m_MirrorBackdropExtents + m_MirrorViewHeight));
  Positions.PushBack(MirrorRotation *
  ...
  IVertexDeclaration* const pVertexDeclaration =
      pRenderer->GetVertexDeclaration(VD_POSITIONS | VD_FLOATCOLORS_SM2 |
                                      VD_UVS);
...
/*virtual*/ UIScreen::ETickReturn UIScreenEldMirror::Tick(float DeltaTime,
                                                          bool HasFocus) {
  XTRACE_FUNCTION;
