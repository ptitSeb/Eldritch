...
/*virtual*/ UIScreen::ETickReturn UIScreenEldCredits::Tick(float DeltaTime,
                                                           bool HasFocus) {
  XTRACE_FUNCTION;
  ...
  const float Bottom = pCreditsText->m_TopLeft.y +
                       pCreditsText->m_Mesh->m_AABB.m_Max.z -
                       pCreditsText->m_Mesh->m_AABB.m_Min.z;
  if (Bottom < 0.0f) {
    STATICHASH(DisplayHeight);
    const float DisplayHeight = ConfigManager::GetFloat(sDisplayHeight);
    const float ParentHeight = pCreditsText->m_OriginParent
                                   ? pCreditsText->m_OriginParent->GetHeight()
                                   : DisplayHeight;
