...
class EldritchTools {
 public:
  ...
  void Reinitialize(const uint RoomScalarX = 1, const uint RoomScalarY = 1,
 ...
 private:
  ...
  struct SPlacedSpawner {
    SPlacedSpawner() : m_SpawnerDef(0), m_Orientation(ESO_North) {}
  ...
  void CreatePaletteMesh(const vval_t VoxelValue,
                         const EldritchWorld::SVoxelDef& VoxelDef);
  ...
  // Helper function, trace world from camera view (which is all I expect I'll
  // ever really need for tools)
  ...
  void GetCoordsForVoxel(const vidx_t VoxelIndex, vpos_t& X, vpos_t& Y,
                         vpos_t& Z);
  ...
  Array<SSpawnerButton>
      m_SpawnerButtons;                  // Should be parallel with spawner defs
  Map<uint, uint> m_SpawnerCategoryMap;  // Maps from a spawner def index to its
                                         // containing category
