...
static const int kMaxExits =
    1 + (EldritchWorldGen::ERE_North | EldritchWorldGen::ERE_South |
         EldritchWorldGen::ERE_East | EldritchWorldGen::ERE_West |
         EldritchWorldGen::ERE_Up | EldritchWorldGen::ERE_Down);
static const int kMaxTransforms =
    1 + (EldritchWorldGen::ERT_MirrorX | EldritchWorldGen::ERT_MirrorY |
...
void EldritchWorldGen::Initialize(const SimpleString& WorldGenDefinitionName) {
  ...
  const uint NumFeatureRoomDefs = ConfigManager::GetInheritedInt(
      sNumFeatureRoomDefs, 0, sWorldGenDefinitionName);
  for (uint FeatureRoomDefIndex = 0; FeatureRoomDefIndex < NumFeatureRoomDefs;
       ++FeatureRoomDefIndex) {
    const SimpleString FeatureRoomDefName =
        ConfigManager::GetInheritedSequenceString("FeatureRoomDef%d",
                                                  FeatureRoomDefIndex, "",
                                                  sWorldGenDefinitionName);
  ...
  const uint NumThemes =
      ConfigManager::GetInheritedInt(sNumThemes, 0, sWorldGenDefinitionName);
  for (uint ThemeIndex = 0; ThemeIndex < NumThemes; ++ThemeIndex) {
    const HashedString Theme = ConfigManager::GetInheritedSequenceHash(
        "Theme%d", ThemeIndex, HashedString::NullString,
        sWorldGenDefinitionName);
    m_Themes.PushBack(Theme);
  }

  STATICHASH(ThemeBias);
  m_ThemeBias = ConfigManager::GetInheritedFloat(sThemeBias, 0.0f,
                                                 sWorldGenDefinitionName);

  STATICHASH(HorizontalRatio);
  m_Expansion_HorizontalRatio = ConfigManager::GetInheritedFloat(
      sHorizontalRatio, 0.0f, sWorldGenDefinitionName);

  STATICHASH(FollowLastNodeRatio);
  m_Expansion_FollowLastNodeRatio = ConfigManager::GetInheritedFloat(
      sFollowLastNodeRatio, 0.0f, sWorldGenDefinitionName);

  STATICHASH(OpenLoopsRatio);
  m_Expansion_OpenLoopsRatio = ConfigManager::GetInheritedFloat(
      sOpenLoopsRatio, 0.0f, sWorldGenDefinitionName);

  // Any resolve groups not listed default to spawning 100% of entities.
  STATICHASH(NumResolveGroups);
  const uint NumResolveGroups = ConfigManager::GetInheritedInt(
      sNumResolveGroups, 0, sWorldGenDefinitionName);
  for (uint ResolveGroupIndex = 0; ResolveGroupIndex < NumResolveGroups;
       ++ResolveGroupIndex) {
    const HashedString GroupName = ConfigManager::GetInheritedSequenceHash(
        "ResolveGroup%dName", ResolveGroupIndex, HashedString::NullString,
        sWorldGenDefinitionName);

    SSpawnResolveGroup& ResolveGroup = m_SpawnResolveGroups[GroupName];

    ResolveGroup.m_ResolveLimit = ConfigManager::GetInheritedSequenceInt(
        "ResolveGroup%dLimit", ResolveGroupIndex, 0, sWorldGenDefinitionName);
    ResolveGroup.m_ResolveChance = ConfigManager::GetInheritedSequenceFloat(
        "ResolveGroup%dChance", ResolveGroupIndex, 1.0f,
        sWorldGenDefinitionName);

    const float MinRadiusFromPlayer = ConfigManager::GetInheritedSequenceFloat(
        "ResolveGroup%dMinRadiusFromPlayer", ResolveGroupIndex, 0.0f,
        sWorldGenDefinitionName);
    const float MinRadiusFromPlayerZ = ConfigManager::GetInheritedSequenceFloat(
        "ResolveGroup%dMinRadiusFromPlayerZ", ResolveGroupIndex, 0.0f,
        sWorldGenDefinitionName);
    ResolveGroup.m_MinRadiusSqFromPlayer = Square(MinRadiusFromPlayer);
    ResolveGroup.m_ScaleFromPlayerZ =
        (MinRadiusFromPlayerZ > 0.0f)
            ? (MinRadiusFromPlayer / MinRadiusFromPlayerZ)
            : 0.0f;
  }
}

void EldritchWorldGen::InitializeFeatureRoomDef(
    const SimpleString& FeatureRoomDefName) {
  ...
  STATICHASH(Required);
  FeatureRoomDef.m_Required =
      ConfigManager::GetBool(sRequired, false, sFeatureRoomDefName);

  STATICHASH(Chance);
  FeatureRoomDef.m_Chance =
      FeatureRoomDef.m_Required
          ? 1.0f
          : ConfigManager::GetFloat(sChance, 1.0f, sFeatureRoomDefName);
  ...
  FeatureRoomDef.m_OpenNeighbors =
      ConfigManager::GetBool(sOpenNeighbors, false, sFeatureRoomDefName);

  STATICHASH(AllowTransforms);
  FeatureRoomDef.m_AllowTransforms =
      ConfigManager::GetBool(sAllowTransforms, true, sFeatureRoomDefName);

  STATICHASH(AllowUnavailable);
  FeatureRoomDef.m_AllowUnavailable =
      ConfigManager::GetBool(sAllowUnavailable, false, sFeatureRoomDefName);

  STATICHASH(IgnoreFit);
  FeatureRoomDef.m_IgnoreFit =
      ConfigManager::GetBool(sIgnoreFit, false, sFeatureRoomDefName);
...
void EldritchWorldGen::InitializeRoomDefs(Array<uint>* pRoomDefs,
                                          const uint NumRoomDefs,
                                          TRoomMap* pRoomMap,
  ...
  for (uint RoomDefIndex = 0; RoomDefIndex < NumRoomDefs; ++RoomDefIndex) {
    ...
    RoomDef.m_Filename = ConfigManager::GetInheritedSequenceString(
        "RoomDef%d", RoomDefIndex, "", sDefinitionName);
    RoomDef.m_Theme = GetThemeFromFilename(RoomDef.m_Filename);

    MAKEHASHFROM(RoomFileName, RoomDef.m_Filename);

    STATICHASH(AllowTransforms);
    RoomDef.m_AllowTransforms =
        ConfigManager::GetBool(sAllowTransforms, true, sRoomFileName);

    STATICHASH(PrescribedExits);
    const bool PrescribedExits =
        ConfigManager::GetBool(sPrescribedExits, false, sRoomFileName);

    if (PrescribedExits) {
      STATICHASH(NumExits);
      const uint NumExits = ConfigManager::GetInt(sNumExits, 0, sRoomFileName);
      for (uint ExitIndex = 0; ExitIndex < NumExits; ++ExitIndex) {
        const HashedString ExitName = ConfigManager::GetSequenceHash(
            "Exit%d", ExitIndex, HashedString::NullString, sRoomFileName);
        AddExit(RoomDef.m_Exits, GetExit(ExitName));
      }
    } else {
      // For the common case, simplify the config file by ignoring subroom
      // fields
    ...
    if (pRoomMap) {
      ...
      for (int Transform = ERT_None; Transform < MaxTransforms; ++Transform) {
        const ERoomExits Exits = static_cast<ERoomExits>(
            GetPrecomputedTransformedExits(ActualExits, Transform));
...
void EldritchWorldGen::ComputeExitTransforms() {
  ...
  for (int ExitTransformIndex = 0; ExitTransformIndex < NumExitTransforms;
       ++ExitTransformIndex) {
    const int Exits = ExitTransformIndex % kMaxExits;
    const int Transform = ExitTransformIndex / kMaxExits;
    m_ExitTransforms[ExitTransformIndex] =
        GetTransformedExits(Exits, Transform);
  }
}

int EldritchWorldGen::GetPrecomputedTransformedExits(const int Exits,
                                                     const int Transform) {
  return m_ExitTransforms[Exits + Transform * kMaxExits];
}

int EldritchWorldGen::GetTransformedExits(const int Exits,
                                          const int Transform) {
...
inline void EldritchWorldGen::AddExit(ERoomExits& Exits,
                                      const ERoomExits NewExit) {
...
HashedString EldritchWorldGen::GetThemeFromFilename(
    const SimpleString& RoomFilename) {
  const char* const TagStart =
      FileUtil::StripLeadingFolders(RoomFilename.CStr());
...
// Where [name] is anything, [exits] is the NSWEUD tag I'm parsing, and version
// is an optional numeric tag.
void EldritchWorldGen::GetExitsFromFilename(const SimpleString& RoomFilename,
                                            ERoomExits& Exits) {
...
EldritchWorldGen::ERoomExits EldritchWorldGen::GetExit(
    const HashedString& ExitName) const {
  ...
  if (ExitName == sNorth || ExitName == sN) {
    return ERE_North;
  }
  if (ExitName == sSouth || ExitName == sS) {
    return ERE_South;
  }
  if (ExitName == sEast || ExitName == sE) {
    return ERE_East;
  }
  if (ExitName == sWest || ExitName == sW) {
    return ERE_West;
  }
  if (ExitName == sUp || ExitName == sU) {
    return ERE_Up;
  }
  if (ExitName == sDown || ExitName == sD) {
    return ERE_Down;
  }
...
inline int EldritchWorldGen::GetRoomIndex(const int RoomX, const int RoomY,
                                          const int RoomZ) {
  return RoomX + (RoomY * m_MapSizeX) + (RoomZ * m_MapSizeX * m_MapSizeY);
}

inline void EldritchWorldGen::GetRoomCoords(const int RoomIndex, int& RoomX,
                                            int& RoomY, int& RoomZ) {
...
bool EldritchWorldGen::FindFittingRooms(const ERoomExits ExitConfig,
                                        const SFeatureRoomDef& FeatureRoomDef,
    ...
    for (int Y = MinRoomY; Y <= MaxRoomY; ++Y) {
      for (int X = MinRoomX; X <= MaxRoomX; ++X) {
        ...
        if (IgnoreFit || IsUnopenNode(RoomIndex)) {
          for (int Transform = ERT_None; Transform < NumTransforms;
               ++Transform) {
            if (IgnoreFit ||
                Fits(ExitConfig, Transform, RoomIndex, AllowUnavailable)) {
...
// Try fitting ExitConfig into graph, taking into account world boundaries and
// adjacent closed/locked nodes.
// (In other words, every neighbor via an exit must be a valid location and
// either unopen or open.)
...
bool EldritchWorldGen::Fits(const ERoomExits ExitConfig, const int Transform,
                            const int RoomIndex, const bool AllowUnavailable) {
  int RoomX, RoomY, RoomZ;
  GetRoomCoords(RoomIndex, RoomX, RoomY, RoomZ);

  const int TransformedExits =
      GetPrecomputedTransformedExits(ExitConfig, Transform);
  ...
  if (RoomX == 0 && (TransformedExits & ERE_West)) {
    return false;
  }
  if (RoomX == HighX && (TransformedExits & ERE_East)) {
    return false;
  }
  if (RoomY == 0 && (TransformedExits & ERE_South)) {
    return false;
  }
  if (RoomY == HighY && (TransformedExits & ERE_North)) {
    return false;
  }
  if (RoomZ == 0 && (TransformedExits & ERE_Down)) {
    return false;
  }
  if (RoomZ == HighZ && (TransformedExits & ERE_Up)) {
    return false;
  }

  // Outgoing neighbor conditions: we can only connect to unopen or open nodes,
  // never closed or locked
  // (This prevents feature rooms being placed adjacent to each other and
  // forming a closed loop.)
  if (AllowUnavailable) {
    // Don't check outgoing neighbors.
  } else {
    if (RoomX > 0 && (TransformedExits & ERE_West) &&
        !IsAvailableNode(Neighbors.m_WestIndex)) {
      return false;
    }
    if (RoomX < HighX && (TransformedExits & ERE_East) &&
        !IsAvailableNode(Neighbors.m_EastIndex)) {
      return false;
    }
    if (RoomY > 0 && (TransformedExits & ERE_South) &&
        !IsAvailableNode(Neighbors.m_SouthIndex)) {
      return false;
    }
    if (RoomY < HighY && (TransformedExits & ERE_North) &&
        !IsAvailableNode(Neighbors.m_NorthIndex)) {
      return false;
    }
    if (RoomZ > 0 && (TransformedExits & ERE_Down) &&
        !IsAvailableNode(Neighbors.m_DownIndex)) {
      return false;
    }
    if (RoomZ < HighZ && (TransformedExits & ERE_Up) &&
        !IsAvailableNode(Neighbors.m_UpIndex)) {
      return false;
    }
  }

  // Incoming neighbor conditions: neighbors that have already been seeded must
  // fit with us.
  if (RoomX > 0 &&
      (m_GeneratedRooms[Neighbors.m_WestIndex].m_Exits & ERE_East) &&
      (0 == (TransformedExits & ERE_West))) {
    return false;
  }
  if (RoomX < HighX &&
      (m_GeneratedRooms[Neighbors.m_EastIndex].m_Exits & ERE_West) &&
      (0 == (TransformedExits & ERE_East))) {
    return false;
  }
  if (RoomY > 0 &&
      (m_GeneratedRooms[Neighbors.m_SouthIndex].m_Exits & ERE_North) &&
      (0 == (TransformedExits & ERE_South))) {
    return false;
  }
  if (RoomY < HighY &&
      (m_GeneratedRooms[Neighbors.m_NorthIndex].m_Exits & ERE_South) &&
      (0 == (TransformedExits & ERE_North))) {
    return false;
  }
  if (RoomZ > 0 && (m_GeneratedRooms[Neighbors.m_DownIndex].m_Exits & ERE_Up) &&
      (0 == (TransformedExits & ERE_Down))) {
    return false;
  }
  if (RoomZ < HighZ &&
      (m_GeneratedRooms[Neighbors.m_UpIndex].m_Exits & ERE_Down) &&
      (0 == (TransformedExits & ERE_Up))) {
    return false;
  }

  // All our exits lead to valid spaces, and all neighboring exits lead validly
  // to this point.
  return true;
}
...
void EldritchWorldGen::InsertAndLockRoom(
    const SRoomDef& RoomDef, const SFittingRoom& FittingRoom,
    const SFeatureRoomDef& FeatureRoomDef) {
  SGenRoom& GenRoom = m_GeneratedRooms[FittingRoom.m_RoomIndex];
  const ERoomExits TransformedExits = static_cast<ERoomExits>(
      GetPrecomputedTransformedExits(RoomDef.m_Exits, FittingRoom.m_Transform));

  GenRoom.m_UsePrescribedRoom = true;
  GenRoom.m_Exits = TransformedExits;
  GenRoom.m_PrescribedFilename = RoomDef.m_Filename;
  GenRoom.m_PrescribedTransform =
      static_cast<ERoomXform>(FittingRoom.m_Transform);
  ...
  TryExpandMaze(TransformedExits, FittingRoom.m_RoomIndex, Neighbors,
                FeatureRoomDef.m_OpenNeighbors);
}
...
void EldritchWorldGen::AddFeatureRooms() {
  ...
  for (uint FeatureRoomDefIndex = 0; FeatureRoomDefIndex < NumFeatureRoomDefs;
       ++FeatureRoomDefIndex) {
    const SFeatureRoomDef& FeatureRoomDef =
        m_FeatureRoomDefs[FeatureRoomDefIndex];
...
void EldritchWorldGen::GetRoomFilenameAndTransform(
    const SGenRoom& GenRoom, SimpleString& OutRoomFilename,
    ERoomXform& OutTransform, const HashedString& ElectedTheme) {
  if (GenRoom.m_UsePrescribedRoom) {
    ...
    if (ElectedTheme == HashedString::NullString) {
      ...
      LookupRandomThemeRoom(GenRoom, OutRoomFilename, OutTransform,
                            ElectedTheme);
    }
  }
}

void EldritchWorldGen::LookupRandomRoom(const SGenRoom& GenRoom,
                                        SimpleString& OutRoomFilename,
                                        ERoomXform& OutTransform) {
...
void EldritchWorldGen::LookupRandomThemeRoom(const SGenRoom& GenRoom,
                                             SimpleString& OutRoomFilename,
                                             ERoomXform& OutTransform,
                                             const HashedString& ElectedTheme) {
  ASSERT(m_RoomMap.Search(GenRoom.m_Exits).IsValid());
  const Array<SRoomDefLookup>& RoomsFittingExit = m_RoomMap[GenRoom.m_Exits];

  // Use a weighted random selection, adding weight to fit level theme for
  // cohesion.
...
void EldritchWorldGen::PopulateWorld() {
  ...
  const HashedString ElectedTheme =
      m_Themes.Empty() ? HashedString::NullString : Math::ArrayRandom(m_Themes);
...
#if BUILD_DEV
    const SimpleString ElectedThemeStr =
        ReverseHash::ReversedHash(ElectedTheme);
    PRINTF("Elected theme %s with %.2f bias.\n", ElectedThemeStr.CStr(),
           m_ThemeBias);
#endif
  }
}
...
void EldritchWorldGen::FilterPreparedSpawns() {
  FOR_EACH_MAP(GroupIter, m_SpawnResolveGroups, HashedString,
               SSpawnResolveGroup) {
    SSpawnResolveGroup& ResolveGroup = GroupIter.GetValue();

    if (ResolveGroup.m_MinRadiusSqFromPlayer > 0.0f) {
      FOR_EACH_ARRAY_REVERSE(PreparedSpawnIter, ResolveGroup.m_PreparedSpawns,
                             SPreparedSpawn) {
...
void EldritchWorldGen::ResolveEntitySpawns() {
  FOR_EACH_MAP(GroupIter, m_SpawnResolveGroups, HashedString,
    ...
    const uint NumDesiredSpawns =
        (ResolveGroup.m_ResolveLimit > 0)
            ? ResolveGroup.m_ResolveLimit
            : static_cast<uint>(ResolveGroup.m_ResolveChance *
                                NumPreparedSpawns);
    ...
    if (NumSpawns < NumDesiredSpawns) {
      ...
      const SimpleString ResolveGroupName =
          ReverseHash::ReversedHash(ResolveGroupHash);
      PRINTF("Not enough spawners in world to satisfy resolve group %s.\n",
             ResolveGroupName.CStr());
    }
#endif
    ASSERTDESC(NumSpawns == NumDesiredSpawns,
               "Not enough spawners in world to satisfy resolve group limit.");

    for (uint SpawnCount = 0; SpawnCount < NumSpawns; ++SpawnCount) {
      const uint PreparedSpawnIndex =
          Math::Random(ResolveGroup.m_PreparedSpawns.Size());
      const SPreparedSpawn& PreparedSpawn =
          ResolveGroup.m_PreparedSpawns[PreparedSpawnIndex];
...
void EldritchWorldGen::SpawnPreparedEntity(
    const SPreparedSpawn& PreparedSpawn) {
  WBEntity* const pEntity =
      WBWorld::GetInstance()->CreateEntity(PreparedSpawn.m_EntityDef);
  DEVASSERT(pEntity);
  ...
  const bool ShouldDebugCheckSpawnClearance = ConfigManager::GetInheritedBool(
      sShouldDebugCheckSpawnClearance, true, sDefinitionName);
  if (ShouldDebugCheckSpawnClearance) {
    ...
    if (m_World->CheckClearance(PreparedSpawn.m_SpawnLocation, TestExtents,
                                Info)) {
      PRINTF("Spawned entity %s in room %s does not have clearance!\n",
             pEntity->GetUniqueName().CStr(),
             PreparedSpawn.m_RoomFilename.CStr());
  ...
  WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(),
                    OnInitialOrientationSet, pEntity);
}
...
void EldritchWorldGen::ValidateSpawnersInRoom(
    const EldritchRoom& Room, const SimpleString& RoomFilename) {
  ...
  for (uint SpawnerIndex = 0; SpawnerIndex < NumSpawners; ++SpawnerIndex) {
    ...
    const bool Deprecated =
        ConfigManager::GetBool(sDeprecated, false, sSpawnerDef);
    if (Deprecated) {
...
void EldritchWorldGen::PrepareSpawnEntitiesFromRoom(const EldritchRoom& Room,
                                                    const ERoomXform Transform,
                                                    const uint RoomX,
                                                    const uint RoomY,
                                                    const uint RoomZ) {
  PROFILE_FUNCTION;
  ...
  for (uint SpawnerIndex = 0; SpawnerIndex < NumSpawners; ++SpawnerIndex) {
    ...
    const SimpleString SpawnerDef =
        TryGetHardSpawner(m_World->GetSpawnerOverride(RoomSpawnerDef));
    ...
    const float UseChance =
        m_IsHardMode ? ConfigManager::GetFloat(sHardChance, Chance, sSpawnerDef)
                     : Chance;
    ...
    const HashedString ResolveGroupName = ConfigManager::GetHash(
        sResolveGroup, HashedString::NullString, sSpawnerDef);
    ...
    PrepareSpawnEntity(SubSpawnerDef, ResolveGroupName, Spawner.m_Location,
                       Direction, Transform, RoomX, RoomY, RoomZ);
  }
}

SimpleString EldritchWorldGen::TryGetHardSpawner(
    const SimpleString& BaseSpawnerDef) {
  if (m_IsHardMode) {
    MAKEHASH(BaseSpawnerDef);

    STATICHASH(HardSpawner);
    return ConfigManager::GetString(sHardSpawner, BaseSpawnerDef.CStr(),
                                    sBaseSpawnerDef);
  } else {
...
SimpleString EldritchWorldGen::GetSubSpawnerDef(
    const SimpleString& SuperSpawnerDef) {
  MAKEHASH(SuperSpawnerDef);

  STATICHASH(NumSubSpawners);
  const uint NumSubSpawners =
      ConfigManager::GetInt(sNumSubSpawners, 0, sSuperSpawnerDef);
  ...
  if (NumSubSpawners > 0) {
    ...
    float WeightSum = 0.0f;
    for (uint SubSpawnerIndex = 0; SubSpawnerIndex < NumSubSpawners;
         ++SubSpawnerIndex) {
      WeightSum += ConfigManager::GetSequenceFloat(
          "SubSpawner%dWeight", SubSpawnerIndex, 1.0f, sSuperSpawnerDef);
    ...
    for (uint SubSpawnerIndex = 0; SubSpawnerIndex < NumSubSpawners;
      ...
      if (RolledWeight <= CumulativeWeightArray[SubSpawnerIndex]) {
        return ConfigManager::GetSequenceString("SubSpawner%d", SubSpawnerIndex,
                                                "", sSuperSpawnerDef);
      }
    }
...
float EldritchWorldGen::GetYaw(const int Direction) {
  if (Direction == ERE_North) {
    return 0.0f;
  }
  if (Direction == ERE_South) {
    return DEGREES_TO_RADIANS(180.0f);
  }
  if (Direction == ERE_East) {
    return DEGREES_TO_RADIANS(270.0f);
  }
  if (Direction == ERE_West) {
    return DEGREES_TO_RADIANS(90.0f);
  }
...
float EldritchWorldGen::GetTransformedYaw(const int Direction,
                                          const int Transform) {
  return GetYaw(GetTransformedExits(Direction, Transform));
}

Vector EldritchWorldGen::GetTransformedOffset(const Vector& Offset,
                                              const int Transform) {
  switch (Transform) {
    case ERT_None:
    ...
    case ERT_MirrorY:
      return Vector(Offset.x, -Offset.y, Offset.z);
      break;
    case (ERT_MirrorX | ERT_MirrorY):
      return Vector(-Offset.x, -Offset.y, Offset.z);
      break;
    case ERT_Rotate90:
      return Vector(-Offset.y, Offset.x, Offset.z);
      break;
    case (ERT_MirrorX | ERT_Rotate90):
      return Vector(-Offset.y, -Offset.x, Offset.z);
      break;
    case (ERT_MirrorY | ERT_Rotate90):
      return Vector(Offset.y, Offset.x, Offset.z);
      break;
    case (ERT_MirrorX | ERT_MirrorY | ERT_Rotate90):
      return Vector(Offset.y, -Offset.x, Offset.z);
      break;
  }
...
EldritchWorldGen::ERoomExits EldritchWorldGen::GetDirection(
    const uint8 Orientation) const {
  if (Orientation == ESO_North) {
    return ERE_North;
  }
  if (Orientation == ESO_South) {
    return ERE_South;
  }
  if (Orientation == ESO_East) {
    return ERE_East;
  }
  if (Orientation == ESO_West) {
    return ERE_West;
  }
...
void EldritchWorldGen::PrepareSpawnEntity(const SimpleString& SpawnerDef,
                                          const HashedString& ResolveGroupName,
                                          const vidx_t VoxelLocation,
                                          const ERoomExits Direction,
                                          const ERoomXform Transform,
                                          const uint RoomX, const uint RoomY,
                                          const uint RoomZ) {
  ...
  const float OffsetXYRange =
      ConfigManager::GetFloat(sOffsetXYRange, 0.0f, sSpawnerDef);

  STATICHASH(OffsetX);
  const float OffsetX = ConfigManager::GetFloat(
      sOffsetX, Math::Random(-OffsetXYRange, OffsetXYRange), sSpawnerDef);

  STATICHASH(OffsetY);
  const float OffsetY = ConfigManager::GetFloat(
      sOffsetY, Math::Random(-OffsetXYRange, OffsetXYRange), sSpawnerDef);

  STATICHASH(OffsetZ);
  const float OffsetZ = ConfigManager::GetFloat(sOffsetZ, 0.0f, sSpawnerDef);

  STATICHASH(OffsetYawRange);
  const float OffsetYawRange =
      ConfigManager::GetFloat(sOffsetYawRange, 0.0f, sSpawnerDef);

  STATICHASH(OffsetYaw);
  const float OffsetYaw = DEGREES_TO_RADIANS(ConfigManager::GetFloat(
      sOffsetYaw, Math::Random(0.0f, OffsetYawRange), sSpawnerDef));

  // OrientOffset: If true, OffsetX/Y is transformed by orientation (yaw)
  STATICHASH(OrientOffset);
  const bool OrientOffset =
      ConfigManager::GetBool(sOrientOffset, true, sSpawnerDef);

  STATICHASH(Entity);
  const SimpleString EntityDef =
      ConfigManager::GetString(sEntity, "", sSpawnerDef);
  ASSERT(EntityDef != "");

  const vidx_t TransformedLocation =
      (Transform == ERT_None)
          ? VoxelLocation
          : GetTransformedRoomVoxelIndex(VoxelLocation, Transform, RoomSize);
  const vidx_t SpawnerWorldVoxelIndex =
      GetWorldVoxelIndex(TransformedLocation, RoomX, RoomY, RoomZ);
  ...
  const Vector OrientedOffset =
      OrientOffset ? (SpawnOffset * OrientationMatrix) : SpawnOffset;
  const Vector TransformedOffset =
      GetTransformedOffset(OrientedOffset, Transform);
  const Vector SpawnLocation =
      m_World->GetVoxelCenter(SpawnerWorldVoxelIndex) + TransformedOffset;
  ...
  // HACK: Identify where the player is spawning. (Hack because I usually
  // avoiding making code be content-aware.)
...
void EldritchWorldGen::GatherStats() {
  ...
  const uint NumStatsIterations =
      ConfigManager::GetInt(sNumStatsIterations, 0, sEldritchWorldGen);

  for (uint GenerateIndex = 0; GenerateIndex < NumStatsIterations;
       ++GenerateIndex) {
    Generate(true);
  }
...
void EldritchWorldGen::PrintStats(const IDataStream& Stream) {
  ...
  FOR_EACH_MAP(StatsIter, m_RoomStats, HashedString, uint) {
    const SimpleString ReversedFilename =
        ReverseHash::ReversedHash(StatsIter.GetKey());
    const uint NumUses = StatsIter.GetValue();
    const float Frequency =
        static_cast<float>(NumUses) / static_cast<float>(NumStatsIterations);
...
vidx_t EldritchWorldGen::GetWorldVoxelIndex(const vidx_t RoomVoxelIndex,
                                            const uint RoomX, const uint RoomY,
                                            const uint RoomZ) {
  ...
  const vpos_t RoomVoxelZ =
      ((RoomVoxelIndex / RoomSizeX) / RoomSizeY) % RoomSizeZ;
...
void EldritchWorldGen::SetVoxelsFromRoom(const EldritchRoom& Room,
                                         const ERoomXform Transform,
                                         const uint RoomX, const uint RoomY,
                                         const uint RoomZ) {
  PROFILE_FUNCTION;
  ...
  ASSERT(RoomSizeX * ScalarX * RoomSizeY * ScalarY * RoomSizeZ * ScalarZ ==
         RoomVoxels.Size());
  ...
  for (vpos_t PosZ = BaseZ; PosZ < HighZ; ++PosZ) {
    for (vpos_t PosY = BaseY; PosY < HighY; ++PosY) {
      for (vpos_t PosX = BaseX; PosX < HighX; ++PosX, ++RoomVoxelIndex) {
        ...
        TransformedRoomVoxelIndex =
            (Transform == ERT_None) ? RoomVoxelIndex
                                    : GetUntransformedRoomVoxelIndex(
                                          RoomVoxelIndex, Transform, RoomSizeX);

        // Calling inline functions in EldritchWorld. This could cause linker
        // errors.
...
int EldritchWorldGen::GetTransformedRoomVoxelIndex(const int RoomVoxelIndex,
                                                   const int Transform,
                                                   const uint RoomSize) {
...
// Apply transforms in reverse order, because we're essentially doing the
// reverse transformation
// to figure out where to sample in the original room in order to produce the
// transformed room.
int EldritchWorldGen::GetUntransformedRoomVoxelIndex(const int RoomVoxelIndex,
                                                     const int Transform,
                                                     const uint RoomSize) {
  int X = RoomVoxelIndex % RoomSize;
  int Y = (RoomVoxelIndex / RoomSize) % RoomSize;
...
void EldritchWorldGen::GetNeighborIndices(const int RoomIndex,
                                          SNeighborIndices& OutIndices) {
...
void EldritchWorldGen::FindOpenLoopOpportunities(
    Array<SOpenLoopOpportunity>& OutOpportunities) {
  const int HighX = static_cast<int>(m_MapSizeX - 1);
  const int HighY = static_cast<int>(m_MapSizeY - 1);
  const int HighZ = static_cast<int>(m_MapSizeZ - 1);

  for (int Z = 0; Z <= HighZ; ++Z) {
    for (int Y = 1; Y <= HighY; ++Y)  // We're looking for west and south exits,
                                      // so we can skip the south face.
    {
      for (int X = 1; X <= HighX; ++X)  // We're looking for west and south
                                        // exits, so we can skip the west face.
      {
        ...
        if (IsClosedNode(RoomIndex)) {
          ...
          ASSERT(IsClosedNode(Neighbors.m_WestIndex) ||
                 IsLockedNode(Neighbors.m_WestIndex));
          ASSERT(IsClosedNode(Neighbors.m_SouthIndex) ||
                 IsLockedNode(Neighbors.m_SouthIndex));
...
void EldritchWorldGen::MazeOpenLoops() {
  ...
  const float fNumOpportunitiesToOpen =
      m_Expansion_OpenLoopsRatio * fNumOpportunities;
  const uint NumOpportunitiesToOpen =
      static_cast<uint>(fNumOpportunitiesToOpen + 0.5f);
...
// PrintMaze();
    ...
    // Modifying Prim's algorithm: expanding from the last expanded room
    // produces more regular floors for me. So do that some portion of the time.
    const bool FollowLastNode = Math::RandomF(m_Expansion_FollowLastNodeRatio);
    const int ClosedRoomIndexIndex =
        FollowLastNode ? (m_Maze_OpenRoomStack.Size() - 1)
                       : Math::Random(m_Maze_OpenRoomStack.Size());
    ...
    if (ClosedX > 0 &&
        m_Maze_UnopenRoomSet.Search(Neighbors.m_WestIndex).IsValid()) {
      HorizontalExits.PushBack(ERE_West);
    }
    if (ClosedX < HighX &&
        m_Maze_UnopenRoomSet.Search(Neighbors.m_EastIndex).IsValid()) {
      HorizontalExits.PushBack(ERE_East);
    }
    if (ClosedY > 0 &&
        m_Maze_UnopenRoomSet.Search(Neighbors.m_SouthIndex).IsValid()) {
      HorizontalExits.PushBack(ERE_South);
    }
    if (ClosedY < HighY &&
        m_Maze_UnopenRoomSet.Search(Neighbors.m_NorthIndex).IsValid()) {
      HorizontalExits.PushBack(ERE_North);
    }
    if (ClosedZ > 0 &&
        m_Maze_UnopenRoomSet.Search(Neighbors.m_DownIndex).IsValid()) {
      VerticalExits.PushBack(ERE_Down);
    }
    if (ClosedZ < HighZ &&
        m_Maze_UnopenRoomSet.Search(Neighbors.m_UpIndex).IsValid()) {
      VerticalExits.PushBack(ERE_Up);
    }

    // If we can't expand anywhere, this node is closed
    if (HorizontalExits.Empty() && VerticalExits.Empty()) {
      // I can't FastRemove because the order of this array matters for follow
      // last node bias.
    ...
    ExpandDirection = ExpandHorizontal ? Math::ArrayRandom(HorizontalExits)
                                       : Math::ArrayRandom(VerticalExits);

    TryExpandMaze(ExpandDirection, ClosedRoomIndex, Neighbors, true);
  }

  // It's rare, but some configurations of feature rooms can result in unvisited
  // nodes in the corners of the world.
...
void EldritchWorldGen::TryExpandMaze(const ERoomExits Exits,
                                     const int OpenRoomIndex,
                                     const SNeighborIndices& Neighbors,
                                     const bool OpenRoom) {
#define TRY_EXPAND_DIR(d, o)                                            \
  if (Exits & ERE_##d) {                                                \
    ExpandMaze(OpenRoomIndex, Neighbors.m_##d##Index, ERE_##d, ERE_##o, \
               OpenRoom);                                               \
  }
...
void EldritchWorldGen::ExpandMaze(const int OpenRoomIndex,
                                  const int UnopenRoomIndex,
                                  const ERoomExits Direction,
                                  const ERoomExits OppositeDirection,
                                  const bool OpenRoom) {
  ...
  if (IsOpenNode(UnopenRoomIndex) || IsClosedNode(UnopenRoomIndex) ||
      IsLockedNode(UnopenRoomIndex)) {
    // We can expand into already opened rooms when seeding the maze with
    // feature rooms.
...
void EldritchWorldGen::PrintMaze() {
  ...
  for (int Z = static_cast<int>(m_MapSizeZ - 1); Z >= 0;
       --Z)  // Print top to bottom
  {
    PRINTF("Floor %d:\n", Z);
    for (int Y = static_cast<int>(m_MapSizeY - 1); Y >= 0;
         --Y)  // Print north to south
    {
      for (int X = 0; X < static_cast<int>(m_MapSizeX);
           ++X)  // Print west to east
      {
        const int RoomIndex = GetRoomIndex(X, Y, Z);
