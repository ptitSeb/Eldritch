...
EldritchTools::~EldritchTools() { ShutDown(); }
...
void EldritchTools::ShutDown() {
  ...
  for (uint VoxelMeshIndex = 0; VoxelMeshIndex < NumVoxelMeshes;
       ++VoxelMeshIndex) {
  ...
  for (uint PaletteVoxelIndex = 0; PaletteVoxelIndex < NumPaletteVoxels;
       ++PaletteVoxelIndex) {
  ...
  for (uint SpawnerCategoryButtonIndex = 0;
       SpawnerCategoryButtonIndex < NumSpawnerCategoryButtons;
       ++SpawnerCategoryButtonIndex) {
    SSpawnerCategoryButton& SpawnerCategoryButton =
        m_SpawnerCategoryButtons[SpawnerCategoryButtonIndex];
    SafeDelete(SpawnerCategoryButton.m_Mesh);
  }
  m_SpawnerCategoryButtons.Clear();

  const uint NumSpawnerButtons = m_SpawnerButtons.Size();
  for (uint SpawnerButtonIndex = 0; SpawnerButtonIndex < NumSpawnerButtons;
       ++SpawnerButtonIndex) {
...
void EldritchTools::InitializeFromDefinition(
    const SimpleString& DefinitionName) {
  MAKEHASH(DefinitionName);

  STATICHASH(NumSpawnerCategories);
  const uint NumSpawnerCategories =
      ConfigManager::GetInt(sNumSpawnerCategories, 0, sDefinitionName);

  for (uint CategoryIndex = 0; CategoryIndex < NumSpawnerCategories;
       ++CategoryIndex) {
    const SimpleString Category = ConfigManager::GetSequenceString(
        "SpawnerCategory%d", CategoryIndex, "", sDefinitionName);
    m_SpawnerCategories.PushBack(Category);

    MAKEHASH(Category);

    STATICHASH(NumSpawnerDefs);
    const uint NumSpawnerDefs =
        ConfigManager::GetInt(sNumSpawnerDefs, 0, sCategory);

    for (uint SpawnerDefCategoryIndex = 0;
         SpawnerDefCategoryIndex < NumSpawnerDefs; ++SpawnerDefCategoryIndex) {
      const uint SpawnerDefIndex = m_SpawnerDefs.Size();
      const SimpleString SpawnerDef = ConfigManager::GetSequenceString(
          "SpawnerDef%d", SpawnerDefCategoryIndex, "", sCategory);
...
void EldritchTools::Reinitialize(const uint RoomScalarX /*= 1*/,
                                 const uint RoomScalarY /*= 1*/,
                                 const uint RoomScalarZ /*= 1*/) {
  ShutDown();
  ...
  m_BoundPlanes.Clear();
  m_BoundPlanes.PushBack(
      Plane(Vector(1.0f, 0.0f, 0.0f), Vector(0.0f, 0.0f, 0.0f)));
  m_BoundPlanes.PushBack(
      Plane(Vector(-1.0f, 0.0f, 0.0f),
            Vector(static_cast<float>(m_RoomSizeX), 0.0f, 0.0f)));
  m_BoundPlanes.PushBack(
      Plane(Vector(0.0f, 1.0f, 0.0f), Vector(0.0f, 0.0f, 0.0f)));
  m_BoundPlanes.PushBack(
      Plane(Vector(0.0f, -1.0f, 0.0f),
            Vector(0.0f, static_cast<float>(m_RoomSizeY), 0.0f)));
  m_BoundPlanes.PushBack(
      Plane(Vector(0.0f, 0.0f, 1.0f), Vector(0.0f, 0.0f, 0.0f)));
  m_BoundPlanes.PushBack(
      Plane(Vector(0.0f, 0.0f, -1.0f),
            Vector(0.0f, 0.0f, static_cast<float>(m_RoomSizeZ))));
...
void EldritchTools::InitializeHelpMeshes() {
  ...
  const SimpleString HelpText = SimpleString(
      "F6: Save\tF7: Clear\tF8: Fill\tF9: Load\nQ: Up\tE: Down\nEnter: "
      "Fill\tShift+Enter: Erase\nHold Alt for more options.");
  m_HelpMesh = pRenderer->Print(
      HelpText.CStr(), pRenderer->GetFontManager()->GetFont(DEFAULT_FONT),
      SRect(), 0);

  const SimpleString AltHelpText = SimpleString(
      "Alt+1: New 1x1x1\tAlt+2: New 1x2x1\tAlt+3: New 1x1x2\tAlt+4: New "
      "2x2x1\nAlt+F7: Clear with floor\tAlt+F8: Clear with shell");
  m_AltHelpMesh = pRenderer->Print(
      AltHelpText.CStr(), pRenderer->GetFontManager()->GetFont(DEFAULT_FONT),
      SRect(), 0);
}

void EldritchTools::CreateGridMesh(const Array<Vector>& Corners,
                                   const uint Color) {
  ASSERT(Corners.Size() == 4);

  IRenderer* const pRenderer = m_Framework->GetRenderer();

  Mesh* const pQuadMesh = pRenderer->GetMeshFactory()->CreateDebugQuad(
      Corners[0], Corners[1], Corners[2], Corners[3], Color);
  pQuadMesh->SetMaterialDefinition(DEFAULT_NEWMATERIAL, pRenderer);
  pQuadMesh->SetTexture(
      0, pRenderer->GetTextureManager()->GetTexture(DEFAULT_TEXTURE));
...
void EldritchTools::InitializeSpawnerMeshes() {
  ...
  for (uint CategoryIndex = 0; CategoryIndex < NumCategories; ++CategoryIndex) {
    ...
    Mesh* const pCategoryMesh = pRenderer->Print(
        SpawnerCategory.CStr(),
        pRenderer->GetFontManager()->GetFont(DEFAULT_FONT), Rect, 0);
    ...
    CategoryButton.m_BoxMin =
        Vector2(pCategoryMesh->m_AABB.m_Min.x, pCategoryMesh->m_AABB.m_Min.z);
    CategoryButton.m_BoxMax =
        Vector2(pCategoryMesh->m_AABB.m_Max.x, pCategoryMesh->m_AABB.m_Max.z);

    const float CategoryWidth =
        pCategoryMesh->m_AABB.m_Max.x - pCategoryMesh->m_AABB.m_Min.x;
  ...
  for (uint SpawnerDefIndex = 0; SpawnerDefIndex < NumSpawnerDefs;
       ++SpawnerDefIndex) {
    ...
    SRect Rect = SRect(kBase + MaxCategoryWidth + kSpacing,
                       kBase + CategoryCount * kSpacing, 0.0f, 0.0f);
    Mesh* const pSpawnerDefMesh = pRenderer->Print(
        SpawnerDef.CStr(), pRenderer->GetFontManager()->GetFont(DEFAULT_FONT),
        Rect, 0);

    SSpawnerButton SpawnerButton;
    SpawnerButton.m_Mesh = pSpawnerDefMesh;
    SpawnerButton.m_SpawnerDefIndex = SpawnerDefIndex;
    SpawnerButton.m_BoxMin = Vector2(pSpawnerDefMesh->m_AABB.m_Min.x,
                                     pSpawnerDefMesh->m_AABB.m_Min.z);
    SpawnerButton.m_BoxMax = Vector2(pSpawnerDefMesh->m_AABB.m_Max.x,
                                     pSpawnerDefMesh->m_AABB.m_Max.z);
...
void EldritchTools::InitializePaletteMeshes() {
  const Map<vval_t, EldritchWorld::SVoxelDef>& VoxelDefs =
      m_World->GetWorldDef().m_VoxelDefs;
...
void EldritchTools::CreatePaletteMesh(
    const vval_t VoxelValue, const EldritchWorld::SVoxelDef& VoxelDef) {
  IRenderer* const pRenderer = m_Framework->GetRenderer();
...
#define PUSH_POSITIONS(a, b, c, d) \
  Positions.PushBack(kV##a);       \
  Positions.PushBack(kV##b);       \
  Positions.PushBack(kV##c);       \
  Positions.PushBack(kV##d)
...
#undef PUSH_POSITIONS
  ...
  const Vector4 VoxelColor =
      ((VoxelValue & 0x80) == 0) ? kDefaultColor : kUnbreakableColor;
...
#define PUSH_UVS(a, b, c, d, s)    \
  UVs.PushBack(UVBase##s + UV##a); \
  UVs.PushBack(UVBase##s + UV##b); \
  UVs.PushBack(UVBase##s + UV##c); \
  UVs.PushBack(UVBase##s + UV##d)
...
#undef PUSH_INDICES
  ...
  IVertexDeclaration* const pVertexDeclaration =
      pRenderer->GetVertexDeclaration(VD_POSITIONS | VD_UVS |
                                      VD_FLOATCOLORS_SM2);
  ...
  Mesh* pPaletteMesh =
      new Mesh(pVertexBuffer, pVertexDeclaration, pIndexBuffer);

  pPaletteMesh->m_AABB =
      AABB(Vector(-0.5f, 0.0f, -0.5f), Vector(0.5f, 0.0f, 0.5f));
  ...
  pPaletteMesh->SetTexture(
      0, pRenderer->GetTextureManager()->GetTexture(WorldTileset.CStr()));
  pPaletteMesh->SetMaterialFlags(MAT_HUD);

  const float fDisplayWidth =
      static_cast<float>(m_Framework->GetDisplay()->m_Width);
  const uint NumPalettesX =
      static_cast<uint>(Floor((fDisplayWidth - kBaseX) / kSpacingX));
...
#define PUSH_POSITIONS(a, b, c, d)       \
  Positions.PushBack(VoxelBase + kV##a); \
  Positions.PushBack(VoxelBase + kV##b); \
  Positions.PushBack(VoxelBase + kV##c); \
  Positions.PushBack(VoxelBase + kV##d)
...
#undef PUSH_POSITIONS
  ...
  static const Vector4 kColors[] = {kBottomColor, kTopColor, kYColor,
                                    kYColor,      kXColor,   kXColor};
  static const Vector4 kUnbreakableColor = Vector4(1.0f, 0.0f, 1.0f, 1.0f);
  for (uint ColorIndex = 0; ColorIndex < kNumVertices; ++ColorIndex) {
    const Vector4 FaceColor = kColors[ColorIndex / 4];
    const Vector4 VoxelColor =
        ((VoxelValue & 0x80) == 0) ? FaceColor : kUnbreakableColor;
...
uint EldritchTools::SafeGetVoxelIndexForCoords(vpos_t X, vpos_t Y, vpos_t Z) {
  if (X >= 0 && X < m_RoomSizeX && Y >= 0 && Y < m_RoomSizeY && Z >= 0 &&
      Z < m_RoomSizeZ) {
...
void EldritchTools::GetCoordsForVoxel(const vidx_t VoxelIndex, vpos_t& X,
                                      vpos_t& Y, vpos_t& Z) {
  ASSERT(VoxelIndex < m_NumVoxels);
...
bool EldritchTools::IsInToolMode() const { return m_ToolMode; }
...
void EldritchTools::ToggleToolMode() {
  ...
  UIScreen* const pOldHUD =
      pUIManager->GetScreen(m_ToolMode ? sHUD : sToolsHUD);
  UIScreen* const pNewHUD =
      pUIManager->GetScreen(m_ToolMode ? sToolsHUD : sHUD);
...
void EldritchTools::Tick(const float DeltaTime) {
  ...
  m_CameraOrientation +=
      m_CameraRotationalVelocity * kRotationVelocity * DeltaTime;

  m_CameraOrientation.Pitch =
      Clamp(m_CameraOrientation.Pitch, -PI * 0.49f, PI * 0.49f);
...
void EldritchTools::TickPaletteInput() {
  ...
  if (pKeyboard->OnRise(Keyboard::EB_Mouse_Left)) {
    ...
    for (uint PaletteVoxelIndex = 0; PaletteVoxelIndex < NumPaletteVoxels;
      ...
      if (fX >= PaletteVoxel.m_BoxMin.x && fY >= PaletteVoxel.m_BoxMin.y &&
          fX <= PaletteVoxel.m_BoxMax.x && fY <= PaletteVoxel.m_BoxMax.y) {
        // Clicked
        if (pKeyboard->IsHigh(Keyboard::EB_LeftShift) ||
...
void EldritchTools::TickSpawnersInput() {
  ...
  if (pKeyboard->OnRise(Keyboard::EB_Mouse_Left)) {
    ...
    for (uint CategoryIndex = 0; CategoryIndex < NumCategoryButtons;
         ++CategoryIndex) {
      const SSpawnerCategoryButton& CategoryButton =
          m_SpawnerCategoryButtons[CategoryIndex];
      ...
      if (fX >= CategoryButton.m_BoxMin.x && fY >= CategoryButton.m_BoxMin.y &&
          fX <= CategoryButton.m_BoxMax.x && fY <= CategoryButton.m_BoxMax.y) {
        // Clicked
    ...
    for (uint SpawnerButtonIndex = 0; SpawnerButtonIndex < NumSpawnerButtons;
         ++SpawnerButtonIndex) {
      const SSpawnerButton& SpawnerButton =
          m_SpawnerButtons[SpawnerButtonIndex];
      ...
      if (fX >= SpawnerButton.m_BoxMin.x && fY >= SpawnerButton.m_BoxMin.y &&
          fX <= SpawnerButton.m_BoxMax.x && fY <= SpawnerButton.m_BoxMax.y) {
        // Clicked
...
void EldritchTools::PrevSubTool() {
  const ESubTool PrevTool =
      static_cast<ESubTool>((m_SubTool + EST_COUNT - 1) % EST_COUNT);
  SetSubTool(PrevTool);
}
...
void EldritchTools::TickNormalInput() {
  ...
  m_CameraSpeed = pKeyboard->IsHigh(Keyboard::EB_LeftShift) ? kFastVelocity
                                                            : kNormalVelocity;
  ...
  if (pKeyboard->IsHigh(Keyboard::EB_S) &&
      !pKeyboard->IsHigh(Keyboard::EB_LeftControl)) {
    m_CameraVelocity -= Y;
  }
  ...
  if (pKeyboard->IsHigh(Keyboard::EB_LeftAlt) &&
      pKeyboard->OnRise(Keyboard::EB_1)) {
    Reinitialize(1, 1, 1);
  }

  if (pKeyboard->IsHigh(Keyboard::EB_LeftAlt) &&
      pKeyboard->OnRise(Keyboard::EB_2)) {
    Reinitialize(1, 2, 1);
  }

  if (pKeyboard->IsHigh(Keyboard::EB_LeftAlt) &&
      pKeyboard->OnRise(Keyboard::EB_3)) {
    Reinitialize(1, 1, 2);
  }

  if (pKeyboard->IsHigh(Keyboard::EB_LeftAlt) &&
      pKeyboard->OnRise(Keyboard::EB_4)) {
    Reinitialize(2, 2, 1);
  }

  if (pKeyboard->OnRise(Keyboard::EB_S) &&
      pKeyboard->IsHigh(Keyboard::EB_LeftControl)) {
    TryQuickSave();
  }
  ...
  if (pKeyboard->OnRise(Keyboard::EB_Mouse_Left)) {
    ...
    const vidx_t AddVoxel = (HitVoxel != kInvalidVoxelIndex &&
                             pKeyboard->IsHigh(Keyboard::EB_LeftControl))
                                ? HitVoxel
                                : FacingVoxel;
  ...
  if (pKeyboard->OnRise(Keyboard::EB_Mouse_Middle)) {
    ...
    const vidx_t BrushVoxel = (HitVoxel != kInvalidVoxelIndex &&
                               pKeyboard->IsHigh(Keyboard::EB_LeftControl))
                                  ? HitVoxel
                                  : FacingVoxel;
  ...
  if (pKeyboard->OnRise(Keyboard::EB_R)) {
    ...
    if (FacingVoxel != kInvalidVoxelIndex) {
      if (pKeyboard->IsHigh(Keyboard::EB_LeftShift)) {
        ...
        Map<vidx_t, SPlacedSpawner>::Iterator SpawnerIter =
            m_Spawners.Search(FacingVoxel);
...
void EldritchTools::TickBrushInput() {
  ...
  if (pKeyboard->IsHigh(Keyboard::EB_LeftControl) &&
      pKeyboard->OnRise(Keyboard::EB_A)) {
    MaximizeBrush();
  }

  if (pKeyboard->IsHigh(Keyboard::EB_LeftControl) &&
      pKeyboard->OnRise(Keyboard::EB_D)) {
    ResetBrush();
  }

  if (pKeyboard->OnRise(Keyboard::EB_Enter) ||
      pKeyboard->OnRise(Keyboard::EB_F)) {
    if (pKeyboard->IsHigh(Keyboard::EB_LeftShift)) {
      EraseBrush();
    } else {
      FillBrush();
  ...
  if (pKeyboard->OnRise(Keyboard::EB_Apostrophe)) {
    if (pKeyboard->IsHigh(Keyboard::EB_LeftShift)) {
      AdjustBrush(1, 0, 1);
    }  // Expand brush forward
    else if (pKeyboard->IsHigh(Keyboard::EB_LeftControl)) {
      AdjustBrush(1, 1, -1);
    }  // Shrink brush forward
    else {
      AdjustBrush(1, 1, 0);
    }  // Move brush forward
  }
...
void EldritchTools::AdjustBrush(const uint Axis, const int BaseChange,
...
void EldritchTools::FillBrush() { FillBrush(true); }

void EldritchTools::EraseBrush() { FillBrush(false, 0); }

void EldritchTools::FillBrush(const bool UsePalette,
                              const vval_t VoxelValue /*= 0*/) {
...
void EldritchTools::TraceVoxel(vidx_t& HitVoxel, vidx_t& FacingVoxel) {
  ...
  const float RayLength =
      2.0f * SqRt(static_cast<float>(Square(m_RoomSizeX) + Square(m_RoomSizeY) +
                                     Square(m_RoomSizeZ)));
  Segment TraceSegment(
      m_CameraLocation,
      m_CameraLocation + m_CameraOrientation.ToVector() * RayLength);
...
bool EldritchTools::TraceBoundPlanes(const Segment& TraceSegment,
                                     CollisionInfo& Info) {
  CollisionInfo MinInfo;

  for (uint BoundPlaneIndex = 0; BoundPlaneIndex < m_BoundPlanes.Size();
       ++BoundPlaneIndex) {
...
bool EldritchTools::Trace(const Segment& TraceSegment, CollisionInfo& Info) {
  ...
  Vector MinCorner;
  Vector MaxCorner;
  Vector::MinMax(TraceSegment.m_Point1, TraceSegment.m_Point2, MinCorner,
                 MaxCorner);
    ...
    for (vpos_t VoxelY = MinVoxelY; VoxelY <= MaxVoxelY; ++VoxelY) {
      for (vpos_t VoxelX = MinVoxelX; VoxelX <= MaxVoxelX; ++VoxelX) {
        const vidx_t VoxelIndex =
            SafeGetVoxelIndexForCoords(VoxelX, VoxelY, VoxelZ);
        if (VoxelIndex == kInvalidVoxelIndex) {
          continue;
        }
...
void EldritchTools::TryClear(const bool WithFloor, const bool WithShell) {
  const int Response = MessageBox(NULL, "Clear world?", "Clear world?",
                                  MB_YESNO | MB_TASKMODAL | MB_SETFOREGROUND);
  if (Response == IDYES) {
...
void EldritchTools::Clear(const bool WithFloor, const bool WithShell) {
  ...
  for (uint VoxelIndex = 0; VoxelIndex < m_NumVoxels; ++VoxelIndex) {
    ...
    const bool IsShell =
        WithShell && (X == 0 || X == m_RoomSizeX - 1 || Y == 0 ||
                      Y == m_RoomSizeY - 1 || Z == 0 || Z == m_RoomSizeZ - 1);
...
void EldritchTools::TryFill() {
  const int Response = MessageBox(NULL, "Fill world?", "Fill world?",
                                  MB_YESNO | MB_TASKMODAL | MB_SETFOREGROUND);
  if (Response == IDYES) {
    Fill();
  }
...
void EldritchTools::RenderCoords() {
  ...
  const float fBrushMidX = static_cast<float>(m_BrushBase[0]) +
                           static_cast<float>(m_BrushSize[0]) * 0.5f;
  const float fBrushMidY = static_cast<float>(m_BrushBase[1]) +
                           static_cast<float>(m_BrushSize[1]) * 0.5f;
  const float fBrushMidZ = static_cast<float>(m_BrushBase[2]) +
                           static_cast<float>(m_BrushSize[2]) * 0.5f;
...
void EldritchTools::RenderBrush() {
  ...
  m_Framework->GetRenderer()->DEBUGDrawBox(
      BrushMin - kBufferVector, BrushMax + kBufferVector, kBrushColor);
  m_Framework->GetRenderer()->DEBUGDrawBox(
      BrushMin + kBufferVector, BrushMax - kBufferVector, kBrushColor);
}
...
void EldritchTools::TickRender() {
  ...
  IRenderTarget* const pPrimaryRenderTarget =
      pTargetManager->GetPrimaryRenderTarget();
  ...
  FOR_EACH_MAP(SpawnerIter, m_Spawners, vidx_t, SPlacedSpawner) {
    ...
    const Vector2 ScaledLocation =
        Vector2(DisplayWidth * (NormalizedLocation.x * 0.5f + 0.5f),
                DisplayHeight * (-NormalizedLocation.y * 0.5f + 0.5f));
    const SRect Rect =
        SRect(Floor(ScaledLocation.x), Floor(ScaledLocation.y), 0.0f, 0.0f);
    const SRect ShadowRect =
        SRect(Rect.m_Left + 1.0f, Rect.m_Top + 1.0f, 0.0f, 0.0f);
    ...
    pRenderer->DEBUGDrawCross(Location, kSpawnerCrossLength,
                              kSpawnerCrossColor);
    pRenderer->DEBUGPrint(SpawnerDef.CStr(), pFont, ShadowRect,
                          ARGB_TO_COLOR(128, 8, 8, 32));
    pRenderer->DEBUGPrint(SpawnerDef.CStr(), pFont, Rect,
                          ARGB_TO_COLOR(255, 255, 255, 192));

    Angles Orientation;
    Orientation.Yaw = GetYaw(PlacedSpawner.m_Orientation);
    pRenderer->DEBUGDrawArrow(Location, Orientation, kSpawnerArrowLength,
                              kSpawnerCrossColor);
  }
  ...
  if (m_SubTool == EST_Palette) {
    ...
    for (uint PaletteVoxelIndex = 0; PaletteVoxelIndex < NumPaletteVoxels;
      ...
      if (m_Palette.Find(PaletteVoxel.m_Voxel)) {
        const Vector BoxMin(PaletteVoxel.m_BoxMin.x, 0.0f,
                            PaletteVoxel.m_BoxMin.y);
        const Vector BoxMax(PaletteVoxel.m_BoxMax.x, 0.0f,
                            PaletteVoxel.m_BoxMax.y);
  ...
  if (m_SubTool == EST_Spawners) {
    // Draw categories
    const uint NumCategoryButtons = m_SpawnerCategoryButtons.Size();
    for (uint CategoryButtonIndex = 0; CategoryButtonIndex < NumCategoryButtons;
         ++CategoryButtonIndex) {
      const SSpawnerCategoryButton& CategoryButton =
          m_SpawnerCategoryButtons[CategoryButtonIndex];
      pRenderer->AddMesh(CategoryButton.m_Mesh);

      // Draw a box around the selected category
      if (m_CurrentSpawnerCategoryIndex ==
          CategoryButton.m_SpawnerCategoryIndex) {
        const Vector BoxMin(CategoryButton.m_BoxMin.x, 0.0f,
                            CategoryButton.m_BoxMin.y);
        const Vector BoxMax(CategoryButton.m_BoxMax.x, 0.0f,
                            CategoryButton.m_BoxMax.y);
        pRenderer->DEBUGDrawBox2D(BoxMin, BoxMax, kSelectedPaletteColor);
      }
    }
    ...
    for (uint SpawnerDefIndex = 0; SpawnerDefIndex < NumSpawnerDefs;
      ...
      if (CategoryIndex == m_CurrentSpawnerCategoryIndex) {
        ...
        if (m_CurrentSpawnerDefIndex == SpawnerButton.m_SpawnerDefIndex) {
          const Vector BoxMin(SpawnerButton.m_BoxMin.x, 0.0f,
                              SpawnerButton.m_BoxMin.y);
          const Vector BoxMax(SpawnerButton.m_BoxMax.x, 0.0f,
                              SpawnerButton.m_BoxMax.y);
          pRenderer->DEBUGDrawBox2D(BoxMin, BoxMax, kSelectedPaletteColor);
        }
      }
    }
...
void EldritchTools::TryQuickSave() {
  ...
  const SimpleString Message =
      SimpleString::PrintF("Save %s?", m_CurrentMapName.CStr());
  const int Response = MessageBox(NULL, Message.CStr(), Message.CStr(),
                                  MB_YESNO | MB_TASKMODAL | MB_SETFOREGROUND);
  if (Response == IDYES) {
...
void EldritchTools::TrySave() {
  ...
  if (FileUtil::GetSaveFile(m_Framework->GetWindow()->GetHWnd(),
                            "Eldritch Room Files", "eldritchroom",
                            SaveFileName)) {
...
void EldritchTools::TryLoad() {
  ...
  if (FileUtil::GetLoadFile(m_Framework->GetWindow()->GetHWnd(),
                            "Eldritch Room Files", "eldritchroom",
                            LoadFileName)) {
...
void EldritchTools::Load(const IDataStream& Stream) {
  ...
  Reinitialize(TempRoom.m_RoomScalarX, TempRoom.m_RoomScalarY,
               TempRoom.m_RoomScalarZ);

  m_VoxelMap = TempRoom.m_VoxelMap;
  RefreshVoxelMeshes();

  m_Spawners.Clear();
  for (uint SpawnerIndex = 0; SpawnerIndex < TempRoom.m_Spawners.Size();
       ++SpawnerIndex) {
    ...
    if (m_SpawnerDefs.Find(Spawner.m_SpawnerDef, SpawnerDefIndex)) {
      ...
      const SimpleString RemappedSpawnerDef =
          ConfigManager::GetString(sRemappedDef, "", sOriginalSpawnerDef);

      STATICHASH(RemapOrientation);
      const bool RemapOrientation =
          ConfigManager::GetBool(sRemapOrientation, false, sOriginalSpawnerDef);

      STATICHASH(RemappedOrientation);
      STATIC_HASHED_STRING(North);
      const HashedString RemappedOrientation = ConfigManager::GetHash(
          sRemappedOrientation, sNorth, sOriginalSpawnerDef);

      if (m_SpawnerDefs.Find(RemappedSpawnerDef, SpawnerDefIndex)) {
        SPlacedSpawner& PlacedSpawner = m_Spawners[Spawner.m_Location];
        PlacedSpawner.m_SpawnerDef = SpawnerDefIndex;
        PlacedSpawner.m_Orientation = RemapOrientation
                                          ? GetOrientation(RemappedOrientation)
                                          : Spawner.m_Orientation;
      } else {
        WARNDESC(
            "Unknown and unremapped spawner loaded; if you resave this map, "
            "the spawners will be lost.");
      }
    }
  }
}
...
uint8 EldritchTools::GetOrientation(const HashedString& Orientation) const {
  ...
  if (Orientation == sNorth || Orientation == sN) {
    return ESO_North;
  }
  if (Orientation == sSouth || Orientation == sS) {
    return ESO_South;
  }
  if (Orientation == sEast || Orientation == sE) {
    return ESO_East;
  }
  if (Orientation == sWest || Orientation == sW) {
    return ESO_West;
  }
