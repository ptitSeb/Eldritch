...
void RodinBTNodeEldMoveTo::InitializeFromDefinition(
    const SimpleString& DefinitionName) {
  MAKEHASH(DefinitionName);

  STATICHASH(MoveTargetBlackboardKey);
  m_MoveTargetBlackboardKey = ConfigManager::GetHash(
      sMoveTargetBlackboardKey, HashedString::NullString, sDefinitionName);

  STATICHASH(Wander);
  m_Wander = ConfigManager::GetBool(sWander, false, sDefinitionName);

  STATICHASH(WanderTargetDistance);
  m_WanderTargetDistance =
      ConfigManager::GetFloat(sWanderTargetDistance, false, sDefinitionName);

  STATICHASH(ReachedThresholdMin);
  m_ReachedThresholdMin =
      ConfigManager::GetFloat(sReachedThresholdMin, 0.0f, sDefinitionName);

  STATICHASH(ReachedThresholdMax);
  m_ReachedThresholdMax =
      ConfigManager::GetFloat(sReachedThresholdMax, 0.0f, sDefinitionName);

  STATICHASH(DeflectionRadius);
  m_FlyingDeflectionRadius =
      ConfigManager::GetFloat(sDeflectionRadius, 0.0f, sDefinitionName);
}

void RodinBTNodeEldMoveTo::OnStart() { m_MoveState = EMTS_Begin; }
...
RodinBTNode::ETickStatus RodinBTNodeEldMoveTo::Tick(float DeltaTime) {
  ...
  if (m_MoveState == EMTS_Begin) {
    if (m_Wander) {
      pAIMotion->SetReachedThreshold(m_ReachedThresholdMin,
                                     m_ReachedThresholdMax);
      ...
      const WBEvent::EType TargetType =
          pAIBlackboard->GetType(m_MoveTargetBlackboardKey);

      if (TargetType == WBEvent::EWBEPT_Vector) {
        const Vector MoveTarget =
            pAIBlackboard->GetVector(m_MoveTargetBlackboardKey);
        ...
        WBEntity* const pMoveTargetEntity =
            pAIBlackboard->GetEntity(m_MoveTargetBlackboardKey);
        if (!pMoveTargetEntity) {
          return ETS_Fail;
        }

        //#if BUILD_DEV
        //				PRINTF( "RodinBTNodeEldMoveTo: %s: %s moving
        //to entity %s\n",
        //					GetName().CStr(),
