...
void EldritchParticles::GetCachedConfig(const SimpleString& DefinitionName) {
  ...
  Map<HashedString, SParticleSystemParams>::Iterator CacheIter =
      sm_ParamCache.Search(HashedDefinitionName);
  ...
  if (CacheIter.IsValid()) {
    ...
    m_Params.m_SystemLifetime = ConfigManager::GetInheritedFloat(
        sSystemLifetime, 0.0f, sDefinitionName);

    STATICHASH(SpawnRate);
    m_Params.m_SpawnRate =
        ConfigManager::GetInheritedFloat(sSpawnRate, 0.0f, sDefinitionName);
    m_Params.m_InvSpawnRate =
        (m_Params.m_SpawnRate > 0.0f) ? (1.0f / m_Params.m_SpawnRate) : 0.0f;

    STATICHASH(MaxParticles);
    m_Params.m_MaxParticles =
        (uint)ConfigManager::GetInheritedInt(sMaxParticles, 1, sDefinitionName);

    STATICHASH(TextureMap);
    m_Params.m_TextureMapName = ConfigManager::GetInheritedString(
        sTextureMap, DEFAULT_TEXTURE, sDefinitionName);

    STATICHASH(ImmediateSpawnMax);
    m_Params.m_ImmediateSpawnMax = ConfigManager::GetInheritedBool(
        sImmediateSpawnMax, false, sDefinitionName);

    STATICHASH(AdditiveDraw);
    m_Params.m_AdditiveDraw =
        ConfigManager::GetInheritedBool(sAdditiveDraw, false, sDefinitionName);

    STATICHASH(AlphaTestDraw);
    m_Params.m_AlphaTestDraw =
        ConfigManager::GetInheritedBool(sAlphaTestDraw, false, sDefinitionName);

    STATICHASH(AlwaysDraw);
    m_Params.m_AlwaysDraw =
        ConfigManager::GetInheritedBool(sAlwaysDraw, false, sDefinitionName);

    STATICHASH(ForegroundDraw);
    m_Params.m_ForegroundDraw = ConfigManager::GetInheritedBool(
        sForegroundDraw, false, sDefinitionName);

    STATICHASH(Collision);
    m_Params.m_Collision =
        ConfigManager::GetInheritedBool(sCollision, false, sDefinitionName);

    STATICHASH(SpawnOffsetOSX);
    m_Params.m_SpawnOffsetOS.x = ConfigManager::GetInheritedFloat(
        sSpawnOffsetOSX, 0.0f, sDefinitionName);

    STATICHASH(SpawnOffsetOSY);
    m_Params.m_SpawnOffsetOS.y = ConfigManager::GetInheritedFloat(
        sSpawnOffsetOSY, 0.0f, sDefinitionName);

    STATICHASH(SpawnOffsetOSZ);
    m_Params.m_SpawnOffsetOS.z = ConfigManager::GetInheritedFloat(
        sSpawnOffsetOSZ, 0.0f, sDefinitionName);

    STATICHASH(SpawnOffsetWSX);
    m_Params.m_SpawnOffsetWS.x = ConfigManager::GetInheritedFloat(
        sSpawnOffsetWSX, 0.0f, sDefinitionName);

    STATICHASH(SpawnOffsetWSY);
    m_Params.m_SpawnOffsetWS.y = ConfigManager::GetInheritedFloat(
        sSpawnOffsetWSY, 0.0f, sDefinitionName);

    STATICHASH(SpawnOffsetWSZ);
    m_Params.m_SpawnOffsetWS.z = ConfigManager::GetInheritedFloat(
        sSpawnOffsetWSZ, 0.0f, sDefinitionName);

    STATICHASH(SpawnExtentsX);
    m_Params.m_SpawnExtents.x =
        ConfigManager::GetInheritedFloat(sSpawnExtentsX, 0.0f, sDefinitionName);

    STATICHASH(SpawnExtentsY);
    m_Params.m_SpawnExtents.y =
        ConfigManager::GetInheritedFloat(sSpawnExtentsY, 0.0f, sDefinitionName);

    STATICHASH(SpawnExtentsZ);
    m_Params.m_SpawnExtents.z =
        ConfigManager::GetInheritedFloat(sSpawnExtentsZ, 0.0f, sDefinitionName);

    STATICHASH(InitialVelocityOSMinX);
    m_Params.m_InitialVelocityOSMin.x = ConfigManager::GetInheritedFloat(
        sInitialVelocityOSMinX, 0.0f, sDefinitionName);

    STATICHASH(InitialVelocityOSMinY);
    m_Params.m_InitialVelocityOSMin.y = ConfigManager::GetInheritedFloat(
        sInitialVelocityOSMinY, 0.0f, sDefinitionName);

    STATICHASH(InitialVelocityOSMinZ);
    m_Params.m_InitialVelocityOSMin.z = ConfigManager::GetInheritedFloat(
        sInitialVelocityOSMinZ, 0.0f, sDefinitionName);

    STATICHASH(InitialVelocityOSMaxX);
    m_Params.m_InitialVelocityOSMax.x = ConfigManager::GetInheritedFloat(
        sInitialVelocityOSMaxX, 0.0f, sDefinitionName);

    STATICHASH(InitialVelocityOSMaxY);
    m_Params.m_InitialVelocityOSMax.y = ConfigManager::GetInheritedFloat(
        sInitialVelocityOSMaxY, 0.0f, sDefinitionName);

    STATICHASH(InitialVelocityOSMaxZ);
    m_Params.m_InitialVelocityOSMax.z = ConfigManager::GetInheritedFloat(
        sInitialVelocityOSMaxZ, 0.0f, sDefinitionName);

    STATICHASH(Elasticity);
    m_Params.m_Elasticity =
        ConfigManager::GetInheritedFloat(sElasticity, 0.0f, sDefinitionName);

    STATICHASH(LifetimeMin);
    m_Params.m_LifetimeMin =
        ConfigManager::GetInheritedFloat(sLifetimeMin, 0.0f, sDefinitionName);

    STATICHASH(LifetimeMax);
    m_Params.m_LifetimeMax =
        ConfigManager::GetInheritedFloat(sLifetimeMax, 0.0f, sDefinitionName);

    STATICHASH(AccelerationWSX);
    m_Params.m_AccelerationWS.x = ConfigManager::GetInheritedFloat(
        sAccelerationWSX, 0.0f, sDefinitionName);

    STATICHASH(AccelerationWSY);
    m_Params.m_AccelerationWS.y = ConfigManager::GetInheritedFloat(
        sAccelerationWSY, 0.0f, sDefinitionName);

    STATICHASH(AccelerationWSZ);
    m_Params.m_AccelerationWS.z = ConfigManager::GetInheritedFloat(
        sAccelerationWSZ, 0.0f, sDefinitionName);

    STATICHASH(ViewBoundRadius);
    m_Params.m_ViewBoundRadius = ConfigManager::GetInheritedFloat(
        sViewBoundRadius, 1.0f, sDefinitionName);

    STATICHASH(InitialSizeMin);
    m_Params.m_InitialSizeMin = ConfigManager::GetInheritedFloat(
        sInitialSizeMin, 1.0f, sDefinitionName);

    STATICHASH(InitialSizeMax);
    m_Params.m_InitialSizeMax = ConfigManager::GetInheritedFloat(
        sInitialSizeMax, 1.0f, sDefinitionName);

    STATICHASH(SizeVelocityMin);
    m_Params.m_SizeVelocityMin = ConfigManager::GetInheritedFloat(
        sSizeVelocityMin, 0.0f, sDefinitionName);

    STATICHASH(SizeVelocityMax);
    m_Params.m_SizeVelocityMax = ConfigManager::GetInheritedFloat(
        sSizeVelocityMax, 0.0f, sDefinitionName);

    STATICHASH(LinkedRGB);
    m_Params.m_LinkedRGB =
        ConfigManager::GetInheritedBool(sLinkedRGB, false, sDefinitionName);

    STATICHASH(ColorMinR);
    m_Params.m_ColorMin.x =
        ConfigManager::GetInheritedFloat(sColorMinR, 1.0f, sDefinitionName);

    STATICHASH(ColorMinG);
    m_Params.m_ColorMin.y =
        ConfigManager::GetInheritedFloat(sColorMinG, 1.0f, sDefinitionName);

    STATICHASH(ColorMinB);
    m_Params.m_ColorMin.z =
        ConfigManager::GetInheritedFloat(sColorMinB, 1.0f, sDefinitionName);

    STATICHASH(ColorMinA);
    m_Params.m_ColorMin.w =
        ConfigManager::GetInheritedFloat(sColorMinA, 1.0f, sDefinitionName);

    STATICHASH(ColorMaxR);
    m_Params.m_ColorMax.x =
        ConfigManager::GetInheritedFloat(sColorMaxR, 1.0f, sDefinitionName);

    STATICHASH(ColorMaxG);
    m_Params.m_ColorMax.y =
        ConfigManager::GetInheritedFloat(sColorMaxG, 1.0f, sDefinitionName);

    STATICHASH(ColorMaxB);
    m_Params.m_ColorMax.z =
        ConfigManager::GetInheritedFloat(sColorMaxB, 1.0f, sDefinitionName);

    STATICHASH(ColorMaxA);
    m_Params.m_ColorMax.w =
        ConfigManager::GetInheritedFloat(sColorMaxA, 1.0f, sDefinitionName);

    STATICHASH(InitialTrace);
    m_Params.m_InitialTrace =
        ConfigManager::GetInheritedBool(sInitialTrace, false, sDefinitionName);

    STATICHASH(InitialTraceOffsetOSX);
    m_Params.m_InitialTraceOffsetOS.x = ConfigManager::GetInheritedFloat(
        sInitialTraceOffsetOSX, 0.0f, sDefinitionName);

    STATICHASH(InitialTraceOffsetOSY);
    m_Params.m_InitialTraceOffsetOS.y = ConfigManager::GetInheritedFloat(
        sInitialTraceOffsetOSY, 0.0f, sDefinitionName);

    STATICHASH(InitialTraceOffsetOSZ);
    m_Params.m_InitialTraceOffsetOS.z = ConfigManager::GetInheritedFloat(
        sInitialTraceOffsetOSZ, 0.0f, sDefinitionName);
    ...
    if (MaxParticles < UpperBound) {
      WARNDESC(
          "Particle system has MaxParticles less than LifetimeMax * SpawnRate. "
          "System may starve.");
    }
#endif
...
/*static*/ void EldritchParticles::FlushConfigCache() { sm_ParamCache.Clear(); }

void EldritchParticles::InitializeFromDefinition(
    const SimpleString& DefinitionName) {
  GetCachedConfig(DefinitionName);

  IRenderer* const pRenderer = EldritchFramework::GetInstance()->GetRenderer();
  m_TextureMap =
      pRenderer->GetTextureManager()->GetTexture(m_Params.m_TextureMapName);
...
void EldritchParticles::Tick(const float DeltaTime) {
  ...
  bool ShouldImmediateSpawnMax =
      m_Params.m_ImmediateSpawnMax && !m_HasImmediateSpawnedMax;
  ...
  if (ShouldImmediateSpawnMax || (!IsExpired() && m_RenderedLastFrame)) {
    ...
    NumParticlesToSpawn =
        Min(NumParticlesToSpawn, m_Params.m_MaxParticles - GetNumParticles());
...
void EldritchParticles::SpawnParticles(uint NumParticlesToSpawn) {
  ...
  for (uint i = 0; i < NumParticlesToSpawn; ++i) {
    ...
    SParticle Particle;
    Particle.m_Lifetime =
        Math::Random(m_Params.m_LifetimeMin, m_Params.m_LifetimeMax);
    Particle.m_Size =
        Math::Random(m_Params.m_InitialSizeMin, m_Params.m_InitialSizeMax);
    Particle.m_SizeVelocity =
        Math::Random(m_Params.m_SizeVelocityMin, m_Params.m_SizeVelocityMax);
    ...
    Particle.m_Velocity = Math::Random(m_Params.m_InitialVelocityOSMin,
                                       m_Params.m_InitialVelocityOSMax) *
                          RotationMatrix;
    ...
    if (m_Params.m_InitialTrace) {
      const Vector OffsetLocation =
          Particle.m_Location +
          m_Params.m_InitialTraceOffsetOS * RotationMatrix;

      EldritchWorld* const pWorld =
          EldritchFramework::GetInstance()->GetWorld();
...
Vector EldritchParticles::GetParticleSpawnLocationOffset() const {
  ...
  const Vector SpawnExtents =
      Math::Random(-m_Params.m_SpawnExtents, m_Params.m_SpawnExtents);
...
void EldritchParticles::TickParticles(const float DeltaTime) {
  if (IsUnsorted()) {
    ...
    FOR_EACH_LIST_NOINCR(ParticleIter, m_ParticlesList,
                         EldritchParticles::SParticle) {
...
bool EldritchParticles::TickParticle(SParticle& Particle, float DeltaTime) {
  ...
  Particle.m_Size =
      Max(0.0f, Particle.m_Size + Particle.m_SizeVelocity * DeltaTime);
  Particle.m_Velocity +=
      TotalAcceleration * DeltaTime;  // Integrating velocity before position is
                                      // Euler-Cromer, stabler than Euler
  ...
  if (m_Params.m_Collision) {
    ...
    const Segment TraceSegment =
        Segment(Particle.m_Location, Particle.m_Location + Offset);
    CollisionInfo Info;
    Info.m_CollideWorld = true;
    ...
    if (pWorld->Trace(TraceSegment, Info)) {
      ...
      Vector NormalVelocity =
          Particle.m_Velocity.ProjectionOnto(Info.m_Plane.m_Normal);
      Vector TangentialVelocity =
          Info.m_Plane.ProjectVector(Particle.m_Velocity);
      Particle.m_Velocity =
          TangentialVelocity + (m_Params.m_Elasticity * -NormalVelocity);
    }
  }
...
void EldritchParticles::CreateMesh() {
  ...
  if (m_Params.m_MaxParticles == 0) {
    ...
    uint VertexSignature =
        VD_POSITIONS | VD_UVS | VD_FLOATCOLORS_SM2 | VD_NORMALS;
    IVertexDeclaration* VertexDeclaration =
        pRenderer->GetVertexDeclaration(VertexSignature);
    ...
    if (m_Params.m_AdditiveDraw) {
      m_DynamicMesh->SetMaterialDefinition("Material_ParticlesAdditive",
                                           pRenderer);
    } else if (m_Params.m_AlphaTestDraw) {
      m_DynamicMesh->SetMaterialDefinition("Material_ParticlesAlphaTest",
                                           pRenderer);
    } else {
      m_DynamicMesh->SetMaterialDefinition("Material_ParticlesAlpha",
                                           pRenderer);
    }
...
void EldritchParticles::UpdateMesh() {
  ...
  // Additive and alpha-tested particles draw the same regardless of order, so
  ...
  // Zero roll so that particles do not roll with player's view--that looks
  // especially silly on foliage.
  ...
  if (IsUnsorted()) {
    ...
    for (uint ParticleIndex = 0; ParticleIndex < NumParticles;
         ++ParticleIndex) {
  ...
  Vector BoundRadius(m_Params.m_ViewBoundRadius, m_Params.m_ViewBoundRadius,
                     m_Params.m_ViewBoundRadius);
  m_DynamicMesh->m_AABB =
      AABB(m_SystemLocation - BoundRadius, m_SystemLocation + BoundRadius);
}

inline void EldritchParticles::AddParticleToMesh(
    const uint Index, const SParticle& Particle,
    const SParticleMeshParams& MeshParams) {
  uint j = Index * 4;
  uint k = j + 1;
  uint l = k + 1;
  uint m = l + 1;
  ...
  const Matrix ParticleMatrix =
      ParticleRotationMatrix * Matrix::CreateTranslation(Particle.m_Location) *
      MeshParams.m_OriginMatrix;  // Translate particles to be relative to
                                  // origin (then set mesh location)
...
/*static*/ void EldritchParticles::DeviceResetCallback(void* pVoid,
                                                       IVertexBuffer* pBuffer) {
  EldritchParticles* const pParticleSystem =
      static_cast<EldritchParticles*>(pVoid);
