...
void WBPEEldHardSwitch::InitializeFromDefinition(
    const SimpleString& DefinitionName) {
  STATICHASH(Normal);
  STATICHASH(Hard);
  MAKEHASH(DefinitionName);

  m_NormalInput = WBParamEvaluatorFactory::Create(
      ConfigManager::GetString(sNormal, "", sDefinitionName));
  m_HardInput = WBParamEvaluatorFactory::Create(
      ConfigManager::GetString(sHard, "", sDefinitionName));

  ASSERT(m_NormalInput);
  ASSERT(m_HardInput);
}

void WBPEEldHardSwitch::Evaluate(
