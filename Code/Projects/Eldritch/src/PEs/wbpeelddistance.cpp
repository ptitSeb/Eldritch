...
/*virtual*/ void WBPEEldDistance::Evaluate(
  ...
  if (ValueA.m_Type != WBParamEvaluator::EPT_Entity ||
      ValueB.m_Type != WBParamEvaluator::EPT_Entity) {
    return;
  }
  ...
  WBCompEldTransform* const pTransformA =
      pEntityA->GetTransformComponent<WBCompEldTransform>();
  WBCompEldTransform* const pTransformB =
      pEntityB->GetTransformComponent<WBCompEldTransform>();

  ASSERT(pTransformA);
  ASSERT(pTransformB);

  const float Distance =
      (pTransformB->GetLocation() - pTransformA->GetLocation()).Length();
