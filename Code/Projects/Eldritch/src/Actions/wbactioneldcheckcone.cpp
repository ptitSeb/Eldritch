...
WBActionEldCheckCone::WBActionEldCheckCone()
    : m_ConeCosTheta(0.0f), m_ConeLengthSq(0.0f), m_CheckTag() {}

WBActionEldCheckCone::~WBActionEldCheckCone() {}

/*virtual*/ void WBActionEldCheckCone::InitializeFromDefinition(
  ...
  m_ConeCosTheta = Cos(DEGREES_TO_RADIANS(
      ConfigManager::GetFloat(sConeAngle, 0.0f, sDefinitionName)));

  STATICHASH(ConeLength);
  m_ConeLengthSq =
      Square(ConfigManager::GetFloat(sConeLength, 0.0f, sDefinitionName));

  STATICHASH(CheckTag);
  m_CheckTag = ConfigManager::GetHash(sCheckTag, HashedString::NullString,
                                      sDefinitionName);
}

// Borrowed (with simplifications) from
// WBActionEldSpawnEntity::GetSpawnTransform. Maybe unify?
void WBActionEldCheckCone::GetConeTransform(Vector& OutLocation,
...
/*virtual*/ void WBActionEldCheckCone::Execute() {
  ...
  const Array<WBCompEldCollision*>* const pCollidables =
  ...
  for (uint CollidableIndex = 0; CollidableIndex < NumCollidables;
       ++CollidableIndex) {
    ...
    const Vector CollidableLocation =
        CollidableBounds.GetClosestPoint(ConeSource);
    ...
    // Check angle to entity's nearest point. (Skip check if distance to nearest
    // point is zero.)
    // NOTE: This check isn't really sufficient; if ConeDirection is tangential
    // to the surface,
    // the nearest point will be perpendicular to ConeDirection even though some
    // of the surface
    ...
    if (pWorld->Trace(TraceSegment, Info) &&
        Info.m_HitEntity != pCollidableEntity) {
      continue;
    }

    // All checks passed, notify this entity that the cone check hit that
    // entity.
