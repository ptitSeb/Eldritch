...
WBActionEldCheckSphere::WBActionEldCheckSphere()
    : m_RadiusSq(0.0f), m_CheckTag() {}

WBActionEldCheckSphere::~WBActionEldCheckSphere() {}

/*virtual*/ void WBActionEldCheckSphere::InitializeFromDefinition(
...
void WBActionEldCheckSphere::GetSphereTransform(Vector& OutLocation) const {
  WBEntity* const
      pEntity = GetEntity();  // GetOwner();	(Unlike cone and line checks,
                              // I'm not using this on a carried weapon)
  DEVASSERT(pEntity);
...
/*virtual*/ void WBActionEldCheckSphere::Execute() {
  ...
  WBEntity* const pOwnerEntity =
      pEntity;  // GetOwner();	(Unlike cone and line checks, I'm not using
                // this on a carried weapon)
  ...
  for (uint CollidableIndex = 0; CollidableIndex < NumCollidables;
    ...
    const Vector CollidableLocation =
        CollidableBounds.GetClosestPoint(SphereSource);
    ...
    // All checks passed, notify this entity that the sphere check hit that
    // entity.
