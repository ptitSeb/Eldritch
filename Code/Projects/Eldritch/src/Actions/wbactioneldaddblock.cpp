...
WBActionEldAddBlock::WBActionEldAddBlock() : m_SourcePE(), m_VoxelValue(0) {}

WBActionEldAddBlock::~WBActionEldAddBlock() {}

/*virtual*/ void WBActionEldAddBlock::InitializeFromDefinition(
  ...
  STATICHASH(Source);
  const SimpleString SourceDef =
      ConfigManager::GetString(sSource, "", sDefinitionName);
  m_SourcePE.InitializeFromDefinition(SourceDef);

  STATICHASH(VoxelValue);
  m_VoxelValue =
      static_cast<byte>(ConfigManager::GetInt(sVoxelValue, 0, sDefinitionName));
}
...
/*virtual*/ void WBActionEldAddBlock::Execute() {
  ...
  WBCompEldTransform* const pTransform =
      pSourceEntity->GetTransformComponent<WBCompEldTransform>();
  if (!pTransform) {
    return;
  }
  ...
  WB_SET_AUTO(OnBlockAdded, Vector, VoxelLocation,
              pWorld->GetVoxelCenter(HitLoc));
  WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(), OnBlockAdded,
                    pEntity);
}