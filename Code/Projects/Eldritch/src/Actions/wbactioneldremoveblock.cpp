...
WBActionEldRemoveBlock::WBActionEldRemoveBlock() : m_SourcePE() {}

WBActionEldRemoveBlock::~WBActionEldRemoveBlock() {}

/*virtual*/ void WBActionEldRemoveBlock::InitializeFromDefinition(
...
void WBActionEldRemoveBlock::TraceFromSourceEntity(
    WBEntity* const pSourceEntity) const {
  if (!pSourceEntity) {
    return;
  }
...
void WBActionEldRemoveBlock::RemoveAtSourceVector(
    const Vector& SourceLocation) const {
  EldritchWorld* const pWorld = EldritchFramework::GetInstance()->GetWorld();
  ASSERT(pWorld);
