...
WBActionEldSetLight::WBActionEldSetLight() : m_AddLight(false) {}

WBActionEldSetLight::~WBActionEldSetLight() {}

/*virtual*/ void WBActionEldSetLight::InitializeFromDefinition(
...
/*virtual*/ void WBActionEldSetLight::Execute() {
  ...
  if (pEntity) {
    if (m_AddLight) {
      // Queue because if we do this during spawning, the world won't accept a
      // light yet.
      WB_MAKE_EVENT(AddLight, pEntity);
      WB_QUEUE_EVENT(WBWorld::GetInstance()->GetEventManager(), AddLight,
                     pEntity);
    } else {
      WB_MAKE_EVENT(RemoveLight, pEntity);
      WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(), RemoveLight,
                        pEntity);
    }
  }
}