...
/*virtual*/ void WBActionEldSpawnEntity::InitializeFromDefinition(
  ...
  STATICHASH(EntityPE);
  const SimpleString EntityDefPE =
  ...
  const SimpleString LocationOverridePE =
      ConfigManager::GetString(sLocationOverridePE, "", sDefinitionName);
  m_LocationOverridePE.InitializeFromDefinition(LocationOverridePE);

  STATICHASH(OrientationOverridePE);
  const SimpleString OrientationOverridePE =
      ConfigManager::GetString(sOrientationOverridePE, "", sDefinitionName);
  ...
  m_SpawnImpulseZ =
      ConfigManager::GetFloat(sSpawnImpulseZ, 0.0f, sDefinitionName);

  STATICHASH(SpawnImpulse);
  m_SpawnImpulse =
      ConfigManager::GetFloat(sSpawnImpulse, 0.0f, sDefinitionName);

  STATICHASH(SpawnImpulsePE);
  const SimpleString SpawnImpulsePE =
      ConfigManager::GetString(sSpawnImpulsePE, "", sDefinitionName);
  m_SpawnImpulsePE.InitializeFromDefinition(SpawnImpulsePE);

  STATICHASH(SpawnOffsetZ);
  m_SpawnOffsetZ =
      ConfigManager::GetFloat(sSpawnOffsetZ, 0.0f, sDefinitionName);
}
...
/*virtual*/ void WBActionEldSpawnEntity::Execute() {
  ...
  const SimpleString EntityDef =
      (m_EntityDefPE.GetType() == WBParamEvaluator::EPT_String)
          ? m_EntityDefPE.GetString()
          : m_EntityDef;

  WBEntity* const pSpawnedEntity =
      WBWorld::GetInstance()->CreateEntity(EntityDef);

  WBCompEldTransform* const pSpawnedTransform =
      pSpawnedEntity->GetTransformComponent<WBCompEldTransform>();
  ...
  GetSpawnTransform(pSpawnedEntity, SpawnLocation, SpawnImpulse,
                    SpawnOrientation);
  ...
  WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(),
                    OnInitialOrientationSet, pSpawnedEntity);
  ...
  WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(),
                    OnSpawnedEntityAction, GetEntity());
}

// Borrowed (with simplifications) from WBCompEldItem spawn drop code. Maybe
// unify?
void WBActionEldSpawnEntity::GetSpawnTransform(WBEntity* const pSpawnedEntity,
                                               Vector& OutLocation,
                                               Vector& OutImpulse,
                                               Angles& OutOrientation) {
  WBEntity* const pEntity = GetOwner();
  DEVASSERT(pEntity);
  ...
  // Get location
  {
    ...
    WBCompEldCollision* const pCollision =
        GET_WBCOMP(pSpawnedEntity, EldCollision);
    if (pCollision) {
      CollisionInfo Info;
      Info.m_CollideWorld = true;
      Info.m_CollideEntities = true;
      Info.m_CollidingEntity = pEntity;  // Using the owner, not the spawned
                                         // entity (which should be at origin at
                                         // the moment)
  ...
  // Get impulse
  {
    ...
    const float SpawnImpulseSize =
        (m_SpawnImpulsePE.GetType() == WBParamEvaluator::EPT_Float)
            ? m_SpawnImpulsePE.GetFloat()
            : m_SpawnImpulse;
