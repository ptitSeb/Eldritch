...
/*virtual*/ void WBActionEldGoToLevel::InitializeFromDefinition(
  ...
  m_GoToNextLevel =
      ConfigManager::GetBool(sGoToNextLevel, false, sDefinitionName);

  STATICHASH(GoToPrevLevel);
  m_GoToPrevLevel =
      ConfigManager::GetBool(sGoToPrevLevel, false, sDefinitionName);
  ...
  STATICHASH(WorldDef);
  m_WorldDef =
      ConfigManager::GetHash(sWorldDef, HashedString::NullString, sm_Level);
}
