...
WBActionEldGiveItem::WBActionEldGiveItem()
    : m_ItemDef(), m_ItemDefPE(), m_GiveToPE() {}

WBActionEldGiveItem::~WBActionEldGiveItem() {}

/*virtual*/ void WBActionEldGiveItem::InitializeFromDefinition(
  ...
  STATICHASH(ItemPE);
  const SimpleString ItemPE =
      ConfigManager::GetString(sItemPE, "", sDefinitionName);
  m_ItemDefPE.InitializeFromDefinition(ItemPE);

  STATICHASH(GiveTo);
  const SimpleString GiveToDef =
      ConfigManager::GetString(sGiveTo, "", sDefinitionName);
...
/*virtual*/ void WBActionEldGiveItem::Execute() {
  ...
  const SimpleString ItemDef =
      (m_ItemDefPE.GetType() == WBParamEvaluator::EPT_String)
          ? m_ItemDefPE.GetString()
          : m_ItemDef;
