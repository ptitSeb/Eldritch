...
EldritchNav::EPathStatus EldritchNav::FindPath(const SPathfindingParams& Params,
                                               TPath& OutPath) {
  DEVASSERT(m_World);
...
EldritchNav::EPathStatus EldritchNav::Wander(const SPathfindingParams& Params,
                                             TPath& OutPath) {
  DEVASSERT(m_World);
...
EldritchNav::EPathStatus EldritchNav::FindPath() {
  ...
  // Starting node doesn't need a floor. This fixes cases where the pathing
  // entity is standing
  // slightly off the edge of a block; its first valid move is back onto that
  // block.
  m_Params.m_StartVoxel = m_World->GetIndex(m_Params.m_Start);
  if (!IsAccessible(m_Params.m_StartVoxel, m_Params.m_Height, false)) {
    if (m_Params.m_Verbose) {
      PRINTF("No path found: Start location was inaccessible.\n");
    }
  ...
  if (IsSearch()) {
    ...
    if (!IsAccessible(m_Params.m_DestinationVoxel, m_Params.m_Height, false)) {
      if (m_Params.m_Verbose) {
        PRINTF("No path found: Destination location was inaccessible.\n");
      }
      return EPS_NoPathFound;
    }
  }
  ...
  if (IsSearch()) {
    if (m_Params.m_StartVoxel == m_Params.m_DestinationVoxel) {
      if (m_Params.m_Verbose) {
        PRINTF("Path found: Already at destination.");
      }
  ...
  StartNode.m_G = 0.0f;
  StartNode.m_H =
      IsSearch() ? GetCost(m_Params.m_StartVoxel, m_Params.m_DestinationVoxel)
                 : 0.0f;
...
void EldritchNav::InitializeExpandDirections() {
  ...
  m_ExpandDirections.PushBack(
      SExpandDirection(StrideY, kLevelCost, false, false));  // North
  m_ExpandDirections.PushBack(
      SExpandDirection(-StrideY, kLevelCost, false, false));  // South
  m_ExpandDirections.PushBack(
      SExpandDirection(StrideX, kLevelCost, false, false));  // East
  m_ExpandDirections.PushBack(
      SExpandDirection(-StrideX, kLevelCost, false, false));  // West

  if (IsWalking()) {
    m_ExpandDirections.PushBack(SExpandDirection(StrideY - StrideZ, kUpCost,
                                                 false, true));  // North down
    m_ExpandDirections.PushBack(SExpandDirection(-StrideY - StrideZ, kUpCost,
                                                 false, true));  // South down
    m_ExpandDirections.PushBack(SExpandDirection(StrideX - StrideZ, kUpCost,
                                                 false, true));  // East down
    m_ExpandDirections.PushBack(SExpandDirection(-StrideX - StrideZ, kUpCost,
                                                 false, true));  // West down

    if (m_Params.m_CanJump) {
      m_ExpandDirections.PushBack(SExpandDirection(StrideY + StrideZ, kDownCost,
                                                   true, false));  // North up
      m_ExpandDirections.PushBack(SExpandDirection(
          -StrideY + StrideZ, kDownCost, true, false));  // South up
      m_ExpandDirections.PushBack(SExpandDirection(StrideX + StrideZ, kDownCost,
                                                   true, false));  // East up
      m_ExpandDirections.PushBack(SExpandDirection(
          -StrideX + StrideZ, kDownCost, true, false));  // West up
    }
  } else if (IsFlying()) {
    m_ExpandDirections.PushBack(
        SExpandDirection(StrideZ, kLevelCost, true, false));  // Up
    m_ExpandDirections.PushBack(
        SExpandDirection(-StrideZ, kLevelCost, false, true));  // Down
  }
}

void EldritchNav::RecursiveBuildPath(const vidx_t StepVoxelIndex,
                                     const uint NumSteps) {
  DEVASSERT(m_Path);
  TPath& Path = *m_Path;
  ...
  if (PathNode.m_Backlink == kInvalidVoxelIndex) {
    // Recursion terminating base case; this should be our start node. Allocate
    // the array once.
...
EldritchNav::EPathStatus EldritchNav::DoAStarSearch() {
  ...
  while (m_OpenSet.Size() && Step++ < m_Params.m_MaxSteps) {
    ...
    if ((IsSearch() && OpenNodeIndex == m_Params.m_DestinationVoxel) ||
      ...
      for (uint DirectionIndex = 0; DirectionIndex < NumExpandDirections;
           ++DirectionIndex) {
        TryExpand(m_ExpandDirections[DirectionIndex], ClosedNodeIndex,
                  ClosedNode);
      }
    }
  }
  ...
  if (m_Params.m_UsePartialPath) {
    if (m_Params.m_Verbose) {
      PRINTF("Pathfinding finished with partial path.\n");
    }
    BuildPath(GetBestPartialDestination());
    return EPS_PartialPathFound;
  } else {
    if (m_Params.m_Verbose) {
      PRINTF("Pathfinding finished with no path found.\n");
    }
    return EPS_NoPathFound;
  }
}

void EldritchNav::TryExpand(const EldritchNav::SExpandDirection& Direction,
                            const vidx_t ClosedIndex,
                            const SPathNode& ClosedNode) {
  // Upward path requires extra clearance locally.
  // No need to recheck floor here, we've already closed this node and we're
  // only concerned with the extra clearance.
  if (Direction.m_IsUp &&
      !IsAccessible(ClosedIndex, m_Params.m_Height + 1, false)) {
    return;
  }

  const vidx_t ExpandVoxel = ClosedIndex + Direction.m_Offset;
  const uint ExpandHeight =
      Direction.m_IsDown ? (m_Params.m_Height + 1) : m_Params.m_Height;
...
void EldritchNav::Expand(const vidx_t ExpandVoxel, const vidx_t ClosedIndex,
                         const SPathNode& ClosedNode, const float Cost) {
  ...
  if (ExpandNode.m_Status == SPathNode::ES_Closed) {
    ...
    // For wandering, add a small random weight so we don't bias based on order
    // of directions.
    ExpandNode.m_H = IsSearch()
                         ? GetCost(ExpandVoxel, m_Params.m_DestinationVoxel)
                         : Math::Random(0.0f, 0.1f);
  }
}
...
vidx_t EldritchNav::GetBestPartialDestination() const {
  ...
  for (uint ClosedSetIndex = 0; ClosedSetIndex < ClosedSetSize;
       ++ClosedSetIndex) {
...
bool EldritchNav::IsAccessible(const vidx_t Voxel, const uint Height,
  ...
  for (uint HeightIndex = 0; HeightIndex < Height; ++HeightIndex) {
    const byte WorldValue =
        SafeGetWorldValue(Voxel + HeightIndex * FloorStride);
    if (WorldValue == 0 ||
        (WorldValue == kDoorValue && m_Params.m_CanOpenDoors) ||
        (WorldValue == kLockedDoorValue && m_Params.m_CanOpenDoors &&
         m_Params.m_CanUnlockDoors)) {
      // This is ok.
    } else {
...
inline const EldritchNav::SPathNode& EldritchNav::GetPathNode(
    const vidx_t Voxel) const {
  return m_PathNodes[Voxel];
}
...
inline byte EldritchNav::SafeGetWorldValue(const vidx_t VoxelIndex) const {
  return m_World->IsValidIndex(VoxelIndex) ? GetWorldValue(VoxelIndex)
                                           : kInvalidWorldValue;
}
...
void EldritchNav::UpdateWorldFromChangedVoxels(
...
void EldritchNav::UpdateWorldFromEntity(WBCompEldCollision* const pCollision,
                                        const bool Add) {
  PROFILE_FUNCTION;
  ...
  vpos_t MinVoxelX =
      Clamp(static_cast<vpos_t>(EntityBounds.m_Min.x), 0, NumVoxelsX - 1);
  vpos_t MinVoxelY =
      Clamp(static_cast<vpos_t>(EntityBounds.m_Min.y), 0, NumVoxelsY - 1);
  vpos_t MinVoxelZ =
      Clamp(static_cast<vpos_t>(EntityBounds.m_Min.z), 0, NumVoxelsZ - 1);
  vpos_t MaxVoxelX =
      Clamp(static_cast<vpos_t>(EntityBounds.m_Max.x), 0, NumVoxelsX - 1);
  vpos_t MaxVoxelY =
      Clamp(static_cast<vpos_t>(EntityBounds.m_Max.y), 0, NumVoxelsY - 1);
  vpos_t MaxVoxelZ =
      Clamp(static_cast<vpos_t>(EntityBounds.m_Max.z), 0, NumVoxelsZ - 1);
  ...
  for (vpos_t VoxelZ = MinVoxelZ; VoxelZ <= MaxVoxelZ;
       ++VoxelZ, VoxelIndex += SkipVoxelsZ) {
    for (vpos_t VoxelY = MinVoxelY; VoxelY <= MaxVoxelY;
         ++VoxelY, VoxelIndex += SkipVoxelsY) {
      for (vpos_t VoxelX = MinVoxelX; VoxelX <= MaxVoxelX;
           ++VoxelX, ++VoxelIndex) {
        ...
        if (Add) {
          ...
          if (IsDoor) {
            WorldValue |= kDoorMask;
          }
          if (IsLockedDoor) {
            WorldValue |= kLockedDoorMask;
          }
        } else {
          // Remove
          ...
          WorldValue |= (CurrentCount - 1);
          if (IsDoor) {
            WorldValue &= kDoorNotMask;
          }
          if (IsLockedDoor) {
            WorldValue &= kLockedDoorNotMask;
          }
        }
      }
    }
...
void EldritchNav::SmoothPath(const Vector& Extents,
                             EldritchNav::TPath& InOutPath) {
  PROFILE_FUNCTION;
  ...
  for (uint AnchorIndex = InOutPath.Size() - 1; AnchorIndex > 0;) {
    const uint NextAnchorIndex = GetNextSmoothingNode(
        InOutPath[AnchorIndex], Extents, OffsetZVector, InOutPath, AnchorIndex);
...
uint EldritchNav::GetNextSmoothingNode(const Vector& Anchor,
                                       const Vector& Extents,
                                       const Vector& OffsetZVector,
                                       const TPath& Path,
                                       const uint PathStart) const {
  ...
  for (uint PathIndex = PathStart - 1; PathIndex > 0; --PathIndex) {
    if (CheckForSmoothingNode(Anchor, Path[PathIndex - 1], Extents,
                              OffsetZVector)) {
...
bool EldritchNav::CheckForSmoothingNode(const Vector& Anchor,
                                        const Vector& Location,
                                        const Vector& Extents,
                                        const Vector& OffsetZVector) const {
  if (IsWalking()) {
    const float AnchorFloor = Floor(Anchor.z);
    const float LocationFloor = Floor(Location.z);
    if (AnchorFloor != LocationFloor) {
      // Any significant (block-height) discrepancy between two nodes is
      // critical.
      return true;
    }
  }

  // Elevate sweep segment by OffsetZVector, which puts it at the entity's
  // actual height.
...
bool EldritchNav::SweepFloor(const Segment& SweepSegment,
  ...
  vpos_t MinVoxelX =
      Clamp(static_cast<vpos_t>(SegmentBounds.m_Min.x), 0, NumVoxelsX - 1);
  vpos_t MinVoxelY =
      Clamp(static_cast<vpos_t>(SegmentBounds.m_Min.y), 0, NumVoxelsY - 1);
  vpos_t MinVoxelZ =
      Clamp(static_cast<vpos_t>(SegmentBounds.m_Min.z), 0, NumVoxelsZ - 1);
  vpos_t MaxVoxelX =
      Clamp(static_cast<vpos_t>(SegmentBounds.m_Max.x), 0, NumVoxelsX - 1);
  vpos_t MaxVoxelY =
      Clamp(static_cast<vpos_t>(SegmentBounds.m_Max.y), 0, NumVoxelsY - 1);
  vpos_t MaxVoxelZ =
      Clamp(static_cast<vpos_t>(SegmentBounds.m_Max.z), 0, NumVoxelsZ - 1);
  ...
  for (vpos_t VoxelZ = MinVoxelZ; VoxelZ <= MaxVoxelZ;
    ...
    for (vpos_t VoxelY = MinVoxelY; VoxelY <= MaxVoxelY;
      ...
      for (vpos_t VoxelX = MinVoxelX; VoxelX <= MaxVoxelX;
           ++VoxelX, ++VoxelIndex) {
        // We're expecting a floor to exist, so we only need to sweep against
        // the boxes of empty cells.
        // NOTE: This might behave weirdly with entities that only fill a
        // portion of a cell, but don't
        // worry about that for right now. Just treat those as if they're fully
        // solid and walkable floors.
        ...
        // Contract box on sides where it is adjacent to a floor.
        {
          ...
          if (ValueNorth != EWV_None) {
            VoxelBox.m_Max.y -= Extents.y;
          }
          if (ValueSouth != EWV_None) {
            VoxelBox.m_Min.y += Extents.y;
          }
          if (ValueEast != EWV_None) {
            VoxelBox.m_Max.x -= Extents.x;
          }
          if (ValueWest != EWV_None) {
            VoxelBox.m_Min.x += Extents.x;
          }
        }
