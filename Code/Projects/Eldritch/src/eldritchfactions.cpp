...
EldritchFactions::EFactionCon EldritchFactions::GetCon(
    const HashedString& FactionA, const HashedString& FactionB) {
...
void StaticInitialize() {
  ...
  const uint NumFactionCons =
      ConfigManager::GetInt(sNumFactionCons, 0, sEldritchFactions);

  for (uint FactionConIndex = 0; FactionConIndex < NumFactionCons;
       ++FactionConIndex) {
    const HashedString FactionA = ConfigManager::GetSequenceHash(
        "FactionCon%dA", FactionConIndex, HashedString::NullString,
        sEldritchFactions);
    const HashedString FactionB = ConfigManager::GetSequenceHash(
        "FactionCon%dB", FactionConIndex, HashedString::NullString,
        sEldritchFactions);
    const HashedString FactionC = ConfigManager::GetSequenceHash(
        "FactionCon%dC", FactionConIndex, HashedString::NullString,
        sEldritchFactions);
