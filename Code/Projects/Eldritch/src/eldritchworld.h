...
typedef int
    vpos_t;  // Voxel position: worldspace position of a voxel on an axis
typedef byte vnbr_t;  // Voxel neighbor: bitfield of neighboring cells (see also
                      // EVoxelNeighbor)
typedef uint32 vxnb_t;  // Voxel extended neighbor: bitfield of neighboring
                        // cells including all diagonals (see also
                        // EVoxelNeighbor)
typedef byte vdir_t;  // Voxel direction: bitfield of direction to neighbor (see
                      // also EVoxelDirection)
...
enum EEldritchCollisionFlags {
  ...
  // Special case, overrides blocking flag optimization and tests all entities
  // (which match the given EECF_Mask_EntityTypes type).
  // Should be used sparingly. I'm adding this as a hack for movable static
  // entities (LizardWatson, specifically).
  ...
  // Usage masks
  EECF_Mask_CollideAs = EECF_CollideAsWorld | EECF_CollideAsEntity |
                        EECF_CollideAsOcclusion | EECF_CollideAsTrace,
  EECF_Mask_EntityTypes =
      EECF_CollideStaticEntities | EECF_CollideDynamicEntities,

  // Common combinations
  EECF_Occlusion = EECF_CollideAsOcclusion | EECF_CollideStaticEntities,
  EECF_EntityCollision = EECF_CollideAsEntity | EECF_CollideStaticEntities |
                         EECF_CollideDynamicEntities,
  EECF_Nav = EECF_CollideAsEntity | EECF_CollideStaticEntities,
  EECF_Trace = EECF_CollideAsTrace | EECF_CollideStaticEntities |
               EECF_CollideDynamicEntities,
};
...
class EldritchWorld {
 public:
  ...
  void SetCurrentWorld(const HashedString& WorldDef) {
  ...
  // Primary function for all collision tests: can be used for point, line, and
  // sweep checks against voxels and entities.
  bool Sweep(const Segment& SweepSegment, const Vector& HalfExtents,
             CollisionInfo& Info) const;

  // More specific tests used by Sweep
  bool SweepVoxels(const Segment& SweepSegment, const Vector& HalfExtents,
                   CollisionInfo& Info) const;
  bool SweepEntities(const Segment& SweepSegment, const Vector& HalfExtents,
                     CollisionInfo& Info) const;
  ...
  bool CheckClearance(const Vector& Location, const Vector& HalfExtents,
                      CollisionInfo& Info) const;
  bool PointCheck(const Vector& Location, CollisionInfo& Info) const;
  bool LineCheck(const Vector& Start, const Vector& End,
                 CollisionInfo& Info) const;
  bool SweepCheck(const Vector& Start, const Vector& End,
                  const Vector& HalfExtents, CollisionInfo& Info) const;
  bool FindSpot(Vector& InOutSpot, const Vector& Extents,
                CollisionInfo& Info) const;
  ...
  bool AddLightAt(const Vector& Location, const float Radius,
                  const Vector4& Color);  // Returns true if light was added
  ...
  SVoxelIrradiance BlendIrradiances(const Vector& LocationA,
                                    const Vector& LocationB) const;
  ...
  void GetTileUVs(const uint TileIndex, Vector2& OutUVMin,
  ...
  void GetRoomCoords(const int RoomIndex, int& OutX, int& OutY,
                     int& OutZ) const;
 ...
 private:
  ...
  struct SWorldDef {
    ...
    Map<vval_t, SVoxelDef>
        m_VoxelDefs;  // Map from voxel value to its texture tiles
  ...
  void InitializeLightConfig(const SimpleString& LightDefinitionName,
                             SWorldDef& WorldDef);
  void InitializeFogConfig(const SimpleString& FogDefinitionName,
                           SWorldDef& WorldDef);
  ...
  void BuildVertsForVoxel(const uint X, const uint Y, const uint Z,
                          SVoxelMeshBuffers& Buffers) const;
  Vector ApplyDisplacementNoise(const Vector& Location,
                                const SWorldDef& WorldDef) const;
  void PushColors(const class Vector4& Color, const class Vector4& AOColor,
                  const float AOValues[], SVoxelMeshBuffers& Buffers) const;
  void PushUVs(const Vector2& BaseUV, const bool FlipU, const bool FlipV,
               SVoxelMeshBuffers& Buffers) const;
  void PushIndices(index_t& BaseIndex, const bool CCW,
                   SVoxelMeshBuffers& Buffers) const;

  void AddAllLightInfluences();
  void ComputeAllIrradiance();
  void ComputeIrradianceForVoxels(const Set<vidx_t>& Voxels);
  void ComputePartialIrradiance(vidx_t VoxelIndex, vidx_t LightVoxel,
                                const SVoxelLight& Light);
  void ResetIrradiance(const vidx_t VoxelIndex);
  float GetIrradianceIncidence(const Vector& LightDir,
                               const Vector& IrradianceDir) const;

  // NOTE: The use of Sets here makes it easy to do multiple gathers over the
  // same region without
  // getting duplicate voxels, but makes this more prone to allocator perf
  // problems. Continue to watch.
  void GatherVoxelsInRadius(Set<vidx_t>& Voxels, const vidx_t VoxelIndex,
                            const float Radius) const;
  void GatherVoxelsInBox(Set<vidx_t>& Voxels, const AABB& Box) const;
  void GatherLightInfluenceVoxels(Set<vidx_t>& Voxels,
                                  const vidx_t VoxelIndex) const;

  void AddLightInfluence(const Set<vidx_t>& AffectedVoxels,
                         const vidx_t LightVoxel);
  void RemoveLightInfluence(const Set<vidx_t>& AffectedVoxels,
                            const vidx_t LightVoxel);
  void ClearLightInfluenceMap();

  bool HasAnyNeighbors(const vnbr_t VoxelNeighbors,
                       const vnbr_t NeighborFlags) const;
  bool HasAnyNeighbors(const vxnb_t VoxelExtendedNeighbors,
                       const vxnb_t ExtendedNeighborFlags) const;
  uint CountNeighbors(const vxnb_t VoxelExtendedNeighbors,
                      const vxnb_t ExtendedNeighborFlags) const;
  ...
  void GetCoords(const vidx_t VoxelIndex, vpos_t& OutX, vpos_t& OutY,
                 vpos_t& OutZ) const;
  ...
  vidx_t GetIndexNeighbor(const vidx_t VoxelIndex,
  ...
  void GetMeshCoords(const uint MeshIndex, uint& OutX, uint& OutY,
                     uint& OutZ) const;
  ...
  const Vector2& GetTileUV(const uint TilesheetIndex) const {
    return m_TileUVsMap[TilesheetIndex];
  }
  ...
  Array<vval_t>
      m_VoxelMap;  // Linear array representing a 3D array, ordered XYZ
  Array<byte> m_PackedVoxels;  // Compressed form for sending over network
  Array<Mesh*>
      m_WorldMeshes;  // Linear array representing a 3D array, ordered XYZ

  Array<Vector2>
      m_TileUVsMap;  // Map from tilesheet index to base UV for that tile

  float m_RayTraceLength;

  typedef Map<vidx_t, SVoxelLight> LightMap;
  LightMap m_LightMap;
  Array<SVoxelIrradiance> m_IrradianceMap;  // Note: This is a hefty chunk of
                                            // data. 96 times bigger than voxel
                                            // data! But it's transient.

  typedef Set<vidx_t> LightInfluences;
  typedef Array<LightInfluences> LightInfluenceMap;
  LightInfluenceMap m_LightInfluenceMap;  // Map of the indices of lights
                                          // influencing each voxel.

  Map<HashedString, SWorldDef>
      m_WorldDefs;  // Map from world name to its tileset and voxel defs
  HashedString m_CurrentWorldDef;  // Name of world theme

  Array<uint8> m_Maze;  // Serialized copy of worldgen maze

  bool m_DisableCompute;  // We can add and remove voxels and lights, but meshes
                          // and irradiance will not be computed while this is
                          // true
