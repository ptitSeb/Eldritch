...
class WBCompEldCollision : public WBEldritchComponent {
 public:
  ...
  void OnCollided(const Vector& CollisionNormal,
                  WBEntity* const pCollidedEntity);

  void Jump();
  void Fall();
  bool IsLanded() const { return m_Landed; }
  bool IsRecentlyLanded(const float TimeThreshold) const;  // Returns landed
                                                           // status within a
                                                           // threshold; never
                                                           // returns true if
                                                           // jumping.
  ...
  const AABB& GetBounds()
      const { /*ASSERT( m_Bounds.Equals( GetCurrentBounds() ) );*/
    return m_Bounds;
  }
  ...
  inline uint GetDefaultCollisionFlags() const {
    return m_DefaultCollisionFlags;
  }
  void SetCollisionFlags(const uint Flags, const uint Mask = 0xffffffff,
                         const bool SendEvents = false,
                         const bool UpdateCollisionMap = true);
  bool MatchesAllCollisionFlags(const uint Flags) const {
    return (m_CollisionFlags & Flags) == Flags;
  }
  bool MatchesAnyCollisionFlags(const uint Flags) const {
    return (m_CollisionFlags & Flags) > 0;
  }
 ...
 private:
  ...
  // m_Touching used to be a set for quicker Removes; I'm trying an Array for
  // quicker UpdateTouching.
  Array<WBEntityRef> m_Touching;  // Serialized
  bool m_CanTouch;                // Config/serialized

  uint m_CollisionFlags;         // Config/serialized, see
                                 // EldritchWorld::ECollisionFlags
