...
class WBCompEldFrobbable : public WBEldritchComponent {
 ...
 private:
  ...
  bool m_HoldReleaseMode;    // Config/serialized: supports two frob reactions,
                             // one for held input and one for released
  bool m_HandleHoldRelease;  // Transient, state that says we've received an
                             // OnRise event and will handle a hold or release

  bool m_UseCollisionExtents;  // Config: if true, uses collisions's extents
                               // (else, uses configured extents)
  bool m_UseMeshExtents;       // Config: if true, uses mesh's AABB
  float m_ExtentsFatten;       // If using collision or mesh extents, optionally
                               // fatten up the bounds
