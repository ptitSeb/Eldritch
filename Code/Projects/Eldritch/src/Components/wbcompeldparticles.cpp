...
WBCompEldParticles::WBCompEldParticles()
    : m_ParticleSystems(), m_Hidden(false), m_CullDistanceSq(0.0f) {
...
/*virtual*/ void WBCompEldParticles::InitializeFromDefinition(
  ...
  const SimpleString ParticlesDef =
      ConfigManager::GetInheritedString(sParticles, "", sDefinitionName);
  if (ParticlesDef != "") {
    AddParticleSystem(ParticlesDef, Serialize, Attached, pLocationOverride);
  }

  STATICHASH(NumParticleSystems);
  const uint NumParticleSystems =
      ConfigManager::GetInheritedInt(sNumParticleSystems, 0, sDefinitionName);
  for (uint ParticleSystemIndex = 0; ParticleSystemIndex < NumParticleSystems;
       ++ParticleSystemIndex) {
    const SimpleString ParticleSystemDef =
        ConfigManager::GetInheritedSequenceString(
            "ParticleSystem%d", ParticleSystemIndex, "", sDefinitionName);
  ...
  const float DefaultCullDistance =
      ConfigManager::GetFloat(sCullDistance, 0.0f, sEldParticles);
  m_CullDistanceSq = Square(ConfigManager::GetInheritedFloat(
      sCullDistance, DefaultCullDistance, sDefinitionName));
}

void WBCompEldParticles::AddParticleSystem(
    const SimpleString& DefinitionName, const bool Serialize,
    const bool Attached, const Vector* const pLocationOverride) {
  WBCompEldTransform* const pTransform =
      GetEntity()->GetTransformComponent<WBCompEldTransform>();
  DEVASSERT(pTransform);

  const Vector Location =
      pLocationOverride ? (*pLocationOverride) : pTransform->GetLocation();
...
void WBCompEldParticles::AddParticleSystem(const SimpleString& DefinitionName,
...
void WBCompEldParticles::StopParticleSystem(
    const HashedString& DefinitionNameHash) {
...
/*virtual*/ void WBCompEldParticles::HandleEvent(const WBEvent& Event) {
  ...
  if (EventName == sOnMoved || EventName == sOnWorldLoaded ||
      EventName == sOnTurned) {
    WBCompEldTransform* const pTransform =
        GetEntity()->GetTransformComponent<WBCompEldTransform>();
    DEVASSERT(pTransform);
    ...
    FOR_EACH_ARRAY(PSIter, m_ParticleSystems, SParticleSystem) {
      ...
      if (PS.m_Attached) {
        ...
        const WBCompEldCamera* const pCamera =
            GET_WBCOMP(GetEntity(), EldCamera);
        if (pCamera) {
          PS.m_System->SetLocation(
              Location +
              pCamera->GetViewTranslationOffset(WBCompEldCamera::EVM_All));
          PS.m_System->SetOrientation(
              Orientation +
  ...
  if (Version >= VERSION_SYSTEMS) {
    ...
    for (uint ParticleSystemIndex = 0; ParticleSystemIndex < NumParticleSystems;
      ...
      if (Stream.ReadBool())  // Serialized
      {
        ...
        AddParticleSystem(DefinitionName, Serialize, Attached, Location,
                          Orientation);
      }
    }
  }
