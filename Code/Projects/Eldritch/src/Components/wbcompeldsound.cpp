...
WBCompEldSound::WBCompEldSound() : m_SoundInstances() {}
...
/*virtual*/ void WBCompEldSound::InitializeFromDefinition(
  ...
  STATICHASH(Sound);
  const SimpleString Sound =
      ConfigManager::GetInheritedString(sSound, "", sDefinitionName);
  if (Sound == "") {
    // No ambient sound; this component may still be used to house attached SFX
  } else {
    ISoundInstance* const pSoundInstance =
        GetFramework()->GetAudioSystem()->CreateSoundInstance(Sound);
    ...
    FOR_EACH_ARRAY(SoundInstIter, m_SoundInstances, SSoundInstance) {
      ...
      if (!SoundInstance.m_SoundInstance->IsPlaying()) {
        SoundInstance.m_SoundInstance
            ->Tick();  // Tick to commit location change
...
void WBCompEldSound::PlaySoundDef(const SimpleString& SoundDef,
                                  const Vector& Location, const bool Attached,
  ...
  IAudioSystem* const pAudioSystem = GetFramework()->GetAudioSystem();
  ISoundInstance* const pSoundInstance =
      pAudioSystem->CreateSoundInstance(SoundDef);
  ASSERT(pSoundInstance);

  pSoundInstance->SetBaseVolume((VolumeOverride > 0.0f) ? VolumeOverride
                                                        : 1.0f);
...
/*static*/ void WBCompEldSound::InstanceDeleteCallback(
