...
/*virtual*/ void WBCompEldSpikes::InitializeFromDefinition(
  ...
  m_SpeedThresholdSq = Square(
      ConfigManager::GetInheritedFloat(sSpeedThreshold, 0.0f, sDefinitionName));

  STATICHASH(CheckMovingDown);
  m_CheckMovingDown =
      ConfigManager::GetInheritedBool(sCheckMovingDown, false, sDefinitionName);

  STATICHASH(RecentlyLandedThreshold);
  m_RecentlyLandedThreshold = ConfigManager::GetInheritedFloat(
      sRecentlyLandedThreshold, 0.0f, sDefinitionName);
}
...
bool WBCompEldSpikes::ShouldSendSpikedEvent(WBEntity* const pTouched) {
  ...
  WBCompEldTransform* const pTouchedTransform =
      pTouched->GetTransformComponent<WBCompEldTransform>();
  ASSERT(pTouchedTransform);

  WBCompEldCollision* const pTouchedCollision =
      GET_WBCOMP(pTouched, EldCollision);
  ASSERT(pTouchedCollision);

  const Vector Velocity = pTouchedTransform->GetVelocity();

  if (m_CheckMovingDown &&
      !pTouchedCollision->IsRecentlyLanded(m_RecentlyLandedThreshold)) {
