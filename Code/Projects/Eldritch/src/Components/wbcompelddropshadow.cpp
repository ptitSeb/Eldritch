...
/*virtual*/ void WBCompEldDropShadow::InitializeFromDefinition(
  ...
  STATICHASH(Radius);
  const float Radius =
      ConfigManager::GetInheritedFloat(sRadius, 0.0f, sDefinitionName);

  STATICHASH(Alpha);
  const float Alpha =
      ConfigManager::GetInheritedFloat(sAlpha, 0.0f, sDefinitionName);
  ...
  m_VoxelCheckOffsetZ = ConfigManager::GetInheritedFloat(sVoxelCheckOffsetZ,
                                                         0.0f, sDefinitionName);

  STATICHASH(ShadowFloatZ);
  m_ShadowFloatZ =
      ConfigManager::GetInheritedFloat(sShadowFloatZ, 0.0f, sDefinitionName);

  STATICHASH(UseMeshCenter);
  m_UseMeshCenter =
      ConfigManager::GetInheritedBool(sUseMeshCenter, false, sDefinitionName);
}
...
bool WBCompEldDropShadow::ShouldBeHidden() const {
  ...
  const Vector VoxelCheckBelowOffset =
      Vector(0.0f, 0.0f, m_VoxelCheckOffsetZ - 1.0f);

  CollisionInfo Info;
  Info.m_CollideWorld = true;

  if (GetWorld()->PointCheck(pTransform->GetLocation() + VoxelCheckOffset,
                             Info)) {
    // We're inside collision, hide mesh
    return true;
  }

  if (!GetWorld()->PointCheck(pTransform->GetLocation() + VoxelCheckBelowOffset,
                              Info)) {
...
void WBCompEldDropShadow::Update() {
  ...
  const Vector DropLocation = m_UseMeshCenter
                                  ? (pMesh->GetMesh()->m_AABB.GetCenter())
...
/*virtual*/ void WBCompEldDropShadow::DebugRender() const {
  GetFramework()->GetRenderer()->DEBUGDrawBox(m_Mesh->m_AABB.m_Min,
                                              m_Mesh->m_AABB.m_Max,
                                              ARGB_TO_COLOR(255, 0, 128, 255));
}
#endif
