...
/*virtual*/ void WBCompEldSensorVision::InitializeFromDefinition(
  ...
  m_EyeOffsetZ =
      ConfigManager::GetInheritedFloat(sEyeOffsetZ, 0.0f, sDefinitionName);

  STATICHASH(Radius);
  m_RadiusSq =
      Square(ConfigManager::GetInheritedFloat(sRadius, 0.0f, sDefinitionName));

  STATICHASH(ConeAngle);
  m_ConeCos = Cos(DEGREES_TO_RADIANS(
      ConfigManager::GetInheritedFloat(sConeAngle, 0.0f, sDefinitionName)));

  STATICHASH(ConeScaleZ);
  const float ConeScaleZ =
      ConfigManager::GetInheritedFloat(sConeScaleZ, 0.0f, sDefinitionName);
  m_ConeInvZScale = (ConeScaleZ > 0.0f) ? (1.0f / ConeScaleZ) : ConeScaleZ;

  STATICHASH(CertaintyFalloffRadius);
  m_CertaintyFalloffRadius = ConfigManager::GetInheritedFloat(
      sCertaintyFalloffRadius, 0.0f, sDefinitionName);

  STATICHASH(DistanceCertaintyFactor);
  m_DistanceCertaintyFactor = ConfigManager::GetInheritedFloat(
      sDistanceCertaintyFactor, 0.0f, sDefinitionName);

  STATICHASH(CertaintyVelocity);
  m_CertaintyVelocity = ConfigManager::GetInheritedFloat(sCertaintyVelocity,
...
bool WBCompEldSensorVision::IsVisible(WBEntity* const pVisibleEntity) const {
  ...
  WBCompEldHeadTracker* const pHeadTracker =
      GET_WBCOMP(pEntity, EldHeadTracker);

  const Vector Location = pTransform->GetLocation();
  const Vector EyeLocation = Location + Vector(0.0f, 0.0f, m_EyeOffsetZ);
  const Vector EyeDirection = pHeadTracker
                                  ? pHeadTracker->GetLookDirection()
                                  : pTransform->GetOrientation().ToVector();
  ...
  const Vector EffectiveOffset =
      OffsetOtherPart + OffsetUpPart * m_ConeInvZScale;
...
/*virtual*/ void WBCompEldSensorVision::PollTick(const float DeltaTime) const {
  const Array<WBCompEldVisible*>* pVisibleComponents =
      WBComponentArrays::GetComponents<WBCompEldVisible>();
  ...
  for (uint VisibleIndex = 0; VisibleIndex < NumVisibles; ++VisibleIndex) {
    ...
    WBCompRodinKnowledge::TKnowledge* const pKnowledgeEntry =
        pKnowledge->GetKnowledge(pVisibleEntity);
    if (pKnowledgeEntry) {
      ...
      VisionCertainty =
          Max(0.0f, VisionCertainty - m_CertaintyDecay * DeltaTime);
    ...
    // Entity is visible.
    WBCompEldTransform* const pVisibleTransform =
        pVisibleEntity->GetTransformComponent<WBCompEldTransform>();
    ...
    const float DistanceCertainty =
        Attenuate(SeenDistance, m_CertaintyFalloffRadius);
    const float DistanceCertaintyFactor =
        Lerp(1.0f - m_DistanceCertaintyFactor, 1.0f, DistanceCertainty);

    // TODO: Multiply other factors in here (luminance, periphery, velocity,
    // crouching, etc.)
    const float Certainty = DistanceCertaintyFactor;

    WBCompRodinKnowledge::TKnowledge& Knowledge =
        pKnowledge->UpdateEntity(pVisibleEntity);

    STATIC_HASHED_STRING(VisionCertainty);
    float VisionCertainty = Knowledge.GetFloat(sVisionCertainty);
    // This is a bit kludgy. Accelerate toward certainty faster if target
    // certainty is higher.
    VisionCertainty =
        Min(Certainty,
            VisionCertainty + m_CertaintyVelocity * Certainty * DeltaTime);
...
/*virtual*/ void WBCompEldSensorVision::DebugRender() const {
  ...
  const Vector EyeLocation =
      pTransform->GetLocation() + Vector(0.0f, 0.0f, m_EyeOffsetZ);

  GetFramework()->GetRenderer()->DEBUGDrawCross(EyeLocation, 0.5f,
