...
WBCompEldHands::WBCompEldHands() : m_HideHandsRefs(0) {
...
/*virtual*/ void WBCompEldHands::HandleEvent(const WBEvent& Event) {
  ...
  if (EventName == sOnWorldLoaded) {
    ...
    if (pItem == GetItemInRightHand() || pItem == GetItemInLeftHand()) {
      AddAnimationsToHand(pItem);
    }

    if (pItem == GetWeaponAlt()) {
      // When traveling to a new world and spawning an item in the alt slot,
      // immediately hide it
      WB_MAKE_EVENT(Hide, GetEntity());
    ...
    if (pItem == GetWeapon()) {
      ...
      // HACK: Play the idle animation. I could add an event for "restored equip
      // focus"
    ...
    if (pAnimatingEntity == GetItemInRightHand() ||
        pAnimatingEntity == GetItemInLeftHand()) {
    ...
    SetHandMeshes(LeftHandMesh, LeftHandTexture, RightHandMesh,
                  RightHandTexture);
    RestoreHandAnimations();
  }
}
...
void WBCompEldHands::ShowWeaponHUD() const {
...

  {
    ...
    WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(), SetWidgetImage,
                      NULL);
  }
...
void WBCompEldHands::HideWeaponHUD() const {
  ...
  WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(), SetWidgetHidden,
                    NULL);
}
...
void WBCompEldHands::PlayAnimation(WBEntity* const pAnimatingEntity,
                                   const HashedString& AnimationName,
                                   const EHand Hand) const {
  if (!pAnimatingEntity) {
    return;
  }
  ...
  WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(), PlayAnim,
                    pAnimatingEntity);
  WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(), PlayAnim,
                    GetHand(Hand));
}
...
void WBCompEldHands::AddAnimationsToHand(WBEntity* const pItem,
                                         const EHand Hand) const {
  if (!pItem) {
    return;
  }
  ...
  WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(), CopyAnimations,
                    GetHand(Hand));
}

void WBCompEldHands::SetHandMeshes(const SimpleString& LeftHandMesh,
...
WBEntity* WBCompEldHands::GetItemInLeftHand() const { return GetPower(); }
