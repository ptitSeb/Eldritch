...
/*virtual*/ void WBCompEldCamera::InitializeFromDefinition(
  ...
  m_ViewOffsetZ =
      ConfigManager::GetInheritedFloat(sViewOffsetZ, 0.0f, sDefinitionName);
  m_LastViewOffsetZ = m_ViewOffsetZ;

  STATICHASH(LeanVelocity);
  m_LeanVelocity = DEGREES_TO_RADIANS(
      ConfigManager::GetInheritedFloat(sLeanVelocity, 0.0f, sDefinitionName));

  STATICHASH(LeanRollMax);
  m_LeanRollMax = DEGREES_TO_RADIANS(
      ConfigManager::GetInheritedFloat(sLeanRollMax, 0.0f, sDefinitionName));

  STATICHASH(LeanRadius);
  m_LeanRadius =
      ConfigManager::GetInheritedFloat(sLeanRadius, 0.0f, sDefinitionName);

  STATICHASH(LeanExtent);
  m_LeanExtent =
      ConfigManager::GetInheritedFloat(sLeanExtent, 0.0f, sDefinitionName);
}
...
/*virtual*/ void WBCompEldCamera::Tick(float DeltaTime) {
  ...
  const Vector ViewTranslation =
      pTransform->GetLocation() + GetViewTranslationOffset(EVM_All);
  const Angles ViewOrientation =
      pTransform->GetOrientation() + GetViewOrientationOffset(EVM_All);
  EldritchFramework::GetInstance()->SetMainViewTransform(ViewTranslation,
                                                         ViewOrientation);
...
Vector WBCompEldCamera::GetViewTranslationOffset(
    const WBCompEldCamera::EViewModifiers Modifiers) const {
  Vector Offset;
...
Angles WBCompEldCamera::GetViewOrientationOffset(
    const WBCompEldCamera::EViewModifiers Modifiers) const {
  Angles Offset;
...
Vector WBCompEldCamera::GetLeanOffset(const float LeanRoll) const {
  if (LeanRoll == 0.0f) {
    ...
    const Angles LeanAngle =
        Angles(0.0f, LeanRoll, pTransform->GetOrientation().Yaw);
...
float WBCompEldCamera::GetDesiredLean(const float LeanPosition) const {
  ...
  const Vector ViewBase =
      pTransform->GetLocation() + GetViewTranslationOffset(EVM_OffsetZ);
...
void WBCompEldCamera::UpdateLean(const float TargetRoll,
