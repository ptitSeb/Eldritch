...
/*virtual*/ void WBCompEldHeadTracker::InitializeFromDefinition(
  ...
  m_HeadBoneName = ConfigManager::GetInheritedHash(
      sHeadBoneName, HashedString::NullString, sDefinitionName);

  STATICHASH(HeadOffsetX);
  m_HeadOffset.x =
      ConfigManager::GetInheritedFloat(sHeadOffsetX, 0.0f, sDefinitionName);

  STATICHASH(HeadOffsetY);
  m_HeadOffset.y =
      ConfigManager::GetInheritedFloat(sHeadOffsetY, 0.0f, sDefinitionName);

  STATICHASH(HeadOffsetZ);
  m_HeadOffset.z =
      ConfigManager::GetInheritedFloat(sHeadOffsetZ, 0.0f, sDefinitionName);

  STATICHASH(EyesOffsetX);
  m_EyesOffset.x =
      ConfigManager::GetInheritedFloat(sEyesOffsetX, 0.0f, sDefinitionName);

  STATICHASH(EyesOffsetY);
  m_EyesOffset.y =
      ConfigManager::GetInheritedFloat(sEyesOffsetY, 0.0f, sDefinitionName);

  STATICHASH(EyesOffsetZ);
  m_EyesOffset.z =
      ConfigManager::GetInheritedFloat(sEyesOffsetZ, 0.0f, sDefinitionName);

  STATICHASH(MaxRotation);
  m_MaxRotationRadians = DEGREES_TO_RADIANS(
      ConfigManager::GetInheritedFloat(sMaxRotation, 0.0f, sDefinitionName));

  STATICHASH(LookVelocity);
  m_LookVelocity = DEGREES_TO_RADIANS(
      ConfigManager::GetInheritedFloat(sLookVelocity, 0.0f, sDefinitionName));
}
...
/*virtual*/ void WBCompEldHeadTracker::Tick(float DeltaTime) {
  ...
  // NOTE: Because we don't have the animation matrices at this point, we can't
  // get the eyes' actual transformed location.
  const Vector EyesLocation =
      pTransform->GetLocation() +
      (m_EyesOffset * pTransform->GetOrientation().ToMatrix());
  ...
  if (m_TrackMode == ETM_Entity) {
    ...
    if (pLookAtTarget) {
      WBCompEldTransform* const pLookAtTransform =
          pLookAtTarget->GetTransformComponent<WBCompEldTransform>();
      ...
      WBCompRodinKnowledge* const pKnowledge =
          GET_WBCOMP(pEntity, RodinKnowledge);
      if (pKnowledge->IsCurrentlyVisible(pLookAtTarget)) {
        TargetLocation = pVisible ? pVisible->GetVisibleLocation()
                                  : pLookAtTransform->GetLocation();
        DirectionToTarget = TargetLocation - EyesLocation;
      } else {
        const bool HasLastKnownLocation =
            pKnowledge->GetLastKnownLocationFor(pLookAtTarget, TargetLocation);
  ...
  const Quat ConstrainedToTarget =
      (ToTarget.GetAngle() <= m_MaxRotationRadians)
          ? ToTarget
          : Quat::CreateRotation(ToTarget.GetAxis(), m_MaxRotationRadians);
  const Quat Rotation = ConstrainedToTarget - m_LookRotationOS;
  const float AmountToTurn = Min(m_LookVelocity * DeltaTime,
                                 Rotation.Length());  // Prevent overturning

  m_LookRotationOS += Rotation.GetNormalized() * AmountToTurn;
}

/*virtual*/ void WBCompEldHeadTracker::Modify(const BoneArray* pBones,
                                              Matrix* pInOutBoneMatrices) {
  ...
  const Vector HeadTrackedHeadOffset =
      TransformedHeadOffset * HeadTrackRotation;
  HeadTrackRotation.SetTranslationElements(TransformedHeadOffset -
                                           HeadTrackedHeadOffset);
...
void WBCompEldHeadTracker::StopLooking() { m_TrackMode = ETM_None; }
...
/*virtual*/ void WBCompEldHeadTracker::DebugRender() const {
  ...
  const Vector HeadLocation =
      pTransform->GetLocation() + pMesh->GetMeshOffset() +
      (m_HeadOffset * pTransform->GetOrientation().ToMatrix());
  const Vector EyeLocation =
      pTransform->GetLocation() +
      (m_EyesOffset * pTransform->GetOrientation().ToMatrix());

  GetFramework()->GetRenderer()->DEBUGDrawCross(
      HeadLocation, 0.25f, ARGB_TO_COLOR(255, 128, 0, 255));
  GetFramework()->GetRenderer()->DEBUGDrawCross(
      EyeLocation, 0.25f, ARGB_TO_COLOR(255, 128, 0, 255));

  const Vector LookDirectionOS =
      Vector(0.0f, 1.0f, 0.0f) * m_LookRotationOS.ToMatrix();
  const Vector LookDirectionWS =
      LookDirectionOS * pTransform->GetOrientation().ToMatrix();
  GetFramework()->GetRenderer()->DEBUGDrawLine(
      EyeLocation - LookDirectionWS * 5.0f,
      EyeLocation + LookDirectionWS * 5.0f, ARGB_TO_COLOR(255, 255, 255, 255));
  GetFramework()->GetRenderer()->DEBUGDrawLine(
      HeadLocation - LookDirectionWS * 5.0f,
      HeadLocation + LookDirectionWS * 5.0f, ARGB_TO_COLOR(255, 255, 255, 255));
}
#endif
