...
/*virtual*/ void WBCompEldTransform::InitializeFromDefinition(
  ...
  const bool UseGravity =
      ConfigManager::GetInheritedBool(sUseGravity, false, sDefinitionName);

  STATICHASH(Gravity);
  const float DefaultGravity =
      ConfigManager::GetFloat(sGravity, 0.0f, sEldTransform);
  m_Gravity =
      -ConfigManager::GetInheritedFloat(
          sGravity, UseGravity ? DefaultGravity : 0.0f, sDefinitionName);

  STATICHASH(SpeedLimit);
  m_SpeedLimit =
      ConfigManager::GetInheritedFloat(sSpeedLimit, 0.0f, sDefinitionName);
  m_UseSpeedLimit = (m_SpeedLimit > 0.0f);

  STATICHASH(AllowImpulses);
  m_AllowImpulses =
      ConfigManager::GetInheritedBool(sAllowImpulses, true, sDefinitionName);

  STATICHASH(Yaw);
  m_Orientation.Yaw = DEGREES_TO_RADIANS(
      ConfigManager::GetInheritedFloat(sYaw, 0.0f, sDefinitionName));

  STATICHASH(IsAttachedToOwner);
  m_IsAttachedToOwner = ConfigManager::GetInheritedBool(sIsAttachedToOwner,
                                                        false, sDefinitionName);

  STATICHASH(OwnerOffsetX);
  m_OwnerOffset.x =
      ConfigManager::GetInheritedFloat(sOwnerOffsetX, 0.0f, sDefinitionName);

  STATICHASH(OwnerOffsetY);
  m_OwnerOffset.y =
      ConfigManager::GetInheritedFloat(sOwnerOffsetY, 0.0f, sDefinitionName);

  STATICHASH(OwnerOffsetZ);
  m_OwnerOffset.z =
      ConfigManager::GetInheritedFloat(sOwnerOffsetZ, 0.0f, sDefinitionName);
}
...
void WBCompEldTransform::TickAcceleration(float DeltaTime) {
  ...
  if (m_UseSpeedLimit && m_Acceleration.LengthSquared() > EPSILON) {
    ...
    m_Acceleration.GetNormalized(ComponentDirection, AccelerationLength,
                                 AccelerationLengthOverOne);

    const Vector ComponentVelocity =
        m_Velocity.ProjectionOnto(ComponentDirection);
    const Vector ComplementVelocity = m_Velocity - ComponentVelocity;
    const float ComponentSpeed = ComponentVelocity.Length();
    const float SignedSpeed = (ComponentVelocity.Dot(ComponentDirection) > 0.0f)
                                  ? ComponentSpeed
                                  : -ComponentSpeed;

    // Allow component velocity to exceed the limit, but *not* by adding to it
    // with acceleration.
    if (SignedSpeed < WB_MODDED(SpeedLimit)) {
      const float NewComponentSpeed = Min(
          SignedSpeed + AccelerationLength * DeltaTime, WB_MODDED(SpeedLimit));
...
void WBCompEldTransform::TickMotion(float DeltaTime) {
  ...
  if (m_Velocity.LengthSquared() > EPSILON)  // Optimization, don't want to pay
                                             // MoveBy costs for static entities
  {
...
void WBCompEldTransform::MoveWithOwner() {
  ...
  if (pOwnerEntity) {
    WBCompEldTransform* const pOwnerTransform =
        pOwnerEntity->GetTransformComponent<WBCompEldTransform>();
    DEVASSERT(pOwnerTransform);

    // TEMPHACK: What I'll probably want is a flag that says whether this is
    // attached to the camera or to a bone on the owner's mesh.
    ...
    const Angles Orientation =
        pOwnerTransform->GetOrientation() +
        pOwnerCamera->GetViewOrientationOffset(WBCompEldCamera::EVM_All);
    const Vector TransformedOffset = m_OwnerOffset * Orientation.ToMatrix();
    const Vector Location =
        pOwnerTransform->GetLocation() +
        pOwnerCamera->GetViewTranslationOffset(WBCompEldCamera::EVM_All) +
        TransformedOffset;
...
/*virtual*/ void WBCompEldTransform::DebugRender() const {
  GetFramework()->GetRenderer()->DEBUGDrawCross(
      m_Location, 0.25f, ARGB_TO_COLOR(255, 255, 255, 255));
}
#endif
...
void WBCompEldTransform::Report() const {
  ...
  if (!m_Acceleration.IsZero()) {
    PRINTF(WBPROPERTY_REPORT_PREFIX "Acceleration: %s\n",
