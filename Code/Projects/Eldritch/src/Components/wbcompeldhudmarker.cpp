...
/*virtual*/ void WBCompEldHUDMarker::InitializeFromDefinition(
  ...
  m_UIScreenName = ConfigManager::GetInheritedHash(
      sUIScreenName, HashedString::NullString, sDefinitionName);

  STATICHASH(UIWidgetName);
  m_UIWidgetName = ConfigManager::GetInheritedHash(
      sUIWidgetName, HashedString::NullString, sDefinitionName);

  STATICHASH(OccludedImage);
  m_OccludedImage = ConfigManager::GetInheritedHash(
      sOccludedImage, HashedString::NullString, sDefinitionName);

  STATICHASH(UnoccludedImage);
  m_UnoccludedImage = ConfigManager::GetInheritedHash(
      sUnoccludedImage, m_OccludedImage, sDefinitionName);

  STATICHASH(FalloffRadius);
  m_FalloffRadius =
      ConfigManager::GetInheritedFloat(sFalloffRadius, 0.0f, sDefinitionName);
...
void WBCompEldHUDMarker::UpdateMarkerPosition() const {
...

  {
    ...
    WB_SET_AUTO(SetWidgetImage, Hash, Image,
                Occluded ? m_OccludedImage : m_UnoccludedImage);
