...
WBCompEldMesh::~WBCompEldMesh() { SafeDelete(m_Mesh); }
...
void WBCompEldMesh::SetMesh(const SimpleString& Mesh) {
  ...
  m_Mesh->SetMaterialFlags((m_DrawForeground ? MAT_FOREGROUND : MAT_WORLD) |
                           MAT_DYNAMIC);
...
void WBCompEldMesh::SetTexture(const SimpleString& Texture) {
  ...
  if (Texture != "") {
    m_Mesh->SetTexture(
        0, pRenderer->GetTextureManager()->GetTexture(Texture.CStr()));
  }
}

/*virtual*/ void WBCompEldMesh::InitializeFromDefinition(
  ...
  m_TextureOverride =
      ConfigManager::GetInheritedString(sTextureOverride, "", sDefinitionName);

  STATICHASH(MaterialOverride);
  m_MaterialOverride =
      ConfigManager::GetInheritedString(sMaterialOverride, "", sDefinitionName);

  STATICHASH(DrawForeground);
  m_DrawForeground =
      ConfigManager::GetInheritedBool(sDrawForeground, false, sDefinitionName);

  STATICHASH(Mesh);
  const SimpleString Mesh =
      ConfigManager::GetInheritedString(sMesh, "", sDefinitionName);

  SetMesh(Mesh);

  STATICHASH(OffsetZ);
  m_Offset.z =
      ConfigManager::GetInheritedFloat(sOffsetZ, 0.0f, sDefinitionName);

  STATICHASH(IrradianceOffsetZ);
  m_IrradianceOffsetZ = ConfigManager::GetInheritedFloat(sIrradianceOffsetZ,
                                                         0.0f, sDefinitionName);

  STATICHASH(UseTwoPointIrradiance);
  m_UseTwoPointIrradiance = ConfigManager::GetInheritedBool(
      sUseTwoPointIrradiance, false, sDefinitionName);

  STATICHASH(UseBlendedIrradiance);
  const bool DefaultUseBlendedIrradiance =
      ConfigManager::GetBool(sUseBlendedIrradiance, false, sEldMesh);
  m_UseBlendedIrradiance = ConfigManager::GetInheritedBool(
      sUseBlendedIrradiance, DefaultUseBlendedIrradiance, sDefinitionName);

  STATICHASH(BlendRate);
  const float DefaultBlendRate =
      ConfigManager::GetFloat(sBlendRate, 0.0f, sEldMesh);
  m_BlendRate = ConfigManager::GetInheritedFloat(sBlendRate, DefaultBlendRate,
                                                 sDefinitionName);

  STATICHASH(ConstantIrradianceR);
  m_ConstantIrradiance.r = ConfigManager::GetInheritedFloat(
      sConstantIrradianceR, 0.0f, sDefinitionName);

  STATICHASH(ConstantIrradianceG);
  m_ConstantIrradiance.g = ConfigManager::GetInheritedFloat(
      sConstantIrradianceG, 0.0f, sDefinitionName);

  STATICHASH(ConstantIrradianceB);
  m_ConstantIrradiance.b = ConfigManager::GetInheritedFloat(
      sConstantIrradianceB, 0.0f, sDefinitionName);

  STATICHASH(CullDistance);
  const float DefaultCullDistance =
      ConfigManager::GetFloat(sCullDistance, 0.0f, sEldMesh);
  m_CullDistanceSq = Square(ConfigManager::GetInheritedFloat(
      sCullDistance, DefaultCullDistance, sDefinitionName));
}
...
void WBCompEldMesh::PlayAnimation(const HashedString& AnimationName,
                                  const bool Loop,
  ...
  AnimationState::SPlayAnimationParams PlayParams;
  PlayParams.m_EndBehavior =
      Loop ? AnimationState::EAEB_Loop : AnimationState::EAEB_Stop;
...
void WBCompEldMesh::CopyAnimationsFrom(WBEntity* const pSourceEntity,
                                       const bool SuppressAnimEvents) const {
...
/*static*/ void WBCompEldMesh::NotifyAnimationFinished(
    void* pVoid, class Mesh* pMesh, class Animation* pAnimation,
    bool Interrupted) {
