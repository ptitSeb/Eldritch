...
/*virtual*/ void WBCompEldAmmoBag::InitializeFromDefinition(
  ...
  m_HideAmmoHUD =
      ConfigManager::GetInheritedBool(sHideAmmoHUD, false, sDefinitionName);

  STATICHASH(DefaultAmmo);
  m_DefaultAmmo = ConfigManager::GetInheritedHash(
      sDefaultAmmo, HashedString::NullString, sDefinitionName);
...
void WBCompEldAmmoBag::PublishToHUD() const {
  const HashedString& PublishAmmo =
      m_CurrentAmmo ? m_CurrentAmmo : m_DefaultAmmo;
...
void WBCompEldAmmoBag::PushPersistence() const {
  ...
  FOR_EACH_MAP(AmmoIter, m_Ammo, HashedString, uint) {
    Persistence.SetHash(SimpleString::PrintF("Ammo%dKey", AmmoIndex),
                        AmmoIter.GetKey());
    Persistence.SetInt(SimpleString::PrintF("Ammo%dValue", AmmoIndex),
                       AmmoIter.GetValue());
    ++AmmoIndex;
  }
}
...
void WBCompEldAmmoBag::PullPersistence() {
  ...
  for (uint AmmoIndex = 0; AmmoIndex < NumAmmoTypes; ++AmmoIndex) {
    const HashedString AmmoKey =
        Persistence.GetHash(SimpleString::PrintF("Ammo%dKey", AmmoIndex));
    const uint AmmoValue =
        Persistence.GetInt(SimpleString::PrintF("Ammo%dValue", AmmoIndex));
