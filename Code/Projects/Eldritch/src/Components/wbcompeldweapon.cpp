...
WBCompEldWeapon::WBCompEldWeapon() : m_Ammo(), m_WeaponIcon() {}

WBCompEldWeapon::~WBCompEldWeapon() {}

/*virtual*/ void WBCompEldWeapon::InitializeFromDefinition(
  ...
  STATICHASH(Ammo);
  m_Ammo = ConfigManager::GetInheritedHash(sAmmo, HashedString::NullString,
                                           sDefinitionName);

  STATICHASH(WeaponIcon);
  m_WeaponIcon =
      ConfigManager::GetInheritedString(sWeaponIcon, "", sDefinitionName);
}
