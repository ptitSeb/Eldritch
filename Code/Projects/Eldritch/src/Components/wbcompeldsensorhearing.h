...
class WBCompEldSensorHearing : public WBCompEldSensor {
 public:
  ...
  bool GetNoiseCertainty(const Vector& NoiseLocation, const float NoiseRadius,
 ...
 private:
  void HandleNoise(WBEntity* const pNoiseEntity, const Vector& NoiseLocation,
                   const float NoiseRadius, const float NoiseUpdateTime,
                   const float ExpireTimeBonus) const;

  float m_Radius;  // Config; adds to noise's radius, so some AIs can hear
                   // further than others. Can be negative.
