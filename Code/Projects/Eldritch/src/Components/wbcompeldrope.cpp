...
/*virtual*/ void WBCompEldRope::InitializeFromDefinition(
  ...
  m_HookLength =
      ConfigManager::GetInheritedFloat(sHookLength, 0.0f, sDefinitionName);

  STATICHASH(DangleHeight);
  m_DangleHeight =
      ConfigManager::GetInheritedFloat(sDangleHeight, 0.0f, sDefinitionName);

  ASSERT(m_DangleHeight < m_HookLength);

  STATICHASH(HookEntity);
  m_HookEntity =
      ConfigManager::GetInheritedString(sHookEntity, "", sDefinitionName);
  ASSERT(m_HookEntity != "");
}
