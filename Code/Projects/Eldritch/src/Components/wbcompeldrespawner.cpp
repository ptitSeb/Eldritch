...
/*virtual*/ void WBCompEldRespawner::InitializeFromDefinition(
  ...
  m_RetryRespawnTime = ConfigManager::GetInheritedFloat(sRetryRespawnTime, 0.0f,
                                                        sDefinitionName);

  STATICHASH(RespawnMinPlayerDistance);
  m_RespawnMinPlayerDistanceSq = Square(ConfigManager::GetInheritedFloat(
      sRespawnMinPlayerDistance, 0.0f, sDefinitionName));
}
...
/*virtual*/ void WBCompEldRespawner::HandleEvent(const WBEvent& Event) {
  ...
  if (EventName == sOnSpawnedQueued) {
    ASSERT(!m_OriginSet);  // This isn't really meaningful anymore since I use
                           // OnSpawnedQueued instead of OnMoved
    m_OriginSet = true;
...
void WBCompEldRespawner::TryRespawn() {
  if (CanRespawn()) {
    ...
    WB_QUEUE_EVENT_DELAY(GetEventManager(), Respawn, GetEntity(),
                         m_RetryRespawnTime);
  }
}
...
bool WBCompEldRespawner::CanOriginBeSeenByPlayer() {
  ...
  const bool Occluded = GetWorld()->LineCheck(
      EldritchGame::GetPlayerViewLocation(), m_OriginLocation, Info);
...
void WBCompEldRespawner::Respawn() {
  ...
  WBEntity* const pSpawnedEntity =
      WBWorld::GetInstance()->CreateEntity(GetEntity()->GetName());
