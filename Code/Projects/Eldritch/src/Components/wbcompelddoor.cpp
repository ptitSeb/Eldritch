...
/*virtual*/ void WBCompEldDoor::InitializeFromDefinition(
  ...
  m_InterpTime =
      ConfigManager::GetInheritedFloat(sInterpTime, 0.0f, sDefinitionName);

  STATICHASH(ClosedOffsetX);
  m_ClosedOffset.x =
      ConfigManager::GetInheritedFloat(sClosedOffsetX, 0.0f, sDefinitionName);

  STATICHASH(ClosedOffsetY);
  m_ClosedOffset.y =
      ConfigManager::GetInheritedFloat(sClosedOffsetY, 0.0f, sDefinitionName);

  STATICHASH(ClosedOffsetZ);
  m_ClosedOffset.z =
      ConfigManager::GetInheritedFloat(sClosedOffsetZ, 0.0f, sDefinitionName);

  STATICHASH(ClosedAngle);
  m_ClosedOrientation.Yaw = DEGREES_TO_RADIANS(
      ConfigManager::GetInheritedFloat(sClosedAngle, 0.0f, sDefinitionName));

  STATICHASH(ClosedFrobOffsetX);
  m_ClosedFrobOffset.x = ConfigManager::GetInheritedFloat(
      sClosedFrobOffsetX, 0.0f, sDefinitionName);

  STATICHASH(ClosedFrobOffsetY);
  m_ClosedFrobOffset.y = ConfigManager::GetInheritedFloat(
      sClosedFrobOffsetY, 0.0f, sDefinitionName);

  STATICHASH(ClosedFrobOffsetZ);
  m_ClosedFrobOffset.z = ConfigManager::GetInheritedFloat(
      sClosedFrobOffsetZ, 0.0f, sDefinitionName);

  STATICHASH(ClosedFrobBoxX);
  m_ClosedFrobExtents.x =
      ConfigManager::GetInheritedFloat(sClosedFrobBoxX, 0.0f, sDefinitionName);

  STATICHASH(ClosedFrobBoxY);
  m_ClosedFrobExtents.y =
      ConfigManager::GetInheritedFloat(sClosedFrobBoxY, 0.0f, sDefinitionName);

  STATICHASH(ClosedFrobBoxZ);
  m_ClosedFrobExtents.z =
      ConfigManager::GetInheritedFloat(sClosedFrobBoxZ, 0.0f, sDefinitionName);

  STATICHASH(ClosedIrrX);
  m_ClosedIrradianceOffset.x =
      ConfigManager::GetInheritedFloat(sClosedIrrX, 0.0f, sDefinitionName);

  STATICHASH(ClosedIrrY);
  m_ClosedIrradianceOffset.y =
      ConfigManager::GetInheritedFloat(sClosedIrrY, 0.0f, sDefinitionName);

  STATICHASH(ClosedIrrZ);
  m_ClosedIrradianceOffset.z =
      ConfigManager::GetInheritedFloat(sClosedIrrZ, 0.0f, sDefinitionName);

  STATICHASH(OpenOffsetX);
  m_OpenOffset.x =
      ConfigManager::GetInheritedFloat(sOpenOffsetX, 0.0f, sDefinitionName);

  STATICHASH(OpenOffsetY);
  m_OpenOffset.y =
      ConfigManager::GetInheritedFloat(sOpenOffsetY, 0.0f, sDefinitionName);

  STATICHASH(OpenOffsetZ);
  m_OpenOffset.z =
      ConfigManager::GetInheritedFloat(sOpenOffsetZ, 0.0f, sDefinitionName);

  STATICHASH(OpenAngle);
  m_OpenOrientation.Yaw = DEGREES_TO_RADIANS(
      ConfigManager::GetInheritedFloat(sOpenAngle, 0.0f, sDefinitionName));

  STATICHASH(OpenFrobOffsetX);
  m_OpenFrobOffset.x =
      ConfigManager::GetInheritedFloat(sOpenFrobOffsetX, 0.0f, sDefinitionName);

  STATICHASH(OpenFrobOffsetY);
  m_OpenFrobOffset.y =
      ConfigManager::GetInheritedFloat(sOpenFrobOffsetY, 0.0f, sDefinitionName);

  STATICHASH(OpenFrobOffsetZ);
  m_OpenFrobOffset.z =
      ConfigManager::GetInheritedFloat(sOpenFrobOffsetZ, 0.0f, sDefinitionName);

  STATICHASH(OpenFrobBoxX);
  m_OpenFrobExtents.x =
      ConfigManager::GetInheritedFloat(sOpenFrobBoxX, 0.0f, sDefinitionName);

  STATICHASH(OpenFrobBoxY);
  m_OpenFrobExtents.y =
      ConfigManager::GetInheritedFloat(sOpenFrobBoxY, 0.0f, sDefinitionName);

  STATICHASH(OpenFrobBoxZ);
  m_OpenFrobExtents.z =
      ConfigManager::GetInheritedFloat(sOpenFrobBoxZ, 0.0f, sDefinitionName);

  STATICHASH(OpenIrrX);
  m_OpenIrradianceOffset.x =
      ConfigManager::GetInheritedFloat(sOpenIrrX, 0.0f, sDefinitionName);

  STATICHASH(OpenIrrY);
  m_OpenIrradianceOffset.y =
      ConfigManager::GetInheritedFloat(sOpenIrrY, 0.0f, sDefinitionName);

  STATICHASH(OpenIrrZ);
  m_OpenIrradianceOffset.z =
      ConfigManager::GetInheritedFloat(sOpenIrrZ, 0.0f, sDefinitionName);
  ...
  m_UnlockedMesh =
      ConfigManager::GetInheritedString(sUnlockedMesh, "", sDefinitionName);

  STATICHASH(LockedMesh);
  m_LockedMesh =
      ConfigManager::GetInheritedString(sLockedMesh, "", sDefinitionName);

  STATICHASH(UnlockedTexture);
  m_UnlockedTexture =
      ConfigManager::GetInheritedString(sUnlockedTexture, "", sDefinitionName);

  STATICHASH(LockedTexture);
  m_LockedTexture =
      ConfigManager::GetInheritedString(sLockedTexture, "", sDefinitionName);

  STATICHASH(UnlockedFriendlyName);
  m_UnlockedFriendlyName = ConfigManager::GetInheritedString(
      sUnlockedFriendlyName, "", sDefinitionName);

  STATICHASH(LockedFriendlyName);
  m_LockedFriendlyName = ConfigManager::GetInheritedString(sLockedFriendlyName,
                                                           "", sDefinitionName);
}
...
WBCompEldDoor::EDoorRotation WBCompEldDoor::GetRotation() const {
  ...
  if (Equal(Yaw, DEGREES_TO_RADIANS(0.0f))) {
    return EDR_0;
  }
  if (Equal(Yaw, DEGREES_TO_RADIANS(90.0f))) {
    return EDR_90;
  }
  if (Equal(Yaw, DEGREES_TO_RADIANS(180.0f))) {
    return EDR_180;
  }
  if (Equal(Yaw, DEGREES_TO_RADIANS(270.0f))) {
    return EDR_270;
  }
...
Vector WBCompEldDoor::RotateExtents(const Vector& Extents,
                                    const EDoorRotation Rotation) const {
  ASSERT(Rotation != EDR_0);

  return (Rotation == EDR_180) ? Extents
                               : Vector(Extents.y, Extents.x, Extents.z);
}

Vector WBCompEldDoor::RotateOffset(const Vector& Offset,
                                   const EDoorRotation Rotation) const {
  ASSERT(Rotation != EDR_0);
...
float WBCompEldDoor::RotateYaw(const float Yaw,
                               const EDoorRotation Rotation) const {
  ASSERT(Rotation != EDR_0);
...
/*virtual*/ void WBCompEldDoor::AddContextToEvent(WBEvent& Event) const {
  ...
  WB_SET_CONTEXT(Event, Bool, IsDoor,
                 true);  // To distinguish from e.g. EldLock component
...
void WBCompEldDoor::TryToggle(WBEntity* const pFrobber) {
  if (m_Open) {
    ...
    if (pWorld->CheckClearance(pTransform->GetLocation(),
                               pCollision->GetExtents(), Info)) {
...
void WBCompEldDoor::Lock() {
  ...
  // If the door is closed, clear current nav state since we'll be adding the
  // locked flag
  if (!m_Open) {
...
void WBCompEldDoor::Unlock() {
  ...
  // If the door is closed, clear current nav state since we'll be removing the
  // locked flag
  if (!m_Open) {
    EldritchNav::GetInstance()->UpdateWorldFromEntity(pCollision, false);
  }
...
void WBCompEldDoor::UpdateFromOpenState(const bool InitialSetup,
                                        const bool SuppressWorldChange) {
  EldritchWorld* const pWorld = GetWorld();
  DEVASSERT(pWorld);
  ...
  const float InterpTime =
      InitialSetup ? 0.0f : (m_InterpTime * CurrentInterpT);

  const uint OcclusionFlag =
      pCollision->GetDefaultCollisionFlags() & EECF_BlocksOcclusion;
  const uint DoorCollisionMask =
      OcclusionFlag | EECF_BlocksEntities | EECF_BlocksTrace;

  if (m_Open) {
    m_OffsetInterpolator.Reset(Interpolator<Vector>::EIT_Linear,
                               m_OffsetInterpolator.GetValue(), m_OpenOffset,
                               InterpTime);
    m_OrientationInterpolator.Reset(Interpolator<Angles>::EIT_Linear,
                                    m_OrientationInterpolator.GetValue(),
                                    m_OpenOrientation, InterpTime);
    ...
    if (InitialSetup) {
      // Do nothing, a serialized open door should already be removed from the
      // nav world.
    } else {
      EldritchNav::GetInstance()->UpdateWorldFromEntity(pCollision, false);
    }
    ...
    m_OffsetInterpolator.Reset(Interpolator<Vector>::EIT_Linear,
                               m_OffsetInterpolator.GetValue(), m_ClosedOffset,
                               InterpTime);
    m_OrientationInterpolator.Reset(Interpolator<Angles>::EIT_Linear,
                                    m_OrientationInterpolator.GetValue(),
                                    m_ClosedOrientation, InterpTime);
    ...
    if (InitialSetup) {
      // HACK: Don't double up where EldCollision already updates the nav world
      // from this entity.
      // TODO: I might need to do this for OnBoxChanged, too; it's possible that
      // those problems (false
      // shadows, etc.) are just hidden by virtue of a whole mesh getting
      // updated when the box changes.
      // If I ever see inexplicable shadows near doors, save the random seed and
      // try that.
    } else {
      EldritchNav::GetInstance()->UpdateWorldFromEntity(pCollision, true);
    }
