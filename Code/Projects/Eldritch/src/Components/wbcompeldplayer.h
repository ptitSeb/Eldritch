...
class WBCompEldPlayer : public WBEldritchComponent,
                        public IInputSystemObserver {
 public:
  WBCompEldPlayer();
 ...
 private:
  ...
  SimpleString GetDamageDesc(WBEntity* const pDamager,
                             const HashedString& DamageType) const;
  ...
  int m_ClimbRefs;  // Serialized; refcount climbing so we can transfer
                    // climbables without issue
