...
class WBCompEldMinimap : public WBEldritchComponent {
 ...
 private:
  ...
  void UpdateMarker(WBEntity* const pEntity, const Vector& Location,
                    const Angles& Orientation, const bool MarkAsRoom);
  void RemoveMarker(WBEntity* const pEntity);

  uint GetAtlasIndex(const uint RoomExits, const uint RoomStatus,
  ...
  Array<Array<Mesh*> > m_MinimapFloorMeshes;  // Transient, same meshes as
                                              // m_MinimapRoomMeshes but sorted
                                              // by floor
