...
/*virtual*/ void WBCompEldWatson::InitializeFromDefinition(
  ...
  m_MinAttackDistSq = Square(
      ConfigManager::GetInheritedFloat(sMinAttackDist, 0.0f, sDefinitionName));

  STATICHASH(MaxAttackDist);
  m_MaxAttackDistSq = Square(
      ConfigManager::GetInheritedFloat(sMaxAttackDist, 0.0f, sDefinitionName));

  STATICHASH(TeleportDist);
  m_TeleportDist =
      ConfigManager::GetInheritedFloat(sTeleportDist, 0.0f, sDefinitionName);
...
void WBCompEldWatson::TickPrimed() {
  ...
  WBCompEldTransform* const pPlayerTransform =
...
void WBCompEldWatson::TickActivated() {
  ...
  // Don't check min distance, because we don't want to re-prime right after an
  // attack.
...
bool WBCompEldWatson::IsPlayerVulnerable(const bool CheckMinDistance) const {
  ...
  // when player is looking up or down at it, and 2D is necessary so Watson
  // doesn't
...
void WBCompEldWatson::TickUnactivated() {
  ...
  WBCompEldSensorVision* const pVision =
      GET_WBCOMP(GetEntity(), EldSensorVision);
  ASSERT(pVision);
