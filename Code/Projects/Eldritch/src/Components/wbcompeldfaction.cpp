...
WBCompEldFaction::WBCompEldFaction() : m_Faction(), m_Immutable(false) {
  EldritchFactions::AddRef();
}

WBCompEldFaction::~WBCompEldFaction() { EldritchFactions::Release(); }

/*virtual*/ void WBCompEldFaction::InitializeFromDefinition(
  ...
  STATICHASH(Faction);
  m_Faction = ConfigManager::GetInheritedHash(
      sFaction, HashedString::NullString, sDefinitionName);

  STATICHASH(Immutable);
  m_Immutable =
      ConfigManager::GetInheritedBool(sImmutable, false, sDefinitionName);
}
...
EldritchFactions::EFactionCon WBCompEldFaction::GetCon(
    const WBEntity* const pEntityB) {
  ASSERT(pEntityB);
...
EldritchFactions::EFactionCon WBCompEldFaction::GetCon(
    const HashedString& FactionB) {
  return EldritchFactions::GetCon(m_Faction, FactionB);
}

/*static*/ EldritchFactions::EFactionCon WBCompEldFaction::GetCon(
    const WBEntity* const pEntityA, const WBEntity* const pEntityB) {
  ASSERT(pEntityA);
  ASSERT(pEntityB);
