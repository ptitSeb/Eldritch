...
class WBCompEldCharacter : public WBEldritchComponent {
 public:
  ...
  SimpleString GetCurrentVoiceSet() const {
    return GetCurrentHeadOption().m_VoiceSet;
  }
 ...
 private:
  ...
  const SHeadOption& GetCurrentHeadOption() const {
    return m_HeadOptions[m_CurrentHeadIndex];
  }
  const SBodyOption& GetCurrentBodyOption() const {
    return m_BodyOptions[m_CurrentBodyIndex];
  }
