...
/*virtual*/ void WBCompEldFrobbable::InitializeFromDefinition(
  ...
  m_IsFrobbable =
      ConfigManager::GetInheritedBool(sIsFrobbable, true, sDefinitionName);

  STATICHASH(HoldReleaseMode);
  m_HoldReleaseMode =
      ConfigManager::GetInheritedBool(sHoldReleaseMode, false, sDefinitionName);

  STATICHASH(UseCollisionExtents);
  m_UseCollisionExtents = ConfigManager::GetInheritedBool(
      sUseCollisionExtents, false, sDefinitionName);

  STATICHASH(UseMeshExtents);
  m_UseMeshExtents =
      ConfigManager::GetInheritedBool(sUseMeshExtents, false, sDefinitionName);

  STATICHASH(ExtentsFatten);
  m_ExtentsFatten =
      ConfigManager::GetInheritedFloat(sExtentsFatten, 0.0f, sDefinitionName);

  STATICHASH(ExtentsX);
  m_BoundExtents.x =
      ConfigManager::GetInheritedFloat(sExtentsX, 0.0f, sDefinitionName);

  STATICHASH(ExtentsY);
  m_BoundExtents.y =
      ConfigManager::GetInheritedFloat(sExtentsY, 0.0f, sDefinitionName);

  STATICHASH(ExtentsZ);
  m_BoundExtents.z =
      ConfigManager::GetInheritedFloat(sExtentsZ, 0.0f, sDefinitionName);
  ...
  const float DefaultHighlightR =
      ConfigManager::GetFloat(sHighlightR, 0.0f, sEldFrobbable);
  m_Highlight.r = ConfigManager::GetInheritedFloat(
      sHighlightR, DefaultHighlightR, sDefinitionName);

  STATICHASH(HighlightG);
  const float DefaultHighlightG =
      ConfigManager::GetFloat(sHighlightG, 0.0f, sEldFrobbable);
  m_Highlight.g = ConfigManager::GetInheritedFloat(
      sHighlightG, DefaultHighlightG, sDefinitionName);

  STATICHASH(HighlightB);
  const float DefaultHighlightB =
      ConfigManager::GetFloat(sHighlightB, 0.0f, sEldFrobbable);
  m_Highlight.b = ConfigManager::GetInheritedFloat(
      sHighlightB, DefaultHighlightB, sDefinitionName);

  STATICHASH(FriendlyName);
  m_FriendlyName = ConfigManager::GetInheritedString(
      sFriendlyName, GetEntity()->GetName().CStr(), sDefinitionName);

  STATICHASH(FrobVerb);
  m_FrobVerb =
      ConfigManager::GetInheritedString(sFrobVerb, "", sDefinitionName);

  STATICHASH(HoldVerb);
  m_HoldVerb =
      ConfigManager::GetInheritedString(sHoldVerb, "", sDefinitionName);
}
...
void WBCompEldFrobbable::MarshalFrob(WBEntity* const pFrobber,
                                     const int InputEdge) {
  if (m_HoldReleaseMode) {
    if (InputEdge == InputSystem::EIE_OnRise) {
      ...
      m_HandleHoldRelease =
          false;  // NOTE: This means we won't get the OnRelease event for this
                  // input! That's what I want currently, but maybe not always.
...
void WBCompEldFrobbable::SendOnFrobbedHeldEvent(
    WBEntity* const pFrobber) const {
