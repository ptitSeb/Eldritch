...
/*virtual*/ void WBCompEldCharacter::InitializeFromDefinition(
  ...
  const uint NumHeadOptions =
      ConfigManager::GetInheritedInt(sNumHeadOptions, 0, sDefinitionName);
  for (uint HeadOptionIndex = 0; HeadOptionIndex < NumHeadOptions;
       ++HeadOptionIndex) {
    SHeadOption& HeadOption = m_HeadOptions.PushBack();

    const SimpleString HeadOptionDef =
        ConfigManager::GetInheritedSequenceString(
            "HeadOption%d", HeadOptionIndex, "", sDefinitionName);
    ...
    STATICHASH(Texture);
    HeadOption.m_Texture =
        ConfigManager::GetString(sTexture, "", sHeadOptionDef);

    STATICHASH(VoiceSet);
    HeadOption.m_VoiceSet =
        ConfigManager::GetString(sVoiceSet, "", sHeadOptionDef);
  }

  STATICHASH(NumBodyOptions);
  const uint NumBodyOptions =
      ConfigManager::GetInheritedInt(sNumBodyOptions, 0, sDefinitionName);
  for (uint BodyOptionIndex = 0; BodyOptionIndex < NumBodyOptions;
       ++BodyOptionIndex) {
    SBodyOption& BodyOption = m_BodyOptions.PushBack();

    const SimpleString BodyOptionDef =
        ConfigManager::GetInheritedSequenceString(
            "BodyOption%d", BodyOptionIndex, "", sDefinitionName);
    ...
    STATICHASH(Texture);
    BodyOption.m_Texture =
        ConfigManager::GetString(sTexture, "", sBodyOptionDef);

    STATICHASH(LeftHandMesh);
    BodyOption.m_LeftHandMesh =
        ConfigManager::GetString(sLeftHandMesh, "", sBodyOptionDef);

    STATICHASH(LeftHandTexture);
    BodyOption.m_LeftHandTexture =
        ConfigManager::GetString(sLeftHandTexture, "", sBodyOptionDef);

    STATICHASH(RightHandMesh);
    BodyOption.m_RightHandMesh =
        ConfigManager::GetString(sRightHandMesh, "", sBodyOptionDef);

    STATICHASH(RightHandTexture);
    BodyOption.m_RightHandTexture =
        ConfigManager::GetString(sRightHandTexture, "", sBodyOptionDef);
  }
}
...
/*virtual*/ void WBCompEldCharacter::HandleEvent(const WBEvent& Event) {
  ...
  if (EventName == sOnSpawned) {
    ...
    // Everything in the world should already be serialized properly, but we
    // need to push to UI screen.
    PushCharacterOptions();
...
void WBCompEldCharacter::PushCharacterOptions() const {
  ...
  // Update hands
  {
    ...
    WB_SET_AUTO(SetHandMeshes, Hash, LeftHandTexture,
                CurrentBody.m_LeftHandTexture);
    WB_SET_AUTO(SetHandMeshes, Hash, RightHandMesh,
                CurrentBody.m_RightHandMesh);
    WB_SET_AUTO(SetHandMeshes, Hash, RightHandTexture,
                CurrentBody.m_RightHandTexture);
  ...
  // Update head in mirror
  {
    ...
    WB_DISPATCH_EVENT(GetEventManager(), SetHeadMesh,
                      EldritchGame::GetMirrorScreen());
  }
  ...
  // Update body in mirror
  {
    ...
    WB_DISPATCH_EVENT(GetEventManager(), SetBodyMesh,
                      EldritchGame::GetMirrorScreen());
  }
...
void WBCompEldCharacter::ChangeHead(int IndexStep) {
  ...
  m_CurrentHeadIndex =
      (m_CurrentHeadIndex + NumHeadOptions + IndexStep) % NumHeadOptions;
...
void WBCompEldCharacter::ChangeBody(int IndexStep) {
  ...
  m_CurrentBodyIndex =
      (m_CurrentBodyIndex + NumBodyOptions + IndexStep) % NumBodyOptions;
