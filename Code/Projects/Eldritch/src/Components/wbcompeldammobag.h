...
class WBCompEldAmmoBag : public WBEldritchComponent {
 public:
  ...
  bool HasAmmo(const HashedString& AmmoType) const {
 ...
 private:
  ...
  HashedString m_CurrentAmmo;  // Transient, cache so we don't need to look it
                               // up on the current weapon
  ...
  HashedString m_DefaultAmmo;  // Config, the universal ammo type when no weapon
                               // is equipped.
