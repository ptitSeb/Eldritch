...
WBCompEldAnchor::WBCompEldAnchor()
    : m_IsAnchored(false), m_AnchorPoint(), m_AnchorDirection() {
...
/*virtual*/ void WBCompEldAnchor::InitializeFromDefinition(
  ...
  m_AnchorDirection.x = ConfigManager::GetInheritedFloat(sAnchorDirectionX,
                                                         0.0f, sDefinitionName);

  STATICHASH(AnchorDirectionY);
  m_AnchorDirection.y = ConfigManager::GetInheritedFloat(sAnchorDirectionY,
                                                         0.0f, sDefinitionName);

  STATICHASH(AnchorDirectionZ);
  m_AnchorDirection.z = ConfigManager::GetInheritedFloat(sAnchorDirectionZ,
                                                         0.0f, sDefinitionName);
}
...
/*virtual*/ void WBCompEldAnchor::HandleEvent(const WBEvent& Event) {
  ...
  if (EventName == sOnSpawnedQueued) {
    SetAnchor();
             ...
             EventName == sOnStaticCollisionChanged) {
    if (m_IsAnchored) {
      CheckAnchor();
    }
...
void WBCompEldAnchor::SetAnchor() {
  ...
  if (pWorld->Trace(TraceRay, Info)) {
    ...
    ASSERT(pWorld->PointCheck(
        m_AnchorPoint,
        Info));  // Since hard-coded 0.1 might break for thin surfaces.
    m_IsAnchored = true;
  }
}
