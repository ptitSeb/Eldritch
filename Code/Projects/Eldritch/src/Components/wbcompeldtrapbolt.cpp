...
/*virtual*/ void WBCompEldTrapBolt::InitializeFromDefinition(
  ...
  m_CollisionFatten =
      ConfigManager::GetInheritedFloat(sCollisionFatten, 0.0f, sDefinitionName);

  STATICHASH(MeshFatten);
  m_MeshFatten =
      ConfigManager::GetInheritedFloat(sMeshFatten, 0.0f, sDefinitionName);

  STATICHASH(EndpointSpacing);
  m_EndpointSpacing =
      ConfigManager::GetInheritedFloat(sEndpointSpacing, 0.0f, sDefinitionName);

  STATICHASH(AnchorDepth);
  m_AnchorDepth =
      ConfigManager::GetInheritedFloat(sAnchorDepth, 0.0f, sDefinitionName);
}
...
void WBCompEldTrapBolt::LaunchTrapBolt() {
  ...
  // Since we're using AABBs for collision, we can only support trap bolts on
  // primary axes.
  ...
  if (pWorld->Trace(ForwardRay, ForwardInfo) &&
    ...
    // Set launched before setting extents, so anything we touch when extents
    // are set will immediately trigger this.
    m_Launched = true;

    // Reset orientation now that it is being "baked" into extents.
    pTransform->SetOrientation(Angles());

    const Vector ForwardEndpoint =
        ForwardInfo.m_Intersection +
        ForwardInfo.m_Plane.m_Normal * m_EndpointSpacing;
    const Vector BackEndpoint =
        BackInfo.m_Intersection + BackInfo.m_Plane.m_Normal * m_EndpointSpacing;
    pTransform->SetLocation(0.5f * (ForwardEndpoint + BackEndpoint));

    const Vector HalfExtents =
        (0.5f * (ForwardEndpoint - BackEndpoint)).GetAbs();
...
void WBCompEldTrapBolt::TriggerTrapBolt() {
  ...
  for (uint TouchingEntityIndex = 0; TouchingEntityIndex < NumTouchingEntities;
       ++TouchingEntityIndex) {
