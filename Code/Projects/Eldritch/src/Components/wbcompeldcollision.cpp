...
/*static*/ WBCompEldCollision::TCollisionMap
    WBCompEldCollision::sm_CollisionMap;
/*static*/ WBCompEldCollision::TCollisionArray
    WBCompEldCollision::sm_TouchingArray;
...
/*virtual*/ void WBCompEldCollision::InitializeFromDefinition(
  ...
  // "Static" is a poor name, but these entities will be included in collision
  // (not touching)
  // tests, occlusion tests, and nav tests. See EEldritchCollisionFlags for
  // exact definition.
  STATICHASH(IsStatic);
  const bool IsStatic =
      ConfigManager::GetInheritedBool(sIsStatic, false, sDefinitionName);
  SetCollisionFlags(IsStatic ? EECF_IsStatic : EECF_IsDynamic,
                    EECF_Mask_EntityTypes, false, false);

  STATICHASH(CanTouch);
  m_CanTouch =
      ConfigManager::GetInheritedBool(sCanTouch, !IsStatic, sDefinitionName);

  STATICHASH(HalfExtentsXY);
  const float HalfExtentsXY =
      ConfigManager::GetInheritedFloat(sHalfExtentsXY, 0.0f, sDefinitionName);

  STATICHASH(HalfExtentsX);
  const float HalfExtentsX = ConfigManager::GetInheritedFloat(
      sHalfExtentsX, HalfExtentsXY, sDefinitionName);

  STATICHASH(HalfExtentsY);
  const float HalfExtentsY = ConfigManager::GetInheritedFloat(
      sHalfExtentsY, HalfExtentsXY, sDefinitionName);

  STATICHASH(HalfExtentsZ);
  const float HalfExtentsZ =
      ConfigManager::GetInheritedFloat(sHalfExtentsZ, 0.0f, sDefinitionName);
  ...
  const float DefaultFrictionTargetTime =
      ConfigManager::GetFloat(sFrictionTargetTime, 0.0f, sEldCollision);
  m_FrictionTargetTime = ConfigManager::GetInheritedFloat(
      sFrictionTargetTime, DefaultFrictionTargetTime, sDefinitionName);
  ASSERT(m_FrictionTargetTime > 0.0f);

  STATICHASH(FrictionTargetPercentVelocity);
  const float DefaultFrictionTargetPercentVelocity = ConfigManager::GetFloat(
      sFrictionTargetPercentVelocity, 0.0f, sEldCollision);
  m_FrictionTargetPercentVelocity = ConfigManager::GetInheritedFloat(
      sFrictionTargetPercentVelocity, DefaultFrictionTargetPercentVelocity,
      sDefinitionName);

  STATICHASH(BlocksWorld);
  const bool BlocksWorld =
      ConfigManager::GetInheritedBool(sBlocksWorld, true, sDefinitionName);
  SetCollisionFlags(BlocksWorld ? EECF_BlocksWorld : EECF_None,
                    EECF_BlocksWorld, false, false);

  STATICHASH(BlocksEntities);
  const bool BlocksEntities =
      ConfigManager::GetInheritedBool(sBlocksEntities, false, sDefinitionName);
  SetCollisionFlags(BlocksEntities ? EECF_BlocksEntities : EECF_None,
                    EECF_BlocksEntities, false, false);

  STATICHASH(BlocksOcclusion);
  const bool BlocksOcclusion =
      ConfigManager::GetInheritedBool(sBlocksOcclusion, false, sDefinitionName);
  SetCollisionFlags(BlocksOcclusion ? EECF_BlocksOcclusion : EECF_None,
                    EECF_BlocksOcclusion, false, false);

  STATICHASH(BlocksTrace);
  const bool BlocksTrace =
      ConfigManager::GetInheritedBool(sBlocksTrace, false, sDefinitionName);
  SetCollisionFlags(BlocksTrace ? EECF_BlocksTrace : EECF_None,
                    EECF_BlocksTrace, false, false);
...
void WBCompEldCollision::SetCollisionFlags(
    const uint Flags, const uint Mask /*= 0xffffffff*/,
    const bool SendEvents /*= false*/,
    const bool UpdateCollisionMap /*= true*/) {
  XTRACE_FUNCTION;
...
float WBCompEldCollision::GetFrictionCoefficient() const {
  ...
  // Relationship between friction and the velocity we want to be reduced to at
  // time t:
  ...
  const float Mu =
      Pow(m_FrictionTargetPercentVelocity,
          DeltaTime / m_FrictionTargetTime);  // Friction coefficient
  return Mu;
}

void WBCompEldCollision::Collide(const Vector& StartLocation,
                                 Vector& InOutMovement) {
  XTRACE_FUNCTION;
  ...
  do {
    ...
    if (GetWorld()->Sweep(SweepSegment, m_HalfExtents, Info)) {
      // Collision response
      // TODO: Apply friction, reflection, zero out velocity on transform
      // component, whatever.
      ...
      if (Info.m_Plane.m_Normal.LengthSquared() > 0.0f) {
        ...
        // (because you don't want to grip walls/ceilings the way you do
        // floors).
        // Or maybe a factor of the entity's velocity on the normal? Is that how
        // friction works?
        const Vector OldVelocity = pTransform->GetVelocity();
        const Vector VelocityInCollisionPlane =
            Info.m_Plane.ProjectVector(OldVelocity);
        const Vector OrthogonalVelocityComponent =
            VelocityInCollisionPlane * Friction;

        const Vector VelocityInCollisionNormal =
            OldVelocity.ProjectionOnto(Info.m_Plane.m_Normal);
        const Vector ParallelVelocityComponent =
            VelocityInCollisionNormal * -m_Elasticity;

        const Vector NewVelocity =
            OrthogonalVelocityComponent + ParallelVelocityComponent;
        ...
        // This was commented out for a long time... but I can't see any reason
        // not to do it,
        // and it fixes things acquiring a huge downward velocity from gravity
        // while they're stuck.
...
void WBCompEldCollision::OnLanded(const float LandedMagnitude,
                                  WBEntity* const pCollidedEntity) {
  XTRACE_FUNCTION;
...
void WBCompEldCollision::OnCollided(const Vector& CollisionNormal,
                                    WBEntity* const pCollidedEntity) {
  XTRACE_FUNCTION;

  // OnCollided actually means *on collided with non-ground surface*, as
  // distinguished from OnLanded.
...
AABB WBCompEldCollision::GetCurrentBounds() const {
  ...
  return AABB::CreateFromCenterAndExtents(pTransform->GetLocation(),
                                          m_HalfExtents);
}
...
/*virtual*/ void WBCompEldCollision::DebugRender() const {
  ...
  GetFramework()->GetRenderer()->DEBUGDrawBox(
      Location - m_HalfExtents, Location + m_HalfExtents,
...
/*virtual*/ void WBCompEldCollision::HandleEvent(const WBEvent& Event) {
  ...
  if (EventName == sOnJumped) {
    ...
    // For initialization of certain classes of static entities, fix up extents
    ...
    if (!m_CanTouch) {
      m_CanTouch = true;
      SetCollisionFlags(m_DefaultCollisionFlags & EECF_BlocksTrace,
                        EECF_BlocksTrace);
...
/*virtual*/ void WBCompEldCollision::AddContextToEvent(WBEvent& Event) const {
  ...
  WB_SET_CONTEXT(Event, Bool, IsStatic,
                 MatchesAllCollisionFlags(EECF_IsStatic));
}
...
void WBCompEldCollision::SetExtents(const Vector& HalfExtents) {
  // If this entity is a nav blocker, remove its old bounds before updating the
  // extents.
  ConditionalSetNavBlocking(false);
  ...
  // If this entity is a nav blocker, adds its new bounds after updating the
  // extents.
  ConditionalSetNavBlocking(true);
}
...
void WBCompEldCollision::GatherTouching(Array<WBEntityRef>& OutTouching) const {
  ...
  WBCompEldTransform* const pThisTransform =
      pThisEntity->GetTransformComponent<WBCompEldTransform>();
  DEVASSERT(pThisTransform);

  const AABB ThisBox = AABB::CreateFromCenterAndExtents(
      pThisTransform->GetLocation(), m_HalfExtents);

  const Array<WBCompEldCollision*>& CollisionComponents = GetTouchingArray();
  const uint NumEntities = CollisionComponents.Size();
  for (uint CollisionEntityIndex = 0; CollisionEntityIndex < NumEntities;
       ++CollisionEntityIndex) {
    WBCompEldCollision* const pCollision =
        CollisionComponents[CollisionEntityIndex];
...
// NOTE: This does not capture touches that occur during a sweep; as such, it
// would
// be possible for an object traveling very fast to tunnel through another
// object
...
void WBCompEldCollision::SendTouchEvent(const WBEntityRef& TouchingEntity) {
  ...
  WBCompEldCollision* const pTouchingCollision =
      GET_WBCOMP(pTouchingEntity, EldCollision);
...
void WBCompEldCollision::SendUntouchEvent(const WBEntityRef& TouchingEntity) {
  ...
  WBEntity* const pTouchingEntity = TouchingEntity.Get();
  WBCompEldCollision* const pTouchingCollision =
      SAFE_GET_WBCOMP(pTouchingEntity, EldCollision);
...
void WBCompEldCollision::GetTouchingEntities(
    Array<WBEntity*>& OutTouchingEntities) const {
...
#define CONDITIONAL_ADD_TO_COLLISION_MAP(flag) \
  if (MatchesAllCollisionFlags(flag)) {        \
    AddToCollisionArray(flag);                 \
  }
...
#define CONDITIONAL_REMOVE_FROM_COLLISION_MAP(flag) \
  if (MatchesAllCollisionFlags(flag)) {             \
    RemoveFromCollisionArray(flag);                 \
  }
...
/*static*/ const WBCompEldCollision::TCollisionArray*
WBCompEldCollision::GetCollisionArray(const uint Flags) {
