...
/*virtual*/ void WBCompEldMinimap::InitializeFromDefinition(
  ...
  const SimpleString RoomAtlasTexture =
      ConfigManager::GetString(sRoomAtlasTexture, "", sEldMinimap);
  m_RoomAtlasTexture =
      pRenderer->GetTextureManager()->GetTexture(RoomAtlasTexture.CStr());
  ASSERT(m_RoomAtlasTexture);

  STATICHASH(Material);
  const SimpleString Material =
      ConfigManager::GetString(sMaterial, "", sEldMinimap);
  m_Material.SetDefinition(Material, pRenderer, pRenderer->GetVertexDeclaration(
                                                    VD_POSITIONS | VD_UVS));

  EldritchWorld* const pWorld = GetWorld();
  m_RoomStatus.ResizeZero(pWorld->GetMapSizeX() * pWorld->GetMapSizeY() *
                          pWorld->GetMapSizeZ());
  ...
  for (uint FloorIndex = 0; FloorIndex < NumFloors; ++FloorIndex) {
    ...
    FOR_EACH_ARRAY(MeshIter, FloorMeshes, Mesh*) {
      ...
      const uint AtlasIndex =
          GetAtlasIndex(RoomExits, RoomStatus, IsActiveFloor);
...
uint WBCompEldMinimap::GetAtlasIndex(const uint RoomExits,
                                     const uint RoomStatus,
                                     const bool ActiveFloor) const {
  // Atlas is arranged as a 2x2 map of 8x8 tiles, ordered: inactive floor (0),
  // hidden (1), inactive (2), active (3)
...
Mesh* WBCompEldMinimap::CreateQuad(const uint AtlasIndex,
                                   Mesh* const pMesh) const {
  XTRACE_FUNCTION;
  ...
  if (pMesh) {
    ...
    pMesh->~Mesh();
    pQuadMesh =
        new (pMesh) Mesh(pVertexBuffer, pVertexDeclaration, pIndexBuffer);
  } else {
    pQuadMesh = new Mesh(pVertexBuffer, pVertexDeclaration, pIndexBuffer);
  }

  pQuadMesh->m_AABB =
      AABB(Vector(-0.5f, -0.5f, 0.0f), Vector(0.5f, 0.5f, 0.0f));
...
void WBCompEldMinimap::AddMarker(WBEntity* const pEntity,
                                 const SimpleString& Texture) {
  DEVASSERT(pEntity);
  ...
  // CreateQuad gives us a mesh with color, normal, tangent, etc., and we don't
  // want all that.
  ...
  Marker.m_Mesh =
      pRenderer->GetMeshFactory()->CreateQuad(1.0f, XY_PLANE, false);
  ...
  Marker.m_Mesh->SetTexture(
      0, pRenderer->GetTextureManager()->GetTexture(Texture.CStr()));
}

void WBCompEldMinimap::UpdateMarker(WBEntity* const pEntity,
                                    const Vector& Location,
                                    const Angles& Orientation,
                                    const bool MarkAsRoom) {
  ...
  const Vector RoomOffset =
      Vector(-1.0f, -1.0f, -1.0f);  // Offset for indestructible world bounds
...
/*virtual*/ void WBCompEldMinimap::Render() {
  ...
  FOR_EACH_MAP(MarkerIter, m_Markers, WBEntity*, SMarker) {
    ...
    if (Marker.m_Hidden || !Marker.m_MarkAsRoom ||
