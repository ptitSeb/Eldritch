...
/*virtual*/ void WBCompEldFootsteps::InitializeFromDefinition(
  ...
  m_FootstepDistanceSq = Square(ConfigManager::GetInheritedFloat(
      sFootstepDistance, 0.0f, sDefinitionName));
}
...
/*virtual*/ void WBCompEldFootsteps::HandleEvent(const WBEvent& Event) {
  ...
  if (EventName == sOnMoved) {
    ...
    const float DistanceSq =
        (Location - m_LastFootstepLocation).LengthSquared();
