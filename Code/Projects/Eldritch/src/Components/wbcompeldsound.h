...
class WBCompEldSound : public WBEldritchComponent {
 ...
 private:
  ...
  void PlaySoundDef(const SimpleString& SoundDef, const Vector& Location,
                    const bool Attached, const float VolumeOverride);

  struct SSoundInstance {
    SSoundInstance() : m_SoundInstance(NULL), m_Attached(false) {}
