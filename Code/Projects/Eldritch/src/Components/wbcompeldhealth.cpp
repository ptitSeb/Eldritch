...
/*virtual*/ void WBCompEldHealth::InitializeFromDefinition(
  ...
  m_MaxHealth =
      ConfigManager::GetInheritedInt(sMaxHealth, m_Health, sDefinitionName);

  STATICHASH(PublishToHUD);
  m_PublishToHUD =
      ConfigManager::GetInheritedBool(sPublishToHUD, false, sDefinitionName);
  ...
  m_HidePickupScreenDelay = ConfigManager::GetInheritedFloat(
      sHidePickupScreenDelay, 0.0f, sDefinitionName);

  STATICHASH(DefaultDamageModMul);
  m_DefaultDamageMod.m_Multiply = ConfigManager::GetInheritedFloat(
      sDefaultDamageModMul, 1.0f, sDefinitionName);

  STATICHASH(DefaultDamageModAdd);
  m_DefaultDamageMod.m_Add = ConfigManager::GetInheritedFloat(
      sDefaultDamageModAdd, 0.0f, sDefinitionName);

  STATICHASH(NumDamageTypeMods);
  const uint NumDamageTypeMods =
      ConfigManager::GetInheritedInt(sNumDamageTypeMods, 0, sDefinitionName);
  for (uint DamageTypeModIndex = 0; DamageTypeModIndex < NumDamageTypeMods;
       ++DamageTypeModIndex) {
    const HashedString DamageType = ConfigManager::GetInheritedSequenceHash(
        "DamageTypeMod%dType", DamageTypeModIndex, HashedString::NullString,
        sDefinitionName);

    SDamageTypeMod& DamageTypeMod = m_DamageTypeMods[DamageType];

    DamageTypeMod.m_Multiply = ConfigManager::GetInheritedSequenceFloat(
        "DamageTypeMod%dMul", DamageTypeModIndex, 1.0f, sDefinitionName);
    DamageTypeMod.m_Add = ConfigManager::GetInheritedSequenceFloat(
        "DamageTypeMod%dAdd", DamageTypeModIndex, 0.0f, sDefinitionName);
  }
}
...
void WBCompEldHealth::TakeDamage(const int DamageAmount,
                                 WBEntity* const pDamager,
                                 const HashedString& DamageType) {
  XTRACE_FUNCTION;
...
int WBCompEldHealth::ModifyDamageAmount(const int DamageAmount,
                                        const HashedString& DamageType) {
  const float fDamageAmount = static_cast<float>(DamageAmount);
  float fModifiedDamageAmount = fDamageAmount;

  Map<HashedString, SDamageTypeMod>::Iterator ModIter =
      m_DamageTypeMods.Search(DamageType);
  if (ModIter.IsValid()) {
    const SDamageTypeMod& DamageTypeMod = ModIter.GetValue();
    fModifiedDamageAmount = (fModifiedDamageAmount * DamageTypeMod.m_Multiply) +
                            DamageTypeMod.m_Add;
  } else {
    fModifiedDamageAmount =
        (fModifiedDamageAmount * m_DefaultDamageMod.m_Multiply) +
        m_DefaultDamageMod.m_Add;
  }
...
void WBCompEldHealth::GiveMaxHealth(const int MaxHealthAmount,
...
void WBCompEldHealth::RestoreHealth(const int TargetHealth) {
  const int ActualTargetHealth =
      (TargetHealth > 0) ? TargetHealth : m_InitialHealth;
