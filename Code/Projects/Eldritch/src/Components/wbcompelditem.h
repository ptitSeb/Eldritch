...
class WBCompEldItem : public WBEldritchComponent {
 ...
 private:
  ...
  bool GetSpawnDropTransform(Vector& OutLocation, Vector& OutImpulse,
  ...
  bool m_SuppressDropDupe;  // Config; suppress drop spawn if item is replaced
                            // in inventory by the same type of item
};
