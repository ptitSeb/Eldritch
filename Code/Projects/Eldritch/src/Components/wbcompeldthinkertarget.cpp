...
/*virtual*/ void WBCompEldThinkerTarget::InitializeFromDefinition(
  ...
  m_OutputCombatTargetBlackboardKey = ConfigManager::GetInheritedHash(
      sOutputCombatTargetBlackboardKey, HashedString::NullString,
      sDefinitionName);

  STATICHASH(OutputSearchTargetBlackboardKey);
  m_OutputSearchTargetBlackboardKey = ConfigManager::GetInheritedHash(
      sOutputSearchTargetBlackboardKey, HashedString::NullString,
      sDefinitionName);

  STATICHASH(CombatTargetScoreThreshold);
  m_CombatTargetScoreThreshold = ConfigManager::GetInheritedFloat(
      sCombatTargetScoreThreshold, 0.0f, sDefinitionName);

  STATICHASH(SearchTargetScoreThreshold);
  m_SearchTargetScoreThreshold = ConfigManager::GetInheritedFloat(
      sSearchTargetScoreThreshold, 0.0f, sDefinitionName);
}
...
/*virtual*/ void WBCompEldThinkerTarget::Tick(float DeltaTime) {
  ...
  FOR_EACH_MAP(KnowledgeIter, KnowledgeMap, WBEntityRef,
    ...
    if (Score >= m_SearchTargetScoreThreshold &&
        Score > SelectedSearchTargetScore) {
    ...
    if (VisionScore >= m_CombatTargetScoreThreshold &&
  ...
  pBlackboard->SetEntity(m_OutputCombatTargetBlackboardKey,
                         pSelectedCombatTarget);
  pBlackboard->SetEntity(m_OutputSearchTargetBlackboardKey,
                         pSelectedSearchTarget);
}