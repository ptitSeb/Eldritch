...
WBCompEldInventory::WBCompEldInventory() : m_InventoryMap(), m_InitialItems() {}

WBCompEldInventory::~WBCompEldInventory() {}

/*virtual*/ void WBCompEldInventory::InitializeFromDefinition(
  ...
  const uint NumInitialItems =
      ConfigManager::GetInheritedInt(sNumInitialItems, 0, sDefinitionName);

  for (uint InitialItemIndex = 0; InitialItemIndex < NumInitialItems;
       ++InitialItemIndex) {
    const SimpleString ItemDef = ConfigManager::GetInheritedSequenceString(
        "InitialItem%d", InitialItemIndex, "", sDefinitionName);
...
void WBCompEldInventory::AddItem(const HashedString& Slot,
                                 WBEntity* const pItem) {
  ASSERT(pItem);
  ...
  if (Slot == sWeapon) {
    ...
    if (pCurrentWeapon && pAltWeapon == nullptr) {
      ...
      if (!pItemComponent->SuppressesDropDupe() ||
          pCurrentWeapon->GetName() != pItem->GetName()) {
  ...
  // Tell ourselves that the item was equipped (do this *before* the item gets
  // notified; fixes some dependencies)
...
void WBCompEldInventory::DropItem(const HashedString& Slot,
                                  const bool SuppressSpawn,
                                  WBEntity* const pReplacingItem) {
...
void WBCompEldInventory::SwapItems(const HashedString& SlotA,
                                   const HashedString& SlotB) {
  ...
  // Inventory map needs to be kept clean, so remove slots that will be empty
  // now.
  if (pItemB) {
...
void WBCompEldInventory::PushPersistence() const {
  ...
  // Push items in order of their UIDs, so we reconstruct them in the same order
  ...
  for (uint ItemIndex = 0; ItemIndex < NumInventoryItems; ++ItemIndex) {
    ...
    Persistence.SetHash(SimpleString::PrintF("InventoryItem%d", ItemIndex),
                        pItem->GetName());
    Persistence.SetHash(SimpleString::PrintF("InventorySlot%d", ItemIndex),
                        pItemComponent->GetSlot());
  }
}
...
void WBCompEldInventory::PullPersistence() {
  ...
  for (uint ItemIndex = 0; ItemIndex < NumInventoryItems; ++ItemIndex) {
    const SimpleString InventoryItem = Persistence.GetString(
        SimpleString::PrintF("InventoryItem%d", ItemIndex));
    const HashedString InventorySlot =
        Persistence.GetHash(SimpleString::PrintF("InventorySlot%d", ItemIndex));
    AddItem(InventoryItem, InventorySlot);
  }

  // Notify that we've respawned inventory, mainly so character can fix up hand
  // meshes.
