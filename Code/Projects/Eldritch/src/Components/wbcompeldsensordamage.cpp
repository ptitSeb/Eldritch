...
// Maybe just make that a parameter of damage dealer, that it doesn't trigger
// sensor, because how could the AI know anything about it?
void WBCompEldSensorDamage::HandleDamage(WBEntity* const pDamager) const {
  ...
  WBCompEldTransform* const pTransform =
      pDamager->GetTransformComponent<WBCompEldTransform>();
  ...
  WBCompRodinKnowledge::TKnowledge& Knowledge =
      pKnowledge->UpdateEntity(pDamager);
