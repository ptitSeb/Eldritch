...
class WBCompEldMesh : public WBEldritchComponent {
 public:
  ...
  virtual int GetTickOrder() {
    return ETO_TickDefault;
  }  // Should tick after transform
  ...
  void SetBlendedIrradianceOffset(const Vector& Offset) {
    m_TwoPointIrradianceOffset = Offset;
  }
  ...
  static void NotifyAnimationFinished(void* pVoid, class Mesh* pMesh,
                                      class Animation* pAnimation,
                                      bool Interrupted);
  void OnAnimationFinished(class Mesh* pMesh, class Animation* pAnimation,
                           bool Interrupted);
 ...
 private:
  ...
  void PlayAnimation(const HashedString& AnimationName, const bool Loop,
                     const bool IgnoreIfAlreadyPlaying,
                     const float PlayRate) const;
  void CopyAnimationsFrom(WBEntity* const pSourceEntity,
  ...
  bool m_UseTwoPointIrradiance;  // Config: Uses a secondary offset and blends
                                 // the two irradiances
  ...
  Vector4 m_CurrentHighlight;  // This couples meshes and frobbables more than
                               // I'd like. :(
  Vector4 m_ConstantIrradiance;

  float m_CullDistanceSq;  // Config: If non-zero, mesh is only drawn within
                           // this distance from camera
};
