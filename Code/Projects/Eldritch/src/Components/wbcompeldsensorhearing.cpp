...
/*virtual*/ void WBCompEldSensorHearing::InitializeFromDefinition(
  ...
  m_OcclusionCertaintyFactor = ConfigManager::GetInheritedFloat(
      sOcclusionCertaintyFactor, 0.0f, sDefinitionName);
}
...
/*virtual*/ void WBCompEldSensorHearing::HandleEvent(const WBEvent& Event) {
  ...
  if (EventName == sOnAINoise) {
    ...
    if (pEventOwner == GetEntity()) {
      ...
      WBEntity* const pUseNoiseEntity =
          pNoiseEntity ? pNoiseEntity : pEventOwner;
      ...
      HandleNoise(pUseNoiseEntity, NoiseLocation, NoiseRadius, NoiseUpdateTime,
                  ExpireTimeBonus);
    }
  }
}

bool WBCompEldSensorHearing::GetNoiseCertainty(const Vector& NoiseLocation,
                                               const float NoiseRadius,
                                               float& OutCertainty) const {
  ...
  const float DistanceCertainty =
      Attenuate(HeardDistance, m_CertaintyFalloffRadius);
  ...
  const float OcclusionCertaintyFactor =
      Lerp(1.0f - m_OcclusionCertaintyFactor, 1.0f, OcclusionCertainty);

  OutCertainty = DistanceCertaintyFactor * OcclusionCertaintyFactor;
  return true;
}

void WBCompEldSensorHearing::HandleNoise(WBEntity* const pNoiseEntity,
                                         const Vector& NoiseLocation,
                                         const float NoiseRadius,
                                         const float NoiseUpdateTime,
                                         const float ExpireTimeBonus) const {
  if (m_Paused) {
    return;
  }
  ...
  WBCompRodinKnowledge::TKnowledge& Knowledge =
      pKnowledge->UpdateEntity(pNoiseEntity, NoiseUpdateTime, ExpireTimeBonus);
