...
class WBCompEldAIMotion : public WBEldritchComponent,
                          public IRodinResourceUser {
 public:
  WBCompEldAIMotion();
  ...
  virtual int GetTickOrder() {
    return ETO_TickFirst;
  }  // Should tick before motion is integrated so input is applied ASAP.
  ...
  void SetReachedThreshold(const float ReachedThresholdMin,
                           const float ReachedThresholdMax);
  void SetDeflectionRadius(const float DeflectionRadius);

  bool IsMoving() const {
    return (m_MotionStatus == EMS_Moving || m_MotionStatus == EMS_Following ||
            m_MotionStatus == EMS_TurningToFace);
  }
  bool IsLocomoting() const {
    return (m_MotionStatus == EMS_Moving || m_MotionStatus == EMS_Following);
  }
 ...
 private:
  ...
  void InternalStartMove(const Vector& Destination,
                         const WBEntity* const pDestinationEntity);
  void PlayAnimation(const HashedString& AnimationName, const bool Loop,
                     const bool IgnoreIfAlreadyPlaying, const float PlayRate);
  bool GetLastKnownLocationFor(const WBEntity* const pFollowEntity,
                               Vector& OutLocation) const;
