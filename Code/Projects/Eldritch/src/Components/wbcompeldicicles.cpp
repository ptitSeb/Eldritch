...
WBCompEldIcicles::WBCompEldIcicles() : m_CheckDistance(0.0f) {}

WBCompEldIcicles::~WBCompEldIcicles() {}

/*virtual*/ void WBCompEldIcicles::InitializeFromDefinition(
  ...
  m_CheckDistance =
      ConfigManager::GetInheritedFloat(sCheckDistance, 0.0f, sDefinitionName);
}
...
/*virtual*/ void WBCompEldIcicles::Tick(float DeltaTime) {
  ...
  WBCompEldCollision* const pPlayerCollision =
      GET_WBCOMP(pPlayer, EldCollision);
  ...
  const AABB TraceBox =
      AABB(TraceEnd - EntityExtents, TraceStart + EntityExtents);
  const AABB PlayerBox = pPlayerCollision->GetBounds();
  if (!TraceBox.Intersects(PlayerBox)) {
    return;
  }

  // Move trace up to the top of player so we don't check occlusion beyond
  // player.
