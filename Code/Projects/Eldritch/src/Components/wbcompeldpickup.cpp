...
WBCompEldPickup::~WBCompEldPickup() { UnregisterForPurchaseEvents(); }

/*virtual*/ void WBCompEldPickup::InitializeFromDefinition(
  ...
  STATICHASH(GiveItem);
  m_GiveItemDef =
      ConfigManager::GetInheritedString(sGiveItem, "", sDefinitionName);
  ...
  m_FriendlyName =
      ConfigManager::GetInheritedString(sFriendlyName, "", sDefinitionName);

  STATICHASH(FriendlyDesc);
  m_FriendlyDesc =
      ConfigManager::GetInheritedString(sFriendlyDesc, "", sDefinitionName);
}
