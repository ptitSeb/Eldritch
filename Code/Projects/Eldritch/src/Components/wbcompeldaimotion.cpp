...
void WBCompEldAIMotion::InitializeFromDefinition(
  ...
  m_TurnSpeed = DEGREES_TO_RADIANS(
      ConfigManager::GetInheritedFloat(sTurnSpeed, 0.0f, sDefinitionName));

  STATICHASH(FollowValidateTime);
  m_FollowValidateTime = ConfigManager::GetInheritedFloat(
      sFollowValidateTime, 0.0f, sDefinitionName);

  STATICHASH(StepReachedThreshold);
  m_StepReachedThresholdSq = Square(ConfigManager::GetInheritedFloat(
      sStepReachedThreshold, 0.0f, sDefinitionName));

  STATICHASH(TurnReachedThreshold);
  const float TurnReachedComplementDegrees =
      90.0f - ConfigManager::GetInheritedFloat(sTurnReachedThreshold, 0.0f,
                                               sDefinitionName);
  m_TurnReachedThreshold =
      Cos(DEGREES_TO_RADIANS(TurnReachedComplementDegrees));

  STATICHASH(IsFlying);
  m_IsFlying =
      ConfigManager::GetInheritedBool(sIsFlying, false, sDefinitionName);
  ...
  m_CanUnlockDoors =
      ConfigManager::GetInheritedBool(sCanUnlockDoors, false, sDefinitionName);

  STATICHASH(CanOpenDoors);
  m_CanOpenDoors =
      m_CanUnlockDoors ||
      ConfigManager::GetInheritedBool(sCanOpenDoors, false, sDefinitionName);

  STATICHASH(MaxPathSteps);
  m_MaxPathSteps =
      ConfigManager::GetInheritedInt(sMaxPathSteps, 0, sDefinitionName);

  STATICHASH(IdleAnimation);
  m_IdleAnimationName = ConfigManager::GetInheritedHash(
      sIdleAnimation, HashedString::NullString, sDefinitionName);

  STATICHASH(WalkAnimation);
  m_WalkAnimationName = ConfigManager::GetInheritedHash(
      sWalkAnimation, HashedString::NullString, sDefinitionName);

  STATICHASH(JumpAnimation);
  m_JumpAnimationName = ConfigManager::GetInheritedHash(
      sJumpAnimation, HashedString::NullString, sDefinitionName);

  STATICHASH(AnimationResource);
  m_AnimationResource = ConfigManager::GetInheritedHash(
      sAnimationResource, HashedString::NullString, sDefinitionName);
}
...
/*virtual*/ void WBCompEldAIMotion::HandleEvent(const WBEvent& Event) {
  ...
  if (EventName == sOnCollided) {
    ...
    WBCompEldDoor* const pCollidedDoor =
        SAFE_GET_WBCOMP(pCollidedEntity, EldDoor);

    if (pCollidedDoor && m_CanOpenDoors) {
      // This was intended to be more general purpose, but doors are the only
      // thing that can be pushed.
      ...
      // If we collided with anything except a door, or if we can't open doors,
      // jump!
      TryJump();
    }
    ...
    if (IsLocomoting()) {
      PlayAnimation(m_WalkAnimationName, true, true,
                    GetWalkAnimationPlayRate());
    } else {
      PlayAnimation(m_IdleAnimationName, true, true, 1.0f);
    }
...
// Bastardized from TickMove. I should refactor to handle this case properly
...
void WBCompEldAIMotion::TickTurn() {
  ...
  if (NeedsLeftTurn || NeedsRightTurn) {
    const float TurnYaw =
        NeedsRightTurn ? -1.0f : (NeedsLeftTurn ? 1.0f : 0.0f);
...
void WBCompEldAIMotion::TickWalking() {
  ...
  if (DistanceRemainingSq <= m_ReachedThresholdMinSq &&
      DistanceRemainingZ <= pCollision->GetExtents().z) {
  ...
  const float AccelerationSize = pCollision->IsRecentlyLanded(0.0f)
                                     ? GetLandAcceleration()
                                     : m_AirAcceleration;
...
void WBCompEldAIMotion::TickFlying() {
  ...
  const float DistanceRemainingSq = RemainingOffset.LengthSquared2D();
  const float DistanceRemainingZ = Abs(RemainingOffset.z);
  if (IsLocomoting() && DistanceRemainingSq <= m_ReachedThresholdMinSq &&
      (!pCollision || DistanceRemainingZ <= pCollision->GetExtents().z)) {
  ...
  const Vector CompensateAcceleration =
      -pTransform->GetVelocity() * kCompensateReciprocal;
  ...
  if (m_MotionStatus == EMS_Following &&
      DistanceRemainingSq < m_FlyingDeflectionRadiusSq) {
  ...
  CombinedAcceleration.GetNormalized(CombinedNormal, CombinedSize,
                                     CombinedSizeReciprocal);
...
void WBCompEldAIMotion::SetReachedThreshold(const float ReachedThresholdMin,
                                            const float ReachedThresholdMax) {
...
bool WBCompEldAIMotion::GetLastKnownLocationFor(
    const WBEntity* const pFollowEntity, Vector& OutLocation) const {
  ...
  if (pKnowledge) {
    ...
    // HACKHACK: For Yog, who doesn't need to actually acquire knowledge of the
    // player.
    WBCompEldTransform* const pFollowTransform =
        pFollowEntity->GetTransformComponent<WBCompEldTransform>();
...
void WBCompEldAIMotion::InternalStartMove(
    const Vector& Destination, const WBEntity* const pDestinationEntity) {
  ...
  if (pDestinationEntity) {
    if (GetLastKnownLocationFor(pDestinationEntity, ActualDestination)) {
      ...
      // Call this a success. It's not really, but it's not a pathing failure;
      // it's more like
      // the request wasn't valid in the first place. This prevents stalling in
      // BT loops that
  ...
  if (!m_Wander) {
    ...
    const float DistanceRemainingSq =
        (CurrentLocation - ActualDestination).LengthSquared();
  ...
  if (!m_IsFlying) {
    ...
    // This is sort of arbitrary. I'm solving a case where the destination is
    // about 1.4m off the ground.
    static const float kTraceDistance = 2.0f;
    const Vector Extents = pCollision->GetExtents();

    // Sweep down to get actual start.
    const Segment SweepStartSegment = Segment(
        CurrentLocation, CurrentLocation + Vector(0.0f, 0.0f, -kTraceDistance));
    pWorld->Sweep(SweepStartSegment, Extents, Info);
    if (Info.m_Collision) {
      ActualStart = Info.m_Intersection;
      ActualStart.z += 0.5f - Extents.z;  // Start in the center of the voxel
                                          // touching the ground; fixes tall AIs
                                          // pathing funny
    }

    // Sweep down to get actual destination.
    // Don't assume extents actually fits at ActualDestination yet!
    const Segment SweepDestinationSegment =
        Segment(ActualDestination,
                ActualDestination + Vector(0.0f, 0.0f, -kTraceDistance));
  ...
  if (BypassPathing) {
    ...
    Params.m_Height =
        static_cast<uint>(Ceiling(pCollision->GetExtents().z * 2.0f));
    Params.m_MotionType =
        m_IsFlying ? EldritchNav::EMT_Flying : EldritchNav::EMT_Walking;
    ...
    const EldritchNav::EPathStatus Status =
        m_Wander ? pNav->Wander(Params, m_Path)
                 : pNav->FindPath(Params, m_Path);
    ...
    const float DistanceRemainingSq =
        (CurrentLocation - m_Path.Last()).LengthSquared();
    if (DistanceRemainingSq <= m_ReachedThresholdMaxSq) {
      // We're within our reached threshold, we're done!
      m_MotionStatus = EMS_TurningToFace;
      m_LastDestination = ActualDestination;
  ...
  // Cache the bound of our path so we can easily invalidate the path when the
  // world changes.
  if (pCollision) {
    ...
    m_PathBound.ExpandBy(pCollision->GetExtents());  // Expand by collision
                                                     // extents so we get all
                                                     // possible changes.
    m_PathBound.m_Min.z -= 1.0f;  // Also expand down by one unit so we detect
...
void WBCompEldAIMotion::PlayAnimation(const HashedString& AnimationName,
                                      const bool Loop,
                                      const bool IgnoreIfAlreadyPlaying,
                                      const float PlayRate) {
  if (m_Paused) {
    return;
  }

  WBCompRodinResourceMap* const pResourceMap =
      GET_WBCOMP(GetEntity(), RodinResourceMap);
  if (pResourceMap &&
      pResourceMap->ClaimResource(this, m_AnimationResource, false)) {
...
/*virtual*/ bool WBCompEldAIMotion::OnResourceStolen(
...
/*virtual*/ void WBCompEldAIMotion::OnResourceReturned(
    const HashedString& Resource) {
  Unused(Resource);
  ASSERT(Resource == m_AnimationResource);
...
/*virtual*/ void WBCompEldAIMotion::DebugRender() const {
  ...
  for (uint PathIndex = 1; PathIndex < NumPathSteps; ++PathIndex) {
    ...
    pRenderer->DEBUGDrawLine(PrevStep, NextStep,
                             ARGB_TO_COLOR(255, 0, 255, 255));
  }

  if (m_PathIndex < NumPathSteps) {
    pRenderer->DEBUGDrawCross(m_Path[m_PathIndex], 0.5f,
