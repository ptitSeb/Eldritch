...
class EldritchNav {
 public:
  ...
  struct SPathfindingParams {
    ...
    bool m_UsePartialPath;  // If there's no path, or a path isn't found after
                            // MaxSteps, return the partial path anyway.
    bool m_Verbose;
  ...
  void UpdateWorldFromEntity(WBCompEldCollision* const pCollision,
                             const bool Add);
 ...
 private:
  ...
  struct SExpandDirection {
    SExpandDirection(int Offset, float Cost, bool Up, bool Down)
        : m_Offset(Offset), m_Cost(Cost), m_IsUp(Up), m_IsDown(Down) {}
  ...
  enum EWorldValue {
    ...
    EWV_Entities =
        0x1f,  // Remaining bits are used to count collidable entities
  };
  ...
  void TryExpand(const SExpandDirection& Direction, const vidx_t ClosedIndex,
                 const SPathNode& ClosedNode);
  void Expand(const vidx_t ExpandVoxel, const vidx_t ClosedIndex,
  ...
  bool IsAccessible(const vidx_t Voxel, const uint Height,
  ...
  uint GetNextSmoothingNode(const Vector& Anchor, const Vector& Extents,
                            const Vector& OffsetZVector, const TPath& Path,
                            const uint PathStart) const;
  bool CheckForSmoothingNode(const Vector& Anchor, const Vector& Location,
                             const Vector& Extents,
                             const Vector& OffsetZVector) const;
  bool SweepFloor(const Segment& SweepSegment,
                  const Vector& Extents) const;  // sweep floor lol
  ...
  Array<byte> m_WorldValues;  // Parallel to m_PathNodes, one per voxel.
                              // Represents collision and other path-relevant
                              // flags.
