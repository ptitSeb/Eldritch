...
EldritchSaveLoad::EldritchSaveLoad() : m_WorldFiles() {}
...
SimpleString EldritchSaveLoad::GetMasterFile() const {
  ...
  const SimpleString MasterFile =
      SaveLoadPath + SimpleString("main.eldritchmastersave");
  return MasterFile;
}
...
void EldritchSaveLoad::SaveMaster() { SaveMaster(GetMasterFile()); }
...
bool EldritchSaveLoad::TryLoadMaster(const SimpleString& MasterFile) {
  ...
  uncompress(UncompressedBuffer.GetData(), &DestinationSize,
             CompressedBuffer.GetData(), CompressedSize);

  MemoryStream MasterMemoryStream(UncompressedBuffer.GetData(),
                                  UncompressedSize);
...
void EldritchSaveLoad::SaveMaster(const SimpleString& MasterFile) {
  ...
  compress(CompressedBuffer.GetData(), &CompressedSize,
           UncompressedBuffer.GetData(), UncompressedSize);
  CompressedBuffer.Resize(CompressedSize);
  ...
  MasterFileStream.Write(CompressedBuffer.MemorySize(),
...
bool EldritchSaveLoad::ShouldSaveCurrentWorld() const {
  ...
  // Don't save if we have initiated the ending sequence (or are disabling pause
...
void EldritchSaveLoad::SaveMaster(const IDataStream& Stream) {
  ...
  if (WorldSaved) {
    ...
    // If we're not saving this world, we shouldn't be saving any other worlds
    // either.
    FlushWorldFiles();
  }
...
bool EldritchSaveLoad::LoadMaster(const IDataStream& Stream) {
  ...
  for (uint WorldFileIndex = 0; WorldFileIndex < NumWorldFiles;
       ++WorldFileIndex) {
