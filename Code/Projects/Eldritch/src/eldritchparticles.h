...
class EldritchParticles {
 public:
  ...
  void SetOrientation(const Angles& Orientation) {
    m_SystemOrientation = Orientation;
  }
 ...
 private:
  struct SParticle {
    ...
    bool operator<(const SParticle& P) {
      return m_ViewDistanceSq > P.m_ViewDistanceSq;
    }  // Further particles belong first in list
  ...
  struct SParticleMeshParams {
    SParticleMeshParams()
        : m_ViewLocation(), m_ViewAxis(), m_AngleMatrix(), m_OriginMatrix() {}
  ...
  void UpdateMesh();
  void AddParticleToMesh(const uint Index, const SParticle& Particle,
                         const SParticleMeshParams& MeshParams);
  void TickParticles(const float DeltaTime);
  bool TickParticle(
      SParticle& Particle,
      float DeltaTime);  // Returns false if this particle has expired
  ...
  typedef Map<HashedString, EldritchParticles::SParticleSystemParams>
      TParamCache;
