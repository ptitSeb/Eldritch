...
// Voxel dimensions are hard-coded (here and in other places not based on this
// variable).
// This simplifies a bunch of stuff and should never need to change for
// Eldritch.
...
EldritchWorld::SVoxelLight::SVoxelLight() : m_Color(), m_Radius(0.0f) {}
...
EldritchWorld::~EldritchWorld() {
  ...
  for (uint WorldMeshIndex = 0; WorldMeshIndex < m_WorldMeshes.Size();
       ++WorldMeshIndex) {
...
void EldritchWorld::PackVoxels() {
  ...
  compress(m_PackedVoxels.GetData(), &PackedVoxelSize, m_VoxelMap.GetData(),
           UnpackedVoxelSize);
  m_PackedVoxels.Resize(PackedVoxelSize);
}
...
void EldritchWorld::UnpackVoxels() {
  ...
  uint32 DestinationSize =
      UnpackedVoxelSize;  // zlib writes the size of the compressed buffer
                          // (m_PackedVoxels.Size()?) to this for some reason.
  uncompress(m_VoxelMap.GetData(), &DestinationSize, m_PackedVoxels.GetData(),
             m_PackedVoxels.Size());
}
...
void EldritchWorld::InitializeConfig(const SimpleString& DefinitionName) {
  ...
  m_RayTraceLength = SqRt(static_cast<float>(
      Square(m_NumVoxelsX) + Square(m_NumVoxelsY) + Square(m_NumVoxelsZ)));
  ...
  m_MaxVertices = m_MeshSize * 4 * 6 / 2;  // Theoretical max: enough for every
                                           // other block to have 4 verts on all
                                           // 6 faces
  m_MaxIndices = m_MeshSize * 3 * 2 * 6 / 2;  // Theoretical max: enough for
                                              // every other block to have 2
                                              // tris on all 6 faces
  ...
  const float UVAdjustment =
      ConfigManager::GetFloat(sUVAdjustment, 0.0f, sDefinitionName);
  ...
  const uint NumWorldDefs =
      ConfigManager::GetInt(sNumWorldDefs, 0, sDefinitionName);

  for (uint WorldDefIndex = 0; WorldDefIndex < NumWorldDefs; ++WorldDefIndex) {
    const SimpleString WorldDefName = ConfigManager::GetSequenceString(
        "WorldDef%d", WorldDefIndex, "", sDefinitionName);
    ASSERT(WorldDefName != "");
...
void EldritchWorld::InitializeWorldDefConfig(
    const SimpleString& WorldDefinitionName) {
  MAKEHASH(WorldDefinitionName);

  SWorldDef& NewWorldDef = m_WorldDefs[WorldDefinitionName];

  STATICHASH(Tileset);
  NewWorldDef.m_Tileset =
      ConfigManager::GetInheritedString(sTileset, "", sWorldDefinitionName);

  STATICHASH(ColorGrading);
  NewWorldDef.m_ColorGrading = ConfigManager::GetInheritedString(
      sColorGrading, "", sWorldDefinitionName);

  STATICHASH(LightDef);
  const SimpleString LightDefinitionName =
      ConfigManager::GetInheritedString(sLightDef, "", sWorldDefinitionName);
  InitializeLightConfig(LightDefinitionName, NewWorldDef);

  STATICHASH(FogDef);
  const SimpleString FogDefinitionName =
      ConfigManager::GetInheritedString(sFogDef, "", sWorldDefinitionName);
  InitializeFogConfig(FogDefinitionName, NewWorldDef);

  STATICHASH(Music);
  NewWorldDef.m_Music =
      ConfigManager::GetInheritedString(sMusic, "", sWorldDefinitionName);

  // I only need to do this with Audiere, because it doesn't support streams.
  // Doing it with FMOD causes memory leaks because the sounds are never
  // instanced
  ...
  NewWorldDef.m_DisplacementNoiseOctaves = ConfigManager::GetInheritedInt(
      sDisplacementNoiseOctaves, 0, sWorldDefinitionName);

  if (NewWorldDef.m_DisplacementNoiseOctaves > 0) {
    STATICHASH(DisplacementNoiseScale);
    const float DisplacementNoiseScaleXY = ConfigManager::GetInheritedFloat(
        sDisplacementNoiseScale, 0.0f, sWorldDefinitionName);
    ...
    NewWorldDef.m_DisplacementNoiseScale.z = ConfigManager::GetInheritedFloat(
        sDisplacementNoiseScaleZ, DisplacementNoiseScaleXY,
        sWorldDefinitionName);

    const float OctaveScale =
        1.0f / static_cast<float>(Noise::GetOctaveScale(
                   NewWorldDef.m_DisplacementNoiseOctaves));
    NewWorldDef.m_DisplacementNoiseScale *= OctaveScale;

    STATICHASH(DisplacementNoiseLookupScale);
    NewWorldDef.m_DisplacementNoiseLookupScale =
        ConfigManager::GetInheritedFloat(sDisplacementNoiseLookupScale, 0.0f,
                                         sWorldDefinitionName);
  }

  STATICHASH(NumSpawnerOverrides);
  const uint NumSpawnerOverrides = ConfigManager::GetInheritedInt(
      sNumSpawnerOverrides, 0, sWorldDefinitionName);
  for (uint SpawnerOverrideIndex = 0;
       SpawnerOverrideIndex < NumSpawnerOverrides; ++SpawnerOverrideIndex) {
    const HashedString OldSpawner = ConfigManager::GetInheritedSequenceHash(
        "SpawnerOverride%dOld", SpawnerOverrideIndex, "", sWorldDefinitionName);
    const SimpleString NewSpawner = ConfigManager::GetInheritedSequenceString(
        "SpawnerOverride%dNew", SpawnerOverrideIndex, "", sWorldDefinitionName);
  ...
  const uint NumVoxelDefs =
      ConfigManager::GetInheritedInt(sNumVoxelDefs, 0, sWorldDefinitionName);

  for (uint VoxelDefIndex = 0; VoxelDefIndex < NumVoxelDefs; ++VoxelDefIndex) {
    const SimpleString VoxelDefName = ConfigManager::GetInheritedSequenceString(
        "VoxelDef%d", VoxelDefIndex, "", sWorldDefinitionName);
    ...
    const vval_t VoxelValue = static_cast<vval_t>(
        ConfigManager::GetInt(sVoxelValue, 0, sVoxelDefName));
    ...
    NewVoxelDef.m_BottomTile =
        ConfigManager::GetInt(sBottomTile, 0, sVoxelDefName);
  }
  ...
  const SimpleString WorldGenDefName =
      ConfigManager::GetInheritedString(sWorldGenDef, "", sWorldDefinitionName);
...
void EldritchWorld::InitializeLightConfig(
    const SimpleString& LightDefinitionName, SWorldDef& WorldDef) {
  MAKEHASH(LightDefinitionName);

  STATICHASH(HiColorH);
  WorldDef.m_HiColorMin.x =
      ConfigManager::GetFloat(sHiColorH, 0.0f, sLightDefinitionName);

  STATICHASH(HiColorHRange);
  WorldDef.m_HiColorMax.x =
      WorldDef.m_HiColorMin.x +
      ConfigManager::GetFloat(sHiColorHRange, 0.0f, sLightDefinitionName);

  STATICHASH(HiColorS);
  WorldDef.m_HiColorMin.y =
      ConfigManager::GetFloat(sHiColorS, 0.0f, sLightDefinitionName);

  STATICHASH(HiColorSRange);
  WorldDef.m_HiColorMax.y =
      WorldDef.m_HiColorMin.y +
      ConfigManager::GetFloat(sHiColorSRange, 0.0f, sLightDefinitionName);

  STATICHASH(HiColorV);
  WorldDef.m_HiColorMin.z =
      ConfigManager::GetFloat(sHiColorV, 0.0f, sLightDefinitionName);

  STATICHASH(HiColorVRange);
  WorldDef.m_HiColorMax.z =
      WorldDef.m_HiColorMin.z +
      ConfigManager::GetFloat(sHiColorVRange, 0.0f, sLightDefinitionName);

  Vector LoColorMin;
  Vector LoColorMax;

  STATICHASH(LoColorH);
  LoColorMin.x = ConfigManager::GetFloat(sLoColorH, WorldDef.m_HiColorMin.x,
                                         sLightDefinitionName);

  STATICHASH(LoColorHRange);
  LoColorMax.x = LoColorMin.x + ConfigManager::GetFloat(
                                    sLoColorHRange, WorldDef.m_HiColorMax.x -
                                                        WorldDef.m_HiColorMin.x,
                                    sLightDefinitionName);

  STATICHASH(LoColorS);
  LoColorMin.y = ConfigManager::GetFloat(sLoColorS, WorldDef.m_HiColorMin.y,
                                         sLightDefinitionName);

  STATICHASH(LoColorSRange);
  LoColorMax.y = LoColorMin.y + ConfigManager::GetFloat(
                                    sLoColorSRange, WorldDef.m_HiColorMax.y -
                                                        WorldDef.m_HiColorMin.y,
                                    sLightDefinitionName);

  STATICHASH(LoColorV);
  LoColorMin.z = ConfigManager::GetFloat(sLoColorV, WorldDef.m_HiColorMin.z,
                                         sLightDefinitionName);

  STATICHASH(LoColorVRange);
  LoColorMax.z = LoColorMin.z + ConfigManager::GetFloat(
                                    sLoColorVRange, WorldDef.m_HiColorMax.z -
                                                        WorldDef.m_HiColorMin.z,
                                    sLightDefinitionName);
  ...
  WorldDef.m_LoColorDeltaMin.x = ConfigManager::GetFloat(
      sLoColorDeltaH, WorldDef.m_LoColorDeltaMin.x, sLightDefinitionName);

  STATICHASH(LoColorDeltaHRange);
  WorldDef.m_LoColorDeltaMax.x =
      WorldDef.m_LoColorDeltaMin.x +
      ConfigManager::GetFloat(
          sLoColorDeltaHRange,
          WorldDef.m_LoColorDeltaMax.x - WorldDef.m_LoColorDeltaMin.x,
          sLightDefinitionName);

  STATICHASH(LoColorDeltaS);
  WorldDef.m_LoColorDeltaMin.y = ConfigManager::GetFloat(
      sLoColorDeltaS, WorldDef.m_LoColorDeltaMin.y, sLightDefinitionName);

  STATICHASH(LoColorDeltaSRange);
  WorldDef.m_LoColorDeltaMax.y =
      WorldDef.m_LoColorDeltaMin.y +
      ConfigManager::GetFloat(
          sLoColorDeltaSRange,
          WorldDef.m_LoColorDeltaMax.y - WorldDef.m_LoColorDeltaMin.y,
          sLightDefinitionName);

  STATICHASH(LoColorDeltaV);
  WorldDef.m_LoColorDeltaMin.z = ConfigManager::GetFloat(
      sLoColorDeltaV, WorldDef.m_LoColorDeltaMin.z, sLightDefinitionName);

  STATICHASH(LoColorDeltaVRange);
  WorldDef.m_LoColorDeltaMax.z =
      WorldDef.m_LoColorDeltaMin.z +
      ConfigManager::GetFloat(
          sLoColorDeltaVRange,
          WorldDef.m_LoColorDeltaMax.z - WorldDef.m_LoColorDeltaMin.z,
          sLightDefinitionName);

  static const float kMidColorAlphaX = 1.0f / 3.0f;

  STATICHASH(MidColorAlphaXH);
  WorldDef.m_MidColorAlphaX.x = ConfigManager::GetFloat(
      sMidColorAlphaXH, kMidColorAlphaX, sLightDefinitionName);

  STATICHASH(MidColorAlphaXS);
  WorldDef.m_MidColorAlphaX.y = ConfigManager::GetFloat(
      sMidColorAlphaXS, kMidColorAlphaX, sLightDefinitionName);

  STATICHASH(MidColorAlphaXV);
  WorldDef.m_MidColorAlphaX.z = ConfigManager::GetFloat(
      sMidColorAlphaXV, kMidColorAlphaX, sLightDefinitionName);

  static const float kMidColorAlphaY = 2.0f / 3.0f;

  STATICHASH(MidColorAlphaYH);
  WorldDef.m_MidColorAlphaY.x = ConfigManager::GetFloat(
      sMidColorAlphaYH, kMidColorAlphaY, sLightDefinitionName);

  STATICHASH(MidColorAlphaYS);
  WorldDef.m_MidColorAlphaY.y = ConfigManager::GetFloat(
      sMidColorAlphaYS, kMidColorAlphaY, sLightDefinitionName);

  STATICHASH(MidColorAlphaYV);
  WorldDef.m_MidColorAlphaY.z = ConfigManager::GetFloat(
      sMidColorAlphaYV, kMidColorAlphaY, sLightDefinitionName);

  STATICHASH(AOColorV);
  WorldDef.m_AOColorVMin =
      ConfigManager::GetFloat(sAOColorV, 0.0f, sLightDefinitionName);

  STATICHASH(AOColorVRange);
  WorldDef.m_AOColorVMax =
      WorldDef.m_AOColorVMin +
      ConfigManager::GetFloat(sAOColorVRange, 0.0f, sLightDefinitionName);
}

void EldritchWorld::InitializeFogConfig(const SimpleString& FogDefinitionName,
                                        SWorldDef& WorldDef) {
  ...
  STATICHASH(Texture);
  WorldDef.m_FogTexture =
      ConfigManager::GetString(sTexture, "", sFogDefinitionName);
}
...
void EldritchWorld::Initialize() {
  ...
  m_LightInfluenceMap
      .MemoryZero();  // This could have side effects on inner maps.

  PRINTF("Voxel mem.: %d Irradiance mem.: %d\n", m_VoxelMap.MemorySize(),
         m_IrradianceMap.MemorySize());
}
...
void EldritchWorld::Create() {
  ...
  pGame->SetFogParams(CurrentWorldDef.m_FogNear, CurrentWorldDef.m_FogFar,
                      CurrentWorldDef.m_FogTexture);
...
void EldritchWorld::RollLightColors() {
  ...
  const Vector HiColor =
      Math::Random(WorldDef.m_HiColorMin, WorldDef.m_HiColorMax);
  const Vector LoColorDelta =
      Math::Random(WorldDef.m_LoColorDeltaMin, WorldDef.m_LoColorDeltaMax);
  const Vector LoColor = HiColor + LoColorDelta;
  const float AOColorV =
      Math::Random(WorldDef.m_AOColorVMin, WorldDef.m_AOColorVMax);
...
void EldritchWorld::GetMeshCoords(const uint MeshIndex, uint& OutX, uint& OutY,
                                  uint& OutZ) const {
...
uint EldritchWorld::GetMeshIndex(const uint MeshX, const uint MeshY,
                                 const uint MeshZ) const {
  return MeshX + (MeshY * m_NumMeshesX) + (MeshZ * m_NumMeshesX * m_NumMeshesY);
}

void EldritchWorld::BuildMeshesForChangedVoxels(
    const Set<vidx_t>& ChangedVoxels) {
  PROFILE_FUNCTION;
  ...
  FOR_EACH_SET(VoxelIter, ChangedVoxels, vidx_t) {
    ...
    // If the voxel is on the edge of a mesh, gather adjacent meshes which may
    // need updated faces or AO.
    uint AddAdjacentDirections = EDirectionNone;
    if (LocalMeshX == 0 && MeshX > 0) AddAdjacentDirections |= EDirectionLeft;
    if (LocalMeshX == m_MeshSizeX - 1 && MeshX < m_NumMeshesX - 1)
      AddAdjacentDirections |= EDirectionRight;
    if (LocalMeshY == 0 && MeshY > 0) AddAdjacentDirections |= EDirectionBack;
    if (LocalMeshY == m_MeshSizeY - 1 && MeshY < m_NumMeshesY - 1)
      AddAdjacentDirections |= EDirectionFront;
    if (LocalMeshZ == 0 && MeshZ > 0) AddAdjacentDirections |= EDirectionDown;
    if (LocalMeshZ == m_MeshSizeZ - 1 && MeshZ < m_NumMeshesZ - 1)
      AddAdjacentDirections |= EDirectionUp;
...
#define TEST_DIR_FLAGS(dirs) ((AddAdjacentDirections & (dirs)) == (dirs))
    ...
    // Edges
    if (TEST_DIR_FLAGS(EDirectionLBC))
      MeshesToBuild.Insert(MeshIndex - dX - dY);
    if (TEST_DIR_FLAGS(EDirectionLFC))
      MeshesToBuild.Insert(MeshIndex - dX + dY);
    if (TEST_DIR_FLAGS(EDirectionLCD))
      MeshesToBuild.Insert(MeshIndex - dX - dZ);
    if (TEST_DIR_FLAGS(EDirectionLCU))
      MeshesToBuild.Insert(MeshIndex - dX + dZ);
    if (TEST_DIR_FLAGS(EDirectionRBC))
      MeshesToBuild.Insert(MeshIndex + dX - dY);
    if (TEST_DIR_FLAGS(EDirectionRFC))
      MeshesToBuild.Insert(MeshIndex + dX + dY);
    if (TEST_DIR_FLAGS(EDirectionRCD))
      MeshesToBuild.Insert(MeshIndex + dX - dZ);
    if (TEST_DIR_FLAGS(EDirectionRCU))
      MeshesToBuild.Insert(MeshIndex + dX + dZ);
    if (TEST_DIR_FLAGS(EDirectionCBD))
      MeshesToBuild.Insert(MeshIndex - dY - dZ);
    if (TEST_DIR_FLAGS(EDirectionCBU))
      MeshesToBuild.Insert(MeshIndex - dY + dZ);
    if (TEST_DIR_FLAGS(EDirectionCFD))
      MeshesToBuild.Insert(MeshIndex + dY - dZ);
    if (TEST_DIR_FLAGS(EDirectionCFU))
      MeshesToBuild.Insert(MeshIndex + dY + dZ);

    // Vertices
    if (TEST_DIR_FLAGS(EDirectionLBD))
      MeshesToBuild.Insert(MeshIndex - dX - dY - dZ);
    if (TEST_DIR_FLAGS(EDirectionLBU))
      MeshesToBuild.Insert(MeshIndex - dX - dY + dZ);
    if (TEST_DIR_FLAGS(EDirectionLFD))
      MeshesToBuild.Insert(MeshIndex - dX + dY - dZ);
    if (TEST_DIR_FLAGS(EDirectionLFU))
      MeshesToBuild.Insert(MeshIndex - dX + dY + dZ);
    if (TEST_DIR_FLAGS(EDirectionRBD))
      MeshesToBuild.Insert(MeshIndex + dX - dY - dZ);
    if (TEST_DIR_FLAGS(EDirectionRBU))
      MeshesToBuild.Insert(MeshIndex + dX - dY + dZ);
    if (TEST_DIR_FLAGS(EDirectionRFD))
      MeshesToBuild.Insert(MeshIndex + dX + dY - dZ);
    if (TEST_DIR_FLAGS(EDirectionRFU))
      MeshesToBuild.Insert(MeshIndex + dX + dY + dZ);
...
void EldritchWorld::BuildMesh(const uint MeshIndex) {
  ...
  IVertexBuffer* VertexBuffer =
      m_Framework->GetRenderer()->CreateVertexBuffer();
  IVertexDeclaration* VertexDeclaration =
      m_Framework->GetRenderer()->GetVertexDeclaration(
          VD_POSITIONS | VD_FLOATCOLORS_SM2 | VD_UVS);
  ...
  const Vector AABBMin(static_cast<float>(BaseVoxelX),
                       static_cast<float>(BaseVoxelY),
                       static_cast<float>(BaseVoxelZ));
  const Vector AABBMax(static_cast<float>(EndVoxelX),
...
#if BUILD_DEV
  if (m_TestLight) {
    NewWorldMesh->SetTexture(
        0, m_Framework->GetRenderer()->GetTextureManager()->GetTexture(
               DEFAULT_TEXTURE));
  } else
#endif
  {
  ...
  NewWorldMesh->SetMaterialDefinition("Material_World",
                                      m_Framework->GetRenderer());
  NewWorldMesh->SetMaterialFlags(MAT_WORLD);

  m_WorldMeshes[MeshIndex] = NewWorldMesh;
}

// NOTE: Intentionally never shuffling the noise hasher because I don't want to
// serialize it for each world.
Vector EldritchWorld::ApplyDisplacementNoise(const Vector& Location,
                                             const SWorldDef& WorldDef) const {
  if (WorldDef.m_DisplacementNoiseOctaves == 0) {
    return Location;
  }

  const Vector ScaledVector =
      Location * WorldDef.m_DisplacementNoiseLookupScale;

  // Use transformed locations for lookups so each component has a different
  // value.
  const float NoiseX =
      Noise::SumNoise3(ScaledVector.x, ScaledVector.y, ScaledVector.z,
                       WorldDef.m_DisplacementNoiseOctaves, Noise::CubicNoise3);
  const float NoiseY =
      Noise::SumNoise3(ScaledVector.y, ScaledVector.z, ScaledVector.x,
                       WorldDef.m_DisplacementNoiseOctaves, Noise::CubicNoise3);
  const float NoiseZ =
      Noise::SumNoise3(ScaledVector.z, ScaledVector.x, ScaledVector.y,
                       WorldDef.m_DisplacementNoiseOctaves, Noise::CubicNoise3);
...
void EldritchWorld::BuildVertsForVoxel(const uint X, const uint Y, const uint Z,
                                       SVoxelMeshBuffers& Buffers) const {
...
#define ADD_MIDPOINT_FIXED_DISPLACEMENT(x)
    ...
    const Vector V0 =
        APPLY_DISPLACEMENT(Vector(fX1, fY1, fZ1), CurrentWorldDef);
    const Vector V1 =
        APPLY_DISPLACEMENT(Vector(fX2, fY1, fZ1), CurrentWorldDef);
    const Vector V2 =
        APPLY_DISPLACEMENT(Vector(fX1, fY2, fZ1), CurrentWorldDef);
    const Vector V3 =
        APPLY_DISPLACEMENT(Vector(fX2, fY2, fZ1), CurrentWorldDef);
    const Vector V4 =
        APPLY_DISPLACEMENT(Vector(fX1, fY1, fZ2), CurrentWorldDef);
    const Vector V5 =
        APPLY_DISPLACEMENT(Vector(fX2, fY1, fZ2), CurrentWorldDef);
    const Vector V6 =
        APPLY_DISPLACEMENT(Vector(fX1, fY2, fZ2), CurrentWorldDef);
    const Vector V7 =
        APPLY_DISPLACEMENT(Vector(fX2, fY2, fZ2), CurrentWorldDef);
    const Vector VXp =
        APPLY_MIDPOINT_DISPLACEMENT(Vector(fX2, fYh, fZh), CurrentWorldDef)
        ADD_MIDPOINT_FIXED_DISPLACEMENT(
            Vector(CurrentWorldDef.m_DisplacementNoiseScale.x, 0.0f, 0.0f));
    const Vector VXn =
        APPLY_MIDPOINT_DISPLACEMENT(Vector(fX1, fYh, fZh), CurrentWorldDef)
        ADD_MIDPOINT_FIXED_DISPLACEMENT(
            Vector(-CurrentWorldDef.m_DisplacementNoiseScale.x, 0.0f, 0.0f));
    const Vector VYp =
        APPLY_MIDPOINT_DISPLACEMENT(Vector(fXh, fY2, fZh), CurrentWorldDef)
        ADD_MIDPOINT_FIXED_DISPLACEMENT(
            Vector(0.0f, CurrentWorldDef.m_DisplacementNoiseScale.y, 0.0f));
    const Vector VYn =
        APPLY_MIDPOINT_DISPLACEMENT(Vector(fXh, fY1, fZh), CurrentWorldDef)
        ADD_MIDPOINT_FIXED_DISPLACEMENT(
            Vector(0.0f, -CurrentWorldDef.m_DisplacementNoiseScale.y, 0.0f));
    const Vector VZp =
        APPLY_MIDPOINT_DISPLACEMENT(Vector(fXh, fYh, fZ2), CurrentWorldDef)
        ADD_MIDPOINT_FIXED_DISPLACEMENT(
            Vector(0.0f, 0.0f, CurrentWorldDef.m_DisplacementNoiseScale.z));
    const Vector VZn =
        APPLY_MIDPOINT_DISPLACEMENT(Vector(fXh, fYh, fZ1), CurrentWorldDef)
        ADD_MIDPOINT_FIXED_DISPLACEMENT(
            Vector(0.0f, 0.0f, -CurrentWorldDef.m_DisplacementNoiseScale.z));
    ...
    if (!HasAnyNeighbors(Neighbors, ENeighborRight)) {
      ...
      AO[0] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborRBC | ENeighborRBD | ENeighborRCD));
      AO[1] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborRFC | ENeighborRFD | ENeighborRCD));
      AO[2] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborRBC | ENeighborRBU | ENeighborRCU));
      AO[3] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborRFC | ENeighborRFU | ENeighborRCU));

      const SVoxelIrradiance& Irradiance =
          GetIrradiance(GetIndexNeighborRight(VoxelIndex));
      const Vector4 Color = m_GlobalLight.m_Light[IRRDIR_Right] +
                            Irradiance.m_Light[IRRDIR_Right];
    ...
    if (!HasAnyNeighbors(Neighbors, ENeighborLeft)) {
      ...
      AO[0] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborLBC | ENeighborLBD | ENeighborLCD));
      AO[1] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborLFC | ENeighborLFD | ENeighborLCD));
      AO[2] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborLBC | ENeighborLBU | ENeighborLCU));
      AO[3] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborLFC | ENeighborLFU | ENeighborLCU));

      const SVoxelIrradiance& Irradiance =
          GetIrradiance(GetIndexNeighborLeft(VoxelIndex));
      const Vector4 Color =
          m_GlobalLight.m_Light[IRRDIR_Left] + Irradiance.m_Light[IRRDIR_Left];
    ...
    if (!HasAnyNeighbors(Neighbors, ENeighborFront)) {
      ...
      AO[0] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborLFC | ENeighborLFD | ENeighborCFD));
      AO[1] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborRFC | ENeighborRFD | ENeighborCFD));
      AO[2] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborLFC | ENeighborLFU | ENeighborCFU));
      AO[3] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborRFC | ENeighborRFU | ENeighborCFU));

      const SVoxelIrradiance& Irradiance =
          GetIrradiance(GetIndexNeighborFront(VoxelIndex));
      const Vector4 Color = m_GlobalLight.m_Light[IRRDIR_Front] +
                            Irradiance.m_Light[IRRDIR_Front];
    ...
    if (!HasAnyNeighbors(Neighbors, ENeighborBack)) {
      ...
      AO[0] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborLBC | ENeighborLBD | ENeighborCBD));
      AO[1] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborRBC | ENeighborRBD | ENeighborCBD));
      AO[2] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborLBC | ENeighborLBU | ENeighborCBU));
      AO[3] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborRBC | ENeighborRBU | ENeighborCBU));

      const SVoxelIrradiance& Irradiance =
          GetIrradiance(GetIndexNeighborBack(VoxelIndex));
      const Vector4 Color =
          m_GlobalLight.m_Light[IRRDIR_Back] + Irradiance.m_Light[IRRDIR_Back];
    ...
    if (!HasAnyNeighbors(Neighbors, ENeighborUp)) {
      ...
      AO[0] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborLCU | ENeighborLBU | ENeighborCBU));
      AO[1] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborRCU | ENeighborRBU | ENeighborCBU));
      AO[2] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborLCU | ENeighborLFU | ENeighborCFU));
      AO[3] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborRCU | ENeighborRFU | ENeighborCFU));

      const SVoxelIrradiance& Irradiance =
          GetIrradiance(GetIndexNeighborUp(VoxelIndex));
      const Vector4 Color =
          m_GlobalLight.m_Light[IRRDIR_Up] + Irradiance.m_Light[IRRDIR_Up];
    ...
    if (!HasAnyNeighbors(Neighbors, ENeighborDown)) {
      ...
      AO[0] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborLCD | ENeighborLBD | ENeighborCBD));
      AO[1] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborRCD | ENeighborRBD | ENeighborCBD));
      AO[2] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborLCD | ENeighborLFD | ENeighborCFD));
      AO[3] = GetAOForNeighbors(CountNeighbors(
          Neighbors, ENeighborRCD | ENeighborRFD | ENeighborCFD));

      const SVoxelIrradiance& Irradiance =
          GetIrradiance(GetIndexNeighborDown(VoxelIndex));
      const Vector4 Color =
          m_GlobalLight.m_Light[IRRDIR_Down] + Irradiance.m_Light[IRRDIR_Down];
...
inline void EldritchWorld::PushColors(const Vector4& Color,
                                      const Vector4& AOColor,
                                      const float AOValues[],
                                      SVoxelMeshBuffers& Buffers) const {
  const float AverageAO =
      0.25f * (AOValues[0] + AOValues[1] + AOValues[2] + AOValues[3]);
...
void EldritchWorld::PushUVs(const Vector2& BaseUV, const bool FlipU,
                            const bool FlipV,
                            struct SVoxelMeshBuffers& Buffers) const {
...
inline void EldritchWorld::PushIndices(
    index_t& BaseIndex, const bool CCW,
    struct SVoxelMeshBuffers& Buffers) const {
  if (CCW) {
  ...
  // HACK: Advance the index pointer. Just trying to minimize code in the
  // calling function.
  BaseIndex += 5;
}
...
void EldritchWorld::Render() const {
  ...
  for (uint WorldMeshIndex = 0; WorldMeshIndex < m_NumMeshes;
       ++WorldMeshIndex) {
...
void EldritchWorld::DebugRender() const {
  ...
  static const bool RenderCrosses =
      ConfigManager::GetBool(sDebugRenderLightCrosses, false, sEldritchWorld);

  STATICHASH(DebugRenderLightSpheres);
  static const bool RenderSpheres =
      ConfigManager::GetBool(sDebugRenderLightSpheres, false, sEldritchWorld);
  ...
  if (RenderCrosses || RenderSpheres) {
    FOR_EACH_MAP(LightIter, m_LightMap, vidx_t, SVoxelLight) {
      ...
      if (RenderCrosses) {
        pRenderer->DEBUGDrawCross(LightLocation, Light.m_Radius,
                                  Light.m_Color.ToColor());
      }
      if (RenderSpheres) {
        pRenderer->DEBUGDrawSphere(LightLocation, Light.m_Radius,
                                   Light.m_Color.ToColor());
      }
    }
  }
...
bool EldritchWorld::Sweep(const Segment& SweepSegment,
...
bool EldritchWorld::SweepVoxels(const Segment& SweepSegment,
                                const Vector& HalfExtents,
                                CollisionInfo& Info) const {
  CollisionInfo MinInfo;
  ...
  Vector MinCorner;
  Vector MaxCorner;
  Vector::MinMax(SweepSegment.m_Point1, SweepSegment.m_Point2, MinCorner,
                 MaxCorner);

  const AABB MovementBox(MinCorner - HalfExtents, MaxCorner + HalfExtents);
  vpos_t MinVoxelX =
      Clamp(static_cast<vpos_t>(MovementBox.m_Min.x), 0, m_NumVoxelsX - 1);
  vpos_t MinVoxelY =
      Clamp(static_cast<vpos_t>(MovementBox.m_Min.y), 0, m_NumVoxelsY - 1);
  vpos_t MinVoxelZ =
      Clamp(static_cast<vpos_t>(MovementBox.m_Min.z), 0, m_NumVoxelsZ - 1);
  vpos_t MaxVoxelX =
      Clamp(static_cast<vpos_t>(MovementBox.m_Max.x), 0, m_NumVoxelsX - 1);
  vpos_t MaxVoxelY =
      Clamp(static_cast<vpos_t>(MovementBox.m_Max.y), 0, m_NumVoxelsY - 1);
  vpos_t MaxVoxelZ =
      Clamp(static_cast<vpos_t>(MovementBox.m_Max.z), 0, m_NumVoxelsZ - 1);
  ...
  for (vpos_t VoxelZ = MinVoxelZ; VoxelZ <= MaxVoxelZ && ContinueSweep;
       ++VoxelZ, VoxelIndex += SkipVoxelsZ) {
    for (vpos_t VoxelY = MinVoxelY; VoxelY <= MaxVoxelY && ContinueSweep;
         ++VoxelY, VoxelIndex += SkipVoxelsY) {
      for (vpos_t VoxelX = MinVoxelX; VoxelX <= MaxVoxelX && ContinueSweep;
           ++VoxelX, ++VoxelIndex) {
        // I can turn these back on if I get paranoid, but this seems to work
        // fine.
        ...
        // Sweep moving box against voxel box by testing segment against
        // expanded voxel box.
        ...
        AABB VoxelBox =
            AABB::CreateFromCenterAndExtents(VoxelCenter, ExpandedExtents);
...
bool EldritchWorld::SweepEntities(const Segment& SweepSegment,
                                  const Vector& HalfExtents,
                                  CollisionInfo& Info) const {
  // This gets called enough that the profiler hook is a cost. >_<
  // PROFILE_FUNCTION;
  ...
  ASSERT(1 ==
         CountBits(CollideAsType));  // We should only be colliding as one type
                                     // (optimization; can be changed, see
                                     // WBCompEldCollision::AddToCollisionMap)
  ASSERT(0 !=
         (EntityType));  // We need to be colliding against some type of entity

  const Array<WBCompEldCollision*>* const pCollisionComponents =
      CollideAll ? WBComponentArrays::GetComponents<WBCompEldCollision>()
  ...
  for (uint CollisionComponentIndex = 0;
       CollisionComponentIndex < NumCollisionComponents && ContinueSweep;
       ++CollisionComponentIndex) {
    WBCompEldCollision* const pCollision =
        (*pCollisionComponents)[CollisionComponentIndex];
...
bool EldritchWorld::Trace(const Ray& TraceRay, CollisionInfo& Info) const {
  // HACK: So that everything is just implemented via a segment sweep, use a
  // fixed trace length.
  // This is set (in ::InitializeConfig) to a size big enough to cross the
  // entire world at its
  // furthest corners, so it should be sufficient.
  const Segment TraceSegment(
      TraceRay.m_Point,
      TraceRay.m_Point + TraceRay.m_Direction * m_RayTraceLength);

  // Since a sweep is just a trace against expanded boxes, a trace is just a
  // sweep with zero extents!
  return Sweep(TraceSegment, Vector(), Info);
}

bool EldritchWorld::Trace(const Segment& TraceSegment,
                          CollisionInfo& Info) const {
  // Since a sweep is just a trace against expanded boxes, a trace is just a
  // sweep with zero extents!
  return Sweep(TraceSegment, Vector(), Info);
...
bool EldritchWorld::CheckClearance(const Vector& Location,
                                   const Vector& HalfExtents,
                                   CollisionInfo& Info) const {
...
bool EldritchWorld::PointCheck(const Vector& Location,
                               CollisionInfo& Info) const {
...
bool EldritchWorld::LineCheck(const Vector& Start, const Vector& End,
                              CollisionInfo& Info) const {
  const Segment TraceSegment(Start, End);
  return Sweep(TraceSegment, Vector(), Info);
}

bool EldritchWorld::SweepCheck(const Vector& Start, const Vector& End,
                               const Vector& HalfExtents,
                               CollisionInfo& Info) const {
  const Segment TraceSegment(Start, End);
  return Sweep(TraceSegment, HalfExtents, Info);
...
bool EldritchWorld::FindSpot(Vector& InOutSpot, const Vector& Extents,
                             CollisionInfo& Info) const {
  PROFILE_FUNCTION;

  // HACKHACK: If we have a zero-extents entity, just use the given spot.
  // This fixes some behavior that got broken by the cardinal direction check
  // below.
  if (Extents.IsZero()) {
    return true;
  }
  ...
  if (PointCheck(InOutSpot, Info)) {
    ...
    if (!PointCheck(InOutSpot + UnitX, Info)) {
      InOutSpot += UnitX;
    } else if (!PointCheck(InOutSpot - UnitX, Info)) {
      InOutSpot -= UnitX;
    } else if (!PointCheck(InOutSpot + UnitY, Info)) {
      InOutSpot += UnitY;
    } else if (!PointCheck(InOutSpot - UnitY, Info)) {
      InOutSpot -= UnitY;
    } else if (!PointCheck(InOutSpot + UnitZ, Info)) {
      InOutSpot += UnitZ;
    } else if (!PointCheck(InOutSpot - UnitZ, Info)) {
      InOutSpot -= UnitZ;
    } else {
      // Moving in cardinal directions didn't help. For now, just bail out and
      // don't try anymore.
      return false;
    }
  }
  ...
  // We might be able to fit now, by pushing away from occlusions directly along
  // each axis.
  if (!CheckClearance(InOutSpot, Extents, Info)) {
    return true;
  }

  // If not, then we need to sweep a line segment along each axis to find more
  ...
  // Finally, we need to sweep a rectangle along each axis to find more complex
  // intersections.
  for (uint Dir = 0; Dir < 3; ++Dir) {
    static const float kSmallDistance = 0.1f;
...
inline bool EldritchWorld::HasAnyNeighbors(const vnbr_t VoxelNeighbors,
                                           const vnbr_t NeighborFlags) const {
  return (VoxelNeighbors & NeighborFlags) > 0;
}

inline bool EldritchWorld::HasAnyNeighbors(
    const vxnb_t VoxelExtendedNeighbors,
    const vxnb_t ExtendedNeighborFlags) const {
  return (VoxelExtendedNeighbors & ExtendedNeighborFlags) > 0;
}

inline uint EldritchWorld::CountNeighbors(
    const vxnb_t VoxelExtendedNeighbors,
    const vxnb_t ExtendedNeighborFlags) const {
  const vxnb_t MatchedNeighbors =
      VoxelExtendedNeighbors & ExtendedNeighborFlags;
...
// NOTE: This used to be inline, but not every compiler would link it properly.
/*inline*/ void EldritchWorld::GetCoords(const vidx_t VoxelIndex, vpos_t& OutX,
                                         vpos_t& OutY, vpos_t& OutZ) const {
  DEVASSERT(IsValidIndex(VoxelIndex));
...
// NOTE: This used to be inline, but not every compiler would link it properly.
/*inline*/ vidx_t EldritchWorld::GetIndex(const vpos_t X, const vpos_t Y,
                                          const vpos_t Z) const {
...
Vector EldritchWorld::GetVoxelCenter(const Vector& Location) const {
  return Vector(Floor(Location.x) + 0.5f, Floor(Location.y) + 0.5f,
                Floor(Location.z) + 0.5f);
}
...
inline vidx_t EldritchWorld::GetIndexNeighborRight(
    const vidx_t VoxelIndex) const {
  return VoxelIndex + 1;
}

inline vidx_t EldritchWorld::GetIndexNeighborLeft(
    const vidx_t VoxelIndex) const {
  return VoxelIndex - 1;
}

inline vidx_t EldritchWorld::GetIndexNeighborFront(
    const vidx_t VoxelIndex) const {
  return VoxelIndex + m_NumVoxelsX;
}

inline vidx_t EldritchWorld::GetIndexNeighborBack(
    const vidx_t VoxelIndex) const {
  return VoxelIndex - m_NumVoxelsX;
}
...
inline vidx_t EldritchWorld::GetIndexNeighborDown(
    const vidx_t VoxelIndex) const {
  return VoxelIndex - m_NumVoxelsX * m_NumVoxelsY;
}

inline vidx_t EldritchWorld::GetIndexNeighbor(const vidx_t VoxelIndex,
                                              const vdir_t Direction) const {
  ...
  if (Direction & EDirectionFront) {
    RetVal += m_NumVoxelsX;
  }
  if (Direction & EDirectionBack) {
    RetVal -= m_NumVoxelsX;
  }
  if (Direction & EDirectionUp) {
    RetVal += m_NumVoxelsX * m_NumVoxelsY;
  }
  if (Direction & EDirectionDown) {
    RetVal -= m_NumVoxelsX * m_NumVoxelsY;
  }
...
// NOTE: This used to be inline, but not every compiler would link it properly.
/*inline*/ void EldritchWorld::SetVoxel(const vidx_t VoxelIndex,
                                        const vval_t VoxelValue) {
  DEBUGASSERT(IsValidIndex(VoxelIndex));
...
inline void EldritchWorld::SafeSetVoxel(const vidx_t VoxelIndex,
                                        const vval_t VoxelValue) {
...
inline const SVoxelIrradiance& EldritchWorld::GetIrradiance(
    const vidx_t VoxelIndex) const {
  DEBUGASSERT(IsValidIndex(VoxelIndex));
...
inline vnbr_t EldritchWorld::GetNeighbors(const vidx_t VoxelIndex) const {
  ...
  if (NeighborRight > 0) {
    RetVal |= ENeighborRight;
  }
  if (NeighborLeft > 0) {
    RetVal |= ENeighborLeft;
  }
  if (NeighborFront > 0) {
    RetVal |= ENeighborFront;
  }
  if (NeighborBack > 0) {
    RetVal |= ENeighborBack;
  }
  if (NeighborUp > 0) {
    RetVal |= ENeighborUp;
  }
  if (NeighborDown > 0) {
    RetVal |= ENeighborDown;
  }

  return RetVal;
}

inline vxnb_t EldritchWorld::GetExtendedNeighbors(
    const vidx_t VoxelIndex) const {
  vxnb_t RetVal = 0;

#define TRY_ADD_NEIGHBOR(tag)                                            \
  if (SafeGetVoxel(GetIndexNeighbor(VoxelIndex, EDirection##tag)) > 0) { \
    RetVal |= ENeighbor##tag;                                            \
  }
...
void EldritchWorld::SetMaze(const Array<uint8>& Maze) { m_Maze = Maze; }
...
void EldritchWorld::Load(const IDataStream& Stream) {
  ...
  // I *think* this was important for some bug. Else I could just load WBWorld
  // first.
  UnpackVoxels();
  AddAllLightInfluences();
  ComputeAllIrradiance();
...
void EldritchWorld::ClearLightInfluenceMap() {
  ...
  for (uint LightInfluenceIndex = 0;
       LightInfluenceIndex < m_LightInfluenceMap.Size();
       ++LightInfluenceIndex) {
...
void EldritchWorld::ExpandBoxAroundVoxels(const Set<vidx_t>& Voxels,
                                          AABB& InOutBox) const {
  PROFILE_FUNCTION;
...
void EldritchWorld::SendOnWorldChangedEvent(const AABB& ChangedBox) const {
  ...
  WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(), OnWorldChanged,
                    NULL);
}

bool EldritchWorld::AddVoxelAt(const Vector& Location,
                               const vval_t VoxelValue) {
  CollisionInfo Info;
  Info.m_CollideEntities = true;
  Info.m_UserFlags = EECF_CollideAsWorld | EECF_CollideStaticEntities |
...
void EldritchWorld::RemoveVoxelsAt(const Vector& Location, const float Radius) {
  ...
  const AABB ChangedBox = AABB::CreateFromCenterAndExtents(
      GetVoxelCenter(VoxelIndex), RadiusExtents);
...
// For all the lights that influence a voxel, gather all the other voxels they
// influence.
void EldritchWorld::GatherLightInfluenceVoxels(Set<vidx_t>& Voxels,
                                               const vidx_t VoxelIndex) const {
  PROFILE_FUNCTION;
  ...
  FOR_EACH_SET(InfluenceIter, LocalLightInfluences, vidx_t) {
    ...
    // If this fails, something is probably happening out of order. Remove
    // light, compute irradiance, then do this.
...
void EldritchWorld::GatherVoxelsInRadius(Set<vidx_t>& Voxels,
                                         const vidx_t VoxelIndex,
...
void EldritchWorld::GatherVoxelsInBox(Set<vidx_t>& Voxels,
...
void EldritchWorld::AddLightInfluence(const Set<vidx_t>& AffectedVoxels,
                                      const vidx_t LightVoxel) {
  FOR_EACH_SET(VoxelIter, AffectedVoxels, vidx_t) {
    const vidx_t VoxelIndex = VoxelIter.GetValue();
...
void EldritchWorld::RemoveLightInfluence(const Set<vidx_t>& AffectedVoxels,
                                         const vidx_t LightVoxel) {
  FOR_EACH_SET(VoxelIter, AffectedVoxels, vidx_t) {
    const vidx_t VoxelIndex = VoxelIter.GetValue();
    LightInfluences& LocalLightInfluences = m_LightInfluenceMap[VoxelIndex];
...
bool EldritchWorld::AddLightAt(const Vector& Location, const float Radius,
                               const Vector4& Color) {
  vidx_t VoxelIndex = GetIndex(Location);
  if (VoxelIndex == kInvalidVoxelIndex) {
    return false;
...
#if BUILD_DEV
  // Make sure light influences are already cleared, else we need to call
  // ClearLightInfluenceMap first.
  for (uint LightInfluenceIndex = 0;
       LightInfluenceIndex < m_LightInfluenceMap.Size();
       ++LightInfluenceIndex) {
    const LightInfluences& Influences =
        m_LightInfluenceMap[LightInfluenceIndex];
...
void EldritchWorld::ComputeAllIrradiance() {
  ...
  for (vidx_t VoxelIndex = 0; VoxelIndex < m_NumVoxels; ++VoxelIndex) {
    ...
    const LightInfluences& LocalLightInfluences =
        m_LightInfluenceMap[VoxelIndex];
    FOR_EACH_SET(InfluenceIter, LocalLightInfluences, vidx_t) {
      const vidx_t LightVoxel = InfluenceIter.GetValue();
      const SVoxelLight& Light = m_LightMap[LightVoxel];
...
void EldritchWorld::ComputePartialIrradiance(vidx_t VoxelIndex,
                                             vidx_t LightVoxel,
                                             const SVoxelLight& Light) {
  // This gets called enough that the profiler hook is a cost. >_<
  // PROFILE_FUNCTION;
  ...
  if (VoxelIndex == LightVoxel) {
    ...
    // 0.5f corresponds with tangential lighting from the half Lambert equation
    // in GetIrradianceIncidence
    ...
    if (Trace(LightTrace, Info)) {
      ...
      Irradiance.m_Light[0] +=
          FalloffColor * GetIrradianceIncidence(LightDir, kIrradianceDirs[0]);
      Irradiance.m_Light[1] +=
          FalloffColor * GetIrradianceIncidence(LightDir, kIrradianceDirs[1]);
      Irradiance.m_Light[2] +=
          FalloffColor * GetIrradianceIncidence(LightDir, kIrradianceDirs[2]);
      Irradiance.m_Light[3] +=
          FalloffColor * GetIrradianceIncidence(LightDir, kIrradianceDirs[3]);
      Irradiance.m_Light[4] +=
          FalloffColor * GetIrradianceIncidence(LightDir, kIrradianceDirs[4]);
      Irradiance.m_Light[5] +=
          FalloffColor * GetIrradianceIncidence(LightDir, kIrradianceDirs[5]);
    }
  }
}

inline float EldritchWorld::GetIrradianceIncidence(
    const Vector& LightDir, const Vector& IrradianceDir) const {
  ...
  const float BentIncidence =
      (Incidence * 0.5f) + 0.5f;  // Wrap light around surfaces.
  return Saturate(BentIncidence);
}

const SVoxelIrradiance& EldritchWorld::GetIrradianceAt(
...
SVoxelIrradiance EldritchWorld::BlendIrradiances(
    const Vector& LocationA, const Vector& LocationB) const {
  ...
  for (uint DirectionIndex = 0; DirectionIndex < 6; ++DirectionIndex) {
    ...
    BlendedIrradiance.m_Light[DirectionIndex] =
        FactorA * IrradianceA.m_Light[DirectionIndex] +
        FactorB * IrradianceB.m_Light[DirectionIndex];
  }

  return BlendedIrradiance;
}

void EldritchWorld::GetRoomDimensions(uint& SizeX, uint& SizeY,
                                      uint& SizeZ) const {
  SizeX = m_RoomSizeX;
  SizeY = m_RoomSizeY;
  SizeZ = m_RoomSizeZ;
...
const EldritchWorld::SVoxelDef& EldritchWorld::GetVoxelDef(
    const vval_t Voxel) const {
  const SWorldDef& CurrentWorldDef = GetWorldDef();
...
void EldritchWorld::GetTileUVs(const uint TileIndex, Vector2& OutUVMin,
                               Vector2& OutUVMax) const {
  OutUVMin = GetTileUV(TileIndex);
  OutUVMax.x = OutUVMin.x + GetTileU();
  OutUVMax.y = OutUVMin.y + GetTileV();
}

SimpleString EldritchWorld::GetSpawnerOverride(
    const SimpleString& OldSpawner) const {
  const SWorldDef& WorldDef = GetWorldDef();
  SWorldDef::TSpawnerMap::Iterator SpawnerIter =
      WorldDef.m_SpawnerOverrides.Search(OldSpawner);
...
void EldritchWorld::GetRoomCoords(const int RoomIndex, int& OutX, int& OutY,
                                  int& OutZ) const {
