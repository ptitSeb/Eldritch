...
int FilePacker::UnpackFile(const char* PackFilename) {
  if (FileUtil::Exists(PackFilename)) {
    ...
    for (uint i = 0; i < m_FileEntries.Size(); ++i) {
      ...
      WriteUnpackedFile(
          FileStream(FileEntry.m_Filename.CStr(), FileStream::EFM_Write),
          FileEntry);
    }
  }
  return 0;
...
void FilePacker::WriteUnpackedFile(const IDataStream& Stream,
...
void FilePacker::AddInFile(const char* InFilename, bool Compress) {
  ...
  if (Compress) {
    ...
    compress(m_InFileCompressionBuffer, &FileEntry.m_CompressedSize,
             m_InFileBuffer, m_InFileSize);
  } else {
    FileEntry.m_Compressed = 0;
    FileEntry.m_CompressedSize = FileEntry.m_DecompressedSize;
    memcpy_s(m_InFileCompressionBuffer, m_InFileCompressionSize, m_InFileBuffer,
             FileEntry.m_CompressedSize);
  }
...
uint FilePacker::GetFilesOffset() {
  // NOTE: This sizeof was causing a problem because the struct was word-aligned
  // but
  // I wasn't writing the extra padding. No real good, general solution to this.
  uint SizeOfEntryMinusString =
      sizeof(SPackageFileEntry) - sizeof(SimpleString);
  uint RetVal = 0;
  ...
  for (uint i = 0; i < m_FileEntries.Size(); ++i) {
    RetVal +=
        sizeof(uint32);  // Because we write the length of the string first
