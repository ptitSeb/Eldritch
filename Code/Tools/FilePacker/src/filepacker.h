...
class FilePacker {
 public:
  ...
  void WriteUnpackedFile(const IDataStream& Stream,
  ...
  uint m_InFileCompressionSize;  // The size of the whole buffer before
                                 // compression; the actual compressed size
                                 // after
  ...
  byte* m_ExistingPackFileBuffer;  // The blob of compressed data read in from
                                   // package file
};
