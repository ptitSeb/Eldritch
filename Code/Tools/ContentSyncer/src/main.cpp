...
uint g_MainMemorySize = 128 * 1024 * 1024;  // Make sure this is big enough for
                                            // the buffer defined in the syncer
...
void BeginUpdating();
void Reinit();
void SplitIntoLines(const char* Message, uint MessageLength,
                    Array<SimpleString>& OutLines);
void FinishPatching();
...
#if BUILD_DEBUG
  Allocator::GetDefault().Report(FileStream(
      "content-syncer-memory-exit-report.txt", FileStream::EFM_Write));
#endif
...
bool HasCommandLineArgument(const SimpleString& Argument) {
  for (uint ArgumentIndex = 0; ArgumentIndex < g_CommandLineArguments.Size();
       ++ArgumentIndex) {
...
void Initialize() {
  ...
  g_Renderer->Init();
  g_Renderer->SetFlickerFreeMode(
      ConfigManager::GetBool(sOnlyRedrawDirtyCells, false, sContentSyncer));

  SetConsoleTitle(ConfigManager::GetString(sTitle, "", sContentSyncer));

  g_CommandLine = new SCommandLine;
  g_CommandLine->m_App =
      ConfigManager::GetString(sApplication, "", sContentSyncer);
  ResolveCommandLine();
  ...
  AddStatus(
      SimpleString::PrintF("%s initialized.", ConfigManager::GetString(
                                                  sTitle, "", sContentSyncer)),
      g_StatusColor);
...
bool Launch(SimpleString& CommandLine) {
  ...
  DWORD Flags = CREATE_DEFAULT_ERROR_MODE |  // Don't inherit error mode
                CREATE_NEW_CONSOLE;  // Create new console (DETACHED_PROCESS
                                     // also means this console isn't inherited;
                                     // but a new console must be created with
                                     // AllocConsole)

  BOOL Result = CreateProcess(
      NULL,  // Application name (optional--use the first argument from command
             // line if NULL)
      CommandLine
          .MutableCStr(),  // Command line (optional, includes application name)
...
bool LaunchApp() { return Launch(g_CommandLine->m_CommandLine); }
...
bool RelaunchTempSelf() {
  ...
  SimpleString CommandLine = SimpleString::PrintF(
      "%s %s", ConfigManager::GetString(sTempSelf, "", sContentSyncer),
...
bool RelaunchTrueSelf() {
  ...
  SimpleString CommandLine =
      ConfigManager::GetString(sSelf, "", sContentSyncer);
...
void RedrawScene() {
  ...
  if (g_NumStatusLines > 0) {
    uint NumStatusesToDraw =
        Min(g_NumStatusLines, g_Statuses.Size() - g_StatusOffset);
    for (uint LineIndex = 0; LineIndex < NumStatusesToDraw; ++LineIndex) {
      ...
      g_MenuSurface->Print(Status.m_Status.CStr(), g_ConsoleWidth,
                           Status.m_Color, 0, LineIndex, ASURF_FG);
    }
  }
}
...
void AddStatus(const SimpleString& Status, uint8 Color) {
  ...
  g_StatusOffset = g_Statuses.Size() > g_NumStatusLines
                       ? g_Statuses.Size() - g_NumStatusLines
                       : 0;
...
void TryLaunchApp() {
  if (LaunchApp()) {
    ...
    AddStatus(SimpleString::PrintF("Failed to launch %s",
                                   g_CommandLine->m_App.CStr()),
              g_StatusWarningColor);
    ++g_NumWarnings;
  }
}
...
void Patcher_Tick() {
  ...
  if (g_Patcher_WaitingToConnect && Patcher_IsRequestFinished()) {
    if (g_HTTPSocket->AsyncConnectSucceeded()) {
      // NOTE: For async socket, buffer isn't actually filled here. Just a bad
      // interface design.
      g_HTTPSocket->Get(
          g_HTTPResultBuffer,
          ConfigManager::GetString(sPatcherAgent, "", sContentSyncer));
...
void Patcher_HandleReceipt_NextPatchFile() {
  ...
  // Compute the checksum for the payload and compare with the data in the
  // manifest file
  uint32 ContentChecksum =
      Checksum::Adler32((uint8*)MessageContent, ContentSize);
  SimpleString ContentLengthString = SimpleString::PrintF("%d", ContentSize);
  SimpleString ContentChecksumString =
      SimpleString::PrintF("0x%08X", ContentChecksum);

  SManifestFile& ManifestFile = g_FilesInManifest[g_NextPatchFileIndex];
  SimpleString PatchFile = ManifestFile.m_Filename;

  if (ContentLengthString == ManifestFile.m_Length &&
      ContentChecksumString == ManifestFile.m_Checksum) {
    ManifestFile.m_Validated = true;

    FileStream TempFile(GetTempFileForPatchFile(PatchFile).CStr(),
                        FileStream::EFM_Write);
    TempFile.Write(ContentSize, MessageContent);

    AddStatus(SimpleString::PrintF("%s received.", PatchFile.CStr()),
              g_StatusColor);
  } else {
    AddStatus(
        SimpleString::PrintF("Checksum failure for %s.", PatchFile.CStr()),
        g_StatusWarningColor);
    ++g_NumWarnings;
  }
...
void Patcher_ReportFailure() {
  if (g_Patcher_GettingVersionNumber) {
    ...
    AddStatus(SimpleString::PrintF("Error receiving %s.",
                                   g_FilesInManifest[g_NextPatchFileIndex]),
              g_StatusWarningColor);
    ++g_NumWarnings;
  } else {
    AddStatus("Unknown error connecting to update server.",
              g_StatusWarningColor);
    ++g_NumWarnings;
  }
}
...
void Patcher_GetVersionNumber() {
  ...
  SocketInit.m_HostName =
      ConfigManager::GetString(sPatcherHost, "", sContentSyncer);
  SocketInit.m_Path =
      ConfigManager::GetString(sPatcherVersionPath, "", sContentSyncer);
...
void Patcher_GetManifestFile() {
  ...
  SocketInit.m_HostName =
      ConfigManager::GetString(sPatcherHost, "", sContentSyncer);
  SocketInit.m_Path =
      ConfigManager::GetString(sPatcherManifestPath, "", sContentSyncer);
  SocketInit.m_Async = true;
  g_HTTPSocket->Connect(SocketInit);
}
...
void Patcher_GetNextPatchFile() {
  ...
  for (; g_NextPatchFileIndex < g_FilesInManifest.Size();
       ++g_NextPatchFileIndex) {
    ...
    if (FileUtil::Exists(PatchFilename.CStr())) {
      ...
      SimpleString ExtantChecksumString =
          SimpleString::PrintF("0x%08X", ExtantChecksum);

      if (ExtantLengthString == PatchFile.m_Length &&
          ExtantChecksumString == PatchFile.m_Checksum) {
        AddStatus(
            SimpleString::PrintF("%s is up to date.", PatchFilename.CStr()),
            g_StatusColor);
      } else {
  ...
  if (g_NextPatchFileIndex < g_FilesInManifest.Size()) {
    ...
    AddStatus(SimpleString::PrintF("Getting %s...", PatchFile.CStr()),
              g_StatusColor);
    ...
    SocketInit.m_HostName =
        ConfigManager::GetString(sPatcherHost, "", sContentSyncer);
    SocketInit.m_Path = SimpleString::PrintF(
        "%s%s",
        ConfigManager::GetString(sPatcherContentPath, "", sContentSyncer),
        PatchFile.CStr());
    SocketInit.m_Async = true;
    g_HTTPSocket->Connect(SocketInit);
  } else {
...
void CopyTempPathFiles() {
  ...
  for (uint PatchFileIndex = 0; PatchFileIndex < g_FilesInManifest.Size();
       ++PatchFileIndex) {
    ...
    if (ManifestFile.m_Validated) {
      ...
      if (FileUtil::Copy(TempFile.CStr(), PatchFile.CStr(), false)) {
        ...
        AddStatus(
            SimpleString::PrintF("Failed to copy %s.", PatchFile.CStr()).CStr(),
            g_StatusWarningColor);
        ++g_NumWarnings;
      }
    }
