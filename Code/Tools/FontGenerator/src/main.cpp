...
// Defines properties about a particular glyph or code point in a particular
// locale in a font.
...
void ComputeWidth(SFGCodePointProps& CodePointProps) {
  uint Width = 1 + Max(0, CodePointProps.m_A) + CodePointProps.m_B +
               Max(0, CodePointProps.m_C);
...
// Defines properties about a particular locale for the font (e.g. Latin
// alphabet).
struct SFGLocaleProps {
  ...
  int m_TextureWidth;
  int m_TextureHeight;
  SimpleString m_BitmapTextureName;  // For bitmap fonts only, passed through
                                     // without wrangling
  SimpleString m_LocaleTag;
  SimpleString m_LocaleContext;  // Config context
  SimpleString m_ImageFilename;
  SimpleString m_RuntimeImageFilename;  // Same as m_ImageFilename with leading
                                        // ../Intermediate/ trimmed, for use as
                                        // a relative path at runtime.
  Array<SimpleString> m_Languages;
  Array<SFGCodePointProps>
      m_CodePointProps;  // Populated in DrawFont/SaveBitmapFont, of course. >_<
  Array<SBitmapSubstitute> m_BitmapSubstitutes;  // Locale-specific; can be in
                                                 // different places on
                                                 // different texture sheets.
};
...
struct SFGProps {
  ...
  bool m_BitmapFont;  // If true, we're using provided image(s) instead of
                      // drawing a new font sheet. (HACK for Eldritch, remove
                      // later.)
  int m_PointSize;
  int m_Weight;
  bool m_Italics;
...
void CreateBitmapSubstitute(const SimpleString& Filename,
                            SBitmapSubstitute& OutSub) {
...

  {
    ...
    OutSub.m_Pixels = new byte[Size];  // TODO LOC LATER: Ever free this memory.
...
void DrawFont(Surface& LocaleSurface, SFGLocaleProps& LocaleProps,
              SFGProps& Props) {
  ...
  LogFont.lfHeight =
      -(int)(0.5f + static_cast<float>(Props.m_PointSize * PixelsY) / 72.0f);
  LogFont.lfWidth = 0;
  ...
  LogFont.lfQuality =
      Props.m_Antialias ? ANTIALIASED_QUALITY : NONANTIALIASED_QUALITY;
  ...
  SelectObject(
      SurfaceHDC,
      CreateFontIndirect(
          &LogFont));  // Should really DeleteObject() the returned object...
  ...
  LocaleSurface.Clear(RGB_TO_COLOR(255, 255, 255));  // NOTE: Change this to a
                                                     // different color to see
                                                     // the space between fonts

  FOR_EACH_ARRAY(CodePointPropsIter, LocaleProps.m_CodePointProps,
                 SFGCodePointProps) {
    ...
    FOR_EACH_ARRAY(GlyphSubIter, Props.m_GlyphSubstitutes, SGlyphSubstitute) {
      ...
      if (GlyphSub.m_CodePoint == CodePoint) {
        ...
        SBitmapSubstitute& BitmapSub =
            LocaleProps.m_BitmapSubstitutes.PushBack();
    ...
    if (CurrentSubType == SGlyphSubstitute::EST_None) {
      // For now, assume I'll never need code points beyond the range U+FFFF,
      // and just cast the code point to a wide char.
      ASSERT(CodePoint <= 0xffff);
      WideCharBuffer[0] = static_cast<wchar_t>(CodePoint & 0xffff);
      GetCharABCWidthsW(SurfaceHDC, CodePoint, CodePoint, &FontWidths);
    }

    // Add 1 pixel at the edges and between each font to prevent any kind of
    // bleeding
    ...
    ASSERTDESC(DrawRect.top + TextMetric.tmHeight + 1 < DrawRect.bottom,
               "Texture is not large enough to contain all specified glyphs.");

    if (CurrentSubType == SGlyphSubstitute::EST_None) {
      DrawTextW(SurfaceHDC, WideCharBuffer, 1, &DrawRect,
                DT_LEFT | DT_NOCLIP | DT_SINGLELINE | DT_TOP | DT_NOPREFIX);
    } else if (CurrentSubType == SGlyphSubstitute::EST_Icon) {
      // Don't actually draw the icon now. Just mark its spot in the surface,
      // and copy it in later when exporting the TGA.
      ...
      BitmapSub.m_Y = (DrawRect.top + BaseLine) - BitmapSub.m_Height +
                      BitmapSub.m_HeightOffset;
    }
    ...
    CodePointProps.m_V2 =
        (static_cast<float>(DrawRect.top + TextMetric.tmHeight) + 0.5f) * RecH;
...
void SaveTGA(const IDataStream& Stream, const Surface& WindowSurface,
             const SFGLocaleProps& LocaleProps, const SFGProps& Props) {
  struct STGAHeader {
    ...
    // TODO LOC LATER: I could also #pragma pack(push, 2) like I do in
    // bmp-format.h
  ...
  for (int j = Height - 1; j >= 0; --j) {
    for (int i = 0; i < Width; ++i) {
      ...
      FOR_EACH_ARRAY(BitmapSubIter, LocaleProps.m_BitmapSubstitutes,
                     SBitmapSubstitute) {
        const SBitmapSubstitute& Bitmap = BitmapSubIter.GetValue();
        if (i >= Bitmap.m_X && i < Bitmap.m_X + Bitmap.m_Width &&
            j >= Bitmap.m_Y && j < Bitmap.m_Y + Bitmap.m_Height) {
          int LocalX = i - Bitmap.m_X;
          int LocalY = Bitmap.m_Height - (j - Bitmap.m_Y) - 1;

          if (LocalX >= 0 && LocalX < Bitmap.m_Width && LocalY >= 0 &&
              LocalY < Bitmap.m_Height) {
            UsedBitmapSub = true;
            byte* const pPixel =
                Bitmap.m_Pixels + LocalY * Bitmap.m_Width * 4 + LocalX * 4;
      ...
      if (!UsedBitmapSub) {
        ...
        const float AdjustedAlpha =
            Pow((Alpha * Props.m_AlphaScalar), Props.m_AlphaExponent);
        const float FinalAlpha =
            Saturate(Props.m_Antialias ? AdjustedAlpha : Alpha);
...
void SaveCodePointProps(const IDataStream& Stream,
                        const SFGCodePointProps& CodePointProps) {
...
void SaveLocaleProps(const IDataStream& Stream,
                     const SFGLocaleProps& LocaleProps) {
  // Write locale tag
...
void PopulateBitmapCodePoints(const SFGProps& Props,
                              SFGLocaleProps& LocaleProps) {
  const float fCharWidth =
      static_cast<float>(Max(0, Props.m_BitmapabcA) + Props.m_BitmapabcB +
                         Max(0, Props.m_BitmapabcC));
  const float fCharHeight = static_cast<float>(Props.m_Height);
  const uint CharsPerRow =
      static_cast<uint>(LocaleProps.m_TextureWidth / (fCharWidth + 1.0f));
  ...
  FOR_EACH_ARRAY(CodePointPropsIter, LocaleProps.m_CodePointProps,
    ...
    CodePointProps.m_V2 =
        (Y + fCharHeight + 0.5f) / LocaleProps.m_TextureHeight;
...
void WrangleLocaleFilenames(SFGLocaleProps& LocaleProps,
                            const SFGProps& Props) {
  ...
  // 1. Add locale suffix
  Name = SimpleString::PrintF("%s-%s", Name.CStr(),
                              LocaleProps.m_LocaleTag.CStr());
...
void AddUnicodeRange(const unicode_t RangeLow, const unicode_t RangeHigh,
                     Array<SFGCodePointProps>& CodePointProps) {
...
void AddLocales(SFGProps& Props) {
  ...
  if (NumLocales == 0) {
    ...
    for (uint LocaleIndex = 0; LocaleIndex < NumLocales; ++LocaleIndex) {
      ...
      const SimpleString Locale =
          ConfigManager::GetSequenceString("Locale%d", LocaleIndex, "");
...
void AddGlyphSubstitutes(SFGProps& Props) {
  ...
  for (uint SubstituteIndex = 0; SubstituteIndex < NumSubstitutes;
       ++SubstituteIndex) {
    SGlyphSubstitute NewSubstitute;

    const SimpleString SubstituteDef =
        ConfigManager::GetSequenceString("Substitute%d", SubstituteIndex, "");
    MAKEHASH(SubstituteDef);

    STATICHASH(CodePoint);
    const uint CodePoint = ConfigManager::GetInt(sCodePoint, 0, sSubstituteDef);
    const SimpleString CodePointString =
        ConfigManager::GetString(sCodePoint, "", sSubstituteDef);
    NewSubstitute.m_CodePoint = Pick(CodePoint, CodePointString.GetCodePoint());

    NewSubstitute.m_SubType = SGlyphSubstitute::EST_Icon;  // This is the only
                                                           // substitution I
                                                           // support anymore.

    STATICHASH(Image);
    NewSubstitute.m_IconName =
        ConfigManager::GetString(sImage, NULL, sSubstituteDef);

    STATICHASH(SpacingLeft);
    NewSubstitute.m_SpacingLeft =
        ConfigManager::GetInt(sSpacingLeft, 0, sSubstituteDef);

    STATICHASH(SpacingRight);
    NewSubstitute.m_SpacingRight =
        ConfigManager::GetInt(sSpacingRight, 0, sSubstituteDef);

    STATICHASH(HeightOffset);
    NewSubstitute.m_HeightOffset =
        ConfigManager::GetInt(sHeightOffset, 0, sSubstituteDef);
...
void AddLanguages(SFGLocaleProps& LocaleProps) {
  ...
  if (NumLanguages == 0) {
    ...
    for (uint LanguageIndex = 0; LanguageIndex < NumLanguages;
         ++LanguageIndex) {
      const SimpleString Language = ConfigManager::GetSequenceString(
          "Language%d", LanguageIndex, "", sLocale);
...
void AddCodePoints(SFGLocaleProps& LocaleProps) {
  ...
  const uint NumCodePointRanges =
      ConfigManager::GetInt(sNumCodePointRanges, 0, sLocale);
  ...
  if (NumCodePointRanges == 0) {
    ...
    for (uint RangeIndex = 0; RangeIndex < NumCodePointRanges; ++RangeIndex) {
      const SimpleString RangeLowCode = ConfigManager::GetSequenceString(
          "CodePointRange%dLow", RangeIndex, "", sLocale);
      const SimpleString RangeHighCode = ConfigManager::GetSequenceString(
          "CodePointRange%dHigh", RangeIndex, "", sLocale);
...
void PopulateLocaleProps(SFGLocaleProps& LocaleProps) {
  ...
  STATICHASH(Width);
  STATICHASH(Height);
  LocaleProps.m_TextureWidth =
      ConfigManager::GetInt(sWidth, ConfigManager::GetInt(sWidth), sLocale);
  LocaleProps.m_TextureHeight =
      ConfigManager::GetInt(sHeight, ConfigManager::GetInt(sHeight), sLocale);

  STATICHASH(Texture);
  LocaleProps.m_BitmapTextureName = ConfigManager::GetString(
      sTexture, ConfigManager::GetString(sTexture, ""), sLocale);
...
void DrawAndSaveImages(SFGProps& Props) {
  FOR_EACH_ARRAY(LocaleIter, Props.m_LocaleProps, SFGLocaleProps) {
    ...
    Surface LocaleSurface(LocaleProps.m_TextureWidth,
                          LocaleProps.m_TextureHeight);
    DrawFont(LocaleSurface, LocaleProps, Props);

    // Save bitmaps as we process locales, but only save .fnp at the end.
    SaveTGA(
        FileStream(LocaleProps.m_ImageFilename.CStr(), FileStream::EFM_Write),
        LocaleSurface, LocaleProps, Props);
  }
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  ASSERT(OutFontFile);
  const char* OutImageFile = strtok_s(
      NULL, " ", &NextArg);  // NOTE: The actual output filename(s) will be
                             // modified per locale and according to settings.
  ASSERT(OutImageFile);
  ...
  SaveProps(
      FileStream(Props.m_OutputFontFilename.CStr(), FileStream::EFM_Write),
      Props);
