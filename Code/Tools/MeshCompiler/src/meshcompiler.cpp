...
void MeshCompiler::Write( const IDataStream& Stream )
{
	...
	if( m_Header.m_HasColors )
	{
...
#if USE_HDR
#ifdef __amigaos4__
		for( uint i = 0; i < m_FloatColors1.Size(); ++i )
		{
			Vector4 vtmp = m_FloatColors1[ i ];
		...
		for( uint i = 0; i < m_FloatColors2.Size(); ++i )
		{
			Vector4 vtmp = m_FloatColors2[ i ];
			littleBigEndian( &vtmp.w );
			littleBigEndian( &vtmp.x );
			littleBigEndian( &vtmp.y );
			littleBigEndian( &vtmp.z );
		...
		for( uint i = 0; i < m_FloatColors3.Size(); ++i )
		{
			Vector4 vtmp = m_FloatColors3[ i ];
			littleBigEndian( &vtmp.w );
			littleBigEndian( &vtmp.x );
			littleBigEndian( &vtmp.y );
			littleBigEndian( &vtmp.z );
	...
	{
#ifdef __amigaos4__
		for( uint i = 0; i < m_UVs.Size(); ++i )
		{
			Vector2 vtmp = m_UVs[ i ];
			littleBigEndian( &vtmp.x );
			littleBigEndian( &vtmp.y );
			Stream.Write( sizeof( Vector2 ), &vtmp );
		}
#else
	...
	{
#ifdef __amigaos4__
		for( uint i = 0; i < m_Tangents.Size(); ++i )
		{
			Vector4 vtmp = m_Tangents[ i ];
			littleBigEndian( &vtmp.w );
			littleBigEndian( &vtmp.x );
			littleBigEndian( &vtmp.y );
			littleBigEndian( &vtmp.z );
	...
	if( m_Header.m_HasSkeleton )
	{
		...
		for( uint i = 0; i < m_Header.m_NumFrames; ++i )
		{
			...
			{
#ifdef __amigaos4__
				Quat qtmp = m_Bones[j].m_FrameQuats[i];
				littleBigEndian( &qtmp.w );
				littleBigEndian( &qtmp.x );
				littleBigEndian( &qtmp.y );
				littleBigEndian( &qtmp.z );
				Stream.Write( sizeof( Quat ), &qtmp );
				Vector vtmp = m_Bones[j].m_FrameTransVecs[i];
				littleBigEndian( &vtmp.x );
				littleBigEndian( &vtmp.y );
				littleBigEndian( &vtmp.z );
				Stream.Write( sizeof( Vector ), &vtmp );
...
#warning TODO
	...
	{
		littleBigEndian( &aabb.m_Min.v[ jj ] );
		littleBigEndian( &aabb.m_Max.v[ jj ] );
	}
	Stream.Write( sizeof( AABB ), &aabb );
#else
