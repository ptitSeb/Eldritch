...
#define FOR_EACH_SET(iter, set, type) \
  for (Set<type>::Iterator iter = (set).Begin(); iter != (set).End(); ++iter)
#define FOR_EACH_SET_NOINCR(iter, set, type) \
  for (Set<type>::Iterator iter = (set).Begin(); iter != (set).End();)
#define FOR_EACH_SET_REVERSE(iter, set, type) \
  for (Set<type>::Iterator iter = (set).Last(); iter != (set).End(); --iter)
#define FOR_EACH_SET_REVERSE_NOINCR(iter, set, type) \
  for (Set<type>::Iterator iter = (set).Last(); iter != (set).End();)
...
class Set {
 ...
 public:
  ...
  Set(const Set& Other, Allocator* pAllocator = NULL)
      : m_Root(NULL), m_Size(0), m_Allocator(pAllocator) {
    for (Set<C>::Iterator OtherIterator = Other.Begin();
