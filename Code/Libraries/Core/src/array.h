...
// If defined, array reallocates to half its capacity when size falls below a
// quarter
// capacity and will be deallocated completely when the array is cleared.
#define DEFLATE

#define FOR_EACH_ARRAY(iter, array, type) \
  for (Array<type>::Iterator iter = (array).Begin(); iter.IsValid(); ++iter)
#define FOR_EACH_ARRAY_NOINCR(iter, array, type) \
  for (Array<type>::Iterator iter = (array).Begin(); iter.IsValid();)
#define FOR_EACH_ARRAY_REVERSE(iter, array, type) \
  for (Array<type>::Iterator iter = (array).End(); iter.IsValid(); --iter)
#define FOR_EACH_ARRAY_REVERSE_NOINCR(iter, array, type) \
  for (Array<type>::Iterator iter = (array).End(); iter.IsValid();)

// This doesn't really belong here, but whatever.
#define FOR_EACH_INDEX(idx, num) for (uint idx = 0; idx < num; ++idx)

// Dynamic array (i.e., vector--but not named such to avoid conflict with math
// vector)
...
class Array {
 public:
  ...
  class _Iterator {
   ...
   private:
    _Iterator<IC>() : m_Array(NULL), m_Index(0) {}

   public:
    _Iterator<IC>(const Array<IC>* const pArray, const uint Index)
        : m_Array(const_cast<Array<IC>*>(pArray)), m_Index(Index) {}
    ...
    IC& operator*() const { return (*m_Array)[m_Index]; }

    // Same as operator*
    IC& GetValue() const { return (*m_Array)[m_Index]; }

    uint GetIndex() const { return m_Index; }

    bool IsValid() const { return m_Array && m_Index < (*m_Array).Size(); }

    bool IsNull() const { return !m_Array || m_Index >= (*m_Array).Size(); }
 ...
 public:
  ...
  void MemoryZero() { memset(GetData(), 0, MemorySize()); }
  ...
  C& PushBack(const C& Item) {
    ...
    new (&m_Array[m_Size++]) C(TempItem);  // Construct the object now using the
                                           // copy constructor--it hasn't been
                                           // constructed yet

    return Last();
  }

  C& PushBack() { return PushBack(C()); }

  void Insert(const C& Item) { PushBack(Item); }
  ...
  void Insert(const C& Item, uint Index) {
    if (Index == m_Size) {
      ...
      for (uint c = 0, i = m_Size - 1; c < Count;
           --i, ++c)  // Doing this madness to avoid wrapping the uint
      {
        memcpy(m_Array + i, m_Array + i - 1, sizeof(C));
      }

      new (&m_Array[Index]) C(TempItem);  // Construct the object now using the
                                          // copy constructor--it hasn't been
                                          // constructed yet
    }
  }
  ...
  void Remove(const uint Index, uint Num) {
    ...
    for (uint DestructIndex = Index; DestructIndex < DestructEndIndex;
         ++DestructIndex) {
  ...
  // FIXME: used in animation.cpp to fix 64-bit issue where m_Reserved is a
  // ridiculous size, causing
  ...
  void Remove(const Iterator& Iter) { Remove(Iter.m_Index); }

  void FastRemove(const Iterator& Iter) { FastRemove(Iter.m_Index); }
  ...
  // Provided for convenience, but if frequent sorting is needed, use a
  // different structure.
  // Requires < operator to be implemented for type C.
  // Quicksort is O(n log n) and requires O(n) space. (Use insertion sort if
  // allocation cost is a problem.)
  void QuickSort() {
    if (m_Size > 1) {
  ...
  void InsertionSort() {
    for (uint Index = 0; Index < m_Size; ++Index) {
      // Create a copy of the item. It's possible I don't want to do this in
      // some cases.
      // A single copy of the whole array upfront might be more cost effective,
      // but this
 ...
 private:
  void ExpandTo(uint Capacity) {
    ...
    // Allocate new array (use byte, not C, because I'm *not* constructing
    // these)
