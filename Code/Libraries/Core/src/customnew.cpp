...
void*
operator new [](const size_t Size, Allocator& Alloc, const uint Alignment) {
  return Alloc.Allocate(static_cast<uint>(Size), Alignment);
}

// These forms aren't actually used.
void
operator delete(void* pObj, Allocator& Alloc) {
  Unused(pObj);
  Unused(Alloc);
...
void
operator delete [](void* pObj, Allocator& Alloc, const uint Alignment) {
  Unused(pObj);
  Unused(Alloc);
  Unused(Alignment);
...
#if USE_ALLOCATOR
void*
operator new(const size_t Size) NEW_THROW {
  void* pObj = NULL;
...
void*
operator new [](const size_t Size, uint Alignment) {
  return Allocator::GetDefault().Allocate(static_cast<uint>(Size), Alignment);
}

void
operator delete(void* pObj) DELETE_THROW {
  if (!pObj) {
    return;
  }

  if (Allocator::IsEnabled()) {
    AllocatorChunk* const pChunk =
        *(AllocatorChunk**)(static_cast<byte*>(pObj) -
...
void operator delete(void* pObj, uint Alignment) {
  ...
  // No need to check if Allocator::IsEnabled(), if this form is used then it
  // should be
  AllocatorChunk* const pChunk =
      *(AllocatorChunk**)(static_cast<byte*>(pObj) -
                          Allocator::m_BlockHeaderSize);
...
void
    operator delete [](void* pObj, uint Alignment) {
      Unused(Alignment);
      ...
      // No need to check if Allocator::IsEnabled(), if this form is used then
      // it should be
      AllocatorChunk* const pChunk =
          *(AllocatorChunk**)(static_cast<byte*>(pObj) -
                              Allocator::m_BlockHeaderSize);
