...
void Clock::ShutDown() { ClearMultiplierRequests(); }

void Clock::Tick(bool GamePaused /*= false*/,
                 const bool DoGameTick /*= true*/) {
  XTRACE_FUNCTION;
  ...
  m_MachineDeltaTime = m_UseFixedMachineDeltaTime
                           ? m_FixedMachineDeltaTime
                           : CounterToSeconds(m_PhysicalDeltaTime);

  m_PhysicalCurrentTime = PhysicalTime;
  m_MachineCurrentTime = m_UseFixedMachineDeltaTime
                             ? (m_MachineCurrentTime + m_FixedMachineDeltaTime)
                             : CounterToSeconds(m_PhysicalCurrentTime);
  ...
  if (DoGameTick) {
    ...
    m_GameCurrentTime =
        m_GameCurrentTime + m_GameDeltaTime;  // NOTE: Accumulates drift
  }
...
float Clock::GetGameCurrentTime() const { return m_GameCurrentTime; }

float Clock::GetGameDeltaTime() const { return m_GameDeltaTime; }

float Clock::GetMachineCurrentTime() const { return m_MachineCurrentTime; }

float Clock::GetMachineDeltaTime() const { return m_MachineDeltaTime; }

CLOCK_T Clock::GetPhysicalCurrentTime() const { return m_PhysicalCurrentTime; }

CLOCK_T Clock::GetPhysicalDeltaTime() const { return m_PhysicalDeltaTime; }

// Used by profiler, could probably be compiled out for Release
double Clock::GetResolution() const { return m_Resolution; }

uint Clock::GetTickCount() const { return m_TickCount; }
...
void Clock::Report() const {
  ...
  DEBUGPRINTF("Game time drift: %.4f\n",
              m_GameCurrentTime - m_MachineCurrentTime);  // Only meaningful
                                                          // before time effects
                                                          // are applied
}

Clock::MultiplierRequest* Clock::AddMultiplierRequest(
...
Clock::MultiplierRequest* Clock::AddMultiplierRequest(float Duration,
                                                      float Multiplier) {
  if (Multiplier < 0.0f) {
    Multiplier = 0.0f;
...
void Clock::RemoveMultiplierRequest(
    Clock::MultiplierRequest** pMultiplierRequest) {
  // For safety, check that the request is actually still in the list before
  // deleting.
  // (Else, it might have already been removed via ClearMultiplierRequests, and
  // the
  // pointer could be dangling.)
  for (List<MultiplierRequest*>::Iterator MultiplierIterator =
           m_MultiplierRequests.Begin();
       MultiplierIterator != m_MultiplierRequests.End();) {
...
void Clock::SetGameDeltaTimeMax(float Limit) { m_GameDeltaTimeMax = Limit; }
...
void Clock::UseActualMachineDeltaTime() { m_UseFixedMachineDeltaTime = false; }