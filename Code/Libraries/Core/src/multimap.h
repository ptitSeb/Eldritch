...
#define FOR_EACH_MULTIMAP(iter, map, ktype, vtype)            \
  for (Multimap<ktype, vtype>::Iterator iter = (map).Begin(); \
       iter != (map).End(); ++iter)
#define FOR_EACH_MULTIMAP_NOINCR(iter, map, ktype, vtype)     \
  for (Multimap<ktype, vtype>::Iterator iter = (map).Begin(); \
       iter != (map).End();)
#define FOR_EACH_MULTIMAP_REVERSE(iter, map, ktype, vtype)   \
  for (Multimap<ktype, vtype>::Iterator iter = (map).Last(); \
       iter != (map).End(); --iter)
#define FOR_EACH_MULTIMAP_REVERSE_NOINCR(iter, map, ktype, vtype) \
  for (Multimap<ktype, vtype>::Iterator iter = (map).Last();      \
       iter != (map).End();)
#define FOR_EACH_MULTIMAP_SEARCH(iter, map, ktype, vtype, key) \
  const ktype& AutoMultimapSearchKey = key;                    \
  for (Multimap<ktype, vtype>::Iterator iter =                 \
           (map).Search(AutoMultimapSearchKey);                \
       !iter.IsNull(); iter = (map).Search(AutoMultimapSearchKey, iter))
...
class Multimap {
 public:
  ...
  class _Iterator {
   ...
   public:
    ...
    _Iterator<IK, IV>(MapIterator MapIter, ListIterator ListIter)
        : m_MapIter(MapIter), m_ListIter(ListIter) {}
    _Iterator<IK, IV>(const _Iterator<IK, IV>& Other)
        : m_MapIter(Other.m_MapIter), m_ListIter(Other.m_ListIter) {}
    ...
    IV& operator*() const { return *m_ListIter; }

    // Useful when iterating the map to get back to the key
    IK& GetKey() const { return m_MapIter.GetKey(); }

    // Same as operator*
    IV& GetValue() const { return (*m_ListIter); }
    ...
    bool IsNull() const {
      // No need to check m_MapIter; it would be redundant (unless something's
      // broken)
      return m_ListIter.IsNull();
    }

    bool IsValid() const { return m_ListIter.IsValid(); }
 ...
 public:
  Multimap(Allocator* pAllocator = NULL) : m_Map(pAllocator), m_Size(0) {}

  Multimap(const Multimap& Other, Allocator* pAllocator = NULL)
      : m_Map(pAllocator), m_Size(0) {
    for (Multimap<K, V>::Iterator OtherIterator = Other.Begin();
         OtherIterator != Other.End(); ++OtherIterator) {
  ...
  // Return the next node that matches the key; if the incoming iterator is
  // null, restart from the head.
  ...
  // Return the node that matches the key and value (slightly faster than just
  // searching Value)
  ...
  // This syntax isn't really as useful for a Multimap as a map, but why not
  // support it?
  V& operator[](const K& Key) {
    Iterator Iter = Search(Key);
    if (Iter.IsNull()) {
      Iter = Insert(Key, V());  // Add the default value if it's not found (so
                                // the syntax MyMap[MyKey] = MyValue will work)
    }
    return *Iter;
  }
  ...
  Iterator End() const { return Iterator(MapIterator(), ListIterator()); }
};
