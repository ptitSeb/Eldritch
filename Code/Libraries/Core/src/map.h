...
#define FOR_EACH_MAP(iter, map, ktype, vtype)                                 \
  for (Map<ktype, vtype>::Iterator iter = (map).Begin(); iter != (map).End(); \
       ++iter)
#define FOR_EACH_MAP_NOINCR(iter, map, ktype, vtype) \
  for (Map<ktype, vtype>::Iterator iter = (map).Begin(); iter != (map).End();)
#define FOR_EACH_MAP_REVERSE(iter, map, ktype, vtype)                        \
  for (Map<ktype, vtype>::Iterator iter = (map).Last(); iter != (map).End(); \
       --iter)
#define FOR_EACH_MAP_REVERSE_NOINCR(iter, map, ktype, vtype) \
  for (Map<ktype, vtype>::Iterator iter = (map).Last(); iter != (map).End();)

template <class K, class V>
class Map {
 private:
  template <class NK, class NV>
  class _Node {
    friend class Map;
 ...
 private:
        ...
        if (Uncle && Uncle->m_Color == Node::Red) {
          ...
          if (N == N->m_Parent->m_Right && N->m_Parent == Grandparent->m_Left) {
            ...
            N = N->m_Left;
                     ...
                     N->m_Parent == Grandparent->m_Right) {
          ...
          if (N == N->m_Parent->m_Left && N->m_Parent == Grandparent->m_Left) {
            ...
            // Here, N == N->m_Parent->m_Right && N->m_Parent ==
            // Grandparent->m_Right
  ...
  bool IsBlack(Node* N) { return (!N || N->m_Color == Node::Black); }

  bool IsRed(Node* N) { return (N && N->m_Color == Node::Red); }
  ...
  inline void FreeNode(Node* const pNode) { delete pNode; }

 public:
  Map(Allocator* pAllocator = NULL)
      : m_Root(NULL), m_Size(0), m_Allocator(pAllocator) {}

  Map(const Map& Other, Allocator* pAllocator = NULL)
      : m_Root(NULL), m_Size(0), m_Allocator(pAllocator) {
    for (Map<K, V>::Iterator OtherIterator = Other.Begin();
    ...
    if (m_Root) {
      ...
      while (!NewNode) {
        if (Key == Parent->m_Key) {
          ...
          // (Old behavior was to not replace the value, but in practice, I want
          // that)
  ...
  V& Insert(const K& Key) { return *Insert(Key, V()); }
  ...
  // Safe to do during iteration as long as Iter is the only iterator over this
  // Map
