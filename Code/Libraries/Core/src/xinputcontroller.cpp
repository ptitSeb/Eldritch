...
#define SET_BUTTON(EB, XI) \
  m_CurrentState.m_Buttons[EB] = (XInputState.Gamepad.wButtons & XI) != 0
...
#define SET_ANALOG_BUTTON(EB, XI, RANGE, CMP, TH) \
  m_CurrentState.m_Buttons[EB] = ((float)(XInputState.Gamepad.XI) / RANGE)CMP TH
    SET_ANALOG_BUTTON(EB_LeftThumbUp, sThumbLY, 32768.0f, >=,
                      m_LeftThumbBoolThreshold);
    SET_ANALOG_BUTTON(EB_LeftThumbDown, sThumbLY, 32768.0f, <=,
                      -m_LeftThumbBoolThreshold);
    SET_ANALOG_BUTTON(EB_LeftThumbLeft, sThumbLX, 32768.0f, <=,
                      -m_LeftThumbBoolThreshold);
    SET_ANALOG_BUTTON(EB_LeftThumbRight, sThumbLX, 32768.0f, >=,
                      m_LeftThumbBoolThreshold);
    SET_ANALOG_BUTTON(EB_RightThumbUp, sThumbRY, 32768.0f, >=,
                      m_RightThumbBoolThreshold);
    SET_ANALOG_BUTTON(EB_RightThumbDown, sThumbRY, 32768.0f, <=,
                      -m_RightThumbBoolThreshold);
    SET_ANALOG_BUTTON(EB_RightThumbLeft, sThumbRX, 32768.0f, <=,
                      -m_RightThumbBoolThreshold);
    SET_ANALOG_BUTTON(EB_RightThumbRight, sThumbRX, 32768.0f, >=,
                      m_RightThumbBoolThreshold);
    SET_ANALOG_BUTTON(EB_LeftTrigger, bLeftTrigger, 255.0f, >=,
                      m_LeftTriggerBoolThreshold);
    SET_ANALOG_BUTTON(EB_RightTrigger, bRightTrigger, 255.0f, >=,
                      m_RightTriggerBoolThreshold);
#undef SET_ANALOG_BUTTON

#define SET_AXIS(EA, XI, RANGE) \
  m_CurrentState.m_Axes[EA] = (float)(XInputState.Gamepad.XI) / RANGE
...
#define SET_BUTTON(EB, SDLB)     \
  m_CurrentState.m_Buttons[EB] = \
      SDL_GameControllerGetButton(m_Controller, SDLB) != 0
...
#define SET_ANALOG_BUTTON(EB, SDLA, RANGE, CMP, TH)                        \
  m_CurrentState.m_Buttons[EB] =                                           \
      (static_cast<float>(SDL_GameControllerGetAxis(m_Controller, SDLA)) / \
       RANGE)CMP TH
      SET_ANALOG_BUTTON(
          EB_LeftThumbUp, SDL_CONTROLLER_AXIS_LEFTY, 32768.0f, <=,
          -m_LeftThumbBoolThreshold);  // Y is inverted from XInput
      SET_ANALOG_BUTTON(EB_LeftThumbDown, SDL_CONTROLLER_AXIS_LEFTY, 32768.0f,
                        >=,
                        m_LeftThumbBoolThreshold);  // Y is inverted from XInput
      SET_ANALOG_BUTTON(EB_LeftThumbLeft, SDL_CONTROLLER_AXIS_LEFTX, 32768.0f,
                        <=, -m_LeftThumbBoolThreshold);
      SET_ANALOG_BUTTON(EB_LeftThumbRight, SDL_CONTROLLER_AXIS_LEFTX, 32768.0f,
                        >=, m_LeftThumbBoolThreshold);
      SET_ANALOG_BUTTON(
          EB_RightThumbUp, SDL_CONTROLLER_AXIS_RIGHTY, 32768.0f, <=,
          -m_RightThumbBoolThreshold);  // Y is inverted from XInput
      SET_ANALOG_BUTTON(
          EB_RightThumbDown, SDL_CONTROLLER_AXIS_RIGHTY, 32768.0f, >=,
          m_RightThumbBoolThreshold);  // Y is inverted from XInput
      SET_ANALOG_BUTTON(EB_RightThumbLeft, SDL_CONTROLLER_AXIS_RIGHTX, 32768.0f,
                        <=, -m_RightThumbBoolThreshold);
      SET_ANALOG_BUTTON(EB_RightThumbRight, SDL_CONTROLLER_AXIS_RIGHTX,
                        32768.0f, >=, m_RightThumbBoolThreshold);
      SET_ANALOG_BUTTON(EB_LeftTrigger, SDL_CONTROLLER_AXIS_TRIGGERLEFT,
                        32768.0f, >=, m_LeftTriggerBoolThreshold);
      SET_ANALOG_BUTTON(EB_RightTrigger, SDL_CONTROLLER_AXIS_TRIGGERRIGHT,
                        32768.0f, >=, m_RightTriggerBoolThreshold);
#undef SET_ANALOG_BUTTON

#define SET_AXIS(EA, SDLA, RANGE, FAC)                                      \
  m_CurrentState.m_Axes[EA] =                                               \
      (FAC *                                                                \
       static_cast<float>(SDL_GameControllerGetAxis(m_Controller, SDLA))) / \
      RANGE
      SET_AXIS(EA_LeftThumbX, SDL_CONTROLLER_AXIS_LEFTX, 32768.0f, 1.0f);
      SET_AXIS(EA_LeftThumbY, SDL_CONTROLLER_AXIS_LEFTY, 32768.0f,
               -1.0f);  // Y is inverted from XInput
      SET_AXIS(EA_RightThumbX, SDL_CONTROLLER_AXIS_RIGHTX, 32768.0f, 1.0f);
      SET_AXIS(EA_RightThumbY, SDL_CONTROLLER_AXIS_RIGHTY, 32768.0f,
               -1.0f);  // Y is inverted from XInput
      SET_AXIS(EA_LeftTrigger, SDL_CONTROLLER_AXIS_TRIGGERLEFT, 32768.0f, 1.0f);
      SET_AXIS(EA_RightTrigger, SDL_CONTROLLER_AXIS_TRIGGERRIGHT, 32768.0f,
               1.0f);
#undef SET_AXIS
  ...
  m_ReceivedInputThisTick = false;
  for (uint ButtonIndex = 1; ButtonIndex < EB_Max && !m_ReceivedInputThisTick;
       ++ButtonIndex) {
    if (IsHigh(ButtonIndex)) {
      m_ReceivedInputThisTick = true;
    }
  }
  for (uint AxisIndex = 1; AxisIndex < EA_Max && !m_ReceivedInputThisTick;
       ++AxisIndex) {
...
void XInputController::ApplyDeadZones() {
  ...
  ApplyDeadZone(EA_RightThumbX, m_RightThumbDeadZone,
                m_RightThumbSaturationPoint);
  ApplyDeadZone(EA_RightThumbY, m_RightThumbDeadZone,
                m_RightThumbSaturationPoint);
...
void XInputController::ApplyDeadZone(EAxes Axis, float DeadZone,
                                     float SaturationPoint) {
  m_CurrentState.m_Axes[Axis] =
      Sign(m_CurrentState.m_Axes[Axis]) *
      Saturate(
          InvLerp(Abs(m_CurrentState.m_Axes[Axis]), DeadZone, SaturationPoint));
}
