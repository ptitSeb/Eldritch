...
SimpleString::~SimpleString() { SafeDeleteArray(m_String); }
...
const char* SimpleString::CStr() const { return m_String; }

char* SimpleString::MutableCStr() { return m_String; }

// Doesn't include the terminating null
uint SimpleString::Length() const { return m_Length; }
...
int SimpleString::AsInt() const { return atoi(m_String); }

float SimpleString::AsFloat() const { return (float)atof(m_String); }

void SimpleString::FillArray(Array<char>& OutArray,
                             bool WithNull /*= false*/) const {
...
void SimpleString::Initialize(const char* pString) {
  DEVASSERTDESC(
      pString,
      "SimpleStrings must be initialized--use \"\" instead of NULL");  // SimpleStrings
                                                                       // should
                                                                       // always
                                                                       // represent
                                                                       // something
...
SimpleString SimpleString::Replace(const char* const Find,
                                   const char* const Replace) const {
  DEVASSERT(Find);
  DEVASSERT(Replace);
  ...
  for (size_t Iterator = 0; Iterator < NewStringBuffer.Size();) {
    const char* const SubStr =
        strstr(NewStringBuffer.GetData() + Iterator, Find);
    if (SubStr) {
      const char* const Base = NewStringBuffer.GetData();
      const size_t Offset = SubStr - Base;

      for (size_t DifferenceIndex = 0; DifferenceIndex < Difference;
           ++DifferenceIndex) {
...
void SimpleString::SplitFind(const char Delimiter, SimpleString& Left,
                             SimpleString& Right) const {
  char Find[2];
  Find[0] = Delimiter;
  Find[1] = '\0';
...
void SimpleString::Split(uint Index, SimpleString& Left,
                         SimpleString& Right) const {
  if (Index < m_Length) {
...
// Hurgh, this would look nicer if strings used Arrays so I could more easily
...
#define UTF8_GETCONTINUATIONBYTE                                    \
  ...
  for (uint ByteIndex = 0; ByteIndex < m_Length; ++ByteIndex) {
    ...
    if ((NextByte & UTF8_1BYTEMASK) == UTF8_1BYTEMARKER) {
      // This is a code point in the range U+0000-U+007F and needs no
      // translation.
      CodePoint = NextByte;
      ...
      // This is a 4-byte code point in the range U+10000-U+1FFFFF (constrained
      // to U+10FFFF by definition).
...
/*static*/ SimpleString SimpleString::SetUTF8(const Array<unicode_t>& Unicode) {
  ...
  UTF8Array.Reserve(
      Unicode
          .Size());  // We know we'll have at least as many chars as code points
  ...
  FOR_EACH_ARRAY(CodePointIter, Unicode, unicode_t) {
    ...
    if (CodePoint <= UTF8_1BYTERANGE) {
      ...
      const char Byte1 =
          UTF8_2BYTEMARKER | static_cast<char>((CodePoint >> UTF8_CBYTESHIFT));
      const char Byte2 =
          UTF8_CBYTEMARKER | static_cast<char>(CodePoint & UTF8_CBYTEMASKLOW);
      UTF8Array.PushBack(Byte1);
      UTF8Array.PushBack(Byte2);
    } else if (CodePoint <= UTF8_3BYTERANGE) {
      const char Byte1 = UTF8_3BYTEMARKER |
                         static_cast<char>(CodePoint >> (2 * UTF8_CBYTESHIFT));
      const char Byte2 =
          UTF8_CBYTEMARKER |
          static_cast<char>((CodePoint >> UTF8_CBYTESHIFT) & UTF8_CBYTEMASKLOW);
      const char Byte3 =
          UTF8_CBYTEMARKER | static_cast<char>(CodePoint & UTF8_CBYTEMASKLOW);
      UTF8Array.PushBack(Byte1);
      UTF8Array.PushBack(Byte2);
      ...
      const char Byte1 = UTF8_4BYTEMARKER |
                         static_cast<char>(CodePoint >> (3 * UTF8_CBYTESHIFT));
      const char Byte2 = UTF8_CBYTEMARKER |
                         static_cast<char>(CodePoint >> (2 * UTF8_CBYTESHIFT) &
                                           UTF8_CBYTEMASKLOW);
      const char Byte3 =
          UTF8_CBYTEMARKER |
...
// Returns "U+xxxx" form
/*static*/ SimpleString SimpleString::GetCodePointString(
    const unicode_t CodePoint) {
