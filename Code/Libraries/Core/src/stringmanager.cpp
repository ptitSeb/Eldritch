...
StringManager::StringManager() : m_Strings() {}

StringManager::~StringManager() { FlushStrings(ESL_Permanent); }
...
// Parses strings containing tokens of the form "#{[type]<:context>:[name]}",
// where type is:
...
const char* StringManager::ParseConfigString(EStringLife Life,
...
void StringManager::ParseConfigToken(Array<char>& ParsedString,
                                     const char*& ConfigString) {
  ...
  SimpleString Name = (SecondPartArray.Size() > 1) ? SecondPartArray.GetData()
                                                   : FirstPartArray.GetData();
  ...
  if (TypeChar == 'b') {
    ...
    AppendString =
        ConfigManager::GetLocalizedString((TheBool ? "True" : "False"), "");
  } else if (TypeChar == 'a') {
    bool TheBool = ConfigManager::GetBool(Name, false, Context);
    AppendString = ConfigManager::GetLocalizedString(
        (TheBool ? "Achieved" : "NotAchieved"), "");
  } else if (TypeChar == 'i') {
    ...
    AppendString =
        PrintF(ESL_Transient, "%.2f",
               TheFloat);  // TODO: Might want to format this differently
  } else if (TypeChar == 's') {
...
/*static*/ bool StringManager::ResolveAndEvaluateConditional(
    const char* ConditionalString) {
  // Parse instruction for config vars, if any, and then evaluate as a
  // conditional
  SimpleString Expression = ParseConfigString(ESL_Transient, ConditionalString);
  return ConfigParser::EvaluateConditional(MemoryStream(
      const_cast<char*>(Expression.CStr()), Expression.Length() + 1));
}
...
/*static*/ void StringManager::RemoveString(EStringLife Life,
                                            const char* StringToRemove) {
...
void StringManager::DeleteInstance() { SafeDelete(m_Instance); }
...
#if BUILD_DEBUG
  m_AllocatorPermanent.Report(
      FileStream(Filename.CStr(), FileStream::EFM_Append));
  m_AllocatorTransient.Report(
      FileStream(Filename.CStr(), FileStream::EFM_Append));
#endif
}