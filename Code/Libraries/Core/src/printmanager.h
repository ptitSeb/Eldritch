...
// Print levels:
// -1 = get no output
// 0 = anything that should always be printed, such as major system alerts or
...
#define SETCATPRINTLEVEL(cat, level) \
  PrintManager::GetInstance()->SetPrintLevel(cat, level)
#define SETPRINTLEVEL(level) \
  PrintManager::GetInstance()->SetPrintLevel(HashedString::NullString, level)

// Non-debug printing is mainly intended for tools, where I want output in
// Release mode
#define CATPRINTF(cat, level, ...) \
  PrintManager::GetInstance()->Printf(cat, level, __VA_ARGS__)
#define LEVELPRINTF(level, ...)                                        \
  PrintManager::GetInstance()->Printf(HashedString::NullString, level, \
                                      __VA_ARGS__)
#define PRINTF(...) \
  PrintManager::GetInstance()->Printf(HashedString::NullString, 0, __VA_ARGS__)

#if BUILD_DEBUG
#define DEBUGCATPRINTF(cat, level, ...) \
  PrintManager::GetInstance()->Printf(cat, level, __VA_ARGS__)
#define DEBUGLEVELPRINTF(level, ...)                                   \
  PrintManager::GetInstance()->Printf(HashedString::NullString, level, \
                                      __VA_ARGS__)
#define DEBUGPRINTF(...) \
  PrintManager::GetInstance()->Printf(HashedString::NullString, 0, __VA_ARGS__)
...
#define SETPRINTCHANNELS(channels) \
  PrintManager::GetInstance()->SetChannels(channels)

#define PRINTLOGS(title)      \
  PrintManager::GetInstance() \
      ->LogTo(                \
            #title            \
            "-log.txt")  // Hacky, but enforces using static strings for title
...
#else
  IDataStream* m_LogStream;  // In Release/Final, open stream once and keep it
                             // open. In Debug, open for append for every call.
#endif
};
