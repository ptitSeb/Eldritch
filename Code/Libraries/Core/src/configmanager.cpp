...
void ConfigManager::DeleteInstance() { SafeDelete(m_Instance); }
...
/*static*/ void ConfigManager::LoadTiny(const SimpleString& TinyString) {
  ConfigParser::ParseTiny(MemoryStream(const_cast<char*>(TinyString.CStr()),
                                       TinyString.Length() + 1));
}
...
void ConfigManager::BeginWriting() { GetInstance()->m_LastContextWritten = ""; }

void ConfigManager::Write(const IDataStream& Stream, const SimpleString& Name,
                          const SimpleString& Context /*= EmptyContext*/) {
  ...
  if (ConfigVar.m_Type != ConfigVar::EVT_None) {
    ...
    switch (ConfigVar.m_Type) {
      ...
      case ConfigVar::EVT_String: {
        ...
        const SimpleString EscapeSequencedString =
            ConfigString.EscapeSequenceEncode();
...
const ConfigVar* const ConfigManager::GetConfigVarForRead(
    const STRING_TYPE& Name,
    const STRING_TYPE& Context /*= EmptyContext*/) const {
...
ConfigVar& ConfigManager::GetConfigVarForWrite(
    const STRING_TYPE& Name, const STRING_TYPE& Context /*= EmptyContext*/) {
...
bool ConfigManager::GetBool(const STRING_TYPE& Name, bool Default /*= false*/,
                            const STRING_TYPE& Context /*= EmptyContext*/) {
  const ConfigVar* const pConfigVar =
      GetInstance()->GetConfigVarForRead(Name, Context);

#if STRICT_TYPE_CHECK
  DEBUGASSERT(!pConfigVar || pConfigVar->m_Type == ConfigVar::EVT_Bool ||
              pConfigVar->m_Type == ConfigVar::EVT_None);
#endif
...
int ConfigManager::GetInt(const STRING_TYPE& Name, int Default /*= 0*/,
                          const STRING_TYPE& Context /*= EmptyContext*/) {
  const ConfigVar* const pConfigVar =
      GetInstance()->GetConfigVarForRead(Name, Context);

#if STRICT_TYPE_CHECK
  DEBUGASSERT(!pConfigVar || pConfigVar->m_Type == ConfigVar::EVT_Int ||
              pConfigVar->m_Type == ConfigVar::EVT_None);
#endif
...
float ConfigManager::GetFloat(const STRING_TYPE& Name, float Default /*= 0.0f*/,
                              const STRING_TYPE& Context /*= EmptyContext*/) {
  const ConfigVar* const pConfigVar =
      GetInstance()->GetConfigVarForRead(Name, Context);

#if STRICT_TYPE_CHECK
  DEBUGASSERT(!pConfigVar || pConfigVar->m_Type == ConfigVar::EVT_Float ||
              pConfigVar->m_Type == ConfigVar::EVT_None);
#endif
...
const char* ConfigManager::GetString(
...
#if STRICT_TYPE_CHECK
  DEBUGASSERT(!pConfigVar || pConfigVar->m_Type == ConfigVar::EVT_String ||
              pConfigVar->m_Type == ConfigVar::EVT_None);
#endif
...
HashedString ConfigManager::GetHash(
    const STRING_TYPE& Name,
...
/*static*/ const char* ConfigManager::GetLocalizedString(
    const STRING_TYPE& Name, const char* Default /*= NULL*/) {
#if PARANOID_HASH_CHECK
  // Default to English if the string isn't present in the current language;
  // this way, we avoid showing empty strings.
...
/*static*/ void ConfigManager::SetLocalizedString(const STRING_TYPE& Name,
                                                  const char* Value) {
#if PARANOID_HASH_CHECK
...
/*static*/ bool ConfigManager::GetArchetypeBool(
    const STRING_TYPE& Name, const STRING_TYPE& Archetype,
    bool Default /*= false*/, const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetBool(Name, GetBool(Name, Default, Archetype), Context);
}

/*static*/ int ConfigManager::GetArchetypeInt(
    const STRING_TYPE& Name, const STRING_TYPE& Archetype, int Default /*= 0*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetInt(Name, GetInt(Name, Default, Archetype), Context);
}

/*static*/ float ConfigManager::GetArchetypeFloat(
    const STRING_TYPE& Name, const STRING_TYPE& Archetype,
    float Default /*= 0.0f*/, const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetFloat(Name, GetFloat(Name, Default, Archetype), Context);
}

/*static*/ const char* ConfigManager::GetArchetypeString(
    const STRING_TYPE& Name, const STRING_TYPE& Archetype,
...
/*static*/ HashedString ConfigManager::GetArchetypeHash(
    const STRING_TYPE& Name, const STRING_TYPE& Archetype,
    const HashedString& Default /*= HashedString::NullString*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
...
/*static*/ bool ConfigManager::GetSequenceBool(
    const SimpleString& FormatName, int Index, bool Default /*= false*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetBool(SimpleString::PrintF(FormatName.CStr(), Index), Default,
                 Context);
}

/*static*/ int ConfigManager::GetSequenceInt(
    const SimpleString& FormatName, int Index, int Default /*= 0*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetInt(SimpleString::PrintF(FormatName.CStr(), Index), Default,
                Context);
}

/*static*/ float ConfigManager::GetSequenceFloat(
    const SimpleString& FormatName, int Index, float Default /*= 0.0f*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetFloat(SimpleString::PrintF(FormatName.CStr(), Index), Default,
                  Context);
}

/*static*/ const char* ConfigManager::GetSequenceString(
    const SimpleString& FormatName, int Index, const char* Default /*= NULL*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetString(SimpleString::PrintF(FormatName.CStr(), Index), Default,
                   Context);
}

/*static*/ HashedString ConfigManager::GetSequenceHash(
    const SimpleString& FormatName, int Index,
    const HashedString& Default /*= HashedString::NullString*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetHash(SimpleString::PrintF(FormatName.CStr(), Index), Default,
                 Context);
}

/*static*/ bool ConfigManager::GetArchetypeSequenceBool(
    const SimpleString& FormatName, int Index, const STRING_TYPE& Archetype,
    bool Default /*= false*/, const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetArchetypeBool(SimpleString::PrintF(FormatName.CStr(), Index),
                          Archetype, Default, Context);
}

/*static*/ int ConfigManager::GetArchetypeSequenceInt(
    const SimpleString& FormatName, int Index, const STRING_TYPE& Archetype,
    int Default /*= 0*/, const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetArchetypeInt(SimpleString::PrintF(FormatName.CStr(), Index),
                         Archetype, Default, Context);
}

/*static*/ float ConfigManager::GetArchetypeSequenceFloat(
    const SimpleString& FormatName, int Index, const STRING_TYPE& Archetype,
    float Default /*= 0.0f*/, const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetArchetypeFloat(SimpleString::PrintF(FormatName.CStr(), Index),
                           Archetype, Default, Context);
}

/*static*/ const char* ConfigManager::GetArchetypeSequenceString(
    const SimpleString& FormatName, int Index, const STRING_TYPE& Archetype,
    const char* Default /*= NULL*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetArchetypeString(SimpleString::PrintF(FormatName.CStr(), Index),
                            Archetype, Default, Context);
}

/*static*/ HashedString ConfigManager::GetArchetypeSequenceHash(
    const SimpleString& FormatName, int Index, const STRING_TYPE& Archetype,
    const HashedString& Default /*= HashedString::NullString*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetArchetypeHash(SimpleString::PrintF(FormatName.CStr(), Index),
                          Archetype, Default, Context);
}

/*static*/ bool ConfigManager::GetInheritedBool(
...
/*static*/ int ConfigManager::GetInheritedInt(
...
/*static*/ float ConfigManager::GetInheritedFloat(
...
/*static*/ const char* ConfigManager::GetInheritedString(
    const STRING_TYPE& Name, const char* Default /*= NULL*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  STRING_TYPE NextContext = Context;
  const ConfigVar* pConfigVar = nullptr;
...
/*static*/ HashedString ConfigManager::GetInheritedHash(
    const STRING_TYPE& Name,
    const HashedString& Default /*= HashedString::NullString*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  STRING_TYPE NextContext = Context;
...
/*static*/ bool ConfigManager::GetInheritedSequenceBool(
    const SimpleString& FormatName, int Index, bool Default /*= false*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetInheritedBool(SimpleString::PrintF(FormatName.CStr(), Index),
                          Default, Context);
}

/*static*/ int ConfigManager::GetInheritedSequenceInt(
    const SimpleString& FormatName, int Index, int Default /*= 0*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetInheritedInt(SimpleString::PrintF(FormatName.CStr(), Index),
                         Default, Context);
}

/*static*/ float ConfigManager::GetInheritedSequenceFloat(
    const SimpleString& FormatName, int Index, float Default /*= 0.0f*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetInheritedFloat(SimpleString::PrintF(FormatName.CStr(), Index),
                           Default, Context);
}

/*static*/ const char* ConfigManager::GetInheritedSequenceString(
    const SimpleString& FormatName, int Index, const char* Default /*= NULL*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetInheritedString(SimpleString::PrintF(FormatName.CStr(), Index),
                            Default, Context);
}

/*static*/ HashedString ConfigManager::GetInheritedSequenceHash(
    const SimpleString& FormatName, int Index,
    const HashedString& Default /*= HashedString::NullString*/,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  return GetInheritedHash(SimpleString::PrintF(FormatName.CStr(), Index),
                          Default, Context);
}

void ConfigManager::SetBool(const STRING_TYPE& Name, bool Value,
                            const STRING_TYPE& Context /*= EmptyContext*/) {
  ConfigVar& ConfigVar = GetInstance()->GetConfigVarForWrite(Name, Context);

  DEBUGASSERT(ConfigVar.m_Type == ConfigVar::EVT_Bool ||
              ConfigVar.m_Type == ConfigVar::EVT_None);
...
void ConfigManager::SetInt(const STRING_TYPE& Name, int Value,
                           const STRING_TYPE& Context /*= EmptyContext*/) {
  ConfigVar& ConfigVar = GetInstance()->GetConfigVarForWrite(Name, Context);

  DEBUGASSERT(ConfigVar.m_Type == ConfigVar::EVT_Int ||
              ConfigVar.m_Type == ConfigVar::EVT_None);
...
void ConfigManager::SetFloat(const STRING_TYPE& Name, float Value,
                             const STRING_TYPE& Context /*= EmptyContext*/) {
  ConfigVar& ConfigVar = GetInstance()->GetConfigVarForWrite(Name, Context);

  DEBUGASSERT(ConfigVar.m_Type == ConfigVar::EVT_Float ||
              ConfigVar.m_Type == ConfigVar::EVT_None);
...
void ConfigManager::SetString(const STRING_TYPE& Name, const char* Value,
                              const STRING_TYPE& Context /*= EmptyContext*/) {
  ...
  DEBUGASSERT(ConfigVar.m_Type == ConfigVar::EVT_String ||
              ConfigVar.m_Type == ConfigVar::EVT_None);
  ...
  if (ConfigVar.m_String) {
    StringManager::RemoveString(StringManager::ESL_Permanent,
                                ConfigVar.m_String);
  ...
  ConfigVar.m_Hash = HashedString(
      StringCopy);  // Update the hash whenever the string is changed
...
void ConfigManager::ToggleBool(const STRING_TYPE& Name,
                               const STRING_TYPE& Context /*= EmptyContext*/) {
  SetBool(Name, !GetBool(Name, false, Context), Context);
}

void ConfigManager::AddInt(const STRING_TYPE& Name, int Value,
                           const STRING_TYPE& Context /*= EmptyContext*/) {
  SetInt(Name, Value + GetInt(Name, 0, Context), Context);
}

void ConfigManager::AddFloat(const STRING_TYPE& Name, float Value,
                             const STRING_TYPE& Context /*= EmptyContext*/) {
  SetFloat(Name, Value + GetFloat(Name, 0.0f, Context), Context);
}

void ConfigManager::AppendString(
    const STRING_TYPE& Name, const char* Value,
    const STRING_TYPE& Context /*= EmptyContext*/) {
  SetString(Name, SimpleString::PrintF("%s%s", GetString(Name, "", Context),
                                       Value).CStr(),
            Context);
}

void ConfigManager::Bind(bool* Addr, const STRING_TYPE& Name,
                         bool Default /*= false*/,
                         const STRING_TYPE& Context /*= EmptyContext*/) {
  VarMap& VarMap = GetInstance()->m_Vars[HASH_STRING(Context)];
  ConfigVar& ConfigVar = VarMap[HASH_STRING(Name)];
...
void ConfigManager::Bind(int* Addr, const STRING_TYPE& Name,
                         int Default /*= 0*/,
                         const STRING_TYPE& Context /*= EmptyContext*/) {
  VarMap& VarMap = GetInstance()->m_Vars[HASH_STRING(Context)];
  ConfigVar& ConfigVar = VarMap[HASH_STRING(Name)];
...
void ConfigManager::Bind(float* Addr, const STRING_TYPE& Name,
                         float Default /*= 0.0f*/,
                         const STRING_TYPE& Context /*= EmptyContext*/) {
  VarMap& VarMap = GetInstance()->m_Vars[HASH_STRING(Context)];
  ConfigVar& ConfigVar = VarMap[HASH_STRING(Name)];
...
void ConfigManager::Bind(const char** Addr, const STRING_TYPE& Name,
                         const char* Default /*= NULL*/,
                         const STRING_TYPE& Context /*= EmptyContext*/) {
  VarMap& VarMap = GetInstance()->m_Vars[HASH_STRING(Context)];
  ConfigVar& ConfigVar = VarMap[HASH_STRING(Name)];
...
#if BUILD_DEBUG
  m_Allocator.Report(
      FileStream("memory_exit_report.txt", FileStream::EFM_Append));
#endif
