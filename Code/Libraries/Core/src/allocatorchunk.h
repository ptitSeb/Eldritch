...
class AllocatorChunk {
 public:
  ...
  void ReportMemoryLeaks(const IDataStream& Stream, const uint Bookmark1,
  ...
  void GetStats(uint* const pAllocated, uint* const pOverhead,
 ...
 private:
  ...
  void SplitToAlignment(SAllocatorBlock* const BlockToSplit,
                        const uint Alignment);
  bool CanSplitToAlignment(const SAllocatorBlock* const pBlock,
                           const uint Alignment, const uint Size) const;
  void Coalesce(
      SAllocatorBlock* const First);  // Coalesces this block with its next
  bool IsAligned(const SAllocatorBlock* const pBlock, const uint Alignment)
      const;  // True if this block's *data* is aligned

  Allocator* m_Allocator;
  SAllocatorBlock* m_Head;
  SAllocatorBlock* m_Free;  // A known free block for fast allocation. No other
                            // guarantees about placement.
  uint m_Size;

#if BUILD_DEV
  uint m_AllocSpace;  // Just what is actually allocated by user
  uint m_Overhead;    // Allocated for use internally
  uint m_FreeSpace;   // Actual free space remaining (capacity minus allocated
                      // and overhead)
  uint m_NumBlocks;
  uint m_MaxAlloc;
  uint m_MaxOverhead;
