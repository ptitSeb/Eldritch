...
FileStream::FileStream(const char* FileName, EFileMode FileMode)
  ...
  if (!m_TheFile) {
    ...
    if (pPrintManager && FileName == pPrintManager->GetLogFilename()) {
      // Don't print and don't assert! That would cause endless recursion trying
      // to write to a log file that can't be opened.
      return;
    }
...
FileStream::~FileStream() { fclose(m_TheFile); }
...
int FileStream::GetPos() const { return ftell(m_TheFile); }

int FileStream::EOS() const { return feof(m_TheFile); }
