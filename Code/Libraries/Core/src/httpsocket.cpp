...
DWORD WINAPI ConnectThreadProc(void* Parameter) {
  ...
  if (connect(pSocket->m_Socket, (SOCKADDR*)&pSocket->m_AsyncSocketAddress,
...
DWORD WINAPI SendAndReceiveThreadProc(void* Parameter) {
  ...
  uint BytesSent = send(pSocket->m_Socket, pSocket->m_AsyncSendBuffer.GetData(),
                        pSocket->m_AsyncSendBuffer.Size(), 0);
  Unused(BytesSent);
...

  {
    ...
    do {
      BytesReceived =
          recv(pSocket->m_Socket,
               pSocket->m_AsyncReceiveBuffer.GetData() + TotalBytesReceived,
               pSocket->m_AsyncReceiveBufferSize, 0);
...
bool HTTPSocket::Connect(const SSocketInit& SocketInit) {
  ...
  if (m_SocketInit.m_HostName == "") {
    ...
    int Success =
        getaddrinfo(m_SocketInit.m_HostName.CStr(), "80", &Hints, &AddressInfo);
  ...
  if (m_SocketInit.m_Async) {
    ...
    // Blocking connect
    if (connect(m_Socket, (SOCKADDR*)&SocketAddress, sizeof(SocketAddress)) ==
        SOCKET_ERROR) {
      return false;
    } else {
      return true;
...
void HTTPSocket::Get(Array<char>& OutBuffer, const SimpleString& AgentName) {
  SimpleString Header = "";
  Header += SimpleString("GET ") + m_SocketInit.m_Path +
            SimpleString(" HTTP/1.1" CRLF) + SimpleString("Host: ") +
            m_SocketInit.m_HostName + SimpleString(CRLF) +
  ...
  memcpy_s(RequestBuffer.GetData(), RequestBufferSize, Header.CStr(),
           RequestBufferSize);

  SendAndReceive(RequestBuffer, OutBuffer);
}

void HTTPSocket::Post(const Array<char>& Buffer, const SimpleString& AgentName,
                      Array<char>& OutBuffer) {
  uint BufferSize = Buffer.Size();

  SimpleString Header = "";
  Header += SimpleString("POST ") + m_SocketInit.m_Path +
            SimpleString(" HTTP/1.1" CRLF) + SimpleString("Host: ") +
            m_SocketInit.m_HostName + SimpleString(CRLF) +
            SimpleString("Content-Type: ") + m_SocketInit.m_ContentType +
            SimpleString(CRLF) + SimpleString("Content-Length: ") +
            SimpleString::PrintF("%d", BufferSize) + SimpleString(CRLF) +
            SimpleString("User-Agent: ") + AgentName + SimpleString(CRLF);
  ...
  Array<char> RequestBuffer;
  uint RequestBufferSize = Header.Length();  // Don't include the terminating
                                             // null; we're appending to the
                                             // header here
  RequestBuffer.Resize(RequestBufferSize + BufferSize);
  memcpy_s(RequestBuffer.GetData(), RequestBufferSize, Header.CStr(),
           RequestBufferSize);
  memcpy_s(RequestBuffer.GetData() + RequestBufferSize, BufferSize,
           Buffer.GetData(), BufferSize);
...
void HTTPSocket::Post(const SimpleString& Buffer, const SimpleString& AgentName,
                      Array<char>& OutBuffer) {
...
void HTTPSocket::SendAndReceive(const Array<char>& Buffer,
                                Array<char>& OutBuffer) {
#if BUILD_DEV
  // Assert that we can send a message of the given size all at once
  uint MaxSize;
  int SizeOf = sizeof(uint);
  if (getsockopt(m_Socket, SOL_SOCKET, SO_MAX_MSG_SIZE, (char*)&MaxSize,
                 &SizeOf) != SOCKET_ERROR) {
  ...
  if (m_SocketInit.m_Async) {
    ...
    do {
      ...
      memcpy_s(OutBuffer.GetData() + BufferSize, BytesReceived, TempBuffer,
               BytesReceived);
    } while (BytesReceived);
...
bool HTTPSocket::AsyncConnectSucceeded() const { return m_AsyncConnected; }
...
uint HTTPSocket::AsyncGetBytesReceived() const { return m_AsyncBytesReceived; }