...
SPackageFileHeader::SPackageFileHeader()
    : m_MagicID(0), m_NumFiles(0), m_OffsetToFiles(0), m_FilesSize(0) {}
...
PackStream::PackStream(const char* Filename)
  ...
  if (sm_PackageFiles.Size()) {
    ...
    for (uint PackageFileIndex = 0; PackageFileIndex < sm_PackageFiles.Size();
         ++PackageFileIndex) {
      SPackageFile& PackageFile = sm_PackageFiles[PackageFileIndex];

      for (uint PackedFileIndex = 0;
           PackedFileIndex < PackageFile.m_Entries.Size(); ++PackedFileIndex) {
        SPackageFileEntry* const FileEntry =
            PackageFile.m_Entries[PackedFileIndex];
        if (FileUtil::Compare(FileEntry->m_Filename.CStr(), Filename)) {
          ...
          PackageFile.m_Stream->Read(FileEntry->m_CompressedSize,
                                     CompressedBuffer);

          if (FileEntry->m_Compressed) {
            m_PackageFileBuffer = new byte[FileEntry->m_DecompressedSize];
            uncompress(m_PackageFileBuffer, &FileEntry->m_DecompressedSize,
                       CompressedBuffer, FileEntry->m_CompressedSize);
            SafeDeleteArray(CompressedBuffer);
          } else {
            m_PackageFileBuffer = CompressedBuffer;
          }

          m_MemoryStream = new MemoryStream(m_PackageFileBuffer,
                                            FileEntry->m_DecompressedSize);
...
/*static*/ void PackStream::StaticAddPackageFile(
  ...
  if (PackageFilename && FileUtil::Exists(PackageFilename)) {
    sm_PackageFiles.Insert(
        SPackageFile(), PreemptExistingPackages ? 0 : sm_PackageFiles.Size());
    SPackageFile& NewPackageFile = PreemptExistingPackages
                                       ? sm_PackageFiles.First()
                                       : sm_PackageFiles.Last();

    NewPackageFile.m_Filename = new SimpleString(PackageFilename);

    NewPackageFile.m_Stream =
        new FileStream(PackageFilename, FileStream::EFM_Read);
    ...
    for (uint i = 0; i < NewPackageFile.m_Header.m_NumFiles; ++i) {
      ...
      FileEntry->m_DecompressedSize =
          (uint)NewPackageFile.m_Stream->ReadUInt32();
...
uint PackStream::GetSubfileOffset() const {
  if (m_PackedMode) {
    return m_PackageFile->m_Header.m_OffsetToFiles +
           m_FileEntry->m_OffsetToFile;
  } else {
    return 0;
  }
}
