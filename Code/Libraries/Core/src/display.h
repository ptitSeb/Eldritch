...
class Display {
 public:
  ...
  static SDisplayMode GetBestDisplayMode(const uint DesiredWidth,
                                         const uint DesiredHeight);
  static void SetDisplay(bool Reset = true, bool FullScreen = false,
                         int Width = 0, int Height = 0, int BitDepth = 0,
                         int Frequency = 0);

  uint m_Width;
  uint m_Height;
  uint m_ScreenWidth;   // Initial Windows display resolution (as opposed to
                        // game's resolution)
  uint m_ScreenHeight;  // Not updated when user changes resolutions, so it's
                        // always the stored default.
  float m_AspectRatio;
  bool m_Fullscreen;
