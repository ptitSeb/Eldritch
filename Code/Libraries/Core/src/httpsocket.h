...
class HTTPSocket {
 public:
  struct SSocketInit {
    ...
    SimpleString m_IPAddress;  // Only needed if HostName is not provided, and
                               // it should be for doing the HTTP requests
                               // anyway
    SimpleString m_Path;
  ...
  void Post(const Array<char>& Buffer, const SimpleString& AgentName,
            Array<char>& OutBuffer);  // Don't use for posting strings, as it
                                      // will post the terminating null
  void Post(const SimpleString& Buffer, const SimpleString& AgentName,
