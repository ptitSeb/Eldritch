...
PrintManager* PrintManager::GetInstance_NoAlloc() { return m_Instance; }
...
void PrintManager::SetChannels(int Channels) { m_Channels = Channels; }
...
void PrintManager::LogTo(const char* Filename) {
  ...
  FileStream LogStream(
      m_LogFilename,
      FileStream::EFM_Write);  // Open the file for write just to clear it
#else
  // NOTE: I thought about using a buffered filestream to allow concurrent
  // instances of the game,
  // but then the game would be accumulating all printing into a huge buffer at
  // runtime. So no.
