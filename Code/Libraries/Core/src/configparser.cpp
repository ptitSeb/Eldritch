...
// & declares a counter (for RHS value) or preincrements and inserts the counter
// (for LHS name)
...
char* SToken::m_TokenNames[] = {
    "ET_None",  "ET_Name",   "ET_Equals",  "ET_Bool",  "ET_Int",
    "ET_Float", "ET_String", "ET_Context", "ET_Macro", "ET_Counter",
};

SToken::SToken() : m_TokenType(ET_None), m_TokenString() {}

SToken::~SToken() { m_TokenString.Clear(); }
...
bool IsBool(char c) { return (c == 't' || c == 'T' || c == 'f' || c == 'F'); }
...
// InnerParse is poorly named; it really just accepts any character not handled
...
void ConfigParser::Parse(const IDataStream& Stream) {
  ...
  char StrMark =
      0;  // Stores the character (either ' or ") that opened a string
  SToken MacroToken;
    ...
    if (c == '&') {
      ...
      switch (Token.m_TokenType) {
        ...
        case SToken::ET_Macro:
          if (Token.m_TokenString.Empty()) {
            ...
            } else if (Token.m_TokenType == SToken::ET_Context ||
      ...
      if (Token.m_TokenType == SToken::ET_String) {
        ...
        if (c == '!')  // #! and !# indicate the start and end of block comments
        {
          ...
          // Read to end of line
          while (c != '\n' && c != '\v' &&
                 !Stream.EOS())  // Vertical tab stupidity again
          {
            c = Stream.ReadInt8();
          }
      ...
      // Escape sequence, intended to insert linebreaks (and maybe other things
      // in the future)
      // Config string escape sequences are not the same as C++ escape
      // sequences. They can be
      // \\, \n, \?, \", or \xx (where x are hex digits, to specify any
      // character by hex).
      ...
      if (Token.m_TokenType == SToken::ET_String) {
        ...
        // We should only ever declare or insert a macro when we're expecting a
        // name
        ASSERT(Token.m_TokenType == SToken::ET_Name);
        c = Stream.ReadInt8();
        if (c == '@') {
      ...
      InnerParse(c, StrMark, Token.m_TokenString, LineCount, Token.m_TokenType,
                 &ClosedString);
      if (ClosedString) {
        Tokens.PushBack(Token);
        DEBUGCATPRINTF("Core", 2, "%s: %s\n",
  ...
  for (uint i = 0; i < Tokens.Size(); ++i) {
    ...
    if (NameToken.m_TokenType == SToken::ET_Name) {
      ...
      CATPRINTF("Core", 2, "%s: %s: %s\n", Name.CStr(),
                SToken::m_TokenNames[ValueToken.m_TokenType], ValueString);
      ...
      switch (ValueToken.m_TokenType) {
        ...
        case SToken::ET_Counter: {
          ...
          (*NextCounterIter)++;  // Add one to the last value we incremented,
                                 // and that's the total for this array
      ...
      DEBUGCATPRINTF("Core", 2,
                     "Skipped unexpected token %s (expected ET_Name)\n",
...
void ConfigParser::ParseTiny(const IDataStream& Stream) {
  ...
  while (!Stream.EOS()) {
    ...
    if (ParseState == EPS_FirstPart) {
      ...
      if (((Type != SToken::ET_String || StringClosed) &&
           (c == ' ' || c == '\t')) ||
          c == '\n' || c == '\0') {
        // Parse the value and set the config var
        ValuePart.PushBack('\0');

        SimpleString Name = (SecondPart.Size() > 1) ? SecondPart.GetData()
                                                    : FirstPart.GetData();
        SimpleString Context =
            (SecondPart.Size() > 1) ? FirstPart.GetData() : "";
...
// It might be useful to have some arithmetic parsing for more complex
// expressions,
...
bool ConfigParser::EvaluateConditional(const IDataStream& Stream) {
  ...
  if (LeftSideType == RightSideType ||
    ...
    if (LeftSideType == SToken::ET_Bool) {
      ...
      int RightSideInt = (RightSideType == SToken::ET_Int)
                             ? RightSide.AsInt()
                             : (int)RightSide.AsFloat();
      ...
      float RightSideFloat = (RightSideType == SToken::ET_Float)
                                 ? RightSide.AsFloat()
                                 : (float)RightSide.AsInt();
