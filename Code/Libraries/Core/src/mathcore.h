...
inline unsigned int Pick(const unsigned int A, const unsigned int B) {
  return A != 0 ? A : B;
}  // Not a standard operator, just something useful for me
inline unsigned int Clamp(const unsigned int Value, const unsigned int Low,
...
inline int Pick(const int A, const int B) {
  return A != 0 ? A : B;
}  // Not a standard operator, just something useful for me
...
inline int Mod(const int N, const int D) {
  int R = N % D;
  if (R < 0) return R + D;
  return R;
}  // Handles negative numbers the way I want, i.e., (-7) % 8 == 1
...
inline void MinMax(const float A, const float B, float& OutMin, float& OutMax) {
  if (A < B) {
    OutMin = A;
    OutMax = B;
  } else {
...
inline float Pick(const float A, const float B) {
  return A != 0.0f ? A : B;
}  // Not a standard operator, just something useful for me
inline float Clamp(const float Value, const float Low, const float High) {
  return (Value < Low) ? Low : (Value > High) ? High : Value;
}
inline bool Equal(const float A, const float B, const float E = EPSILON) {
  return Abs(A - B) < E;
}
...
inline float SignedPow(const float V, const float E) {
  return Pow(Abs(V), E) * ((V > 0.0f) ? 1.0f : -1.0f);
}
...
inline float LogBase(const float F, const float Base) {
...
inline float InvLerp(const float F, const float A, const float B) {
  return (F - A) / (B - A);
}
inline float Hermite(const float A, const float B, const float T) {
...
inline float Attenuate(const float Distance, const float RadiusHalf) {
  return RadiusHalf / (RadiusHalf + Distance);
}
...
inline void CheckFloatingPointControlWord() {
  ...
  //_controlfp_s(&control_word, _CW_DEFAULT, MCW_PC);	// If I ever need it,
  //this resets the FPCW to default
  PRINTF("FPCW: 0x%08X\n", control_word);
#endif
}
...
inline float FastInvSqRt(float F) {
  ...
  F = *(float*)&FAsI;
  F = F * (1.5f - Half * F * F);
  // F = F * ( 1.5f - Half * F * F ); // Another iteration of Newton's method
  // can be done to get more precision.
  return F;
}
