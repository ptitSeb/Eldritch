...
#define FOR_EACH_LIST(iter, list, type) \
  for (List<type>::Iterator iter = (list).Begin(); iter != (list).End(); ++iter)
#define FOR_EACH_LIST_NOINCR(iter, list, type) \
  for (List<type>::Iterator iter = (list).Begin(); iter != (list).End();)
#define FOR_EACH_LIST_REVERSE(iter, list, type) \
  for (List<type>::Iterator iter = (list).Back(); iter != (list).End(); --iter)
#define FOR_EACH_LIST_REVERSE_NOINCR(iter, list, type) \
  for (List<type>::Iterator iter = (list).Back(); iter != (list).End();)
...
class List {
 ...
 public:
  List(Allocator* pAllocator = NULL)
      : m_Head(NULL), m_Tail(NULL), m_Size(0), m_Allocator(pAllocator) {}

  List(const List& Other, Allocator* pAllocator = NULL)
      : m_Head(NULL), m_Tail(NULL), m_Size(0), m_Allocator(pAllocator) {
    for (List<C>::Iterator OtherIterator = Other.Begin();
 ...
 private:
  // Can be the head, tail, or any item in the middle, so has to handle many
  // cases
 ...
 public:
  Iterator Front() const { return Iterator(m_Head); }
