...
ProfilerTable::ProfilerTable() : m_Entries(), m_Overhead(), m_Inclusive() {
  m_Overhead.Push(0);
  m_Inclusive.Push(0);
}
...
void ProfilerTable::AddEntry(const char* Name, const HashedString& HashedName,
                             __int64 InclusiveTime, __int64 ExclusiveTime) {
...
void ProfilerTable::Dump(const IDataStream& Stream) {
  ...
  // Write header
  Stream.PrintF(
      "FuncSig\tAvgExcTimeSec\tAvgIncTimeSec\tMaxExcTimeSec\tMaxIncTimeSec\tNum"
      "Calls\tTotIncTimeCyc\tTotIncTimeSec\tAvgIncTimeCyc\tMaxIncTimeCyc\tTotEx"
      "cTimeCyc\tTotExcTimeSec\tAvgExcTimeCyc\tMaxExcTimeCyc\n");
  ...
  for (; Iter != End; ++Iter) {
    ...
    Stream.PrintF("\t%f", ((double)Entry.m_TotalExclusiveTime /
                           (double)Entry.m_NumCalls) *
                              ClockResolution);

    // Average inclusive time in seconds
    Stream.PrintF("\t%f", ((double)Entry.m_TotalInclusiveTime /
                           (double)Entry.m_NumCalls) *
                              ClockResolution);
    ...
    Stream.PrintF(
        "\t%f", (double)Entry.m_TotalInclusiveTime / (double)Entry.m_NumCalls);
    ...
    Stream.PrintF(
        "\t%f", (double)Entry.m_TotalExclusiveTime / (double)Entry.m_NumCalls);
...
void ProfilerTable::Flush() { m_Entries.Clear(); }

const ProfilerMap& ProfilerTable::GetEntries() { return m_Entries; }
