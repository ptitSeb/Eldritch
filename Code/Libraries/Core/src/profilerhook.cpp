...
#define GET_TABLE           \
  ProfilerTable& TheTable = \
      (m_Permanent ? pProfiler->m_Table : pProfiler->m_FrameTable)
...
ProfilerHook::ProfilerHook(const char* Name, const HashedString& HashedName,
                           bool Permanent)
    : m_Name(Name),
...
ProfilerHook::~ProfilerHook() {
  ...
  // Pop the overhead time off the stack and subtract from this hook's running
  // time
