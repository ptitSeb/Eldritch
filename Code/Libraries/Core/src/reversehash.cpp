...
void ReverseHash::Initialize() { gReverseHashEnabled = true; }
...
bool ReverseHash::IsEnabled() { return gReverseHashEnabled; }

void ReverseHash::RegisterHash(const HashedString& Hash,
                               const SimpleString& String) {
  if (gReverseHashEnabled) {
#if BUILD_DEV
    const Map<HashedString, SimpleString>::Iterator HashIter =
        gReverseHashMap.Search(Hash);
    if (HashIter.IsValid()) {
      const SimpleString ExistingString = HashIter.GetValue();
      if (ExistingString != String) {
        PRINTF(
            "ReverseHash: Hash collision detected between \"%s\" and \"%s\"!\n",
    ...
    // Even though it's safe, this shouldn't be called if the reverse hash isn't
    // enabled,
...
// NOTE: Doesn't account for fragmentation, which is probably a big part of
// string memory.
