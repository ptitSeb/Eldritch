...
void ConsoleManager::DeleteInstance() { SafeDelete(m_Instance); }
...
void ConsoleManager::PushDeferredCommands(const SimpleString& Commands,
                                          uint Frames) {
...
void ConsoleManager::Tick(float DeltaTime, bool GetInput) {
  for (List<SDeferredCommands>::Iterator DeferredCommandIter =
           m_DeferredCommands.Begin();
       DeferredCommandIter != m_DeferredCommands.End();) {
...
bool ConsoleManager::IsCheater() const { return m_IsCheater; }

void ConsoleManager::SetIsCheater(bool Cheater) { m_IsCheater = Cheater; }
...
void ConsoleManager::SubmitCommandString() {
  ...
  if (m_CommandHistory.Size() >
      (uint)ConfigManager::GetInt(sCommandHistorySize, 10)) {
