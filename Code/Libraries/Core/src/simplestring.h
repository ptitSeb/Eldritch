...
class SimpleString {
 public:
  explicit SimpleString(bool NoInit);  // To prevent static scope strings from
                                       // being initialized with the wrong
                                       // allocator
  ...
  SimpleString(const Array<char>&
                   String);  // Be careful, the array must be null-terminated
  ~SimpleString();

  SimpleString& operator=(const SimpleString& String);
  SimpleString& operator=(const char* pString);
  SimpleString& operator=(const Array<
      char>& String);  // Be careful that the array might not be null-terminated
  ...
  void SplitFind(const char Delimiter, SimpleString& Left, SimpleString& Right)
      const;  // Split at the first instance of Delimiter
  ...
  void FillArray(Array<char>& OutArray,
                 bool WithNull = false) const;  // Clears and fills given array,
                                                // optionally including
                                                // terminating null

  void UTF8ToUnicode(Array<unicode_t>& OutUnicode) const;  // Converts string
                                                           // (assumed in UTF-8
                                                           // form without BOM)
                                                           // to an array of
                                                           // Unicode code
                                                           // points
  static SimpleString SetUTF8(const Array<unicode_t>& Unicode);  // Converts
                                                                 // array of
                                                                 // Unicode code
                                                                 // points to
                                                                 // UTF-8 form
  unicode_t GetCodePoint() const;  // Converts string (assumed in "U+xxxx" form)
                                   // to a Unicode code point
  static SimpleString GetCodePointString(
      const unicode_t CodePoint);  // Returns "U+xxxx" form
