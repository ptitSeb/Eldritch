...
Thread::Thread(ThreadFunc Function, void* Parameter) : m_Thread(NULL) {
...
Thread::~Thread() {
  // No need to ExitThread, it happens automatically when the thread function
  // returns
}

void Thread::Wait() { WaitForSingleObject(m_Thread, INFINITE); }
