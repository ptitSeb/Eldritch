...
// WM_NOTIFY_SIZE is used to notify a program of the intended size of a window
// prior to its creation.
// This allows the window's max size to be properly adjusted when
// WM_GETMINMAXINFO is received.
// wParam: Unused
// lParam: Pointer to a POINT representing the requested window's dimensions.

// Style notes:
// For basic window (thick border, resizing, min/max buttons):
// WS_OVERLAPPEDWINDOW
...
class Window {
 public:
...
#if BUILD_WINDOWS_NO_SDL
  int Init(const char* Title, const char* ClassName, DWORD Style, DWORD ExStyle,
           int Width, int Height, HINSTANCE hInstance, WNDPROC WndProc = NULL,
           uint Icon = 0, int ScreenWidth = 0, int ScreenHeight = 0);
#endif
#if BUILD_SDL
  ...
  int SetSize(int Width, int Height, int ScreenWidth, int ScreenHeight,
              bool AdjustForStyle);  // Resize and center window
