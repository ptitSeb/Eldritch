...
class Clock {
 public:
  ...
  // Clocks are automatically initialized and shut down on
  // construction/destruction;
  // these just exist to allow reinitialization or deletion of memory when
  // needed.
  void Initialize();
  void ShutDown();
  ...
  MultiplierRequest* AddMultiplierRequest(
      float Duration,
      float Multiplier);  // Set Duration to 0.0f to make it infinite
 ...
 private:
  ...
  // NOTE: It might be a bit redundant, but I should probably precompute and
  // store all
  // the various things this class can return, for two reasons: 1) to prevent
  // recomputing
  // things I don't need to, and 2) so there's no way I could possibly get two
  // different
