...
/*static*/ void ConsoleInput::SetHandlerCallback(
    ConsoleInputHandlerCallback NewCallback) {
...
ConsoleInput::ConsoleInput()
    : m_hConsoleInput(NULL),
      ...
      m_CursorY(0),
      m_HasFocus(
          true)  // NOTE: Assume we start with focus. Might cause problems?
{
  m_hConsoleInput = GetStdHandle(STD_INPUT_HANDLE);
  ASSERT(m_hConsoleInput);

// TOTAL HACK!! Because the SDK I have doesn't define these, I need to do it
// myself
#define ENABLE_EXTENDED_FLAGS 0x0080
#define ENABLE_QUICK_EDIT_MODE 0x0040
#define ENABLE_INSERT_MODE 0x0020
  SetConsoleCtrlHandler(ConsoleHandler, TRUE);
  SetConsoleMode(m_hConsoleInput, ENABLE_EXTENDED_FLAGS | ENABLE_INSERT_MODE |
...
void ConsoleInput::Tick() {
  ...
  // Don't read until we have events, because ReadConsoleInput blocks until it
  // can return something
  if (NumEvents > 0) {
    ReadConsoleInput(m_hConsoleInput, InputBuffer, CONSOLE_INPUT_BUFFER_SIZE,
                     &NumEvents);
...
int ConsoleInput::GetCursorX() { return m_CursorX; }

int ConsoleInput::GetCursorY() { return m_CursorY; }

bool ConsoleInput::HasFocus() { return m_HasFocus; }
...
SimpleString ConsoleInput::GetUserInput() const {
  ...
  if (ReadConsole(m_hConsoleInput, Buffer, 256, &OutNum, NULL)) {
    // The last two characters should be a CRLF, unless we Ctrl+C or Ctrl+Break
    // out of the zinput.
