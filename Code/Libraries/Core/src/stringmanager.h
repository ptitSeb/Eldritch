...
class StringManager {
 public:
  ...
  static const char* ParseConfigString(EStringLife Life,
                                       const char* ConfigString);  // In this
                                                                   // case, a
                                                                   // "config
                                                                   // string"
                                                                   // has config
                                                                   // var tokens
                                                                   // embedded
  static bool ResolveAndEvaluateConditional(
      const char* ConditionalString);  // This isn't really the best place for
                                       // this, but it kinda doesn't fit
                                       // anywhere

  static void AddString(EStringLife Life, const char* String);
  static void FlushStrings(EStringLife Life);
  static void RemoveString(EStringLife Life,
                           const char* StringToRemove);  // Life needed as hint
                                                         // for where string is
                                                         // stored; if this is a
                                                         // problem, I could
                                                         // make a different
 ...
 private:
  ...
  static void ParseConfigToken(Array<char>& ParsedString,
