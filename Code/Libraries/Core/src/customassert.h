...
// CHECK evaluates the expression in every build but only checks it in specified
// builds
...
// Disable for perf. When this is set, assert expressions are not compiled out
// of Final builds
...
#define INNERASSERT(exp, desc, type)                               \
  do {                                                             \
    ...
    const char* Assertion =                                        \
        type " failed: \"" #exp "\" (" desc ")\nin " FUNCTION_NAME \
             " in " __FILE__ " at line " LINE_AS_STRING "\n";      \
    if (!Disabled && !(exp)) {                                     \
      PRINTF(Assertion);                                           \
      Disabled = true;                                             \
    }                                                              \
