...
struct SPackageFileEntry {
  ...
  uint32 m_Compressed;  // uint32 instead of bool to remind me that it would be
                        // padded out to this size anyway
};
...
class PackStream : public IDataStream {
 public:
  ...
  static void StaticAddPackageFile(const char* PackageFilename,
                                   const bool PreemptExistingPackages =
                                       false);  // If PreemptExistingPackages,
                                                // this package will be searched
                                                // first for a match
  static void StaticShutDown();
  static bool StaticFileExists(const char* Filename);

  virtual int Read(int NumBytes, void* Buffer) const;
  virtual int Write(int NumBytes, const void* Buffer)
      const;  // Not implemented for PackStream
  virtual int PrintF(const char* Str,
                     ...) const;  // Not implemented for PackStream
 ...
 private:
  ...
  SimpleString m_Filename;  // The full name of the file that was requested,
                            // regardless of packed mode
  SPackageFile*
      m_PackageFile;  // Which package file m_MemoryStream is reading from
  SPackageFileEntry*
      m_FileEntry;               // Which packed file m_MemoryStream represents
  ...
  bool m_PackedMode;  // Is this stream reading from the decompressed memory
                      // stream or a loose file on disk
