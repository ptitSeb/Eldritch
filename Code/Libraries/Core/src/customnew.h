...
// They are just here to fix warnings about there not being deletes to match
// news.
...
void* operator new(
    size_t Size) NEW_THROW;  // Allocates from the default allocator
void* operator new[](size_t Size)
    NEW_THROW;  // Allocates from the default allocator
void operator delete(void* pObj) DELETE_THROW;
void operator delete[](void* pObj)DELETE_THROW;

void* operator new(size_t Size,
                   uint Alignment);  // Allocates from the default allocator
void* operator new[](size_t Size,
                     uint Alignment);  // Allocates from the default allocator
void operator delete(void* pObj, uint Alignment);  // Never needs to be called,
                                                   // just in case the compiler
                                                   // wants it automatically
void operator delete[](void* pObj, uint Alignment);  // Never needs to be
                                                     // called, just in case the
                                                     // compiler wants it
                                                     // automatically
