...
class Interpolator {
 public:
  ...
  Interpolator(EInterpolationType InterpolationType = EIT_None,
               const C& StartValue = C(), const C& EndValue = C(),
               float Duration = 0.0f) {
  ...
  void Reset(EInterpolationType InterpolationType, const C& StartValue,
             const C& EndValue, float Duration) {
    m_InterpolationType = InterpolationType;
    m_CurrentValue =
        (Duration <= 0.0f)
            ? EndValue
            : StartValue;  // So the reset takes even before the first tick
  ...
  const C& GetValue() const { return m_CurrentValue; }

  bool IsFinished() const { return m_ElapsedTime >= m_Duration; }
