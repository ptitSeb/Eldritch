...
void AllocatorChunk::Initialize(Allocator* const pAllocator, uint Size) {
  ...
  m_Size = Size;
  void* const Where = malloc(Size);  // NOTE: malloc returns an 8-byte aligned
                                     // pointer in 32-bit Windows, 16-byte
                                     // aligned in 64-bit
  m_Head = new (Where) SAllocatorBlock;  // NOTE: Could just cast Where to an
                                         // SAllocatorBlock* and initialize, but
                                         // placement new doesn't have any
                                         // overhead and I like constructors
  m_Head->m_Chunk = this;
...
#if BUILD_DEV
#if DEBUG_FILL
  memset(reinterpret_cast<byte*>(m_Head) + Allocator::m_BlockSize, FILL_BLOCK,
         Allocator::m_BlockHeaderSize - Allocator::m_BlockSize);
  memset(reinterpret_cast<byte*>(m_Head) + Allocator::m_BlockHeaderSize,
         FILL_INIT, m_Size - Allocator::m_BlockHeaderSize);
#endif
  m_AllocSpace = 0;
...
void* AllocatorChunk::Allocate(uint Size, uint Alignment) {
  ...
  Alignment = (Alignment + Allocator::m_GranularityMask) &
  ...
  return (byte*)Fit + Allocator::m_BlockHeaderSize;  // Skip over the block
                                                     // header and return the
                                                     // address of the data
}
...
void AllocatorChunk::Free(void* pObj) {
  ...
  SAllocatorBlock* const Freed = reinterpret_cast<SAllocatorBlock*>(
      reinterpret_cast<byte*>(pObj) - Allocator::m_BlockHeaderSize);
  Freed->m_Used = false;
...
#if BUILD_DEV
#if DEBUG_FILL
  memset(reinterpret_cast<byte*>(Freed) + Allocator::m_BlockHeaderSize,
         FILL_FREE, Freed->m_Size);
#endif
...
#if DEBUG_FILL
  memset(reinterpret_cast<byte*>(m_Head) + Allocator::m_BlockHeaderSize,
         FILL_FLUSH, m_Size - Allocator::m_BlockHeaderSize);
#endif
  m_AllocSpace = 0;
  m_Overhead = Allocator::m_BlockHeaderSize;
  m_MaxOverhead = Allocator::m_BlockHeaderSize;
...
#if BUILD_DEV
#if DEBUG_FILL
  memset(reinterpret_cast<byte*>(NewBlock) + Allocator::m_BlockSize, FILL_BLOCK,
         Allocator::m_BlockHeaderSize - Allocator::m_BlockSize);
#endif
  m_Overhead += Allocator::m_BlockHeaderSize;
...
void AllocatorChunk::SplitToAlignment(SAllocatorBlock* const BlockToSplit,
                                      uint Alignment) {
  const uint AlignPad =
      Alignment -
      (reinterpret_cast<uintptr_t>(reinterpret_cast<byte*>(BlockToSplit) +
                                   (Allocator::m_BlockHeaderSize << 1)) %
       Alignment);
  ...
  NewBlock->m_Chunk = this;
  NewBlock->m_Size =
      BlockToSplit->m_Size - AlignPad - Allocator::m_BlockHeaderSize;
  NewBlock->m_Prev = BlockToSplit;
  NewBlock->m_Next = BlockToSplit->m_Next;
...
#if BUILD_DEV
#if DEBUG_FILL
  memset((byte*)NewBlock + Allocator::m_BlockSize, FILL_BLOCK,
         Allocator::m_BlockHeaderSize - Allocator::m_BlockSize);
#endif
  m_Overhead += Allocator::m_BlockHeaderSize;
  m_MaxOverhead = (m_Overhead > m_MaxOverhead) ? m_Overhead : m_MaxOverhead;
...
bool AllocatorChunk::CanSplitToAlignment(
    const SAllocatorBlock* const BlockToSplit, uint Alignment,
    uint Size) const {
  uint AlignPad =
      Alignment -
      (reinterpret_cast<uintptr_t>(reinterpret_cast<const byte*>(BlockToSplit) +
                                   (Allocator::m_BlockHeaderSize << 1)) %
       Alignment);
  return (BlockToSplit->m_Size >=
          AlignPad + Allocator::m_BlockHeaderSize + Size);
}
...
#if BUILD_DEV
#if DEBUG_FILL
  memset(const_cast<byte*>(reinterpret_cast<const byte*>(Next)) +
             Allocator::m_BlockSize,
         FILL_FREE, Allocator::m_BlockHeaderSize - Allocator::m_BlockSize);
#endif
  m_Overhead -= Allocator::m_BlockHeaderSize;
  m_FreeSpace += Allocator::m_BlockHeaderSize;
  --m_NumBlocks;
...
bool AllocatorChunk::IsAligned(const SAllocatorBlock* const pBlock,
                               uint Alignment) const {
  return (reinterpret_cast<uintptr_t>(reinterpret_cast<const byte*>(pBlock) +
                                      Allocator::m_BlockHeaderSize) %
          Alignment) == 0;
}
...
#if BUILD_DEV
  // TODO: Figure out how to quantify fragmentation (Number of free blocks? Size
  // of free blocks? Divided by what?)

  // Sanity checks, no need to report
  DEBUGASSERTDESC(m_FreeSpace == (m_Size - (m_AllocSpace + m_Overhead)),
                  "Current memory sanity check");
  DEBUGASSERTDESC(m_MinFree >= (m_Size - (m_MaxAlloc + m_MaxOverhead)),
                  "Maximum memory sanity check");

  const float PercentSize = 100.0f / static_cast<float>(m_Size);
  Stream.PrintF("\tCapacity:\t\t\t%d\n", m_Size);
  Stream.PrintF("\tCurrent allocated:\t%d (%f%%)\n", m_AllocSpace,
                static_cast<float>(m_AllocSpace) * PercentSize);
  Stream.PrintF("\tCurrent free:\t\t%d (%f%%)\n", m_FreeSpace,
                static_cast<float>(m_FreeSpace) * PercentSize);
  Stream.PrintF("\tCurrent overhead:\t%d (%f%%)\n", m_Overhead,
                static_cast<float>(m_Overhead) * PercentSize);
  Stream.PrintF("\tCurrent blocks:\t\t%d\n", m_NumBlocks);
  Stream.PrintF("\tMaximum allocated:\t%d (%f%%)\n", m_MaxAlloc,
                static_cast<float>(m_MaxAlloc) * PercentSize);
  Stream.PrintF("\tMinimum free:\t\t%d (%f%%)\n", m_MinFree,
                static_cast<float>(m_MinFree) * PercentSize);
  Stream.PrintF("\tMaximum overhead:\t%d (%f%%)\n", m_MaxOverhead,
                static_cast<float>(m_MaxOverhead) * PercentSize);
  Stream.PrintF("\tMaximum blocks:\t\t%d\n", m_MaxBlocks);

  Stream.PrintF("\n\tCurrent block map:\n");
  for (const SAllocatorBlock* pBlock = m_Head; pBlock;
       pBlock = pBlock->m_Next) {
    Stream.PrintF("\t%s 0x%08X %d\n", pBlock->m_Used ? "USED" : "FREE", pBlock,
                  pBlock->m_Size);
  }
  Stream.PrintF("\n");
#endif  // BUILD_DEV
...
void AllocatorChunk::ReportMemoryLeaks(const IDataStream& Stream,
  ...
  for (const SAllocatorBlock* pBlock = m_Head; pBlock;
       pBlock = pBlock->m_Next) {
    if (pBlock->m_Used && pBlock->m_AllocNum >= Bookmark1 &&
        pBlock->m_AllocNum < Bookmark2) {
      Stream.PrintF("\t0x%08X %d\n", pBlock, pBlock->m_Size);

      static const uint kMaxSizeToPrint = 16;
      const uint SizeToPrint =
          (pBlock->m_Size < kMaxSizeToPrint) ? pBlock->m_Size : kMaxSizeToPrint;

      Stream.PrintF("\t");
      for (uint i = 0; i < SizeToPrint; ++i) {
        Stream.PrintF("%02X", *(reinterpret_cast<const byte*>(pBlock) +
                                Allocator::m_BlockSize + i));
...
bool AllocatorChunk::CheckForLeaks(const uint Bookmark1,
                                   const uint Bookmark2) const {
  Unused(Bookmark1);
  Unused(Bookmark2);
...
void AllocatorChunk::GetStats(uint* const pAllocated, uint* const pOverhead,
