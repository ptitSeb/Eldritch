 ...
 public:
  ...
  class _Iterator {
   ...
   public:
    ...
    void operator=(const _Iterator<IC>& Other) { m_Node = Other.m_Node; }
  ...
  ~HashTable() { SafeDeleteArray(m_Table); }
 ...
 public:
  ...
  void Remove(const HashedString& Key) {
    ...
    while (IterNode) {
      ...
      // Check if the *next* item is equal to Item, so we can update the
      // pointers
  ...
  uint GetNumSlots() const { return m_NumSlots; }

  // Expose the array to allow iterating over every item in table
  Iterator Begin(uint Slot) const { return Iterator(m_Table[Slot]); }
