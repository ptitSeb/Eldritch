...
// This unit is a mess, a mix of standard and Windows-only functions, many with
// special-case
// purposes and some that are unsafe to use in-game and are only ever meant to
// run in tools.
...
SimpleString FileUtil::Normalize(const char* Path) {
  ...
  SimpleString RetVal(NormalizedPath);  // Causes an extra allocation, which
                                        // bugs me, but I don't want to make
                                        // SimpleString's char* accessible
...
bool FileUtil::Copy(const char* InPath, const char* OutPath,
                    bool FailIfExists) {
#if BUILD_WINDOWS
...
// Hacks used to clean up session folders; non-recursive (only returns folders
// in immediate path)
...
void FileUtil::GetAllFolders(const char* Path, Array<SimpleString>& OutFolders,
                             bool AppendPath) {
#if BUILD_WINDOWS
  HANDLE FileHandle;
  SimpleString FilePath;
...
bool FileUtil::SplitLeadingFolder(const char* Path, SimpleString& OutFolder,
                                  SimpleString& OutRemainder) {
  ASSERT(Path);
...
bool TryShellExecute(const char* Filename, const char* Operation) {
  ...
  HINSTANCE Result =
      ShellExecute(NULL,          // hWnd
                   ...
                   NULL,          // Directory
                   SW_SHOWNORMAL  // Show flags (this makes sense in the context
                                  // of opening a file in some editor)
                   );
...
#if BUILD_WINDOWS
// TODO PORT LATER (not currently used)
bool FileUtil::GetSaveFile(const HWND& hWnd, const SimpleString& Desc,
                           const SimpleString& Ext, SimpleString& OutFileName) {
  // HACK, because string functions don't like dealing with \0 as part of a
  // string.
  const SimpleString Filter =
      SimpleString::PrintF("%s (*.%s)0*.%s0All Files (*.*)0*.*0", Desc.CStr(),
                           Ext.CStr(), Ext.CStr());
  Filter.Replace('0', '\0');
...
#if BUILD_WINDOWS
// TODO PORT LATER (not currently used)
bool FileUtil::GetLoadFile(const HWND& hWnd, const SimpleString& Desc,
                           const SimpleString& Ext, SimpleString& OutFileName) {
  // HACK, because string functions don't like dealing with \0 as part of a
  // string.
  const SimpleString Filter =
