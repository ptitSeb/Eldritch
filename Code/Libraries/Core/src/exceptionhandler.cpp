...
#define UPLOADLOG \
  0  // Disabling for final Eldritch release because its usefulness has run its
     // course.

static bool gIsEnabled = false;

LONG WINAPI
CustomUnhandledExceptionFilter(EXCEPTION_POINTERS* pExceptionPointers) {
  ...
  const DWORD FirstExceptionCode =
      pExceptionRecord ? pExceptionRecord->ExceptionCode : 0;
