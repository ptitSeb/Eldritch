...
// TODO: Incorporate control modes (legacy/southpaw) here (could be done with
// axial bindings).
...
void InputSystem::Initialize(const SimpleString& DefinitionName) {
  ...
  const uint NumExposedInputs =
      ConfigManager::GetInt(sNumExposedInputs, 0, sDefinitionName);
  for (uint ExposedInputIndex = 0; ExposedInputIndex < NumExposedInputs;
       ++ExposedInputIndex) {
    const SimpleString ExposedInput = ConfigManager::GetSequenceString(
        "ExposedInput%d", ExposedInputIndex, "", sDefinitionName);
  ...
  for (uint InputIndex = 0; InputIndex < NumInputs; ++InputIndex) {
    const SimpleString Input = ConfigManager::GetSequenceString(
        "Input%d", InputIndex, "", sDefinitionName);
  ...
  const uint NumAnalogInputs =
      ConfigManager::GetInt(sNumAnalogInputs, 0, sDefinitionName);
  for (uint AnalogInputIndex = 0; AnalogInputIndex < NumAnalogInputs;
       ++AnalogInputIndex) {
    const SimpleString Input = ConfigManager::GetSequenceString(
        "AnalogInput%d", AnalogInputIndex, "", sDefinitionName);

    SAnalogInput NewInput;
    NewInput.m_String = Input;
    NewInput.m_Hash = Input;
    NewInput.m_Clamp = ConfigManager::GetSequenceBool(
        "AnalogInput%dClamp", AnalogInputIndex, false, sDefinitionName);
    NewInput.m_ClampMin = ConfigManager::GetSequenceFloat(
        "AnalogInput%dClampMin", AnalogInputIndex, 0.0f, sDefinitionName);
    NewInput.m_ClampMax = ConfigManager::GetSequenceFloat(
        "AnalogInput%dClampMax", AnalogInputIndex, 0.0f, sDefinitionName);
  ...
  const uint NumInputContexts =
      ConfigManager::GetInt(sNumInputContexts, 0, sDefinitionName);
  for (uint InputContextIndex = 0; InputContextIndex < NumInputContexts;
       ++InputContextIndex) {
    const SimpleString InputContextName = ConfigManager::GetSequenceString(
  ...
  m_KeyboardConfigContext =
      ConfigManager::GetString(sKeyboardBindings, "", sDefinitionName);

  STATICHASH(MouseBindings);
  m_MouseConfigContext =
      ConfigManager::GetString(sMouseBindings, "", sDefinitionName);

  STATICHASH(ControllerBindings);
  m_ControllerConfigContext =
      ConfigManager::GetString(sControllerBindings, "", sDefinitionName);

  STATICHASH(NumUnbindableInputs);
  STATICHASH(NumUnbindableAnalogInputs);

  MAKEHASH(m_KeyboardConfigContext);
  const uint NumUnbindableKeyboardInputs =
      ConfigManager::GetInt(sNumUnbindableInputs, 0, sm_KeyboardConfigContext);
  for (uint UnbindableInputIndex = 0;
       UnbindableInputIndex < NumUnbindableKeyboardInputs;
       ++UnbindableInputIndex) {
    m_UnbindableKeyboardInputs.Insert(ConfigManager::GetSequenceHash(
        "UnbindableInput%d", UnbindableInputIndex, HashedString::NullString,
        sm_KeyboardConfigContext));
  }

  MAKEHASH(m_MouseConfigContext);
  const uint NumUnbindableMouseInputs =
      ConfigManager::GetInt(sNumUnbindableInputs, 0, sm_MouseConfigContext);
  for (uint UnbindableInputIndex = 0;
       UnbindableInputIndex < NumUnbindableMouseInputs;
       ++UnbindableInputIndex) {
    m_UnbindableMouseInputs.Insert(ConfigManager::GetSequenceHash(
        "UnbindableInput%d", UnbindableInputIndex, HashedString::NullString,
        sm_MouseConfigContext));
  }
  const uint NumUnbindableMouseAnalogInputs = ConfigManager::GetInt(
      sNumUnbindableAnalogInputs, 0, sm_MouseConfigContext);
  for (uint UnbindableInputIndex = 0;
       UnbindableInputIndex < NumUnbindableMouseAnalogInputs;
       ++UnbindableInputIndex) {
    m_UnbindableMouseAnalogInputs.Insert(ConfigManager::GetSequenceHash(
        "UnbindableAnalogInput%d", UnbindableInputIndex,
        HashedString::NullString, sm_MouseConfigContext));
  }

  MAKEHASH(m_ControllerConfigContext);
  const uint NumUnbindableControllerInputs = ConfigManager::GetInt(
      sNumUnbindableInputs, 0, sm_ControllerConfigContext);
  for (uint UnbindableInputIndex = 0;
       UnbindableInputIndex < NumUnbindableControllerInputs;
       ++UnbindableInputIndex) {
    m_UnbindableControllerInputs.Insert(ConfigManager::GetSequenceHash(
        "UnbindableInput%d", UnbindableInputIndex, HashedString::NullString,
        sm_ControllerConfigContext));
  }
  const uint NumUnbindableControllerAnalogInputs = ConfigManager::GetInt(
      sNumUnbindableAnalogInputs, 0, sm_ControllerConfigContext);
  for (uint UnbindableInputIndex = 0;
       UnbindableInputIndex < NumUnbindableControllerAnalogInputs;
       ++UnbindableInputIndex) {
    m_UnbindableControllerAnalogInputs.Insert(ConfigManager::GetSequenceHash(
        "UnbindableAnalogInput%d", UnbindableInputIndex,
        HashedString::NullString, sm_ControllerConfigContext));
  }

  STATICHASH(NumGearConfigs);
  const uint NumGearConfigs =
      ConfigManager::GetInt(sNumGearConfigs, 0, sm_ControllerConfigContext);
  for (uint GearConfigIndex = 0; GearConfigIndex < NumGearConfigs;
       ++GearConfigIndex) {
    const HashedString GearConfigInput = ConfigManager::GetSequenceHash(
        "GearConfig%dInput", GearConfigIndex, HashedString::NullString,
        sm_ControllerConfigContext);

    DEVASSERT(m_ControllerGears.Search(GearConfigInput).IsValid());
    SControllerGear& Gear = m_ControllerGears[GearConfigInput];

    Gear.m_HighGearScalar = ConfigManager::GetSequenceFloat(
        "GearConfig%dHighScalar", GearConfigIndex, 1.0f,
        sm_ControllerConfigContext);
    Gear.m_HighGearThreshold = ConfigManager::GetSequenceFloat(
        "GearConfig%dHighThreshold", GearConfigIndex, 0.0f,
        sm_ControllerConfigContext);
    Gear.m_HighGearShiftTime = ConfigManager::GetSequenceFloat(
        "GearConfig%dHighShiftTime", GearConfigIndex, 0.0f,
        sm_ControllerConfigContext);
    Gear.m_LowGearScalar = ConfigManager::GetSequenceFloat(
        "GearConfig%dLowScalar", GearConfigIndex, 1.0f,
        sm_ControllerConfigContext);
    Gear.m_LowGearThreshold = ConfigManager::GetSequenceFloat(
        "GearConfig%dLowThreshold", GearConfigIndex, 0.0f,
        sm_ControllerConfigContext);
    Gear.m_LowGearShiftTime = ConfigManager::GetSequenceFloat(
        "GearConfig%dLowShiftTime", GearConfigIndex, 0.0f,
        sm_ControllerConfigContext);
  }
...
void InputSystem::UpdateBindingsFromConfig() {
  ...
  for (uint InputIndex = 0; InputIndex < NumInputs; ++InputIndex) {
    ...
    m_KeyboardBindings[Input.m_Hash] = GetKeyboardSignal(ConfigManager::GetHash(
        sInputName, HashedString::NullString, sm_KeyboardConfigContext));
    m_MouseBindings[Input.m_Hash] = GetMouseSignal(ConfigManager::GetHash(
        sInputName, HashedString::NullString, sm_MouseConfigContext));
    m_ControllerBindings[Input.m_Hash] =
        GetControllerSignal(ConfigManager::GetHash(
            sInputName, HashedString::NullString, sm_ControllerConfigContext));
  }
  ...
  for (uint AnalogInputIndex = 0; AnalogInputIndex < NumAnalogInputs;
    ...
    m_MouseAnalogBindings[Input.m_Hash] =
        GetMouseAnalogSignal(ConfigManager::GetHash(
            sInputName, HashedString::NullString, sm_MouseConfigContext));
    m_ControllerAnalogBindings[Input.m_Hash] =
        GetControllerAnalogSignal(ConfigManager::GetHash(
            sInputName, HashedString::NullString, sm_ControllerConfigContext));
  }
}
...
void InputSystem::Tick() {
  ...
  m_CurrentAnalogState = (m_CurrentAnalogState == &m_AnalogStateA)
                             ? &m_AnalogStateB
                             : &m_AnalogStateA;
  m_LastAnalogState = (m_CurrentAnalogState == &m_AnalogStateA)
                          ? &m_AnalogStateB
                          : &m_AnalogStateA;

  // Assume that devices have already been ticked. InputSystem doesn't own them,
  // it just utilizes them.
  ...
  // Clear all states first, in case their inputs are suppressed or remapped or
  // whatever.
  for (uint InputIndex = 0; InputIndex < NumInputs; ++InputIndex) {
    const SInput& Input = m_Inputs[InputIndex];
  ...
  for (uint InputIndex = 0; InputIndex < NumInputs; ++InputIndex) {
    ...
    CurrentState[ContextualInput] =
        (m_Keyboard && KeyboardSignal > 0 &&
         m_Keyboard->IsHigh(KeyboardSignal)) ||
        (m_Mouse && MouseSignal > 0 && m_Mouse->IsHigh(MouseSignal)) ||
        (m_Controller && ControllerSignal > 0 &&
         m_Controller->IsHigh(ControllerSignal));
  ...
  for (uint AnalogInputIndex = 0; AnalogInputIndex < NumAnalogInputs;
    ...
    if (m_Mouse && MouseAnalogSignal > 0) {
      ...
      State += m_Mouse->GetPosition(MouseAnalogSignal) *
               Adjustment.m_ScaleFactor *
               (Adjustment.m_InvertFactor ? -1.0f : 1.0f);
    }

    const uint ControllerAnalogSignal =
        GetBoundControllerAnalogSignal(OriginalInput);
    if (m_Controller && ControllerAnalogSignal > 0) {
      const float InputPosition =
          m_Controller->GetPosition(ControllerAnalogSignal);

      DEBUGASSERT(m_ControllerAdjustments.Search(OriginalInput).IsValid());
      const SAnalogAdjustment& Adjustment =
          m_ControllerAdjustments[OriginalInput];
      const float InvertFactor = Adjustment.m_InvertFactor ? -1.0f : 1.0f;
      const float ModifiedInputPosition =
          ModifyControllerInputPosition(InputPosition, Adjustment);

      SControllerGear& Gear = m_ControllerGears[OriginalInput];
      TickControllerGear(Gear, InputPosition);
      const float GearScalar =
          Gear.m_HighGear ? Gear.m_HighGearScalar : Gear.m_LowGearScalar;

      State += ModifiedInputPosition * Adjustment.m_ScaleFactor * GearScalar *
               InvertFactor;
    }
...
float InputSystem::ModifyControllerInputPosition(
    const float InputPosition, const SAnalogAdjustment& Adjustment) {
...
void InputSystem::TickControllerGear(SControllerGear& Gear,
                                     const float InputPosition) {
  if (Gear.m_HighGear) {
...
void InputSystem::TickBinding() {
  if ((m_Keyboard && m_Keyboard->OnRise(Keyboard::EB_Escape)) ||
      (m_Controller && m_Controller->OnRise(XInputController::EB_Start)) ||
      (m_Controller && !m_CanBindController &&
       m_Controller->OnRise(XInputController::EB_B))) {
    // Cancel
    m_Binding = false;
  } else if ((m_Keyboard && m_Keyboard->OnRise(Keyboard::EB_Backspace)) ||
             (m_Controller &&
              m_Controller->OnRise(XInputController::EB_Back))) {
    // Unbind
    MAKEHASH(m_KeyboardConfigContext);
    MAKEHASH(m_MouseConfigContext);
    MAKEHASH(m_ControllerConfigContext);
    ...
    if (m_BindingInputIsAnalog) {
      ConfigManager::SetString(sm_BindingInput,
                               GetMouseAnalogSignalName(Mouse::EA_None).CStr(),
                               sm_MouseConfigContext);
      ConfigManager::SetString(
          sm_BindingInput,
          GetControllerAnalogSignalName(XInputController::EA_None).CStr(),
          sm_ControllerConfigContext);
    } else {
      ConfigManager::SetString(sm_BindingInput, GetKeyboardSignalName(0).CStr(),
                               sm_KeyboardConfigContext);
      ConfigManager::SetString(sm_BindingInput,
                               GetMouseSignalName(Mouse::EB_None).CStr(),
                               sm_MouseConfigContext);
      ConfigManager::SetString(
          sm_BindingInput,
          GetControllerSignalName(XInputController::EB_None).CStr(),
          sm_ControllerConfigContext);
    }
...
HashedString InputSystem::GetContextualInput(const HashedString& Input) const {
  for (uint InputContextIndex = m_ActiveInputContexts.Size();
       InputContextIndex > 0; --InputContextIndex) {
    InputContext* const pInputContext =
        m_ActiveInputContexts[InputContextIndex - 1];
...
void InputSystem::PushContext(const HashedString& ContextName) {
  ...
  // Whenever we change contexts, we need to retick to push the changes through
  // immediately.
  Tick();
...
void InputSystem::PopContext(const HashedString& ContextName) {
  ...
  FOR_EACH_ARRAY_REVERSE(ContextIter, m_ActiveInputContexts, InputContext*) {
    ...
    if (pActiveContext == pPopContext) {
      ...
      // Whenever we change contexts, we need to retick to push the changes
      // through immediately.
      Tick();
      NotifyObserversOnInputContextsChanged();
...
// m_Held flag is only high for a single tick, so we don't need to compare with
// last state.
...
void InputSystem::SetControllerScale(const HashedString& Axis,
                                     const float Scale) {
  DEBUGASSERT(m_ControllerAdjustments.Search(Axis).IsValid());
  m_ControllerAdjustments[Axis].m_ScaleFactor = Scale;
}

void InputSystem::SetControllerPower(const HashedString& Axis,
                                     const float Power) {
  DEBUGASSERT(m_ControllerAdjustments.Search(Axis).IsValid());
...
void InputSystem::SetControllerInvert(const HashedString& Axis,
                                      const bool Invert) {
  DEBUGASSERT(m_ControllerAdjustments.Search(Axis).IsValid());
...
void InputSystem::InitializeSignalMaps() {
  ...
  m_KeyboardMap["{"] = Keyboard::EB_LeftBrace;   // Use [] instead of {} so it
                                                 // can be written in config
                                                 // files
  m_KeyboardMap["}"] = Keyboard::EB_RightBrace;  // (because [] are used to
                                                 // denote context blocks)
...
/*inline*/ uint InputSystem::GetBoundKeyboardSignal(
    const HashedString& Input) const {
  DEVASSERT(m_KeyboardBindings.Search(Input).IsValid());
  return m_KeyboardBindings[Input];
}

// NOTE: This used to be inline, but not every compiler would link it properly.
/*inline*/ uint InputSystem::GetBoundMouseSignal(
    const HashedString& Input) const {
...
SimpleString InputSystem::GetAnalogInputBoundToController(
    const uint Signal) const {
  const uint NumAnalogInputs = m_AnalogInputs.Size();
  for (uint InputIndex = 0; InputIndex < NumAnalogInputs; ++InputIndex) {
    const SAnalogInput& Input = m_AnalogInputs[InputIndex];
...
inline const SimpleString& InputSystem::GetKeyboardSignalName(
    const uint Signal) const {
  DEVASSERT(m_ReverseKeyboardMap.Search(Signal).IsValid());
  return m_ReverseKeyboardMap[Signal];
}

inline const SimpleString& InputSystem::GetMouseSignalName(
    const uint Signal) const {
  DEVASSERT(m_ReverseMouseMap.Search(Signal).IsValid());
  return m_ReverseMouseMap[Signal];
}

inline const SimpleString& InputSystem::GetControllerSignalName(
    const uint Signal) const {
...
// Might not be needed if there's just a few things that calling code can
// manage.
...
void InputSystem::BindInput(const SimpleString& InputName) {
  ...
  for (uint AnalogInputIndex = 0; AnalogInputIndex < m_AnalogInputs.Size();
       ++AnalogInputIndex) {
  ...
  m_CanBindController =
      !m_BindingInputIsAnalog && CanBindToController(InputHash);
  m_CanBindMouseAnalog =
      m_BindingInputIsAnalog && CanBindToMouseAnalog(InputHash);
  m_CanBindControllerAnalog =
      m_BindingInputIsAnalog && CanBindToControllerAnalog(InputHash);
}
...
bool InputSystem::TryKeyboardBind(const uint Signal) {
  ...
  if (m_Binding && m_Keyboard->OnRise(Signal)) {
    ...
    if (CurrentInputForKey != "") {
      const SimpleString CurrentKeyForBindingInput = ConfigManager::GetString(
          sm_BindingInput, "", sm_KeyboardConfigContext);
      ConfigManager::SetString(CurrentInputForKey,
                               CurrentKeyForBindingInput.CStr(),
                               sm_KeyboardConfigContext);
    }

    ConfigManager::SetString(sm_BindingInput,
                             GetKeyboardSignalName(Signal).CStr(),
                             sm_KeyboardConfigContext);
    m_Binding = false;
...
bool InputSystem::TryMouseBind(const uint KeyboardSignal,
                               const uint MouseSignal) {
  MAKEHASH(m_MouseConfigContext);
  MAKEHASH(m_BindingInput);

  if (m_Binding && ((KeyboardSignal > Keyboard::EB_None &&
                     m_Keyboard->OnRise(KeyboardSignal)) ||
                    m_Mouse->OnRise(MouseSignal))) {
    // Swap inputs if binding a key that's already in use
    const SimpleString CurrentInputForKey = GetInputBoundToMouse(MouseSignal);
    if (CurrentInputForKey != "") {
      const SimpleString CurrentKeyForBindingInput =
          ConfigManager::GetString(sm_BindingInput, "", sm_MouseConfigContext);
      ConfigManager::SetString(CurrentInputForKey,
                               CurrentKeyForBindingInput.CStr(),
                               sm_MouseConfigContext);
    }
    ...
    ConfigManager::SetString(sm_BindingInput,
                             GetMouseSignalName(MouseSignal).CStr(),
                             sm_MouseConfigContext);
    m_Binding = false;
...
bool InputSystem::TryControllerBind(const uint Signal) {
  ...
  if (m_Binding && m_Controller->OnRise(Signal)) {
    ...
    if (CurrentInputForKey != "") {
      const SimpleString CurrentKeyForBindingInput = ConfigManager::GetString(
          sm_BindingInput, "", sm_ControllerConfigContext);
      ConfigManager::SetString(CurrentInputForKey,
                               CurrentKeyForBindingInput.CStr(),
                               sm_ControllerConfigContext);
    }
    ...
    ConfigManager::SetString(sm_BindingInput,
                             GetControllerSignalName(Signal).CStr(),
                             sm_ControllerConfigContext);
    m_Binding = false;
...
bool InputSystem::TryMouseBindAnalog(const uint Signal) {
  ...
  if (m_Binding &&
      Abs(m_Mouse->GetPosition(Signal)) > skMouseBindAnalogThreshold) {
    // Swap inputs if binding a key that's already in use
    ...
    ConfigManager::SetString(sm_BindingInput,
                             GetMouseAnalogSignalName(Signal).CStr(),
                             sm_MouseConfigContext);
    m_Binding = false;
...
bool InputSystem::TryControllerBindAnalog(const uint Signal) {
  ...
  if (m_Binding &&
      Abs(m_Controller->GetPosition(Signal)) >
          skControllerBindAnalogThreshold) {
    // Swap inputs if binding a key that's already in use
    const SimpleString CurrentInputForKey =
        GetAnalogInputBoundToController(Signal);
    if (CurrentInputForKey != "") {
      const SimpleString CurrentKeyForBindingInput = ConfigManager::GetString(
          sm_BindingInput, "", sm_ControllerConfigContext);
      ConfigManager::SetString(CurrentInputForKey,
    ...
    ConfigManager::SetString(sm_BindingInput,
                             GetControllerAnalogSignalName(Signal).CStr(),
                             sm_ControllerConfigContext);
    m_Binding = false;
...
void InputSystem::AddInputSystemObserver(
    IInputSystemObserver* const pObserver) {
  DEVASSERT(pObserver);
  m_InputSystemObservers.PushBack(pObserver);
}

void InputSystem::RemoveInputSystemObserver(
    IInputSystemObserver* const pObserver) {
  DEVASSERT(pObserver);
