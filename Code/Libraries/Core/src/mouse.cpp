...
Mouse::Mouse(HINSTANCE hInstance, HWND hWnd)
  ...
  m_pMouse->SetCooperativeLevel(
      hWnd, DISCL_EXCLUSIVE | DISCL_FOREGROUND);  // Could do non-exclusive if I
                                                  // don't need exclusive?
...
void Mouse::SetRelativeMode(const bool Relative) {
  if (m_RelativeModeSupported) {
    ...
    SDL_SetWindowGrab(m_Window->GetSDLWindow(),
                      Relative ? SDL_TRUE : SDL_FALSE);
  }
}
#endif
...
void Mouse::Tick(float DeltaTime) {
  ...
  // because wheel isn't accessible through virtual keys like mouse buttons
  // are).
  m_CurrentState.m_Buttons[EB_WheelUp] |= m_BufferedState.m_Buttons[EB_WheelUp];
  m_CurrentState.m_Buttons[EB_WheelDown] |=
      m_BufferedState.m_Buttons[EB_WheelDown];
...
bool Mouse::IsActive() const { return m_Active; }
