...
bool CustomAssert::ShouldAssert(const char* Assertion, bool& Disable) {
#if BUILD_WINDOWS
  char AssertBuffer
      [ASSERTBUFFERSIZE];  // So we don't do any heap allocations in here
  ...
  strcpy_s(AssertBuffer + AssertionLength, ASSERTBUFFERSIZE - AssertionLength,
           "\nAbort to debug\nRetry to continue\nIgnore to continue and "
           "disable this assert");

  const int Response =
      MessageBox(NULL, AssertBuffer, "Assertion failed",
                 MB_ABORTRETRYIGNORE | MB_TASKMODAL | MB_SETFOREGROUND);
