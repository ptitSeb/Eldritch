...
#define DEFINE_ANIMEVENT_FACTORY(type) \
  static class AnimEvent* Factory() { return new AnimEvent##type; }

#define DEFINE_ANIMEVENT_STATICNAME(type)   \
  static const char* GetStaticEventName() { \
    static const char* sEventName = #type;  \
    return sEventName;                      \
  }
#define DEFINE_ANIMEVENT_NAME \
  virtual const char* GetEventName() { return GetStaticEventName(); }
