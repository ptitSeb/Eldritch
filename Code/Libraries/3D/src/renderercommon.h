...
class RendererCommon : public IRenderer {
 public:
  ...
  virtual Mesh* Print(const SimpleString& UTF8String, const Font* const pFont,
                      const SRect& Bounds, unsigned int Flags);
 ...
 protected:
  virtual void RenderBucket(Bucket* pBucket,
                            const ViewportPass* const pViewportPass) = 0;
  void RenderBuckets();
  void PostRenderBuckets();

  virtual void SetViewport(const ViewportPass* const pViewportPass) = 0;

  void ApplyMaterial(const Material& Material, Mesh* const pMesh,
                     const View& View);
  void ApplyRenderState(const SRenderState& RenderState);
  void ApplySamplerState(const uint SamplerStage,
  ...
  List<IRenderTarget*> m_RenderTargets;        // TODO: Replace this with
                                               // functionality in TargetManager
  Set<IVertexBuffer*> m_DynamicVertexBuffers;  // Things wot get released and
                                               // rebuilt when device is reset
