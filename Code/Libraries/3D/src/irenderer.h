...
class IRenderer {
 public:
  ...
  virtual void Tick() = 0;
  virtual void Clear(unsigned int Flags = CLEAR_COLOR | CLEAR_DEPTH |
                                          CLEAR_STENCIL,
                     unsigned int Color = 0xff000000, float Depth = 1.0f,
  ...
  virtual void SetAlphaBlendEnable(
      const EAlphaBlendEnable AlphaBlendEnable) = 0;
  virtual void SetBlend(const EBlend SrcBlend, const EBlend DestBlend) = 0;

  virtual void SetTexture(const uint SamplerStage,
                          ITexture* const pTexture) = 0;
  virtual void SetAddressing(const uint SamplerStage,
                             const ETextureAddress AddressU,
                             const ETextureAddress AddressV) = 0;
  virtual void SetMinMipFilters(const uint SamplerStage,
                                const ETextureFilter MinFilter,
                                const ETextureFilter MipFilter) = 0;
  virtual void SetMagFilter(const uint SamplerStage,
                            const ETextureFilter MagFilter) = 0;

  virtual void SetVertexShaderFloat4(const HashedString& Parameter,
                                     const float* const pFloats,
                                     const uint NumFloat4s) = 0;
  virtual void SetVertexShaderMatrix(const HashedString& Parameter,
                                     const float* const pFloats,
                                     const uint NumMatrices) = 0;
  virtual void SetPixelShaderFloat4(const HashedString& Parameter,
                                    const float* const pFloats,
                                    const uint NumFloat4s) = 0;
  virtual void SetPixelShaderMatrix(const HashedString& Parameter,
                                    const float* const pFloats,
                                    const uint NumMatrices) = 0;
  ...
  struct SRestoreDeviceCallback {
    SRestoreDeviceCallback() : m_Callback(NULL), m_Void(NULL) {}

    RestoreDeviceCallback m_Callback;
    void* m_Void;
  };
  virtual void SetRestoreDeviceCallback(
      const SRestoreDeviceCallback& Callback) = 0;

  virtual void SetFullScreen(bool FullScreen) = 0;

  virtual void SetMultiSampleType(EMultiSampleType MultiSampleType) = 0;
  virtual void GetBestSupportedMultiSampleType(
      EMultiSampleType& OutMultiSampleType,
      uint32* pOutQualityLevels = NULL) = 0;

  virtual ERenderTargetFormat GetBestSupportedRenderTargetFormat(
      const ERenderTargetFormat Format) const = 0;
  ...
  virtual void AddBucket(const HashedString& Name,
                         Bucket* pBucket) = 0;  // Add (ordered) buckets for
                                                // doing different render
                                                // passes. Renderer owns memory.
  virtual Bucket* GetBucket(const HashedString& Name) = 0;
  virtual Bucket* GetBucket(uint Index) = 0;  // This is pretty ugly, may want
                                              // to refactor later (I'm just
                                              // using it for debug stuff at the
                                              // moment)
  ...
  virtual IShaderProgram* CreateShaderProgram(
      ...
      IVertexDeclaration* const pVertexDeclaration) = 0;
  virtual IRenderTarget* CreateRenderTarget(
      const SRenderTargetParams& Params) = 0;
...
#if BUILD_DEV
  virtual void DEBUGDrawLine(const Vector& Start, const Vector& End,
                             unsigned int Color) = 0;
  virtual void DEBUGDrawTriangle(const Vector& V1, const Vector& V2,
                                 const Vector& V3, unsigned int Color) = 0;
  virtual void DEBUGDrawBox(const Vector& Min, const Vector& Max,
                            unsigned int Color) = 0;
  virtual void DEBUGDrawFrustum(const View& rView, unsigned int Color) = 0;
  virtual void DEBUGDrawSphere(const Vector& Center, float Radius,
                               unsigned int Color) = 0;
  virtual void DEBUGDrawEllipsoid(const Vector& Center, const Vector& Extents,
                                  unsigned int Color) = 0;
  virtual void DEBUGDrawCross(const Vector& Center, const float Length,
                              unsigned int Color) = 0;
  virtual void DEBUGDrawArrow(const Vector& Root, const Angles& Direction,
                              const float Length, unsigned int Color) = 0;

  // 2D versions draw to the HUD instead of the world.
  virtual void DEBUGDrawLine2D(const Vector& Start, const Vector& End,
                               unsigned int Color) = 0;
  virtual void DEBUGDrawBox2D(const Vector& Min, const Vector& Max,
                              unsigned int Color) = 0;

  virtual void DEBUGPrint(const SimpleString& UTF8String,
                          const Font* const pFont, const SRect& Bounds,
                          const Vector4& Color) = 0;
#endif  // BUILD_DEV

  // Flags are defined in font.h
  virtual Mesh* Print(const SimpleString& UTF8String, const Font* const pFont,
                      const SRect& Bounds, unsigned int Flags) = 0;
