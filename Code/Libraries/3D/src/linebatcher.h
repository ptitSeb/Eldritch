...
class LineBatcher {
 public:
  ...
  void Add(const Array<Vector>& Positions, const Array<uint>& Colors,
           const Array<index_t>& Indices);
  Mesh* Render();
  ...
  void DrawX(const Vector& Center, const float Radius,
  ...
  void DrawEllipsoid(const Vector& Center, const Vector& Extents,
                     unsigned int Color);
  void DrawGradientLine(const Vector& Start, const Vector& End,
                        unsigned int ColorA, unsigned int ColorB);
  void DrawIrradianceCross(const Vector& Center, const float Radius,
                           const Vector4 Colors[6]);
