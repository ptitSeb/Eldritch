...
const SFontCharProp& Font::GetCharProp(const SFontLocale& Locale,
                                       const unicode_t CodePoint) const {
  const Map<unicode_t, SFontCharProp>::Iterator PropIter =
      Locale.m_CharProps.Search(CodePoint);

  if (PropIter.IsNull()) {
#if BUILD_DEV
    Map<HashedString, SFontLocale>::Iterator LocaleIter =
        m_FontLocales.SearchValue(Locale);
    ...
    const SimpleString CodePointString =
        SimpleString::GetCodePointString(CodePoint);
    PRINTF("Code point %s not found in locale %s.\n", CodePointString.CStr(),
           LocaleString.CStr());
#endif
...
const SFontLocale& Font::GetCurrentLocale() const {
  ...
  Map<HashedString, HashedString>::Iterator LanguageIter =
      m_FontLanguages.Search(Language);
  ASSERT(LanguageIter.IsValid());
  const HashedString Locale = LanguageIter.GetValue();

  Map<HashedString, SFontLocale>::Iterator LocaleIter =
      m_FontLocales.Search(Locale);
  ASSERT(LocaleIter.IsValid());
  return LocaleIter.GetValue();
}

Font::EWordResult Font::GetNextWord(const Array<unicode_t>& CodePoints,
                                    const SFontLocale& Locale, uint& Index,
                                    Array<unicode_t>& WordBuffer,
                                    float& WordWidth) const {
  WordBuffer.Clear();
  WordWidth = 0.0f;
    ...
    if (CodePoint == '\n') {
      if (WordBuffer.Empty()) {
        ...
        return EWR_Word;
               ...
               CodePoint == '\t')  // Just treat Tab as a whitespace character
    {
      if (WordBuffer.Empty()) {
...
// TODO LOC LATER: Add support for center/right alignment. This could be done by
// adjusting
// typeset positions after each line is closed (or as a post process after all
// typesetting).
// I wouldn't be too worried about performance on that, since I almost never use
// it. (Using
// a center or right origin on the UI widget is sufficient for single-line
// elements.)
void Font::Arrange(const Array<unicode_t>& CodePoints, const SRect& Bounds,
                   Array<STypesetGlyph>& OutTypesetting,
                   Vector2& OutDimensions) const {
  ...
  while (EWR_None != (WordResult = GetNextWord(CodePoints, Locale, Index,
                                               WordBuffer, WordWidth))) {
    if (WordResult == EWR_Linebreak) {
      ...
      if (Bounded &&          // Are we doing a bounded print?
          ...
          Cursor.x + WordWidth >
              BoundWidth)  // Does the word not fit within bounds?
      {
        Cursor.x = 0.0f;
        Cursor.y += m_Height;
      ...
      FOR_EACH_ARRAY(WordIter, WordBuffer, unicode_t) {
        ...
        Cursor.x += Min(
            0, CharProp.m_A);  // Retreat cursor if there is leading overhang
        ...
        TypesetGlyph.m_Position.x =
            Bounds.m_Left + Cursor.x -
            1.0f;  // Subtract 1/2 for the half pixel padding in the font
        TypesetGlyph.m_Position.y =
            Bounds.m_Top + Cursor.y - 1.0f;  // and 1/2 to align on pixel grid

        Cursor.x +=
            Max(0, CharProp.m_A);  // Advance cursor if there is leading padding
...
Mesh* Font::Print(const SimpleString& StringUTF8, const SRect& Bounds,
                  uint Flags) const {
...
Mesh* Font::Print(const Array<unicode_t>& CodePoints, const SRect& Bounds,
                  uint Flags) const {
  ...
  FOR_EACH_ARRAY(GlyphIter, TypesetGlyphs, STypesetGlyph) {
    ...
    Positions.PushBack(
        Vector(Glyph.m_Position.x, 0.0f, Glyph.m_Position.y + m_Height + 1.0f));
    Positions.PushBack(Vector(Glyph.m_Position.x + CharProp.m_Width, 0.0f,
                              Glyph.m_Position.y + m_Height + 1.0f));
