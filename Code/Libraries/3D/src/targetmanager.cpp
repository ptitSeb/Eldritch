...
TargetManager::~TargetManager() { ReleaseTargets(); }
...
void TargetManager::CreateTargets(uint Width, uint Height) {
  ...
  m_DepthRenderTarget = m_Renderer->CreateRenderTarget(
      SRenderTargetParams(Width, Height, ERTF_R32F, ERTF_UseDefault));
  m_DepthAlphaRenderTarget = m_Renderer->CreateRenderTarget(
      SRenderTargetParams(Width, Height, ERTF_A16B16G16R16F, ERTF_UseDefault));
#if USE_HDR
  STATICHASH(UseHDR);
  STATICHASH(UseBloom);
  ...
  if (ConfigManager::GetBool(sUseHDR)) {
    m_PrimaryRenderTarget = m_Renderer->CreateRenderTarget(SRenderTargetParams(
        Width, Height, ERTF_A16B16G16R16F, ERTF_UseDefault, true));
    if (ConfigManager::GetBool(sUseBloom)) {
      ...
      m_BloomSmallTarget1 = m_Renderer->CreateRenderTarget(SRenderTargetParams(
          m_BloomWidth, m_BloomHeight, ERTF_A16B16G16R16F, ERTF_None));
      m_BloomSmallTarget2 = m_Renderer->CreateRenderTarget(SRenderTargetParams(
          m_BloomWidth, m_BloomHeight, ERTF_A16B16G16R16F, ERTF_None));
    }
  } else
#endif
  {
    m_PrimaryRenderTarget = m_Renderer->CreateRenderTarget(
        SRenderTargetParams(Width, Height, ERTF_A8R8G8B8, ERTF_UseDefault));
  }
