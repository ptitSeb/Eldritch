...
bool KDTree::Trace(const Segment& s,
  ...
  for (uint i = 0; i < m_NumMeshes; ++i) {
    ...
    if (pMesh->m_CollisionMesh.m_MutableCollidable &&
        pMesh->m_CollisionMesh.m_NumTris) {
  ...
  if (m_FrontChild) {
    ...
    KDTree* pFirstChild =
        s.m_Point1.IsOnFrontSide(m_SplittingPlane) ? m_FrontChild : m_BackChild;
    KDTree* pSecondChild =
        pFirstChild == m_FrontChild ? m_BackChild : m_FrontChild;
...
bool KDTree::Trace(const Ray& r, CollisionInfo* const pInfo /*= NULL*/) const {
  ...
  if (m_FrontChild) {
    ...
    KDTree* pFirstChild =
        r.m_Point.IsOnFrontSide(m_SplittingPlane) ? m_FrontChild : m_BackChild;
    KDTree* pSecondChild =
        pFirstChild == m_FrontChild ? m_BackChild : m_FrontChild;
...
bool KDTree::SweepAgainst(const Ellipsoid& e, const Vector& v,
  ...
  if (m_FrontChild) {
    ...
    KDTree* pFirstChild =
        e.m_Center.IsOnFrontSide(m_SplittingPlane) ? m_FrontChild : m_BackChild;
    KDTree* pSecondChild =
        pFirstChild == m_FrontChild ? m_BackChild : m_FrontChild;
