...
class BoneArray {
 public:
  ...
  void Init(HashedString* BoneNames, SBone* Bones, const char* MeshFilename,
            Animation* Animations, int NumFrames, int NumBones, int NumAnims);
  SBone* GetBone(int Index, int Frame = FRAME_TPOSE) const;
  int GetBoneIndex(const HashedString& Name)
      const;  // Returns the bone index, or INVALID_INDEX if not found

  inline void GetInterpolatedBone(int Index, int Frame, int NextFrame,
                                  float Delta,
                                  SBone& OutInterpolatedBone) const {
  ...
  static inline void GetInterpolatedBone(const SBone& Bone1, const SBone& Bone2,
                                         float Delta,
                                         SBone& OutInterpolatedBone) {
  ...
  int GetAnimationIndex(
      const Animation* pAnimation) const;  // Returns INVALID_INDEX if not found
