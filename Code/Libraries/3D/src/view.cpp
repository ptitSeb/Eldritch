...
View::View(const Vector& Location, const Vector& Direction, float VerticalFOV,
...
View::View(const Vector& Location, const Angles& Rotation, float VerticalFOV,
           float AspectRatio, float NearClip, float FarClip)
    : m_Location(Location),
      m_Rotation(Rotation),
      m_NearClip(NearClip),
...
View::View(const Vector& Location, const Vector& Direction, const SRect& Bounds,
...
View::View(const Vector& Location, const Angles& Rotation, const SRect& Bounds,
           float NearClip, float FarClip)
    : m_Location(Location),
      m_Rotation(Rotation),
      m_NearClip(NearClip),
...
Matrix View::GetProjectionMatrix() const {
  if (m_OrthoMode) {
    return Matrix::CreateOrthoProjectionMatrix(
        m_Bounds.m_Left, m_Bounds.m_Top, m_Bounds.m_Right, m_Bounds.m_Bottom,
        m_NearClip, m_InvRange);
  } else {
    return Matrix::CreateProjectionMatrix(m_VerticalFOV, m_NearClip, m_FarClip,
                                          m_InvRange, m_AspectRatio);
  }
}
...
void View::UpdateInvRange() { m_InvRange = 1.0f / (m_FarClip - m_NearClip); }