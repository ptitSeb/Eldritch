...
// it consolidated.)
void Animation::InitializeFromDefinition(
    const SimpleString& QualifiedAnimationName) {
  STATICHASH(Animation);
  STATICHASH(AnimEventVersion);
  const uint AnimEventVersion =
      ConfigManager::GetInt(sAnimEventVersion, 0, sAnimation);
  ...
  for (int AnimEventIndex = 0; AnimEventIndex < NumAnimEvents;
    ...
    if (AnimEventVersion < 2) {
      // OLD SYSTEM
      const SimpleString AnimEventType = ConfigManager::GetSequenceString(
          "AnimEventType%d", AnimEventIndex, "", sQualifiedAnimationName);
      const SimpleString AnimEventName = ConfigManager::GetSequenceString(
          "AnimEventName%d", AnimEventIndex, "", sQualifiedAnimationName);
      const int AnimEventFrame = ConfigManager::GetSequenceInt(
          "AnimEventFrame%d", AnimEventIndex, 0, sQualifiedAnimationName);
      const float AnimEventTime = ConfigManager::GetSequenceFloat(
          "AnimEventTime%d", AnimEventIndex, 0.0f, sQualifiedAnimationName);

      pAnimEvent =
          AnimEventFactory::GetInstance()->Create(AnimEventType, AnimEventName);
      pAnimEvent->m_Time =
          AnimEventFrame ? (AnimEventFrame / ANIM_FRAMERATE) : AnimEventTime;
    } else {
      // NEW SYSTEM
      const SimpleString AnimEventDef = ConfigManager::GetSequenceString(
          "AnimEvent%dDef", AnimEventIndex, "", sQualifiedAnimationName);
      const int AnimEventFrame = ConfigManager::GetSequenceInt(
          "AnimEvent%dFrame", AnimEventIndex, 0, sQualifiedAnimationName);
      pAnimEvent = AnimEventFactory::GetInstance()->Create(AnimEventDef);
      pAnimEvent->m_Time =
          (static_cast<float>(AnimEventFrame) / ANIM_FRAMERATE);
    }
  ...
  m_Velocity.x =
      ConfigManager::GetFloat(sVelocityX, 0.0f, sQualifiedAnimationName);
  m_Velocity.y =
      ConfigManager::GetFloat(sVelocityY, 0.0f, sQualifiedAnimationName);
  m_RotationalVelocity.Yaw = DEGREES_TO_RADIANS(
      ConfigManager::GetFloat(sVelocityYaw, 0.0f, sQualifiedAnimationName));
}

uint Animation::GetLengthFrames() const { return m_Length; }

float Animation::GetLengthSeconds() const { return m_Length / ANIM_FRAMERATE; }

float Animation::GetNonLoopingLengthSeconds() const {
  return (m_Length - 1) / ANIM_FRAMERATE;
}

void Animation::GetVelocity(Vector& OutVelocity,
                            Angles& OutRotationalVelocity) const {
