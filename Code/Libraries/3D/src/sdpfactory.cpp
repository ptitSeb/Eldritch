...
void SDPFactory::RegisterSDPFactory(const HashedString& TypeName,
                                    SDPFactoryFunc Factory) {
...
ShaderDataProvider* SDPFactory::GetSDPInstance(const HashedString& TypeName) {
  ...
  Map<HashedString, ShaderDataProvider*>::Iterator InstanceIter =
      sSDPInstanceMap.Search(TypeName);
  if (InstanceIter.IsValid()) {
    return InstanceIter.GetValue();
  }

  Map<HashedString, SDPFactoryFunc>::Iterator FactoryIter =
