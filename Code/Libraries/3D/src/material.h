...
#if BUILD_DEV
#define MAT_DEBUG_ALWAYS \
  0x00040000  // Always draw this, even if frustum-culled, etc.
#endif
...
// This sort of thing shouldn't be a material flag, it should be part of a whole
// separate scene with separate buckets.
// But as I'm not in a place to refactor my renderer to support that concept at
...
class Material {
 public:
  ...
  void SetDefinition(const SimpleString& DefinitionName,
                     IRenderer* const pRenderer,
                     IVertexDeclaration* const pVertexDeclaration);
  void SetSamplerDefinition(const uint SamplerStage,
 ...
 private:
  ...
  uint m_Flags;  // TODO: Move to a data-driven/project-specific system for
                 // declaring flags
