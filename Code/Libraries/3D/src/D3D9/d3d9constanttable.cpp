...
struct SConstantInfo {
  uint m_NameOffset;  // Contrary to documentation, this is the offset from the
                      // constant table, *not* from this structure
  uint16 m_RegisterSet;
  uint16 m_RegisterIndex;
  uint16 m_RegisterCount;
  uint16 m_Padding;
  uint m_TypeInfoOffset;      // Probably don't need this.
  uint m_DefaultValueOffset;  // Might need this, if defaults aren't
...
void D3D9ConstantTable::ParseConstantTable(const IDataStream& Stream,
                                           Map<HashedString, uint>& OutTable) {
  const uint Version = Stream.ReadUInt32();
  // If I need it, version number (major, minor) is in low bytes.
  DEVASSERT((Version & VERTEXSHADERMASK) == VERTEXSHADERMASK ||
  ...
  DEVASSERT(ConstantTable.m_NumConstants == 0 ||
            ConstantTable.m_ConstantsOffset == ConstantTable.m_Size);

  for (uint ConstantIndex = 0; ConstantIndex < ConstantTable.m_NumConstants;
       ++ConstantIndex) {
