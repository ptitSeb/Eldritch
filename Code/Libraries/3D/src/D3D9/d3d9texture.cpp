...
D3D9Texture::D3D9Texture(IDirect3DDevice9* D3DDevice,
    ...
    : m_D3DDevice(D3DDevice), m_Texture(Texture) {}

D3D9Texture::~D3D9Texture() { SafeRelease(m_Texture); }

void* D3D9Texture::GetHandle() { return m_Texture; }

/*virtual*/ void D3D9Texture::CreateTexture(byte* const ARGBImage) {
  const int MipLevels = CountMipLevels();

  HRESULT hr = m_D3DDevice->CreateTexture(m_Width, m_Height, MipLevels, 0,
...
/*virtual*/ void D3D9Texture::CreateTextureFromDDS(const IDataStream& Stream) {
  ...
  if (SurfaceDesc.ddpfPixelFormat.dwFourCC == MAKEFOURCC('D', 'X', 'T', '1')) {
    Format = D3DFMT_DXT1;
  } else if (SurfaceDesc.ddpfPixelFormat.dwFourCC ==
             MAKEFOURCC('D', 'X', 'T', '2')) {
    Format = D3DFMT_DXT2;
  } else if (SurfaceDesc.ddpfPixelFormat.dwFourCC ==
             MAKEFOURCC('D', 'X', 'T', '3')) {
  ...
  const HRESULT hr = m_D3DDevice->CreateTexture(
      SurfaceDesc.dwWidth, SurfaceDesc.dwHeight, SurfaceDesc.dwMipMapCount, 0,
  ...
  // DLP NOTE: That's from the DX docs, but I think it's supposed to be width
  // and height DIVIDED BY 4
