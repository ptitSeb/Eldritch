...
/*static*/ D3DFORMAT D3D9RenderTarget::GetD3DFormat(
    const ERenderTargetFormat Format) {
  switch (Format) {
    case ERTF_Unknown:
...
D3D9RenderTarget::D3D9RenderTarget(IRenderer* pRenderer,
...
D3D9RenderTarget::~D3D9RenderTarget() { Release(); }

/*virtual*/ void D3D9RenderTarget::Initialize(
...
void D3D9RenderTarget::Reset() { BuildRenderTarget(); }

void D3D9RenderTarget::Reset(IDirect3DSurface9* ColorSurface,
                             IDirect3DSurface9* DepthStencilSurface) {
  // No need to AddRef these; when called from
  // D3D9Renderer::CreateDefaultRenderTarget, they have
  // already been implicitly AddRef-ed by the
  // GetRenderTarget/GetDepthStencilSurface calls.
...
void D3D9RenderTarget::BuildRenderTarget() {
  ...
  if (m_Params.ColorFormat != ERTF_None) {
    ...
    const ERenderTargetFormat SupportedFormat =
        m_Renderer->GetBestSupportedRenderTargetFormat(m_Params.ColorFormat);
    ...
    HRESULT hr = m_D3DDevice->CreateTexture(
        m_Params.Width, m_Params.Height, 1, ColorUsage,
        GetD3DFormat(SupportedFormat), D3DPOOL_DEFAULT, &ColorTexture, NULL);
    ASSERT(hr == D3D_OK);
  ...
  if (m_Params.DepthStencilFormat != ERTF_None) {
    if (m_Params.DepthStencilFormat != ERTF_UseDefault) {
      CATPRINTF(sRender, 1,
                "Trying to create depth/stencil texture with format %d\n",
                m_Params.DepthStencilFormat);

      const ERenderTargetFormat SupportedFormat =
          m_Renderer->GetBestSupportedRenderTargetFormat(
              m_Params.DepthStencilFormat);
      CATPRINTF(sRender, 1,
                "Creating depth/stencil texture with format %d...\n",
                SupportedFormat);

      IDirect3DTexture9* DepthStencilTexture;
      HRESULT hr = m_D3DDevice->CreateTexture(
          m_Params.Width, m_Params.Height, 1, DepthStencilUsage,
          GetD3DFormat(SupportedFormat), D3DPOOL_DEFAULT, &DepthStencilTexture,
          NULL);
      ASSERT(hr == D3D_OK);
      ...
      m_DepthStencilSurface =
          (IDirect3DSurface9*)(m_Renderer->GetDefaultRenderTarget()
                                   ->GetDepthStencilRenderTargetHandle());
...
void* D3D9RenderTarget::GetColorRenderTargetHandle() { return m_ColorSurface; }

void* D3D9RenderTarget::GetDepthStencilRenderTargetHandle() {
  return m_DepthStencilSurface;
}

ITexture* D3D9RenderTarget::GetColorTextureHandle() { return m_ColorTexture; }
