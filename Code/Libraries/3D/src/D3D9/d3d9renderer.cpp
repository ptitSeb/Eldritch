...
/*virtual*/ ERenderTargetFormat
D3D9Renderer::GetBestSupportedRenderTargetFormat(
    const ERenderTargetFormat Format) const {
...
#define CHECK_AND_DEMOTE(fmt, dmt)                                            \
  if (ReturnFormat == fmt) {                                                  \
    ...
    const HRESULT hr = m_D3D->CheckDeviceFormat(                              \
...
void D3D9Renderer::SetFullScreen(bool FullScreen) { m_Fullscreen = FullScreen; }

void D3D9Renderer::SetMultiSampleType(EMultiSampleType MultiSampleType) {
  m_MultiSampleType = MultiSampleType;
}

void D3D9Renderer::GetBestSupportedMultiSampleType(
    EMultiSampleType& OutMultiSampleType,
    uint32* pOutQualityLevels /*= NULL*/) {
  // NOTE: This assumes the desired resolution is set before calling this
  // function.
  ...
  for (int MultiSampleType = EMST_8X; MultiSampleType > EMST_None;
       --MultiSampleType) {
    D3DMULTISAMPLE_TYPE DesiredMultisampleType =
        GetD3DMultiSampleType(static_cast<EMultiSampleType>(MultiSampleType));
    DWORD NumQualityLevels = 0;
    HRESULT hr = m_D3D->CheckDeviceMultiSampleType(
        D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, D3DDisplayMode.Format,
        !m_Fullscreen, DesiredMultisampleType, &NumQualityLevels);
    if (hr == D3D_OK) {
...
void D3D9Renderer::CreateDefaultRenderTarget() {
  ...
  if (m_DefaultRenderTarget) {
    ...
    static_cast<D3D9RenderTarget*>(m_DefaultRenderTarget)
        ->Reset(ColorRenderTarget, DepthStencilSurface);
  } else {
    m_DefaultRenderTarget = new D3D9RenderTarget(
        this, m_D3DDevice, ColorRenderTarget, DepthStencilSurface);
  }
...
/*virtual*/ void D3D9Renderer::SetRestoreDeviceCallback(
    const SRestoreDeviceCallback& Callback) {
...
/*virtual*/ void D3D9Renderer::SetViewport(
    const ViewportPass* const pViewportPass) {
  XTRACE_FUNCTION;
...
void D3D9Renderer::Clear(unsigned int Flags,
                         unsigned int Color /*= 0xff000000*/,
                         float Depth /*= 1.0f*/, unsigned int Stencil /*= 0*/) {
  XTRACE_FUNCTION;
...
/*virtual*/ IVertexShader* D3D9Renderer::CreateVertexShader(
...
/*virtual*/ IShaderProgram* D3D9Renderer::CreateShaderProgram(
...
void D3D9Renderer::SetRenderTarget(IRenderTarget* RenderTarget) {
  ...
  m_D3DDevice->SetRenderTarget(
      0,
      (IDirect3DSurface9*)m_CurrentRenderTarget->GetColorRenderTargetHandle());
  m_D3DDevice->SetDepthStencilSurface(
      (IDirect3DSurface9*)
      m_CurrentRenderTarget->GetDepthStencilRenderTargetHandle());
}

bool D3D9Renderer::IsValid() { return !m_DeviceLost; }
...
void D3D9Renderer::GetPresentParams(D3DPRESENT_PARAMETERS& Params) {
  ...
  m_D3D->GetAdapterDisplayMode(
      D3DADAPTER_DEFAULT, &D3DDisplayMode);  // NOTE: This assumes the desired
                                             // resolution is set before calling
                                             // this for fullscreen mode
  ...
  Params.BackBufferCount =
      1;  // Could be 1, 2 or 3... Faster? Slower? Test it...

  D3DMULTISAMPLE_TYPE DesiredMultisampleType =
      GetD3DMultiSampleType(m_MultiSampleType);
  DWORD NumQualityLevels = 0;
  HRESULT hr = m_D3D->CheckDeviceMultiSampleType(
      D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, D3DDisplayMode.Format,
      Params.Windowed, DesiredMultisampleType, &NumQualityLevels);
  if (hr == D3D_OK) {
  ...
  Params.SwapEffect = D3DSWAPEFFECT_DISCARD;  // Is apparently fastest (and
  ...
  Params.AutoDepthStencilFormat =
      (D3DDisplayMode.Format == D3DFMT_X8R8G8B8) ? D3DFMT_D24S8 : D3DFMT_D16;

  Params.Flags = D3DPRESENTFLAG_DISCARD_DEPTHSTENCIL;  // Discard z-buffer after
                                                       // Present()ing, can
                                                       // improve performance
  ...
  if (ConfigManager::GetBool(sVSync, true)) {
    Params.PresentationInterval =
        D3DPRESENT_INTERVAL_ONE;  // D3DPRESENT_INTERVAL_DEFAULT is basically
                                  // the same; coarser time resolution but a bit
                                  // faster?
  } else {
...
void D3D9Renderer::EnumerateDisplayModes(Array<SDisplayMode>& DisplayModes,
                                         D3DFORMAT Format) {
...
void* D3D9Renderer::GetDevice() { return m_D3DDevice; }
...
#define TESTFORMAT(adapterfmt, usage, rtype, fmt)                   \
  hr = m_D3D->CheckDeviceFormat(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, \
                                adapterfmt, usage, rtype, fmt);     \
  PRINTF(#adapterfmt " " #usage " " #rtype " " #fmt ": 0x%08X\n", hr)

  TESTFORMAT(D3DFMT_X8R8G8B8, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE,
             D3DFMT_D24S8);
  TESTFORMAT(D3DFMT_X8R8G8B8, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE,
             D3DFMT_D16);
  ...
  TESTFORMAT(D3DFMT_X8R8G8B8, D3DUSAGE_RENDERTARGET, D3DRTYPE_SURFACE,
             D3DFMT_X8R8G8B8);
  TESTFORMAT(D3DFMT_X8R8G8B8, D3DUSAGE_RENDERTARGET, D3DRTYPE_TEXTURE,
             D3DFMT_X8R8G8B8);
  TESTFORMAT(D3DFMT_X8R8G8B8, D3DUSAGE_RENDERTARGET, D3DRTYPE_SURFACE,
             D3DFMT_A8R8G8B8);
  TESTFORMAT(D3DFMT_X8R8G8B8, D3DUSAGE_RENDERTARGET, D3DRTYPE_TEXTURE,
  ...
  TESTFORMAT(D3DFMT_R5G6B5, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE,
             D3DFMT_D24S8);
  TESTFORMAT(D3DFMT_R5G6B5, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE,
             D3DFMT_D16);
  ...
  TESTFORMAT(D3DFMT_R5G6B5, D3DUSAGE_RENDERTARGET, D3DRTYPE_SURFACE,
             D3DFMT_X8R8G8B8);
  TESTFORMAT(D3DFMT_R5G6B5, D3DUSAGE_RENDERTARGET, D3DRTYPE_TEXTURE,
             D3DFMT_X8R8G8B8);
  TESTFORMAT(D3DFMT_R5G6B5, D3DUSAGE_RENDERTARGET, D3DRTYPE_SURFACE,
             D3DFMT_A8R8G8B8);
  TESTFORMAT(D3DFMT_R5G6B5, D3DUSAGE_RENDERTARGET, D3DRTYPE_TEXTURE,
  ...
  TESTFORMAT(D3DFMT_X1R5G5B5, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE,
             D3DFMT_D24S8);
  TESTFORMAT(D3DFMT_X1R5G5B5, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE,
             D3DFMT_D16);
  ...
  TESTFORMAT(D3DFMT_X1R5G5B5, D3DUSAGE_RENDERTARGET, D3DRTYPE_SURFACE,
             D3DFMT_X8R8G8B8);
  TESTFORMAT(D3DFMT_X1R5G5B5, D3DUSAGE_RENDERTARGET, D3DRTYPE_TEXTURE,
             D3DFMT_X8R8G8B8);
  TESTFORMAT(D3DFMT_X1R5G5B5, D3DUSAGE_RENDERTARGET, D3DRTYPE_SURFACE,
             D3DFMT_A8R8G8B8);
  TESTFORMAT(D3DFMT_X1R5G5B5, D3DUSAGE_RENDERTARGET, D3DRTYPE_TEXTURE,
...
/*virtual*/ void D3D9Renderer::SetVertexShader(
  ...
  IDirect3DVertexShader9* const pD3DVertexShader =
      static_cast<IDirect3DVertexShader9*>(pVertexShader->GetHandle());
...
/*virtual*/ SimpleString D3D9Renderer::GetShaderType() const { return "HLSL"; }
...
/*virtual*/ void D3D9Renderer::SetZWriteEnable(
    const EZWriteEnable ZWriteEnable) {
  DEBUGASSERT(m_D3DDevice);
  ...
  const HRESULT Result = m_D3DDevice->SetRenderState(
      D3DRS_ZWRITEENABLE, (ZWriteEnable == EZWE_True) ? TRUE : FALSE);
  DEBUGASSERT(Result == D3D_OK);
  Unused(Result);
}
...
/*virtual*/ void D3D9Renderer::SetBlend(const EBlend SrcBlend,
                                        const EBlend DestBlend) {
  DEBUGASSERT(m_D3DDevice);
  ...
  {
    const HRESULT Result =
        m_D3DDevice->SetRenderState(D3DRS_SRCBLEND, D3DBlend[SrcBlend]);
    DEBUGASSERT(Result == D3D_OK);
    Unused(Result);
  }
  ...
  {
    const HRESULT Result =
        m_D3DDevice->SetRenderState(D3DRS_DESTBLEND, D3DBlend[DestBlend]);
    DEBUGASSERT(Result == D3D_OK);
    Unused(Result);
  }
}

/*virtual*/ void D3D9Renderer::SetTexture(const uint SamplerStage,
                                          ITexture* const pTexture) {
  DEBUGASSERT(m_D3DDevice);
  ...
  IDirect3DTexture9* const pD3DTexture =
      static_cast<IDirect3DTexture9*>(pTexture->GetHandle());
...
/*virtual*/ void D3D9Renderer::SetAddressing(const uint SamplerStage,
                                             const ETextureAddress AddressU,
                                             const ETextureAddress AddressV) {
  DEBUGASSERT(m_D3DDevice);
  DEBUGASSERT(SamplerStage < MAX_TEXTURE_STAGES);
  ...
  {
    const HRESULT Result = m_D3DDevice->SetSamplerState(
        SamplerStage, D3DSAMP_ADDRESSU, D3DTextureAddress[AddressU]);
    DEBUGASSERT(Result == D3D_OK);
    Unused(Result);
  }
  ...
  {
    const HRESULT Result = m_D3DDevice->SetSamplerState(
        SamplerStage, D3DSAMP_ADDRESSV, D3DTextureAddress[AddressV]);
    DEBUGASSERT(Result == D3D_OK);
    Unused(Result);
  }
}
...
/*virtual*/ void D3D9Renderer::SetMinMipFilters(
    const uint SamplerStage, const ETextureFilter MinFilter,
    const ETextureFilter MipFilter) {
  DEBUGASSERT(m_D3DDevice);
  DEBUGASSERT(SamplerStage < MAX_TEXTURE_STAGES);
  ...
  {
    const HRESULT Result = m_D3DDevice->SetSamplerState(
        SamplerStage, D3DSAMP_MINFILTER, D3DTextureFilter[MinFilter]);
    DEBUGASSERT(Result == D3D_OK);
    Unused(Result);
  }
  ...
  {
    const HRESULT Result = m_D3DDevice->SetSamplerState(
        SamplerStage, D3DSAMP_MIPFILTER, D3DTextureFilter[MipFilter]);
    DEBUGASSERT(Result == D3D_OK);
    Unused(Result);
  }
}

/*virtual*/ void D3D9Renderer::SetMagFilter(const uint SamplerStage,
                                            const ETextureFilter MagFilter) {
  DEBUGASSERT(m_D3DDevice);
  DEBUGASSERT(SamplerStage < MAX_TEXTURE_STAGES);
  ...
  const HRESULT Result = m_D3DDevice->SetSamplerState(
      SamplerStage, D3DSAMP_MAGFILTER, D3DTextureFilter[MagFilter]);
  DEBUGASSERT(Result == D3D_OK);
  Unused(Result);
}

/*virtual*/ void D3D9Renderer::SetVertexShaderFloat4(
    const HashedString& Parameter, const float* const pFloats,
    const uint NumFloat4s) {
  ...
  const HRESULT Result =
      m_D3DDevice->SetVertexShaderConstantF(Register, pFloats, NumFloat4s);
  DEBUGASSERT(Result == D3D_OK);
  Unused(Result);
}

/*virtual*/ void D3D9Renderer::SetVertexShaderMatrix(
    const HashedString& Parameter, const float* const pFloats,
  ...
  const HRESULT Result =
      m_D3DDevice->SetVertexShaderConstantF(Register, pFloats, NumMatrices * 4);
  DEBUGASSERT(Result == D3D_OK);
  Unused(Result);
}

/*virtual*/ void D3D9Renderer::SetPixelShaderFloat4(
    const HashedString& Parameter, const float* const pFloats,
    const uint NumFloat4s) {
  ...
  const HRESULT Result =
      m_D3DDevice->SetPixelShaderConstantF(Register, pFloats, NumFloat4s);
  DEBUGASSERT(Result == D3D_OK);
  Unused(Result);
}
