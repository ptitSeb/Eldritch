...
/*virtual*/ void SDPBase::SetShaderParameters(IRenderer* const pRenderer,
                                              Mesh* const pMesh,
                                              const View& View) const {
  Unused(View);

  const Matrix WorldMatrix = pRenderer->GetWorldMatrix();
  const Matrix WVP = WorldMatrix * pRenderer->GetViewMatrix() *
                     pRenderer->GetProjectionMatrix();
  ...
  if (pMesh->IsAnimated()) {
    ...
    pRenderer->SetVertexShaderMatrix(sBoneMatrices,
                                     pMesh->m_BoneMatrices[0].GetArray(),
                                     pMesh->m_Bones->GetNumBones());
  }
}