...
void AnimationState::Tick(float DeltaTime) {
  ...
  if (pLayer->m_EndBehavior == EAEB_Stop && !pLayer->m_Stopped) {
    ...
    pLayer->m_Time = Mod(pLayer->m_Time + LayerDeltaTime,
                         pLayer->m_Animation->GetLengthSeconds());
  }

  DEVASSERT(pLayer->m_Time >= 0.0f);
  DEVASSERT(pLayer->m_Time < pLayer->m_Animation->GetLengthSeconds());
  DEVASSERT(static_cast<uint>(pLayer->m_Time * ANIM_FRAMERATE) <
            pLayer->m_Animation->GetLengthFrames());
...
void AnimationState::TickAnimEvents(const float OldTime) {
  ...
  for (uint AnimEventIndex = 0;
       AnimEventIndex < pLayer->m_Animation->m_AnimEvents.Size();
       ++AnimEventIndex) {
    ...
    const bool WasPreAnimEventTime =
        OldTime <= pAnimEvent->m_Time &&
        pAnimEvent->m_Time >
            0.0f;  // Because 0-frame events are triggered immediately
    const bool HasLooped = OldTime > pLayer->m_Time;  // Catch anim events when
                                                      // animation loops
                                                      // (including 0-frame)
    const bool ShouldCallAnimEvent =
        IsPostAnimEventTime && (WasPreAnimEventTime || HasLooped);
...
void AnimationState::PlayAnimation(Animation* pAnimation,
                                   const SPlayAnimationParams& PlayParams) {
  SAnimationLayer* pLayer = GetAnimationLayer();
  DEVASSERT(pLayer);

  if (pLayer->m_Animation == pAnimation &&
      PlayParams.m_IgnoreIfAlreadyPlaying) {
    // We're already playing this animation and don't want to restart.
    // But do update the play rate if it has changed. Poorly documented side
    // effect!
  ...
  if (pLayer->m_Animation && !pLayer->m_Stopped) {
    FOR_EACH_LIST(NotifyIter, m_AnimationListeners, SAnimationListener) {
      SAnimationListener& AnimationListener = *NotifyIter;
      if (AnimationListener.m_NotifyFinishedFunc) {
        AnimationListener.m_NotifyFinishedFunc(AnimationListener.m_Void, m_Mesh,
                                               pLayer->m_Animation, true);
      }
    }
  }
...
AnimationState::EAnimationEndBehavior AnimationState::GetAnimationEndBehavior()
    const {
  const SAnimationLayer* pLayer = GetConstAnimationLayer();
  if (pLayer) {
...
void AnimationState::GetAnimationVelocity(Vector& OutVelocity,
...
void AnimationState::SetMesh(Mesh* pMesh) { m_Mesh = pMesh; }

const AnimationState::SAnimationLayer* AnimationState::GetConstAnimationLayer()
    const {
...
/*static*/ uint AnimationState::GetFrameFromTime(
    const SAnimationLayer* pLayer) {
  if (pLayer && pLayer->m_Animation) {
    ...
    return 0;  // Always return to T-pose if there's not an animation playing on
               // this layer
  }
}

/*static*/ uint AnimationState::GetNextFrame(const SAnimationLayer* pLayer,
                                             uint Frame) {
  if (pLayer && pLayer->m_Animation) {
    if (pLayer->m_Animation->m_Length > 0) {
      Frame -= pLayer->m_Animation->m_StartFrame;
      ...
      if (Frame >= pLayer->m_Animation->m_Length) {
        if (pLayer->m_EndBehavior == EAEB_Stop) {
          Frame--;  // Keep interpolating into the same frame (see also
                    // ::Tick())
        } else if (pLayer->m_EndBehavior == EAEB_Loop) {
...
void AnimationState::UpdateBones(const BoneArray* pBones,
                                 Matrix* pBoneMatrices) const {
  ...
  for (int i = 0; i < NumBones; ++i) {
    ...
    pBones->GetInterpolatedBone(i, CurrentFrame, NextFrame, FrameDelta,
                                InterpolatedBone);
...
void AnimationState::AddAnimationListener(
    const SAnimationListener& AnimationListener) {
  m_AnimationListeners.PushBack(AnimationListener);
}

void AnimationState::RemoveAnimationListener(
    const SAnimationListener& AnimationListener) {
  m_AnimationListeners.Remove(AnimationListener);
}

void AnimationState::CallAnimEvent(AnimEvent* const pAnimEvent,
                                   const SAnimationLayer* const pLayer) {
  ...
  FOR_EACH_LIST(NotifyIter, m_AnimationListeners, SAnimationListener) {
    ...
    if (AnimationListener.m_NotifyAnimEventFunc) {
      AnimationListener.m_NotifyAnimEventFunc(
          AnimationListener.m_Void, pAnimEvent, m_Mesh, pLayer->m_Animation);
    }
  }
}