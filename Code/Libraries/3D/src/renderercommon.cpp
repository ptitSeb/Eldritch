...
Matrix RendererCommon::GetWorldMatrix() { return m_WorldMatrix; }

Matrix RendererCommon::GetViewMatrix() { return m_ViewMatrix; }

Matrix RendererCommon::GetProjectionMatrix() { return m_ProjectionMatrix; }
...
/*virtual*/ void RendererCommon::FlushBuckets() {
  for (uint BucketIndex = 0; BucketIndex < m_OrderedBuckets.Size();
       ++BucketIndex) {
...
/*virtual*/ void RendererCommon::AddViewportPass(
    ViewportPass* const pViewportPass) {
...
/*virtual*/ void RendererCommon::FreeViewportPasses() {
  ...
  for (uint ViewportPassIndex = 0; ViewportPassIndex < NumViewportPasses;
       ++ViewportPassIndex) {
...
void RendererCommon::FreeRenderTargets() { m_RenderTargets.Clear(); }
...
void RendererCommon::SetClock(Clock* pClock) { m_Clock = pClock; }

Clock* RendererCommon::GetClock() { return m_Clock; }
  ...
  if (m_ViewportPasses.Empty()) {
    ...
    for (uint ViewportPassIndex = 0; ViewportPassIndex < NumViewportPasses;
      ...
      const ViewportPass* const pViewportPass =
          m_ViewportPasses[ViewportPassIndex];
...
#if BUILD_DEV
void RendererCommon::DEBUGDrawLine(const Vector& Start, const Vector& End,
                                   unsigned int Color) {
...
void RendererCommon::DEBUGDrawBox(const Vector& Min, const Vector& Max,
                                  unsigned int Color) {
