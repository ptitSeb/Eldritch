...
const SIrradianceVolume& IrradianceVolumes::GetNearestVolume(
    const Vector& Location) const {
  Vector Indices = ((Location + m_VolumeResolution * 0.5f) - m_Extents.m_Min) /
                   m_VolumeResolution;
...
const SIrradianceVolume& IrradianceVolumes::GetIndexedVolume(uint X, uint Y,
                                                             uint Z) const {
  ASSERT(X < m_NumVolumesX);
  ASSERT(Y < m_NumVolumesY);
  ASSERT(Z < m_NumVolumesZ);

  return *(m_Volumes + Z * m_NumVolumesX * m_NumVolumesY + Y * m_NumVolumesX +
           X);
}

Vector IrradianceVolumes::GetLocationOfIndexedVolume(uint X, uint Y,
                                                     uint Z) const {
  return m_Extents.m_Min +
         Vector((float)X, (float)Y, (float)Z) * m_VolumeResolution;
}

/*static*/ Vector4 IrradianceVolumes::GetColor(const SIrradianceVolume& Volume,
  ...
  return DirectionSquared.x * Volume.m_Light[XNegative] +
...
/*static*/ Vector4 IrradianceVolumes::GetAverage(
    const SIrradianceVolume& Volume) {
  return (0.1666666f * GetSum(Volume));
}

/*static*/ float IrradianceVolumes::GetLuminance(
    const SIrradianceVolume& Volume) {
