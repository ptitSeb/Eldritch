...
void AnimEventFactory::DeleteInstance() { SafeDelete(m_Instance); }

void AnimEventFactory::Register(const HashedString& ClassName,
                                AnimEventFactoryFunc FactoryFunc) {
...
AnimEvent* AnimEventFactory::Create(const SimpleString& DefinitionName) {
  ...
  const HashedString Type =
      ConfigManager::GetHash(sType, HashedString::NullString, sDefinitionName);

  return Create(Type, DefinitionName);
}

AnimEvent* AnimEventFactory::Create(const HashedString& ClassName,
                                    const SimpleString& DefinitionName) {
  Map<HashedString, AnimEventFactoryFunc>::Iterator FactoryFuncIter =
      m_FactoryFuncMap.Search(ClassName);
  ASSERTDESC(!FactoryFuncIter.IsNull(),
             "Unknown AnimEvent class specified in config file.");
