...
Mesh* MeshFactory::CreatePlane(float Length, float Width, int LengthSegments,
                               int WidthSegments, EPlane Plane, bool TwoSided) {
  ...
  for (int j = 0; j < WidP; ++j) {
    for (int i = 0; i < LenP; ++i) {
      ...
      if (Plane == XY_PLANE) {
        ...
        UVs[Index] =
            Vector2((float)i / LengthSegments, (float)j / WidthSegments);
        Normals[Index] = Vector(0.f, 0.f, 1.f);
        Tangents[Index] = Vector4(
            1.f, 0.f, 0.f,
            1.f);  // Bitangent = (Normal x Tangent) * Bitangent orientation
      } else if (Plane == XZ_PLANE) {
        // Because XZ plane is typically used for ortho mode (UI) and my ortho
        // projection flips the vertical component, positions and UVs are
        // backwards
        // here compared to XY and YZ planes. Ugly, but whatever. This is legacy
        // code.
  ...
  IVertexDeclaration* VDecl = m_Renderer->GetVertexDeclaration(
      VD_POSITIONS | VD_COLORS | VD_UVS | VD_NORMALS | VD_TANGENTS);
  ...
  if (Plane == XY_PLANE) {
    NewMesh->m_AABB =
        AABB(Vector(-HalfL, -HalfW, 0.0f), Vector(HalfL, HalfW, 0.0f));
  } else if (Plane == XZ_PLANE) {
    NewMesh->m_AABB =
        AABB(Vector(-HalfL, 0.0f, -HalfW), Vector(HalfL, 0.0f, HalfW));
  } else if (Plane == YZ_PLANE) {
    NewMesh->m_AABB =
        AABB(Vector(0.0f, -HalfL, -HalfW), Vector(0.0f, HalfL, HalfW));
  }
...
Mesh* MeshFactory::CreateSprite() {
  ...
  IVertexDeclaration* VertexDeclaration =
      m_Renderer->GetVertexDeclaration(VD_POSITIONS | VD_UVS);
...
Mesh* MeshFactory::CreateGrid(float Length, float Width, int LengthSegments,
                              int WidthSegments, EPlane Plane) {
  int LenP = LengthSegments + 1;
  int WidP = WidthSegments + 1;
...
Mesh* MeshFactory::CreateCylinder(float Radius, float Height,
  ...
  for (int j = 0; j < HP; ++j) {
    for (int i = 0; i < RP; ++i) {
      ...
      Colors[Index] = 0xffffffff;
      UVs[Index] = Vector2(
          2.f * (float)i / RadialSegments,
          (float)j /
              HeightSegments);  // NOTE: Multiplying u by two to repeat texture
  ...
  Normals[IndexTop] = Vector(0.f, 0.f, 1.f);
  Tangents[IndexTop] = Vector4(
      0.f, 1.f, 0.f, 1.f);  // What's the correct tangent for the center?

  Positions[IndexBottom] = Vector(0.f, 0.f, -HalfH);
  Colors[IndexBottom] = 0xffffffff;
  UVs[IndexBottom] = Vector2(.5f, .5f);
  Normals[IndexBottom] = Vector(0.f, 0.f, -1.f);
  Tangents[IndexBottom] = Vector4(
      0.f, -1.f, 0.f, 1.f);  // What's the correct tangent for the center?
  ...
  // Last one has to be handled separate because there is no duplicated vert on
  // caps
  Indices[IndexIndex++] = (index_t)(VertexIndex);
...
Mesh* MeshFactory::CreateBox(float Length, float Width, float Height,
  ...
  for (int j = 0; j < WP; ++j) {
    for (int i = 0; i < LP; ++i) {
      ...
      UVs[Index1] =
          Vector2((float)i / LengthSegments, (float)j / WidthSegments);
      UVs[Index2] = Vector2((float)i / LengthSegments,
                            (float)(WidthSegments - j) / WidthSegments);
      Normals[Index1] = Vector(0.f, 0.f, 1.f);
      Normals[Index2] = Vector(0.f, 0.f, -1.f);
      Tangents[Index1] = Vector4(
          1.f, 0.f, 0.f,
          1.f);  // Bitangent = (Normal x Tangent) * Bitangent orientation
  ...
  for (int j = 0; j < HP; ++j) {
    for (int i = 0; i < LP; ++i) {
      ...
      Colors[Index2] = 0xffffffff;
      UVs[Index1] =
          Vector2((float)i / LengthSegments, (float)j / HeightSegments);
      UVs[Index2] = Vector2((float)(LengthSegments - i) / LengthSegments,
                            (float)j / HeightSegments);
  ...
  for (int j = 0; j < HP; ++j) {
    for (int i = 0; i < WP; ++i) {
      ...
      Colors[Index2] = 0xffffffff;
      UVs[Index1] =
          Vector2((float)i / WidthSegments, (float)j / HeightSegments);
      UVs[Index2] = Vector2((float)(WidthSegments - i) / WidthSegments,
                            (float)j / HeightSegments);
...
Mesh* MeshFactory::CreateSphere(float Radius, int LongitudinalSegments,
...
Mesh* MeshFactory::CreateGeosphere(float Radius, int Refinement) {
  // TODO: This
  // Use the method of subdividing the faces of an icosahedron (an octahedron
  // won't work) and pushing them out to unit length.
  Unused(Radius);
  Unused(Refinement);
  WARN;
  return nullptr;
...
Mesh* MeshFactory::CreateCapsule(float Radius, float InnerLength,
                                 int CylinderSegments, int LongitudinalSegments,
                                 int LatitudinalSegments) {
  // TODO: This
  Unused(Radius);
  Unused(InnerLength);
...
Mesh* MeshFactory::CreateDebugTriangle(const Vector& V1, const Vector& V2,
  ...
  TriMesh->m_AABB =
      AABB(Vector(Min(Min(V1.x, V2.x), V3.x), Min(Min(V1.y, V2.y), V3.y),
           ...
           Vector(Max(Max(V1.x, V2.x), V3.x), Max(Max(V1.y, V2.y), V3.y),
...
// V1 V2
// V3 V4
Mesh* MeshFactory::CreateDebugQuad(const Vector& V1, const Vector& V2,
...
Mesh* MeshFactory::CreateDebugBox(const Vector& Min, const Vector& Max,
                                  unsigned int Color) {
  Vector Positions[8];
  uint Colors[8];
  index_t Indices[24];
...
Mesh* MeshFactory::CreateDebugFrustum(const View& rView, unsigned int Color) {
  ...
  FrustumMesh->m_AABB =
      AABB(Vector(MinX, MinY, MinZ), Vector(MaxX, MaxY, MaxZ));
...
Mesh* MeshFactory::CreateDebugSphere(const Vector& Center, float Radius,
                                     unsigned int Color) {
...
Mesh* MeshFactory::CreateDebugEllipsoid(const Vector& Center,
...
Mesh* MeshFactory::CreateDebugCross(const Vector& Center, const float Length,
                                    unsigned int Color) {
  Vector Positions[6];
  uint Colors[6];
  index_t Indices[6];
...
Mesh* MeshFactory::CreateDebugArrow(const Vector& Root, const Angles& Direction,
  ...
  const Vector ArrowHeadUp =
      RotationMatrix * Vector(0.0f, -HeadLength, HeadLength);
  const Vector ArrowHeadDown =
      RotationMatrix * Vector(0.0f, -HeadLength, -HeadLength);
  const Vector ArrowHeadLeft =
      RotationMatrix * Vector(-HeadLength, -HeadLength, 0.0f);
  const Vector ArrowHeadRight =
      RotationMatrix * Vector(HeadLength, -HeadLength, 0.0f);
...
Mesh* MeshFactory::CreateDebugChar(float Width, float Height,
                                   const Vector2& UV1, const Vector2& UV2,
                                   EPlane Plane, bool TwoSided) {
  ...
  IVertexBuffer* VertexBuffer = m_Renderer->CreateVertexBuffer();
  IVertexDeclaration* VertexDeclaration =
      m_Renderer->GetVertexDeclaration(VD_POSITIONS | VD_COLORS | VD_UVS);
  IIndexBuffer* IndexBuffer = m_Renderer->CreateIndexBuffer();
  IVertexBuffer::SInit InitStruct;
  InitStruct.NumVertices = 4;
  InitStruct.Positions = Positions;
  ...
  if (Plane == XY_PLANE) {
    CharMesh->m_AABB =
        AABB(Vector(-HalfW, -HalfH, 0.0f), Vector(HalfW, HalfH, 0.0f));
  } else if (Plane == XZ_PLANE) {
    CharMesh->m_AABB =
        AABB(Vector(-HalfW, 0.0f, -HalfH), Vector(HalfW, 0.0f, HalfH));
  } else if (Plane == YZ_PLANE) {
    CharMesh->m_AABB =
        AABB(Vector(0.0f, -HalfW, -HalfH), Vector(0.0f, HalfW, HalfH));
  }
...
void MeshFactory::GetDynamicMesh(const char* Filename, Mesh* pNewMesh) {
  ...
  pNewMesh->Initialize(pManagedMesh->m_VertexBuffer,
                       pManagedMesh->m_VertexDeclaration,
                       pManagedMesh->m_IndexBuffer, pManagedMesh->m_Bones);
...
// NOTE: Filename is only used for mapping anim events. It can be ignored (NULL)
// for static meshes.
// If pMesh is provided, an existing mesh will be initialized from this stream.
// In most cases, this should be null.
Mesh* MeshFactory::Read(const IDataStream& Stream, const char* Filename,
                        Mesh* pMesh /*= NULL*/,
  ...
  if (Bones) {
    ...
    pBoneArray->Init(BoneNames, Bones, Filename, Animations, Header.m_NumFrames,
  ...
  pMesh->SetTexture(0, pTextureManager->GetTexture(
                           DEFAULT_TEXTURE, TextureManager::ETL_Permanent));
  pMesh->SetTexture(1, pTextureManager->GetTexture(
                           DEFAULT_NORMAL, TextureManager::ETL_Permanent));
  pMesh->SetTexture(2, pTextureManager->GetTexture(
                           DEFAULT_SPEC, TextureManager::ETL_Permanent));

  for (int i = 0; i < Header.m_NumMaterials; ++i) {
    pMesh->SetTexture(i,
                      pTextureManager->GetTexture(Materials[i].m_Filename,
                                                  TextureManager::ETL_World));
  }
