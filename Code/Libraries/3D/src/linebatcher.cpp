...
void LineBatcher::DrawLine(const Vector& Start, const Vector& End,
                           unsigned int Color) {
#if BUILD_RELEASE
  if (m_IsDebug) {
    return;
...
void LineBatcher::DrawTriangle(const Vector& A, const Vector& B,
                               const Vector& C, unsigned int Color) {
#if BUILD_RELEASE
  if (m_IsDebug) {
    return;
  }
...
void LineBatcher::DrawBox(const Vector& Min, const Vector& Max,
                          unsigned int Color) {
#if BUILD_RELEASE
  if (m_IsDebug) {
    return;
...
void LineBatcher::DrawX(const Vector& Center, const float Radius,
...
void LineBatcher::DrawSphere(const Vector& Center, float Radius,
                             unsigned int Color) {
#if BUILD_RELEASE
  if (m_IsDebug) {
    return;
  ...
  for (uint i = 0; i < Resolution; ++i) {
    ...
    m_AddPositions[i + Resolution] =
        Vector(Center.x + u, Center.y, Center.z + v);
...
void LineBatcher::DrawGradientLine(const Vector& Start, const Vector& End,
...
void LineBatcher::DrawIrradianceCross(const Vector& Center, const float Radius,
                                      const Vector4 m_AddColors[6]) {
#if BUILD_RELEASE
  if (m_IsDebug) {
    return;
  }
  ...
  DrawGradientLine(Center + X, Center - X, m_AddColors[0].ToColor(),
                   m_AddColors[1].ToColor());
  DrawGradientLine(Center + Y, Center - Y, m_AddColors[2].ToColor(),
                   m_AddColors[3].ToColor());
  DrawGradientLine(Center + Z, Center - Z, m_AddColors[4].ToColor(),
                   m_AddColors[5].ToColor());
}