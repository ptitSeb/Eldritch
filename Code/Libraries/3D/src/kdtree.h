...
class KDTree {
 public:
  ...
  bool SweepAgainst(const Ellipsoid& e, const Vector& v, const AABB& SweepAABB,
  ...
  AABB m_TightBound;  // Tight bound on all meshes in this node and all children
                      // (same as m_FullBound for root node)
  AABB m_LocalBound;  // Tight bound on only meshes in this node (same as
                      // m_TightBound for leaf nodes; has no meaning in non-leaf
                      // nodes if there are no local meshes)
