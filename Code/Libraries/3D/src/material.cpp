...
void Material::SetDefinition(const SimpleString& DefinitionName,
  ...
  const HashedString CullMode =
      ConfigManager::GetHash(sCullMode, sCW, sDefinitionName);
  ...
  const bool ZWriteEnable =
      ConfigManager::GetBool(sZWriteEnable, true, sDefinitionName);
  m_RenderState.m_ZWriteEnable = ZWriteEnable ? EZWE_True : EZWE_False;

  STATICHASH(AlphaBlendEnable);
  const bool AlphaBlendEnable =
      ConfigManager::GetBool(sAlphaBlendEnable, false, sDefinitionName);
  m_RenderState.m_AlphaBlendEnable = AlphaBlendEnable ? EABE_True : EABE_False;

  STATICHASH(SrcBlend);
  STATIC_HASHED_STRING(One);
  const HashedString SrcBlend =
      ConfigManager::GetHash(sSrcBlend, sOne, sDefinitionName);
  m_RenderState.m_SrcBlend = RenderStates::GetBlend(SrcBlend);

  STATICHASH(DestBlend);
  STATIC_HASHED_STRING(Zero);
  const HashedString DestBlend =
      ConfigManager::GetHash(sDestBlend, sZero, sDefinitionName);
  ...
  const SimpleString VertexShader =
      ConfigManager::GetString(sVertexShader, "", sDefinitionName);

  STATICHASH(PixelShader);
  const SimpleString PixelShader =
      ConfigManager::GetString(sPixelShader, "", sDefinitionName);

  const SimpleString ShaderType = pRenderer->GetShaderType();
  MAKEHASH(ShaderType);

  MAKEHASHFROM(VertexShaderDef, VertexShader);
  const SimpleString VertexShaderVersion =
      ConfigManager::GetString(sShaderType, "", sVertexShaderDef);

  MAKEHASHFROM(PixelShaderDef, PixelShader);
  const SimpleString PixelShaderVersion =
      ConfigManager::GetString(sShaderType, "", sPixelShaderDef);

  m_ShaderProgram = pShaderManager->GetShaderProgram(
      VertexShaderVersion, PixelShaderVersion, pVertexDeclaration);
  ...
  for (uint SamplerIndex = 0; SamplerIndex < m_NumSamplers; ++SamplerIndex) {
    const SimpleString SamplerDefinitionName = ConfigManager::GetSequenceString(
        "Sampler%d", SamplerIndex, "", sDefinitionName);
    SetSamplerDefinition(SamplerIndex, SamplerDefinitionName);
  }
}

void Material::SetSamplerDefinition(const uint SamplerStage,
                                    const SimpleString& SamplerDefinitionName) {
  ...
  const HashedString AddressU =
      ConfigManager::GetHash(sAddressU, sWrap, sSamplerDefinitionName);
  SamplerState.m_AddressU = RenderStates::GetTextureAddress(AddressU);

  STATICHASH(AddressV);
  const HashedString AddressV =
      ConfigManager::GetHash(sAddressV, sWrap, sSamplerDefinitionName);
  SamplerState.m_AddressV = RenderStates::GetTextureAddress(AddressV);

  STATICHASH(MinFilter);
  STATIC_HASHED_STRING(Point);
  const HashedString MinFilter =
      ConfigManager::GetHash(sMinFilter, sPoint, sSamplerDefinitionName);
  SamplerState.m_MinFilter = RenderStates::GetTextureFilter(MinFilter);

  STATICHASH(MagFilter);
  const HashedString MagFilter =
      ConfigManager::GetHash(sMagFilter, sPoint, sSamplerDefinitionName);
  SamplerState.m_MagFilter = RenderStates::GetTextureFilter(MagFilter);

  STATICHASH(MipFilter);
  STATIC_HASHED_STRING(None);
  const HashedString MipFilter =
      ConfigManager::GetHash(sMipFilter, sNone, sSamplerDefinitionName);
