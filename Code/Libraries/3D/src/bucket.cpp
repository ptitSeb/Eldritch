...
Bucket::Bucket(View* pView, IRenderTarget* RenderTarget, unsigned int Flags,
               ...
               Color ClearColor /*= 0xff000000*/, float ClearDepth /*= 1.0f*/)
    : m_View(pView),
...
void Bucket::Sort(const View& View) {
  ...
  for (uint MeshIndex = 0; MeshIndex < NumMeshes; ++MeshIndex) {
    ...
    // would be more correct; but it will rarely make a difference, and is more
    // ops.

    Helper.m_Mesh = pMesh;
    Helper.m_SortDistanceSq =
        (pMesh->GetSortLocation() - ViewLocation).LengthSquared();
  }
