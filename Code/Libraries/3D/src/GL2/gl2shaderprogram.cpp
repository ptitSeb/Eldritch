...
void GL2ShaderProgram::BuildUniformTable()
{
	...
	++m_MaxCache;
	m_UniformCache = static_cast<void**>( calloc( m_MaxCache, sizeof( void* ) ) );
	m_UniformSize = static_cast<int*>( calloc( m_MaxCache, sizeof( int ) ) );
}
...
/*virtual*/ bool GL2ShaderProgram::IsCached( uint Register, int Size, const void* Value )
{
	if( m_UniformSize[ Register ] < Size )
	{
		// bigger size, purge the cache...
		free( m_UniformCache[ Register ] );
		m_UniformCache[ Register ] = NULL;
	}
	...
	if( m_UniformCache[ Register ] == NULL )
	{
