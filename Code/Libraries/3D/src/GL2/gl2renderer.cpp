...
void GL2Renderer::Initialize() {
...
#if BUILD_WINDOWS_NO_SDL
  {
    ...
    // TODO PORT: Choose flags to match D3D settings. (Also, maybe move this to
    // a "GetPresentParams" kind of function.)
    // TODO SDL: Do I need to do anything like this? It seemed to work just fine
    // without it.
    ...
    const int PixelFormat =
        ChoosePixelFormat(m_Window->GetHDC(), &PixelFormatDescriptor);
    ASSERT(PixelFormat > 0);

    const BOOL Success =
        SetPixelFormat(m_Window->GetHDC(), PixelFormat, &PixelFormatDescriptor);
    Unused(Success);
    ASSERT(Success == TRUE);
  }
...
#if BUILD_SDL
    const int Error =
        SDL_GL_MakeCurrent(m_Window->GetSDLWindow(), m_RenderContext);
    Unused(Error);
    ASSERT(Error == 0);
#endif
  }
...
/*virtual*/ ERenderTargetFormat GL2Renderer::GetBestSupportedRenderTargetFormat(
    const ERenderTargetFormat Format) const {
  // TODO PORT LATER GetBestSupportedRenderTargetFormat
  // There's a good chance I don't need this, because the OpenGL spec allows the
  // implementation to
  // automatically demote the requested format to a supported format when
  // needed.
  Unused(Format);
  return ERTF_Unknown;
}
...
void GL2Renderer::GetBestSupportedMultiSampleType(
...
bool GL2Renderer::CanReset() { return true; }

bool GL2Renderer::Reset() { return true; }

/*virtual*/ void GL2Renderer::SetRestoreDeviceCallback(
...
#undef SETSTREAM
        ...
        for (int DisableAttribIndex = Index;
...
void GL2Renderer::Clear(unsigned int Flags, unsigned int Color /*= 0xff000000*/,
...
IVertexBuffer* GL2Renderer::CreateVertexBuffer() { return new GL2VertexBuffer; }

IVertexDeclaration* GL2Renderer::CreateVertexDeclaration() {
  return new GL2VertexDeclaration;
}

IIndexBuffer* GL2Renderer::CreateIndexBuffer() { return new GL2IndexBuffer; }
...
void GL2Renderer::SetRenderTarget(IRenderTarget* RenderTarget) {
  ...
  {
    ASSERT(m_Display);
    const uint ViewportWidth = (FrameBufferObject == 0)
                                   ? m_Display->m_Width
                                   : m_CurrentRenderTarget->GetWidth();
    const uint ViewportHeight = (FrameBufferObject == 0)
                                    ? m_Display->m_Height
                                    : m_CurrentRenderTarget->GetHeight();
...
/*virtual*/ void GL2Renderer::SetVertexShader(
...
/*virtual*/ void GL2Renderer::SetShaderProgram(
  ...
  const GLuint ShaderProgram =
      *static_cast<GLuint*>(pShaderProgram->GetHandle());
  glUseProgram(ShaderProgram);
}

/*virtual*/ SimpleString GL2Renderer::GetShaderType() const { return "GLSL"; }
...
/*virtual*/ void GL2Renderer::SetZWriteEnable(
...
/*virtual*/ void GL2Renderer::SetAlphaBlendEnable(
...
/*virtual*/ void GL2Renderer::SetBlend(const EBlend SrcBlend,
...
/*virtual*/ void GL2Renderer::SetTexture(const uint SamplerStage,
...
/*virtual*/ void GL2Renderer::SetAddressing(const uint SamplerStage,
  ...
  GL2Texture* const pTexture =
      static_cast<GL2Texture*>(OuterSamplerState.m_Texture);
...
static GLenum GLMinMipFilters[][ETF_SIZE] = {  //	ETF_Unknown,
                                               //ETF_None,	ETF_Point,
                                               //ETF_Linear,
                                               //ETF_Anisotropic (mip)
    {
    ...
    {
     0, GL_NEAREST, GL_NEAREST_MIPMAP_NEAREST, GL_NEAREST_MIPMAP_LINEAR, 0,
    },  // ETF_Point (min)
    {
     0, GL_LINEAR, GL_LINEAR_MIPMAP_NEAREST, GL_LINEAR_MIPMAP_LINEAR, 0,
    },  // ETF_Linear (min)
    {
     0, 0, 0, 0, 0,
    },  // ETF_Anisotropic (min) (not supported on GL)
};

/*virtual*/ void GL2Renderer::SetMinMipFilters(const uint SamplerStage,
                                               const ETextureFilter MinFilter,
...
/*virtual*/ void GL2Renderer::SetMagFilter(const uint SamplerStage,
...
/*virtual*/ void GL2Renderer::SetVertexShaderFloat4(
...
/*virtual*/ void GL2Renderer::SetVertexShaderMatrix(
    const HashedString& Parameter, const float* const pFloats,
    const uint NumMatrices) {
  SetPixelShaderMatrix(Parameter, pFloats, NumMatrices);
}

/*virtual*/ void GL2Renderer::SetPixelShaderFloat4(
    const HashedString& Parameter, const float* const pFloats,
    const uint NumFloat4s) {
...
/*virtual*/ void GL2Renderer::SetPixelShaderMatrix(
    const HashedString& Parameter, const float* const pFloats,
    const uint NumMatrices) {
  ASSERT(m_ShaderProgram);
