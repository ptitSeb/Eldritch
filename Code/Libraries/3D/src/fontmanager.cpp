...
FontManager::~FontManager() { FreeFonts(); }

void FontManager::FreeFonts() {
  FOR_EACH_MAP(Iter, m_FontTable, HashedString, Font*) { SafeDelete(*Iter); }
  m_FontTable.Clear();
}
...
Font* FontManager::GetFont(const char* Filename) {
  ...
  Map<HashedString, Font*>::Iterator FontIter =
      m_FontTable.Search(HashedFilename);
  if (FontIter.IsValid()) {
