...
void BoneArray::Init(HashedString* BoneNames, SBone* Bones,
                     const char* MeshFilename, Animation* Animations,
                     int NumFrames, int NumBones, int NumAnims) {
  ...
  memcpy_s(m_BoneNames, sizeof(HashedString) * NumBones, BoneNames,
           sizeof(HashedString) * NumBones);

  m_Bones = new SBone[TotalBones];
  memcpy_s(m_Bones, sizeof(SBone) * TotalBones, Bones,
           sizeof(SBone) * TotalBones);
  ...
  SimpleString AnimationMapName =
      ConfigManager::GetString(sMeshFilename, "", sAnimationMap);
  for (int AnimIndex = 0; AnimIndex < NumAnims; ++AnimIndex) {
    Animation& Animation = m_Animations[AnimIndex];
    Animation.InitializeFromDefinition(SimpleString::PrintF(
        "%s:%s", AnimationMapName.CStr(), Animation.m_Name));
  }
}
...
int BoneArray::GetNumFrames() const { return m_NumFrames; }

int BoneArray::GetNumBones() const { return m_NumBones; }

int BoneArray::GetNumAnimations() const { return m_NumAnimations; }

Animation* BoneArray::GetAnimation(int Index) const {
  // This case is sometimes occurring in Eldritch due to hand animation
  // swapping.
