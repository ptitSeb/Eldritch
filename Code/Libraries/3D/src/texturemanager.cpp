...
TextureManager::TextureManager(IRenderer* Renderer)
    : m_TextureTable(), m_Renderer(Renderer) {}

TextureManager::~TextureManager() { FreeTextures(ETL_Permanent); }

void TextureManager::FreeTextures(ETextureLife Life) {
  for (Map<HashedString, SManagedTexture>::Iterator TextureIter =
           m_TextureTable.Begin();
       TextureIter != m_TextureTable.End();) {
...
ITexture* TextureManager::GetTexture(const char* Filename,
                                     ETextureLife Life /*= ETL_Permanent*/) {
  XTRACE_FUNCTION;

  HashedString HashedFilename(Filename);

  SManagedTexture& ManagedTexture =
      m_TextureTable[HashedFilename];  // Creates the managed texture if it
                                       // didn't exist before
