...
extern int stb_vorbis_decode_filename(char *filename, int *channels,
                                      short **output);
#endif
extern int stb_vorbis_decode_memory(unsigned char *mem, int len, int *channels,
                                    short **output);
...
#ifndef STB_VORBIS_NO_STDIO
extern stb_vorbis *stb_vorbis_open_filename(char *filename, int *error,
                                            stb_vorbis_alloc *alloc_buffer);
...
static __forceinline unsigned int crc32_update(unsigned int crc,
                                               unsigned char byte) {
...
static void add_entry(Codebook *c, unsigned int huff_code, int symbol,
                      int count, int len, unsigned int *values) {
  if (!c->sparse) {
...
static int compute_codewords(Codebook *c, unsigned char *len, int n,
                             unsigned int *values) {
  int i, k, m = 0;
  ...
  for (i = 1; i <= len[k]; ++i) available[i] = 1 << (32 - i);
...
static void compute_sorted_huffman(Codebook *c, unsigned char *lengths,
                                   unsigned int *values) {
  int i, len;
...
#define CODEBOOK_ELEMENT(c, off) \
  (c->multiplicands[off] * c->delta_value + c->minimum_value)
...
#ifndef STB_VORBIS_DIVIDES_IN_CODEBOOK
static int codebook_decode_deinterleave_repeat_2(vorb *f, Codebook *c,
                                                 float **outputs,
                                                 int *c_inter_p, int *p_inter_p,
                                                 int len, int total_decode) {
  int c_inter = *c_inter_p;
...
static void decode_residue(vorb *f, float *residue_buffers[], int ch, int n,
                           int rn, unsigned char *do_not_decode) {
  int i, j, pass;
  ...
  unsigned char ***part_classdata = (uint8 ***)temp_block_array(
          ...
          for (i = 0; i < classwords && pcount < part_read; ++i, ++pcount) {
            ...
            if (b >= 0) {
              ...
              if (!codebook_decode_deinterleave_repeat_2(
                      f, book, residue_buffers, &c_inter, &p_inter, n,
                      r->part_size))
                goto done;
#endif
              stb_prof(7);
...
static int do_floor(vorb *f, Mapping *map, int i, int n, float *target,
                    YTYPE *finalY, unsigned char *step2_flag) {
  int n2 = n >> 1;
...
static int start_decoder(vorb *f) {
  ...
  for (i = 0; i < f->codebook_count; ++i) {
    ...
    if (!c->sparse) {
      c->codewords =
          (unsigned int *)setup_malloc(f, sizeof(c->codewords[0]) * c->entries);
      if (!c->codewords) return error(f, VORBIS_outofmem);
    } else {
      unsigned int size;
      if (c->sorted_entries) {
        c->codeword_lengths =
            (unsigned char *)setup_malloc(f, c->sorted_entries);
        if (!c->codeword_lengths) return error(f, VORBIS_outofmem);
        c->codewords = (unsigned int *)setup_temp_malloc(
            f, sizeof(*c->codewords) * c->sorted_entries);
        if (!c->codewords) return error(f, VORBIS_outofmem);
        values = (unsigned int *)setup_temp_malloc(
    ...
    if (c->sorted_entries) {
      ...
      c->sorted_codewords = (unsigned int *)setup_malloc(
...
//   !STB_VORBIS_DIVIDES_IN_CODEBOOK
  ...
  f->floor_config =
      (Floor01 *)setup_malloc(f, f->floor_count * sizeof(*f->floor_config));
  ...
  f->residue_config =
      (Residue *)setup_malloc(f, f->residue_count * sizeof(*f->residue_config));
...
static int vorbis_search_for_page_pushdata(vorb *f, unsigned char *data,
                                           int data_len) {
  int i, n;
...
static unsigned int vorbis_find_page(stb_vorbis *f, unsigned int *end,
                                     unsigned int *last) {
  for (;;) {
    int n;
    if (f->eof) return 0;
...
static int vorbis_analyze_page(stb_vorbis *f, ProbedPage *z) {
  ...
  // parse the header
  getn(f, header, 27);
  assert(header[0] == 'O' && header[1] == 'g' && header[2] == 'g' &&
         header[3] == 'S');
  ...
  z->last_decoded_sample =
      header[6] + (header[7] << 8) + (header[8] << 16) + (header[9] << 16);
  ...
  for (i = 0; i < header[26]; ++i) {
    if (packet_start) {
      ...
      if (num_packet == 0 && f->mode_config[n].blockflag) previous = (m & 1);
  ...
  if (num_packet > 1) samples += f->blocksize[packet_type[num_packet - 1]];
  ...
  for (i = num_packet - 2; i >= 1; --i) {
    ...
    if (packet_type[i] == 1)
      if (packet_type[i + 1] == 1)
        ...
        samples +=
            ((f->blocksize_1 - f->blocksize_0) >> 2) + (f->blocksize_0 >> 1);
    else
...
static int vorbis_seek_frame_from_page(stb_vorbis *f, unsigned int page_start,
                                       unsigned int first_sample,
                                       unsigned int target_sample, int fine) {
  ...
  for (;;) {
    ...
    if (!vorbis_decode_initial(f, &left_start, &left_end, &right_start,
                               &right_end, &mode))
    ...
    if (target_sample < frame_start + right_start - start) break;

    flush_packet(f);
    if (f->eof) return error(f, VORBIS_seek_failed);
  ...
  if (data_to_skip >= 0) {
    ...
    for (i = 0; i < f->channels; ++i)
      for (j = 0; j < n; ++j) f->previous_window[i][j] = 0;
    f->previous_length = n;
  ...
  if (fine) {
    ...
    if (target_sample != frame_start) {
      ...
      assert(f->channel_buffer_start + (int)(target_sample - frame_start) <
...
static int vorbis_seek_base(stb_vorbis *f, unsigned int sample_number,
  ...
  if (sample_number < f->p_first.last_decoded_sample) {
    ...
    while (p[0].page_end < p[1].page_start) {
      ...
      end_offset = p[1].after_previous_page_start;  // an address known to seek
                                                    // to page p[1]
      ...
      if (start_offset + 4000 < end_offset) end_offset -= 4000;

      // now compute an interpolated search loc
      probe = start_offset + (int)floor((float)(end_offset - start_offset) /
    ...
    if (p[0].last_decoded_sample <= sample_number &&
        sample_number < p[1].last_decoded_sample) {
...
stb_vorbis *stb_vorbis_open_filename(char *filename, int *error,
                                     stb_vorbis_alloc *alloc) {
...
int stb_vorbis_decode_memory(uint8 *mem, int len, int *channels,
                             short **output) {
  int data_len, offset, total, limit, error;
  short *data;
...
int stb_vorbis_get_samples_float(stb_vorbis *f, int channels, float **buffer,
  ...
  while (n < num_samples) {
    ...
    if (k) {
      for (i = 0; i < z; ++i)
        memcpy(buffer[i] + n, f->channel_buffers + f->channel_buffer_start,
               sizeof(float) * k);
