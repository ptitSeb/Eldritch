...
class Framework3D : public IWBEventObserver {
...
#if BUILD_WINDOWS_NO_SDL
  // Hacky sack to allow windows larger than the screen size, which fixes
  // ScreenToClient offset and UI problems.
  void SetLastWindowSize(const POINT& WindowSize) {
 ...
 protected:
  ...
  virtual void RefreshDisplay(const bool Fullscreen, const uint DisplayWidth,
  ...
  virtual void CreateSplashWindow(const uint WindowIcon,
