...
CheckForUpdates::CheckForUpdates(UIManagerCommon* pUIManager)
  ...
  const int BufferSize =
      ConfigManager::GetInt(sHTTPSocketAsyncReceiveBufferSize, 512 * 1024);
...
void CheckForUpdates::Tick() {
  ...
  if (m_WaitingToConnect && IsRequestFinished()) {
    if (m_Socket.AsyncConnectSucceeded()) {
      m_Socket.Get(m_ResultBuffer,
                   ConfigManager::GetString(sCheckForUpdatesAgent));
...
void CheckForUpdates::Check(bool Verbose, bool PausesGame) {
  ...
  if (m_WaitingToConnect || m_WaitingToReceive) {
    // If user manually checks for updates while we're already checking, elevate
    // to verbose but don't restart connection
...
bool CheckForUpdates::IsRequestFinished() { return !m_Socket.AsyncIsWaiting(); }
...
#endif
  {
    m_UIManager->ShowYesNoDialog(m_PausesGame, "ShouldUpdate", "", "", "Pop",
                                 "", "",
                                 SUICallback(TryRunContentSyncer, this));
  }
}

/*static*/ void CheckForUpdates::TryRunContentSyncer(void* pUIElement,
                                                     void* pVoid) {
  Unused(pUIElement);
  ...
  if (FileUtil::Launch("ContentSyncer.exe")) {
    ...
    pCheckForUpdates->m_UIManager->ShowOKDialog(pCheckForUpdates->m_PausesGame,
