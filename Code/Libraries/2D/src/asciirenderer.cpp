...
uint8 GetBitWiseColor(uint8 fore_r, uint8 fore_g, uint8 fore_b, uint8 fore_i,
                      uint8 back_r, uint8 back_g, uint8 back_b, uint8 back_i) {
  return (fore_r << 2 | fore_g << 1 | fore_b | fore_i << 3 | back_r << 6 |
          back_g << 5 | back_b << 4 | back_i << 7);
}
...
ASCIIRenderer::~ASCIIRenderer() { SafeDelete(m_FlickerFreeBuffer); }
...
void ASCIIRenderer::Init(uint width /*= 80*/, uint height /*= 25*/) {
  ...
  SetConsoleScreenBufferSize(m_hConsoleOutput,
                             BufferSize);  // Next, set the screen buffer size,
                                           // which cannot be smaller in either
                                           // dimension than the window.
  SetConsoleWindowInfo(m_hConsoleOutput, true,
                       &m_WindowRect);  // Finally, set the window size, which
                                        // cannot be larger in either dimension
                                        // than the buffer.
...
void ASCIIRenderer::Draw(const ASCIISurface& surface) {
  ...
  if (m_FlickerFreeBuffer) {
    ...
    ReadConsoleOutput(m_hConsoleOutput, m_FlickerFreeBuffer->GetSurfData(),
                      m_FlickerFreeBuffer->GetSurfSize(), startLocation,
                      &m_WindowRect);
    ...
    for (int Y = 0; Y < m_Height; ++Y) {
      for (int X = 0; X < m_Width; ++X, ++OldChar, ++NewChar) {
        if (OldChar->Char.AsciiChar != NewChar->Char.AsciiChar ||
          ...
          WriteConsoleOutput(m_hConsoleOutput, NewChar, OneCharSize,
                             startLocation, &WriteRegion);
        }
      }
    }
  } else {
    // Just redraw the whole thing. Somehow, this is slower than the flicker
    // free mode.
    WriteConsoleOutput(m_hConsoleOutput, surface.GetSurfData(),
                       surface.GetSurfSize(), startLocation, &m_WindowRect);
  }
}
...
/*static*/ uint8 ASCIIRenderer::GetColorFromString(const HashedString& Color) {
  ...
  if (Color == sBrightBlack) {
    return FG_BR_BLACK;
  }
  if (Color == sBrightBlue) {
    return FG_BR_BLUE;
  }
  if (Color == sBrightGreen) {
    return FG_BR_GREEN;
  }
  if (Color == sBrightCyan) {
    return FG_BR_CYAN;
  }
  if (Color == sBrightRed) {
    return FG_BR_RED;
  }
  if (Color == sBrightMagenta) {
    return FG_BR_MAGENTA;
  }
  if (Color == sBrightYellow) {
    return FG_BR_YELLOW;
  }
  if (Color == sBrightWhite) {
    return FG_BR_WHITE;
  }
...
static const uint sRGBColors[] = {
    RGB_TO_COLOR(0x00, 0x00, 0x00), RGB_TO_COLOR(0x00, 0x00, 0x80),
    RGB_TO_COLOR(0x00, 0x80, 0x00), RGB_TO_COLOR(0x00, 0x80, 0x80),
    RGB_TO_COLOR(0x80, 0x00, 0x00), RGB_TO_COLOR(0x80, 0x00, 0x80),
    RGB_TO_COLOR(0x80, 0x80, 0x00), RGB_TO_COLOR(0xc0, 0xc0, 0xc0),
    RGB_TO_COLOR(0x80, 0x80, 0x80), RGB_TO_COLOR(0x00, 0x00, 0xff),
    RGB_TO_COLOR(0x00, 0xff, 0x00), RGB_TO_COLOR(0x00, 0xff, 0xff),
    RGB_TO_COLOR(0xff, 0x00, 0x00), RGB_TO_COLOR(0xff, 0x00, 0xff),
    RGB_TO_COLOR(0xff, 0xff, 0x00), RGB_TO_COLOR(0xff, 0xff, 0xff)};
