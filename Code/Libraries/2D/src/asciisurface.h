...
class ASCIISurface {
 public:
  ...
  void Print(const char* str, uint8 color, int x, int y,
             int Flags = 0);  // Very basic print without wrapping
  void PrintCentered(const char* String, uint8 Color, int Y, int XBase = 0,
                     int XSpan = 0, int Flags = 0);
  void Print(const char* str, uint MaxChars, uint8 color, int x, int y,
             int Flags = 0);

  void DrawLine(int StartX, int StartY, int EndX, int EndY, uint8 ch,
                uint8 color, int Flags = 0);

  CHAR_INFO* GetSurfData() const { return m_Surface; }
  COORD GetSurfSize() const { return m_SurfaceSize; }
  uint GetSurfNumBytes() const {
    return sizeof(CHAR_INFO) * m_SurfaceSize.X * m_SurfaceSize.Y;
  }

  static void ABlit(const ASCIISurface& src, ASCIISurface& dst, uint16 srcX = 0,
                    uint16 srcY = 0, uint16 srcW = 0, uint16 srcH = 0,
                    int16 dstX = 0, int16 dstY = 0, int Flags = 0,
                    bool masked = false, uint8 maskChar = ' ');
