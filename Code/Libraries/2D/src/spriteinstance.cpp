...
void SpriteInstance::PlayAnimation(const HashedString& AnimationName,
                                   bool Looping) {
  ASSERT(m_Sprite);
...
void SpriteInstance::BlitToSurface(Surface& Dest, int dx, int dy, uint Flags) {
  ...
  if (m_Sprite->IsAnimated() && m_CurrentAnimation) {
    ...
    Surface::Blit(*m_Sprite->m_pSurface, Dest, dx, dy, m_Sprite->m_Width,
                  m_Sprite->m_Height, Frame.x, Frame.y, Flags,
                  m_Sprite->m_ColorKey, m_Sprite->m_Alpha);
  } else {
    Surface::Blit(*m_Sprite->m_pSurface, Dest, dx, dy, m_Sprite->m_Width,
                  m_Sprite->m_Height, 0, 0, Flags, m_Sprite->m_ColorKey,
                  m_Sprite->m_Alpha);
  }
}