...
void Sprite::TransformedMaskedBlit(Surface& Dest, int dx, int dy, float Scale,
                                   float RotationRadians) {
  Surface::TransformedMaskedBlit(*m_pSurface, Dest, dx, dy, Scale,
                                 RotationRadians, m_ColorKey);
}

void Sprite::SetColorKey(uint Color) { m_ColorKey = Color; }
...
void Sprite::InitializeAnimations(const SimpleString& SpriteName) {
  ...
  m_Height =
      ConfigManager::GetInt(sHeight, m_pSurface->GetHeight(), sSpriteName);
  ...
  for (int AnimIndex = 0; AnimIndex < NumAnimations; ++AnimIndex) {
    ...
    SimpleString AnimationName = ConfigManager::GetSequenceString(
        "Animation%d", AnimIndex, "", sSpriteName);
    ...
    for (int FrameIndex = 0; FrameIndex < NumFrames; ++FrameIndex) {
      ...
      int FrameNum = ConfigManager::GetSequenceInt("Frame%d", FrameIndex, 0,
                                                   sAnimationName);
      Frame.Duration = ConfigManager::GetSequenceFloat("Duration%d", FrameIndex,
                                                       0.0f, sAnimationName);
...
bool Sprite::IsAnimated() const { return m_Animations.Size() > 0; }