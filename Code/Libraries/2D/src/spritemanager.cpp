...
SpriteManager::SpriteManager() : m_Sprites() {}
...
void SpriteManager::DeleteInstance() { SafeDelete(m_Instance); }
...
Sprite* SpriteManager::GetSprite(const SimpleString& SpriteDef) {
  ...
  if (SpriteIter.IsNull()) {
    Surface* pSurface = SurfaceManager::GetInstance()->GetSurface(
        ...
        (Surface::ESurfaceFileType)ConfigManager::GetInt(sSurfaceFileType, 0,
                                                         sSpriteDef));
