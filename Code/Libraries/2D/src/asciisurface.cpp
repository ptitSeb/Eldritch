...
ASCIISurface::ASCIISurface(uint w /*= 80*/, uint h /*=25*/) : m_Surface(NULL) {
  Init(w, h);
}

ASCIISurface::~ASCIISurface() { SafeDeleteArray(m_Surface); }

void ASCIISurface::Init(uint w /*= 80*/, uint h /*= 25*/, uint8 ch /*= ' '*/,
                        uint8 color /*= 0*/) {
...
void ASCIISurface::SetChar(uint8 ch, uint8 color, int x, int y,
                           int Flags /*= 0*/) {
  if (!CheckBounds(x, y)) return;
...
void ASCIISurface::Print(const char* str, uint8 color, int x, int y,
                         int Flags /*= 0*/) {
  Print(str, 0, color, x, y, Flags);
}

void ASCIISurface::PrintCentered(const char* String, uint8 Color, int Y,
                                 int XBase /*= 0*/, int XSpan /*= 0*/,
                                 int Flags /*= 0*/) {
  if (XSpan <= 0) {
...
void ASCIISurface::Print(const char* str, uint MaxChars, uint8 color, int x,
                         int y, int Flags /*= 0*/) {
...
void ASCIISurface::DrawLine(int StartX, int StartY, int EndX, int EndY,
                            uint8 ch, uint8 color, int Flags /*= 0*/) {
  DrawLineChar = ch;
  DrawLineColor = color;
  DrawLineSurface = this;
  DrawLineFlags = Flags;
  Math::Bresenham(StartX, StartY, EndX, EndY, ASCIISurfaceDrawLineCallback,
                  NULL);
}
...
void ASCIISurface::ABlit(const ASCIISurface& src, ASCIISurface& dst,
                         uint16 srcX, uint16 srcY, uint16 srcW, uint16 srcH,
                         int16 dstX, int16 dstY, int Flags /*= 0*/, bool masked,
                         uint8 maskChar) {
  PROFILE_FUNCTION;
