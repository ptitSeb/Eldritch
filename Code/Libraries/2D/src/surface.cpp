...
int Surface::LoadTGA(const IDataStream& Stream) {
  ...
  ASSERTDESC(Header.m_ImageType == TGA::EIT_RLE_RBG,
             "Loading this type of TGA has not been implemented.");

  // Bunch of other checks that may not matter; if ID length and color map are
  // present, I can skip them
  ...
  int Stride = ((Width + 1) * 3) &
               0xffffffc;  // TODO: Generalize to more than 24-bit images
...
void Surface::Reset(int Width, int Height, int BitCount) {
  ...
  if (!m_hDC) {
    m_hDC = CreateCompatibleDC(
        NULL);  // Could take a window's DC, but this should work for now
  }
#endif
...
#if BUILD_WINDOWS_NO_SDL
  // NOTE: This only supports 24-bit images! 32-bit was added for SDL icon
  // support only.
  BITMAPINFO* const pBitmapInfo = reinterpret_cast<BITMAPINFO*>(&m_BitmapInfo);
  m_BMP = CreateDIBSection(m_hDC, pBitmapInfo, DIB_RGB_COLORS,
                           (void**)&m_pPixels, NULL, NULL);
  m_OldBMP = (HBITMAP)SelectObject(m_hDC, m_BMP);
#endif
#if BUILD_SDL
  m_Surface = SDL_CreateRGBSurface(0, m_Width, m_Height, BitCount, 0x00ff0000,
                                   0x0000ff00, 0x000000ff, 0xff000000);
  ASSERT(m_Surface);
  ASSERT(!SDL_MUSTLOCK(m_Surface));  // Surfaces are always in RAM now, so
                                     // there's no reason they should need to be
                                     // locked.
...
#if BUILD_WINDOWS_NO_SDL
  CHECK(
      BitBlt(pWindow->GetHDC(), 0, 0, m_Width, m_Height, m_hDC, 0, 0, SRCCOPY));
#endif
#if BUILD_SDL
...
/*static*/ void Surface::ScaledBlit(const Surface& Src, Surface& Dest,
                                    int Scalar) {
  PROFILE_FUNCTION;
  ...
  int SrcIncrement = Src.GetStride() * 2 -
                     Src.GetPadding();  // Roll forward (padding) to end of line
                                        // then roll back two lines (stride*2)
  int DestIncrement = Dest.GetStride() +
                      Scalar * BYTES;  // Roll back scalar width plus one line
...
int Surface::ComputePadding() { return (m_Width % 4); }
...
void Surface::Clear(unsigned int Color) {
  ...
  // Handle last row separately so that bounds aren't overrun at end (because of
  // 24-bit depth)
  for (int i = 0; i < m_Width; ++i) {
    *Pixels++ = b;
    *Pixels++ = g;
...
/*static*/ void Surface::Blit(const Surface& Src, Surface& Dest, int dx, int dy,
                              int w, int h, int sx, int sy, uint Flags,
                              uint ColorKey /*= DEFAULT_COLOR_KEY*/,
                              float Alpha /*= 0.5f*/) {
  PROFILE_FUNCTION;
...
/*static*/ bool Surface::ClipRectangles(const Surface& Src, const Surface& Dest,
...
int Surface::GetPadding() const { return m_Padding; }

int Surface::GetStride() const { return m_Stride; }

#if BUILD_WINDOWS_NO_SDL
HDC Surface::GetHDC() { return m_hDC; }
#endif
...
/*static*/ void Surface::StandardBlit(const Surface& Src, Surface& Dest, int dx,
                                      int dy, int w, int h, int sx, int sy) {
  ...
  // Would it be faster to iterate forward (Pixels += ...)? for cache
  // reasons...?
  for (int j = 0; j < h; ++j) {
...
/*static*/ void Surface::MaskBlit(const Surface& Src, Surface& Dest, int dx,
                                  int dy, int w, int h, int sx, int sy,
                                  uint ColorKey) {
  if (!ClipRectangles(Src, Dest, dx, dy, w, h, sx, sy)) {
    return;
  ...
  for (int j = 0; j < h; ++j) {
    for (int i = 0; i < w; ++i) {
      // Fixed this when last three bytes can't be converted to a uint because
      // end of block.
      // The other methods need to use this version when/if I ever actually dust
      // this class off.
      byte B = *SrcPixels++;
      byte G = *SrcPixels++;
      byte R = *SrcPixels++;
...
/*static*/ void Surface::MaskBlendBlit(const Surface& Src, Surface& Dest,
                                       int dx, int dy, int w, int h, int sx,
                                       int sy, uint ColorKey) {
  if (!ClipRectangles(Src, Dest, dx, dy, w, h, sx, sy)) {
    return;
  }
...
/*static*/ void Surface::MaskAlphaBlit(const Surface& Src, Surface& Dest,
                                       int dx, int dy, int w, int h, int sx,
                                       int sy, uint ColorKey, float Alpha) {
  if (Alpha == 0.0f) {
    return;
  }
  ...
  for (int j = 0; j < h; ++j) {
    for (int i = 0; i < w; ++i) {
      ...
      if ((*(unsigned int*)SrcPixels & 0x00ffffff) != ColorKey) {
        *DestPixels++ =
            (byte)(*DestPixels * OneMinusAlpha + *SrcPixels++ * Alpha);
        *DestPixels++ =
            (byte)(*DestPixels * OneMinusAlpha + *SrcPixels++ * Alpha);
        *DestPixels++ =
            (byte)(*DestPixels * OneMinusAlpha + *SrcPixels++ * Alpha);
...
/*static*/ void Surface::BlendBlit(const Surface& Src, Surface& Dest, int dx,
                                   int dy, int w, int h, int sx, int sy) {
  if (!ClipRectangles(Src, Dest, dx, dy, w, h, sx, sy)) {
    return;
  }
...
/*static*/ void Surface::AlphaBlit(const Surface& Src, Surface& Dest, int dx,
                                   int dy, int w, int h, int sx, int sy,
                                   float Alpha) {
  if (Alpha == 0.0f) {
    return;
...
/*static*/ void Surface::AdditiveBlit(const Surface& Src, Surface& Dest, int dx,
                                      int dy, int w, int h, int sx, int sy) {
  if (!ClipRectangles(Src, Dest, dx, dy, w, h, sx, sy)) {
    return;
  }
...
/*static*/ void Surface::TransformedMaskedBlit(const Surface& Src,
                                               Surface& Dest, int dx, int dy,
                                               float Scale,
                                               float RotationRadians,
                                               uint ColorKey) {
  // Use -RotationRadians because +Y is down in screen coords
  Matrix Transform = Matrix::CreateRotationAboutZ(-RotationRadians) *
                     Matrix::CreateScale(Vector(Scale, Scale, Scale)) *
                     Matrix::CreateTranslation(Vector((float)dx, (float)dy, 0));
  ...
  Vector BottomRight =
      Vector((float)Src.GetWidth(), (float)Src.GetHeight(), 0) * Transform;

  int MinX =
      (int)(Min(Min(TopLeft.x, TopRight.x), Min(BottomLeft.x, BottomRight.x)));
  int MinY =
      (int)(Min(Min(TopLeft.y, TopRight.y), Min(BottomLeft.y, BottomRight.y)));
  int MaxX =
      (int)(Max(Max(TopLeft.x, TopRight.x), Max(BottomLeft.x, BottomRight.x)));
  int MaxY =
      (int)(Max(Max(TopLeft.y, TopRight.y), Max(BottomLeft.y, BottomRight.y)));

  // TEMP: Draw a debug rectangle around the borders
  uint ColorWhite = RGB_TO_COLOR(255, 255, 255);
  Dest.DrawLine((int)TopLeft.x, (int)TopLeft.y, (int)TopRight.x,
                (int)TopRight.y, ColorWhite);
  Dest.DrawLine((int)TopLeft.x, (int)TopLeft.y, (int)BottomLeft.x,
                (int)BottomLeft.y, ColorWhite);
  Dest.DrawLine((int)TopRight.x, (int)TopRight.y, (int)BottomRight.x,
                (int)BottomRight.y, ColorWhite);
  Dest.DrawLine((int)BottomLeft.x, (int)BottomLeft.y, (int)BottomRight.x,
                (int)BottomRight.y, ColorWhite);
  ...
  for (int y = MinY; y < MaxY; ++y) {
    for (int x = MinX; x < MaxX; ++x) {
      ...
      // TODO: Do masking, clamping, mirroring, or wrapping at edges (for now,
      // mask)
      if (Sample.x < 0.f || Sample.x >= (float)Src.GetWidth() ||
          Sample.y < 0.f || Sample.y >= (float)Src.m_Height) {
        continue;
      }
