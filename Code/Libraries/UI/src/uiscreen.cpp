...
#define LEFT_MOUSE_KEY                                        \
  (UIScreen::m_MouseButtonsSwapped ? Keyboard::EB_Mouse_Right \
                                   : Keyboard::EB_Mouse_Left)
...
void UIScreen::InitializeFromDefinition(const SimpleString& DefinitionName) {
  ...
  const uint NumPushedActions =
      ConfigManager::GetInt(sNumPushedActions, 0, sDefinitionName);
  for (uint PushedActionIndex = 0; PushedActionIndex < NumPushedActions;
       ++PushedActionIndex) {
    const SimpleString Action = ConfigManager::GetSequenceString(
        "PushedAction%d", PushedActionIndex, "", DefinitionName);
  ...
  const uint NumClosedActions =
      ConfigManager::GetInt(sNumClosedActions, 0, sDefinitionName);
  for (uint ClosedActionIndex = 0; ClosedActionIndex < NumClosedActions;
       ++ClosedActionIndex) {
    const SimpleString Action = ConfigManager::GetSequenceString(
        "ClosedAction%d", ClosedActionIndex, "", DefinitionName);
    WBAction* const pAction = WBActionFactory::Create(Action);
    if (pAction) {
  ...
  for (int WidgetIndex = 0; WidgetIndex < NumWidgets; ++WidgetIndex) {
    if (UIScreenVersion < 2) {
      // OLD SYSTEM
      const SimpleString WidgetClass = ConfigManager::GetSequenceString(
          "WidgetClass%d", WidgetIndex, "", sDefinitionName);
      const SimpleString WidgetName = ConfigManager::GetSequenceString(
          "WidgetName%d", WidgetIndex, "", sDefinitionName);
      ...
      if (WidgetClass == "Image"
        ...
        // Do nothing--this is so widgets can be "commented out" in config
        // without throwing asserts in dev builds
      } else {
        DEBUGLEVELPRINTF(PRINTLEVEL_Info,
                         "Bad widget class name \"%s\" in definition \"%s\"\n",
                         WidgetName.CStr(), DefinitionName.CStr());
        WARNDESC("Bad widget class name in configuration file");
      }
    } else {
      // NEW SYSTEM
      // Dev widgets are supported with a DevOnly bool in the config, see
      // UIFactory::CreateWidget.
      const SimpleString WidgetName = ConfigManager::GetSequenceString(
          "Widget%d", WidgetIndex, "", sDefinitionName);
...
bool UIScreen::TakesFocus() { return m_TakesFocus; }

bool UIScreen::PausesGame() { return m_PausesGame; }

void UIScreen::TickMouse() {
  int MousePosX = 0;
  int MousePosY = 0;
  m_UIManager->GetMouse()->GetPosition(MousePosX, MousePosY,
                                       m_UIManager->GetWindow());
  ...
  if (m_FocusedWidget)  // Don't do mouse UI stuff if we're unfocused or binding
                        // keys
  {
  ...
  if (!m_UIManager->GetKeyboard()->IsHigh(LEFT_MOUSE_KEY) &&
      !m_UIManager->GetMouse()->IsHigh(LEFT_MOUSE_BUTTON)) {
    // Only refocus under the cursor when the mouse moves and the button is not
    // held down
    if (MouseOffsetX || MouseOffsetY ||
      ...
      for (uint i = 0; i < m_FocusWidgets.Size(); ++i) {
        ...
        if (  // TODO: Make this test a function of SRect, and maybe make SRect
              // a full-fledged class
            MouseX >= WidgetBounds.m_Left && MouseX <= WidgetBounds.m_Right &&
            MouseY >= WidgetBounds.m_Top && MouseY <= WidgetBounds.m_Bottom) {
          SetFocus(pWidget);
          break;
        }
...
UIScreen::ETickReturn UIScreen::Tick(float DeltaTime, bool HasFocus) {
  ...
  if (HasFocus && m_UIManager->GetWindow()->HasFocus()) {
    ...
    if (m_FocusedWidget && !m_FocusedWidget->IsDisabled() &&
        m_FocusedWidget->HandleInput()) {
...
/*virtual*/ void UIScreen::Render(bool HasFocus) {
  ...
  for (uint RenderWidgetIndex = 0; RenderWidgetIndex < NumRenderWidgets;
       ++RenderWidgetIndex) {
  ...
  if (HasFocus && ConfigManager::GetBool(sDebugRenderUIBounds, false)) {
    if (m_UIManager->GetLineBatcher()) {
      ...
      WARNDESC(
          "UI manager does not have a line batcher and Cannot render debug "
          "bounds.");
    }
  }
#endif
...
void UIScreen::UpdateRender() { SortWidgets(); }
...
void UIScreen::SortWidgets() {
  ...
  for (uint i = 0; i < m_RenderWidgets.Size(); ++i) {
    ...
    int j = i - 1;
    while (j >= 0 &&
           m_RenderWidgets[j]->m_RenderPriority > pWidget->m_RenderPriority) {
...
void UIScreen::FocusNext() { ShiftFocus(1); }

void UIScreen::FocusPrevious() { ShiftFocus(-1); }

void UIScreen::FocusUp() {
  const int FocusShift = m_FocusedWidget
                             ? m_FocusedWidget->OverrideFocusUp(-m_FocusYShift)
                             : -m_FocusYShift;
...
void UIScreen::FocusDown() {
  const int FocusShift = m_FocusedWidget
                             ? m_FocusedWidget->OverrideFocusDown(m_FocusYShift)
                             : m_FocusYShift;
  ShiftFocus(FocusShift);
}

void UIScreen::FocusLeft() {
  const int FocusShift =
      m_FocusedWidget ? m_FocusedWidget->OverrideFocusLeft(-m_FocusXShift)
                      : -m_FocusXShift;
  ShiftFocus(FocusShift);
}
...
void UIScreen::FocusRight() {
  const int FocusShift =
      m_FocusedWidget ? m_FocusedWidget->OverrideFocusRight(m_FocusXShift)
                      : m_FocusXShift;
  ShiftFocus(FocusShift);
}
...
void UIScreen::ShiftFocus(int Amount) {
  ...
  if (NumFocusWidgets > 0) {
    ...
    do {
      ...
      // HACK: Go to the next widget in order instead of continuing to skip by
      // the shift amount.
      // This is specifically to deal with a special case in Eldritch (gear
      // icons on the pause
      // screen being disabled in any combination), and shouldn't break anything
      // anywhere else.
      // Generally, I don't know enough about the layout of a page to infer what
      // should happen.
      Amount = Sign(Amount);
