...
UIWidgetImage::~UIWidgetImage() { SafeDelete(m_Mesh); }
...
void UIWidgetImage::UpdateRender() {
  ...
  m_Mesh->m_Location = Vector(m_TopLeft.x + m_Dimensions.x * 0.5f, 0.0f,
                              m_TopLeft.y + m_Dimensions.y * 0.5f);
...
void UIWidgetImage::InitializeFromDefinition(
  ...
  float X =
      Pick(ConfigManager::GetArchetypeFloat(sPixelX, sm_Archetype, 0.0f,
           ...
           ParentWidth * ConfigManager::GetArchetypeFloat(
                             sScreenX, sm_Archetype, 0.0f, sDefinitionName));
  ...
  float Y =
      Pick(ConfigManager::GetArchetypeFloat(sPixelY, sm_Archetype, 0.0f,
           ...
           ParentHeight * ConfigManager::GetArchetypeFloat(
                              sScreenY, sm_Archetype, 0.0f, sDefinitionName));

  STATICHASH(PixelWidth);
  STATICHASH(ScreenWidth);
  float Width = Pick(
      ConfigManager::GetArchetypeFloat(sPixelWidth, sm_Archetype, 0.0f,
                                       sDefinitionName),
      ParentWidth * ConfigManager::GetArchetypeFloat(sScreenWidth, sm_Archetype,
                                                     0.0f, sDefinitionName));

  STATICHASH(PixelHeight);
  STATICHASH(ScreenHeight);
  float Height = Pick(
      ConfigManager::GetArchetypeFloat(sPixelHeight, sm_Archetype, 0.0f,
                                       sDefinitionName),
      ParentHeight * ConfigManager::GetArchetypeFloat(
                         sScreenHeight, sm_Archetype, 0.0f, sDefinitionName));
  ...
  const float AspectRatio = ConfigManager::GetArchetypeFloat(
      sAspectRatio, sm_Archetype, 1.0f, sDefinitionName);
  if (Width == 0.0f) {
  ...
  STATICHASH(ImageX);
  X = Pick(X, ConfigManager::GetArchetypeFloat(sImageX, sm_Archetype, 0.0f,
                                               sDefinitionName) *
                  Width);

  STATICHASH(ImageY);
  Y = Pick(Y, ConfigManager::GetArchetypeFloat(sImageY, sm_Archetype, 0.0f,
                                               sDefinitionName) *
                  Height);

  AdjustDimensionsToParent(X, Y, Width, Height, ParentX, ParentY, ParentWidth,
                           ParentHeight);
  GetPositionFromOrigin(X, Y, Width, Height);

  STATICHASH(ClampToPixelGrid);
  if (ConfigManager::GetArchetypeBool(sClampToPixelGrid, sm_Archetype, true,
                                      sDefinitionName)) {
  ...
  // If LoadImage is false, we're expecting to dynamically set the texture in
  // code somewhere
  STATICHASH(LoadImage);
  if (ConfigManager::GetArchetypeBool(sLoadImage, sm_Archetype, true,
                                      sDefinitionName)) {
    STATICHASH(Image);
    SetTexture(ConfigManager::GetArchetypeString(
        sImage, sm_Archetype, DEFAULT_TEXTURE, sDefinitionName));
  }

  m_Dimensions = Vector2(Width, Height);

  STATICHASH(Calibration);
  m_Calibration = ConfigManager::GetArchetypeBool(sCalibration, sm_Archetype,
                                                  false, sDefinitionName);

  STATICHASH(MaterialOverride);
  const SimpleString DefaultMaterial("Material_HUD");
  m_Material = ConfigManager::GetArchetypeString(
      sMaterialOverride, sm_Archetype, DefaultMaterial.CStr(), sDefinitionName);

  UpdateRender();
}

void UIWidgetImage::GetBounds(SRect& OutBounds) {
  OutBounds = SRect(m_TopLeft.x, m_TopLeft.y, m_TopLeft.x + m_Dimensions.x,
                    m_TopLeft.y + m_Dimensions.y);
}

void UIWidgetImage::SetTexture(const char* Filename) {
  ASSERT(Filename);
  m_Texture = m_UIManager->GetRenderer()->GetTextureManager()->GetTexture(
      Filename, TextureManager::ETL_Permanent);
}

// HACK: Supporting Eldritch character screen.
// Note that we call UpdateRender here since image widgets don't normally
// refresh their textures.
