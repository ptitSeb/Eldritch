...
Mesh* UIWidgetFrame::CreateMesh() const {
  ...
  const Vector BottomRight =
      TopLeft + Vector(m_Dimensions.x, 0.0f, m_Dimensions.y);
  ...
  IVertexDeclaration* const VertexDeclaration =
...
void UIWidgetFrame::InitializeFromDefinition(
  ...
  // "Screen" values are now relative to parent, which may or may not be screen
  // dimensions.
  ...
  m_Border = ConfigManager::GetArchetypeFloat(sPixelBorder, sm_Archetype, 0.0f,
                                              sDefinitionName);
  ...
  STATICHASH(Image);
  const char* const Filename = ConfigManager::GetArchetypeString(
