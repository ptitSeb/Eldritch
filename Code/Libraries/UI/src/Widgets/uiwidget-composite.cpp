...
UIWidgetComposite::UIWidgetComposite() : m_Children() {}

UIWidgetComposite::~UIWidgetComposite() { ClearChildren(); }
...
void UIWidgetComposite::InitializeFromDefinition(
  ...
  const uint NumChildren = ConfigManager::GetArchetypeInt(
      sNumChildren, sm_Archetype, 0, sDefinitionName);
  for (uint ChildIndex = 0; ChildIndex < NumChildren; ++ChildIndex) {
    const SimpleString ChildDefinitionName =
        ConfigManager::GetArchetypeSequenceString(
            "Child%d", ChildIndex, sm_Archetype, "", sDefinitionName);
    UIWidget* const pChildWidget =
        UIFactory::CreateWidget(ChildDefinitionName, GetOwnerScreen());
    ASSERT(pChildWidget);
