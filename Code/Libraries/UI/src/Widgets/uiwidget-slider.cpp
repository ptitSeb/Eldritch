...
void UIWidgetSlider::InitializeFromDefinition(
  ...
  m_ShiftAmount = ConfigManager::GetArchetypeFloat(sShiftAmount, sm_Archetype,
                                                   0.1f, sDefinitionName);
  ...
  if (UIScreenVersion < 2) {
    m_SliderLabel = new UIWidgetText(ConfigManager::GetArchetypeString(
        sSliderLabel, sm_Archetype, "", sDefinitionName));
    m_SliderBack = new UIWidgetImage(ConfigManager::GetArchetypeString(
        sSliderBack, sm_Archetype, "", sDefinitionName));
    m_Slider = new UIWidgetImage(ConfigManager::GetArchetypeString(
        sSlider, sm_Archetype, "", sDefinitionName));
  } else {
    m_SliderLabel = static_cast<UIWidgetText*>(UIFactory::CreateWidget(
        ConfigManager::GetArchetypeString(sSliderLabel, sm_Archetype, "",
                                          sDefinitionName),
        GetOwnerScreen()));
    m_SliderBack = static_cast<UIWidgetImage*>(UIFactory::CreateWidget(
        ConfigManager::GetArchetypeString(sSliderBack, sm_Archetype, "",
                                          sDefinitionName),
        GetOwnerScreen()));
    m_Slider = static_cast<UIWidgetImage*>(
        UIFactory::CreateWidget(ConfigManager::GetArchetypeString(
                                    sSlider, sm_Archetype, "", sDefinitionName),
                                GetOwnerScreen()));
...
/*virtual*/ void UIWidgetSlider::Released() { OnTrigger(); }
...
void UIWidgetSlider::SendSliderEvent() const {
  ...
  WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(), OnSliderChanged,
                    NULL);
}
...
/*virtual*/ void UIWidgetSlider::Drag(float X, float Y) {
  ...
  m_Slider->m_TopLeft.x =
      Clamp(X - (m_Slider->m_Dimensions.x / 2), m_SliderBack->m_TopLeft.x,
            m_SliderBack->m_TopLeft.x + m_SliderBack->m_Dimensions.x -
                m_Slider->m_Dimensions.x);
  UpdateRender();
...
float UIWidgetSlider::GetSliderValue() const {
  return Saturate(InvLerp(m_Slider->m_TopLeft.x, m_SliderBack->m_TopLeft.x,
                          m_SliderBack->m_TopLeft.x +
                              m_SliderBack->m_Dimensions.x -
                              m_Slider->m_Dimensions.x));
}
