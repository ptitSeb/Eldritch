...
UIManagerCommon::UIManagerCommon() : m_InitialScreens() {}
...
/*virtual*/ void UIManagerCommon::InitializeFromDefinition(
  ...
  const uint NumUIScreens =
      ConfigManager::GetInt(sNumUIScreens, 0, sDefinitionName);
  for (uint UIScreenIndex = 0; UIScreenIndex < NumUIScreens; ++UIScreenIndex) {
    const SimpleString UIScreenName = ConfigManager::GetSequenceString(
        "UIScreen%d", UIScreenIndex, "", sDefinitionName);
  ...
  const uint NumInitialScreens =
      ConfigManager::GetInt(sNumInitialScreens, 0, sDefinitionName);
  for (uint InitialScreenIndex = 0; InitialScreenIndex < NumInitialScreens;
       ++InitialScreenIndex) {
    const HashedString InitialScreen = ConfigManager::GetSequenceHash(
        "InitialScreen%d", InitialScreenIndex, HashedString::NullString,
        sDefinitionName);
  ...
  if (ConfigManager::GetBool(sPushInitialScreensOnInitialize, true,
                             sDefinitionName)) {
...
/*virtual*/ void UIManagerCommon::HandleEvent(const WBEvent& Event) {
  ...
  if (EventName == sShowYesNoDialog) {
    ...
    const Array<WBAction*>* const pYesActions =
        Event.GetPointer<Array<WBAction*> >(sYesActions);
    ...
    const Vector4 StartColor =
        Vector4(FadeColorR, FadeColorG, FadeColorB, FadeColorAStart);
    const Vector4 EndColor =
        Vector4(FadeColorR, FadeColorG, FadeColorB, FadeColorAEnd);
    ...
    const Vector4 StartColor =
        Vector4(FadeColorR, FadeColorG, FadeColorB, 1.0f);
    ...
    const Vector4 StartColor =
        Vector4(FadeColorR, FadeColorG, FadeColorB, 0.0f);
...
void UIManagerCommon::ShowOKDialog(
    bool PauseGame, const SimpleString& OKString,
    const SimpleString& OKDynamicString, const SimpleString& OKEvent,
  ...
  pOKDialog->SetParameters(PauseGame, OKString, OKDynamicString, OKEvent,
                           OKCommand, OKCallback);
  m_UIStack->Push(pOKDialog);
}

void UIManagerCommon::ShowWaitDialog(bool PauseGame,
...
void UIManagerCommon::ShowYesNoDialog(
    bool PauseGame, const SimpleString& YesNoString,
    const SimpleString& YesNoDynamicString, const SimpleString& YesEvent,
    const SimpleString& NoEvent, const SimpleString& YesCommand,
  ...
  UIScreenYesNoDialog* const pYesNoDialog =
      GetScreen<UIScreenYesNoDialog>(sYesNoDialog);
  ASSERT(pYesNoDialog);
...
void UIManagerCommon::ShowYesNoDialog(
  ...
  pYesNoDialog->SetParameters(
      PauseGame, YesNoString, YesNoDynamicString, sPop, sPop, UnusedCommand,
      UnusedCommand, UnusedCallback, UnusedCallback, pYesActions, pNoActions);

  m_UIStack->Push(pYesNoDialog);
}

void UIManagerCommon::Fade(const Vector4& StartColor, const Vector4& EndColor,
                           const float Duration, const bool GameFade) {
  static const HashedString sGameFade("GameFade");
  static const HashedString sFade("Fade");
  UIScreenFade* const pFade =
      GetScreen<UIScreenFade>(GameFade ? sGameFade : sFade);
  ASSERT(pFade);
