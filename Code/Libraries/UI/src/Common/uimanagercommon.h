...
class UIManagerCommon : public UIManager {
 public:
  ...
  void Fade(const Vector4& StartColor, const Vector4& EndColor,
            const float Duration, const bool GameFade);

  void ShowOKDialog(bool PauseGame, const SimpleString& OKString,
                    const SimpleString& OKDynamicString,
                    const SimpleString& OKEvent, const SimpleString& OKCommand,
                    const SUICallback& OKCallback = SUICallback());

  void ShowWaitDialog(bool PauseGame, const SimpleString& WaitString,
                      const SimpleString& WaitDynamicString);

  void HideWaitDialog();

  void ShowYesNoDialog(bool PauseGame, const SimpleString& YesNoString,
