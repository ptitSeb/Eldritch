...
/*virtual*/ bool UIManager::ProcessEventInternal(
    const HashedString& EventName) {
  ASSERT(m_UIStack);
...
/*virtual*/ void UIManager::HandleEvent(const WBEvent& Event) {
  ...
  if (EventName == sReinitializeUI) {
    ...
    UIScreen* const pScreen = GetScreen(Screen);
    if (pScreen) {
      UIWidgetSlider* const pWidget =
          pScreen->GetWidget<UIWidgetSlider>(Widget);
      if (pWidget) {
...
#if BUILD_DEBUG
void UIManager::DEBUGSetIsSafeToUpdateRender(bool IsSafeToUpdateRender,
                                             debug_int64_t Time /*= 0*/) {
...
bool UIManager::DEBUGIsSafeToUpdateRender(
    debug_int64_t LastRenderedTime) const {
  return m_DEBUGIsSafeToUpdateRender ||
         (LastRenderedTime < m_DEBUGLastSafeToUpdateRenderTime);
}
#endif
