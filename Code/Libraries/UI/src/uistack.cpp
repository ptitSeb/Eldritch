...
#if BUILD_DEBUG
#define BEGIN_ITERATING_STACK \
  do {                        \
    m_IteratingStack = true;  \
  } while (0)
#define END_ITERATING_STACK   \
  do {                        \
    m_IteratingStack = false; \
  } while (0)
#define CHECK_ITERATING_STACK       \
  do {                              \
    DEBUGASSERT(!m_IteratingStack); \
  } while (0)
#else
...
void UIStack::Tick(float MachineDeltaTime, float GameDeltaTime, bool Render) {
  ...
  FOR_EACH_ARRAY(ScreenIter, m_ScreenStack, SStackScreen) {
    ...
    const UIScreen::ETickReturn TickReturn = pScreen->Tick(
        pScreen->m_GameTick ? GameDeltaTime : MachineDeltaTime, HasFocus);
    ...
    if (pScreen->AllowClose() && TickReturn == UIScreen::ETR_Close) {
      ...
      // Re-get the reference because the array may have been resized and
...
void UIStack::Push(UIScreen* pScreen) {
  ...
  FOR_EACH_ARRAY(ScreenIter, m_ScreenStack, SStackScreen) {
    ...
    ASSERTDESC(pScreen != StackScreen.m_Screen,
               "Warning: Pushing a screen that is already on the UI stack! "
               "Consider using Repush().");
  }
#endif
...
void UIStack::SetFadeOverlay(UIScreen* pScreen) { m_FadeOverlay = pScreen; }

void UIStack::SetDebugOverlay(UIScreen* pScreen) { m_DebugOverlay = pScreen; }