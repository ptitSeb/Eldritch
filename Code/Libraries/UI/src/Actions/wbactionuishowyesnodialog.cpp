...
/*virtual*/ void WBActionUIShowYesNoDialog::InitializeFromDefinition(
  ...
  const uint NumYesActions =
      ConfigManager::GetInt(sNumYesActions, 0, sDefinitionName);
  for (uint YesActionIndex = 0; YesActionIndex < NumYesActions;
       ++YesActionIndex) {
    const SimpleString YesAction = ConfigManager::GetSequenceString(
        "YesAction%d", YesActionIndex, "", DefinitionName);
...
/*virtual*/ void WBActionUIShowYesNoDialog::Execute() {
  // We can't push directly to the UI stack because we don't know anything about
  // Framework3D
  // or whoever else might own a UI manager and stack. Instead, use Workbench
  // events.
  ...
  WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(), ShowYesNoDialog,
                    NULL);
}