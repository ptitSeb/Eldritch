...
UIScreenFade::UIScreenFade() : m_FadeInterpolator(), m_FadeFinishedCallback() {}

UIScreenFade::UIScreenFade(const SimpleString& DefinitionName)
    : m_FadeInterpolator(), m_FadeFinishedCallback() {
...
void UIScreenFade::Fade(const Vector4& StartColor, const Vector4& EndColor,
                        const float Duration) {
  m_FadeInterpolator.Reset(Interpolator<Vector4>::EIT_Linear, StartColor,
                           EndColor, Duration);
