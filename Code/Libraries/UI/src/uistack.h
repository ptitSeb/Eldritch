...
class UIStack {
 public:
  ...
  void Repush(UIScreen* pScreen);  // Push a screen that may already be on the
                                   // stack; if it is, the existing instance is
                                   // floated to the top
  void Remove(UIScreen* pScreen);  // Remove a screen regardless of its place in
                                   // the stack.
  void Replace(UIScreen* pOldScreen, UIScreen* pNewScreen);  // Replace a screen
                                                             // in the stack
                                                             // with another
                                                             // screen.
  void Pop();
 ...
 private:
  struct SStackScreen {
    SStackScreen() : m_Screen(NULL), m_Popped(false) {}
