...
class UIManager : public IWBEventObserver {
 public:
  ...
  UIScreen* AddScreen(const HashedString& Name,
                      UIScreen* pScreen);  // NOTE: The UIManager takes
                                           // ownership of the memory of this
                                           // UIScreen.
  UIScreen* GetScreen(const HashedString& Name);
  template <class C>
  C* GetScreen(const HashedString& Name) {
    return static_cast<C*>(GetScreen(Name));
  }
...
#if BUILD_DEBUG
  void DEBUGSetIsSafeToUpdateRender(bool IsSafeToUpdateRender,
