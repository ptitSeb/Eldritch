...
void UIFactory::RegisterUIScreenFactory(const HashedString& TypeName,
...
void UIFactory::RegisterUIWidgetFactory(const HashedString& TypeName,
...
#define ADDUIWIDGETFACTORY(type) \
  UIFactory::RegisterUIWidgetFactory(#type, UIWidget##type::Factory);
#include "uiwidgets.h"
...
UIScreen* UIFactory::CreateScreen(const SimpleString& DefinitionName) {
  ...
  HashedString UIScreenType =
      ConfigManager::GetHash(sUIScreenType, "", sDefinitionName);

  Map<HashedString, UIScreenFactoryFunc>::Iterator FactoryIter =
      sScreenFactoryFuncMap.Search(UIScreenType);
...
UIWidget* UIFactory::CreateWidget(const SimpleString& DefinitionName,
  ...
  HashedString UIWidgetType =
      ConfigManager::GetHash(sUIWidgetType, "", sDefinitionName);

  Map<HashedString, UIWidgetFactoryFunc>::Iterator FactoryIter =
      sWidgetFactoryFuncMap.Search(UIWidgetType);
  if (FactoryIter.IsNull()) {
