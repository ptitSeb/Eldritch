...
#define DEFINE_UIWIDGET_FACTORY(type) \
  static class UIWidget* Factory() { return new UIWidget##type; }
...
class UIWidget {
 public:
  ...
  void GetPositionFromOrigin(const float X, const float Y, const float Width,
                             const float Height);

  void AdjustDimensionsToParent(float& X, float& Y, float& Width, float& Height,
                                const float ParentX, const float ParentY,
  ...
  HashedString
      m_EventName;  // Event that gets fired when this widget is triggered
  SimpleString m_Command;  // Literal console command that is issued when this
                           // widget is triggered
  SUICallback
      m_Callback;  // Callback that is invoked when this widget is triggered
  SimpleString m_FocusSound;    // Sound definition that is played when this
                                // widget gets focus
  SimpleString m_TriggerSound;  // Sound definition that is played when this
                                // widget is triggered
  Array<WBAction*>
      m_Actions;  // Actions that are executed when this widget is triggered
  ...
  Vector2 m_TopLeft;
  Vector2 m_Velocity;
  bool m_AllowNegativeOrigin;  // If true, negative origin is used literally
                               // instead of being used as relative to parent's
                               // high bound. Useful for spacers.
