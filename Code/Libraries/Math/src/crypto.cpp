...
void Crypto::Encrypt(const Array<char>& Plaintext, const Array<char>& Key,
                     Array<char>& OutCiphertext) {
  ARC4Initialize(Key);
...
void Crypto::Decrypt(const Array<char>& Ciphertext, const Array<char>& Key,
                     Array<char>& OutPlaintext) {
  ARC4Initialize(Key);
...
void Crypto::Encrypt(const SimpleString& Plaintext,
                     const Array<SimpleString>& Keys,
                     Array<char>& OutCiphertext) {
...
void Crypto::Decrypt(const Array<char>& Ciphertext,
                     const Array<SimpleString>& Keys,
                     SimpleString& OutPlaintext) {
  Array<char> KeyArray;
  Array<char> PlaintextArray;
...
void Crypto::ConstructKeyFromStrings(const Array<SimpleString>& Keys,
                                     Array<char>& OutKey) {
  ASSERT(Keys.Size());
  ...
  // Push arrays one at a time instead of resizing, so they are constructed and
  // not simply allocated
