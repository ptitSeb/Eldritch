...
bool Ray::Intersects(const Triangle& t,
...
bool Ray::Intersects(const AABB& a,
                     CollisionInfo* const pInfo /*= NULL*/) const {
  float tMin = 0.0f;
  float tMax = FLT_MAX;
  ...
  for (uint i = 0; i < 3; ++i) {
    if (Abs(m_Direction.v[i]) < SMALLER_EPSILON) {
      // Ray is (close to) parallel to slab in ith dimension. No intersection if
      // origin is outside slab.
      if (m_Point.v[i] < a.m_Min.v[i] || m_Point.v[i] > a.m_Max.v[i]) {
        ...
        DEVASSERT((m_Point.v[i] < a.m_Min.v[i] &&
                   m_Point.v[i] + m_Direction.v[i] < a.m_Min.v[i]) ||
                  (m_Point.v[i] > a.m_Max.v[i] &&
                   m_Point.v[i] + m_Direction.v[i] > a.m_Max.v[i]));
        return false;
      }
    } else {
...
bool Ray::Intersects(const CollisionMesh& m,
                     CollisionInfo* const pInfo /*= NULL*/) const {
...
bool Ray::Intersects(const Ellipsoid& e,
                     CollisionInfo* const pInfo /*= NULL*/) const {
