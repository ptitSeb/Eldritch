...
Vector HSV::RGBToHSV(const Vector& ColorRGB) {
  ...
  const float Chroma =
      MaxValue - MinValue;  // Roughly, relative colorfulness of components
  ...
  if (MaxValue == ColorRGB.r) {
    ...
    ColorHSV.x =
        kGreenShift + (((ColorRGB.b - ColorRGB.r) / Chroma) * kHueScalar);
  } else {
    ColorHSV.x =
        kBlueShift + (((ColorRGB.r - ColorRGB.g) / Chroma) * kHueScalar);
  }
...
Vector HSV::HSVToRGB(const Vector& ColorHSV) {
  ...
  const float MidValue =
      MinValue + Chroma * (1.0f - Abs(Mod(Hue * 6.0f, 2.0f) - 1.0f));  // wat
