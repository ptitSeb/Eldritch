...
Vector2 Vector2::operator*(const Matrix& m) const { return m * *this; }
...
Vector2 Vector2::ProjectionOnto(const Vector2& v) const {
  return (Dot(v) / v.LengthSquared()) *
         v;  // I assume this is still valid for 2D vectors
}
...
#if BUILD_DEV
bool Vector2::IsValid() const { return FIsValid(x) && FIsValid(y); }
#endif