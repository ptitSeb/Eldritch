...
AABB::AABB() : m_Min(0.0f, 0.0f, 0.0f), m_Max(0.0f, 0.0f, 0.0f) {}

AABB::AABB(const Vector& Point) : m_Min(Point), m_Max(Point) {}
...
bool AABB::IsZero() const { return m_Min.IsZero() && m_Max.IsZero(); }

bool AABB::Intersects(const AABB& a) const {
  // Cases: this contains a, a contains this, other intersection, no
  // intersection
  ...
  return (a.m_Max.x >= m_Min.x && a.m_Max.y >= m_Min.y &&
          a.m_Max.z >= m_Min.z && a.m_Min.x <= m_Max.x &&
          a.m_Min.y <= m_Max.y && a.m_Min.z <= m_Max.z);
}

bool AABB::Intersects(const Vector& v) const {
  return (v.x >= m_Min.x && v.y >= m_Min.y && v.z >= m_Min.z &&
          v.x <= m_Max.x && v.y <= m_Max.y && v.z <= m_Max.z);
}

bool AABB::Intersects(const Frustum& f) const { return f.Intersects(*this); }
...
bool AABB::Intersects(const Plane& p) const { return p.Intersects(*this); }
...
AABB AABB::GetTransformedBound(const Matrix& m) const {
  ...
  RetVal.m_Min.x = Min(Min(Min(V0.x, V1.x), Min(V2.x, V3.x)),
                       Min(Min(V4.x, V5.x), Min(V6.x, V7.x)));
  RetVal.m_Min.y = Min(Min(Min(V0.y, V1.y), Min(V2.y, V3.y)),
                       Min(Min(V4.y, V5.y), Min(V6.y, V7.y)));
  RetVal.m_Min.z = Min(Min(Min(V0.z, V1.z), Min(V2.z, V3.z)),
                       Min(Min(V4.z, V5.z), Min(V6.z, V7.z)));
  RetVal.m_Max.x = Max(Max(Max(V0.x, V1.x), Max(V2.x, V3.x)),
                       Max(Max(V4.x, V5.x), Max(V6.x, V7.x)));
  RetVal.m_Max.y = Max(Max(Max(V0.y, V1.y), Max(V2.y, V3.y)),
                       Max(Max(V4.y, V5.y), Max(V6.y, V7.y)));
  RetVal.m_Max.z = Max(Max(Max(V0.z, V1.z), Max(V2.z, V3.z)),
