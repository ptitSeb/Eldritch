...
namespace Math {
void Bresenham(int x0, int y0, int x1, int y1, BresenhamPointFunc PointFunc,
               void* pContext);
void Bresenham3(int x0, int y0, int z0, int x1, int y1, int z1,
                Bresenham3PointFunc PointFunc, void* pContext);

bool RandomF(float Chance);  // Chance in [0,1], returns rand < Chance.

uint Random(uint Range);  // Returns [0, Range-1], meant for e.g. random array
                          // indexing with Range as size of array
...
template <class C>
C& ArrayRandom(Array<C>& A) {
  return A[Random(A.Size())];
}
template <class C>
const C& ArrayRandom(const Array<C>& A) {
  return A[Random(A.Size())];
}
