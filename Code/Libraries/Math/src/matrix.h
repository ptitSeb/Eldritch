...
class Matrix {
 public:
  ...
  float GetCofactorAt(const int i,
                      const int j) const;  // Get cofactor at (i, j)
  ...
  static Matrix CreateCoordinate(const Vector& X, const Vector& Y,
                                 const Vector& Z);
  ...
  static Matrix CreateReflection(const Vector& n);  // Given normal is the
                                                    // normal to the plane to
                                                    // reflect about
  ...
  static Matrix CreateViewMatrixCoords(const Vector& eye, const Vector& rt,
                                       const Vector& fd, const Vector& up);
  static Matrix CreateProjectionMatrix(float fov, float nearC, float farC,
                                       float invR, float aRatio);
  static Matrix CreateProjectionMatrix(float fov, float nearC, float farC,
                                       float aRatio);

  // Top should be a greater value than Bottom!
  static Matrix CreateOrthoProjectionMatrix(float Left, float Top, float Right,
