...
bool Box2D::Intersects(const Box2D& Box) const {
  if (m_Max.x < Box.m_Min.x || m_Min.x > Box.m_Max.x || m_Max.y < Box.m_Min.y ||
      m_Min.y > Box.m_Max.y) {
    return false;
  }
...
bool Box2D::Contains(const Vector2& Point) const {
  if (m_Max.x < Point.x || m_Min.x > Point.x || m_Max.y < Point.y ||
      m_Min.y > Point.y) {
    return false;
  }
...
bool Box2D::SweepAgainst(const Segment2D& Offset, const Array<Triangle2D>& Tris,
                         CollisionInfo2D* const pInfo /*= NULL*/) const {
...
Segment2D ExpandSegment(const Vector2& Extents, const Vector2& Start,
                        const Vector2& End) {
  const Vector2 EdgeDir = End - Start;
  Segment2D TempSegment = Segment2D(Start, End);

  if (EdgeDir.x > 0.0f) {
    TempSegment.m_Start.y -= Extents.y;
    TempSegment.m_End.y -= Extents.y;
  }
  if (EdgeDir.x < 0.0f) {
    TempSegment.m_Start.y += Extents.y;
    TempSegment.m_End.y += Extents.y;
  ...
  if (EdgeDir.y > 0.0f) {
    TempSegment.m_Start.x += Extents.x;
    TempSegment.m_End.x += Extents.x;
  }
  if (EdgeDir.y < 0.0f) {
    TempSegment.m_Start.x -= Extents.x;
    TempSegment.m_End.x -= Extents.x;
...
bool Box2D::SweepAgainst(const Segment2D& Offset, const Triangle2D& Tri,
                         CollisionInfo2D* const pInfo /*= NULL*/) const {
  CollisionInfo2D Info;
  CollisionInfo2D MinInfo;
  ...
  if (Offset.Intersects(EdgeSegmentA, &Info)) {
    if (Info.m_HitT < MinInfo.m_HitT || !MinInfo.m_Collision) {
      MinInfo = Info;
    }
  }
  if (Offset.Intersects(EdgeSegmentB, &Info)) {
    if (Info.m_HitT < MinInfo.m_HitT || !MinInfo.m_Collision) {
      MinInfo = Info;
    }
  }
  if (Offset.Intersects(EdgeSegmentC, &Info)) {
    if (Info.m_HitT < MinInfo.m_HitT || !MinInfo.m_Collision) {
      MinInfo = Info;
    }
  }
  if (Offset.Intersects(VertexBoxA, &Info)) {
    if (Info.m_HitT < MinInfo.m_HitT || !MinInfo.m_Collision) {
      MinInfo = Info;
    }
  }
  if (Offset.Intersects(VertexBoxB, &Info)) {
    if (Info.m_HitT < MinInfo.m_HitT || !MinInfo.m_Collision) {
      MinInfo = Info;
    }
  }
  if (Offset.Intersects(VertexBoxC, &Info)) {
    if (Info.m_HitT < MinInfo.m_HitT || !MinInfo.m_Collision) {
      MinInfo = Info;
    }
  }
...
Vector2 Box2D::GetExtents() const { return (m_Max - m_Min) * 0.5f; }
