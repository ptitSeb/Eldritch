...
class Plane {
 public:
  ...
  float DistanceTo(const Vector& v)
      const;  // Get the (signed) distance between plane and point
  Vector ProjectPoint(const Vector& v) const;
  Vector ProjectVector(const Vector& v) const;
  Vector Reflect(const Vector& v) const;
  bool OnFrontSide(const Vector& v)
      const;  // These are liberal tests that include points within
  bool OnBackSide(const Vector& v) const;  // an epsilon on the other side.
  bool Contains(const Vector& v) const;    // Test whether the point is in the
                                           // plane--might make more sense as a
                                           // Vector function
  ...
  float m_Distance;  // D	(negative of what I thought it was--it is the
                     // distance from plane to origin along normal)
};
