...
class Vector {
 public:
  ...
  void GetNormalized(Vector& OutNormalized, float& OutLength,
  ...
  Vector LERP(const Vector& t,
              const Vector& w) const;  // Per-component t values
  void Zero();
  ...
  static void MinMax(const Vector& V1, const Vector& V2, Vector& OutMin,
                     Vector& OutMax);
