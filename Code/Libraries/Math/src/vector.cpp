...
Vector::Vector(const Vector2& v) : x(v.x), y(v.y), z(0.0f) {}

Vector::Vector(const Vector4& v) : x(v.x), y(v.y), z(v.z) {}

Vector Vector::operator*(const Matrix& m) const { return m * *this; }
...
Vector Vector::Cross(const Vector& w) const {
  return Vector((y * w.z) - (w.y * z), (z * w.x) - (w.z * x),
                (x * w.y) - (w.x * y));
}
...
void Vector::GetNormalized(Vector& OutNormalized, float& OutLength,
                           float& OutOneOverLength) const {
  OutLength = Length();
  if (Abs(OutLength) < EPSILON) {
    ...
    OutNormalized = Vector(x * OutOneOverLength, y * OutOneOverLength,
                           z * OutOneOverLength);
  }
}
...
float Vector::LengthSquared2D() const { return (x * x) + (y * y); }
...
Vector Vector::LERP(const Vector& t, const Vector& w) const {
  return Vector(Lerp(x, w.x, t.x), Lerp(y, w.y, t.y), Lerp(z, w.z, t.z));
}
...
Vector Vector::GetAbs() const { return Vector(Abs(x), Abs(y), Abs(z)); }
...
Angles Vector::ToAngles() const {
  ...
  float Yaw = ATan2(-Normal.x, Normal.y);  // Convention says atan2(y/x), but
                                           // I'm considering Y+ forward, so a
                                           // zero yaw should be this way
...
uint Vector::ToColor() const {
  ...
  return ARGB_TO_COLOR(255, (byte)(R * 255.0f), (byte)(G * 255.0f),
                       (byte)(B * 255.0f));
}

bool Vector::Intersects(const Frustum& f) const { return f.Intersects(*this); }

bool Vector::Intersects(const AABB& a) const { return a.Intersects(*this); }

bool Vector::Intersects(const Sphere& s) const { return s.Intersects(*this); }

bool Vector::Intersects(const Line& l) const { return l.Intersects(*this); }

bool Vector::IsOnFrontSide(const Plane& p) const {
  return p.OnFrontSide(*this);
}

bool Vector::IsOnBackSide(const Plane& p) const { return p.OnBackSide(*this); }

/*static*/ void Vector::MinMax(const Vector& V1, const Vector& V2,
                               Vector& OutMin, Vector& OutMax) {
...
Vector Vector::Get2D() const { return Vector(x, y, 0.0f); }
...
bool Vector::IsValid() const {
  return FIsValid(x) && FIsValid(y) && FIsValid(z);
}
#endif