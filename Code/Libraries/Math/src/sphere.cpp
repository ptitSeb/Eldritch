...
Sphere::Sphere() : m_Center(Vector(0, 0, 0)), m_Radius(1.f) {}
...
bool Sphere::Intersects(const Frustum& f) const { return f.Intersects(*this); }

bool Sphere::Intersects(const Sphere& s) const {
  return ((s.m_Center - m_Center).LengthSquared() <=
          ((s.m_Radius + m_Radius) * (s.m_Radius + m_Radius)));
}

bool Sphere::Intersects(const Line& l) const { return l.Intersects(*this); }

bool Sphere::Intersects(const Triangle& t,
  ...
  if (v.Dot(v) <= m_Radius * m_Radius) {
    if (pInfo) {
      ...
      pInfo->m_Plane = t.GetPlane();  // NOTE: There's no plane defined for
                                      // inside the sphere, so just use the
                                      // triangle
    }
...
bool Sphere::Intersects(const CollisionMesh& m,
                        CollisionInfo* const pInfo /*= NULL*/) const {
...
bool Sphere::SweepAgainst(const AABB& a, const Vector& v) const {
  ...
  // a liberal test and may return true when the objects do not actually
  // intersect.
...
bool Sphere::SweepAgainst(const Plane& p, const Vector& v,
                          CollisionInfo* const pInfo /*= NULL*/) const {
...
bool Sphere::SweepAgainst(const Triangle& t, const Vector& v,
                          CollisionInfo* const pInfo /*= NULL*/) const {
  CollisionInfo Info;
  if (pInfo) {
  ...
  if (PlaneSweep || Intersects(TriPlane)) {
    if (PlaneSweep && t.Contains(Info.m_Intersection)) {
      ...
      // Test swept sphere against triangle edges (segment-cylinder tests)
      // (return closest)
      // NOTE: Don't test against endcaps of cylinder, because segment would hit
      // a vert-sphere first
      ...
      if (MinInfo.m_Collision) {
        if (pInfo) {
          ...
          // pInfo->m_Plane = Plane( ( m_Center - pInfo->m_Intersection
          // ).GetNormalized(), pInfo->m_Intersection );
          pInfo->m_Intersection -=
              pInfo->m_Plane.m_Normal.GetNormalized() * m_Radius;
        }
        return true;
      }

      // If nothing, test swept sphere against triangles verts (segment-sphere
      // tests) (return closest)
...
bool Sphere::Intersects(const Segment& s,
                        CollisionInfo* const pInfo /*= NULL*/) const {
