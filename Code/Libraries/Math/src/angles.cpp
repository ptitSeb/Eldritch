...
Angles::Angles(float _Pitch, float _Roll, float _Yaw)
...
Angles Angles::operator-() { return Angles(-Pitch, -Roll, -Yaw); }
...
bool Angles::operator==(const Angles& a) const {
  return ((Abs(Pitch - a.Pitch) < EPSILON) && (Abs(Roll - a.Roll) < EPSILON) &&
          (Abs(Yaw - a.Yaw) < EPSILON));
}

bool Angles::operator!=(const Angles& a) const {
  return ((Abs(Pitch - a.Pitch) >= EPSILON) ||
          (Abs(Roll - a.Roll) >= EPSILON) || (Abs(Yaw - a.Yaw) >= EPSILON));
}
...
bool Angles::IsZero() const {
  return Abs(Pitch) < EPSILON && Abs(Roll) < EPSILON && Abs(Yaw) < EPSILON;
}
...
Vector Angles::GetY() const {
  ...
  float sR = Sin(Roll);
  float cR = Cos(Roll);
  return Vector(-(cP * sY) + (sP * sR * cY), (cP * cY) + (sP * sR * sY),
                (sP * cR));
}
...
Vector Angles::GetZ() const {
  ...
  float sR = Sin(Roll);
  float cR = Cos(Roll);
  return Vector((sP * sY) + (cP * sR * cY), -(sP * cY) + (cP * sR * sY),
                (cP * cR));
}
...
Vector Angles::ToVector() const { return GetY(); }
...
Matrix Angles::ToMatrix() const {
  ...
  float cz = Cos(Yaw);
  float sz = Sin(Yaw);
  return Matrix(cy * cz, cy * sz, -sy, 0, -cx * sz + sx * sy * cz,
                cx * cz + sx * sy * sz, sx * cy, 0, sx * sz + cx * sy * cz,
                -sx * cz + cx * sy * sz, cx * cy, 0, 0, 0, 0, 1.f);
}
