...
Line::Line() : m_Point(Vector(0, 0, 0)), m_Direction(Vector(0, 0, 1)) {}
...
bool Line::Intersects(const Plane& p) const {
  return (!Equal(m_Direction.Dot(p.m_Normal),
                 0.f) ||  // If the line isn't parallel to the plane,
          m_Point == p.ProjectPoint(m_Point));  // or it is, but the point is on
                                                // the plane, it intersects
}
