...
CollisionMesh::~CollisionMesh() { SafeDeleteArray(m_Tris); }
...
bool CollisionMesh::Intersects(const Segment& s,
...
bool CollisionMesh::Intersects(const Ray& r,
                               CollisionInfo* const pInfo /*= NULL*/) const {
  CollisionInfo Info;
  CollisionInfo MinInfo;
  if (pInfo) {
...
bool CollisionMesh::Intersects(const Sphere& s,
                               CollisionInfo* const pInfo /*= NULL*/) const {
  CollisionInfo Info;
  CollisionInfo MinInfo;
  if (pInfo) {
  ...
  for (uint i = 0; i < m_NumTris; ++i) {
    if (s.Intersects(m_Tris[i].m_Triangle, &Info)) {
      if ((Info.m_Intersection - s.m_Center).LengthSquared() <
              (MinInfo.m_Intersection - s.m_Center).LengthSquared() ||
          !MinInfo.m_Collision) {
        MinInfo = Info;
      }
    }
