...
void Math::Bresenham(int x0, int y0, int x1, int y1,
                     BresenhamPointFunc PointFunc, void* pContext) {
  bool steep = abs(y1 - y0) > abs(x1 - x0);  // Is the slope greater than 1?
  if (steep) {  // If so, swap all x and y values throughout algorithm
    int t = x0;
    x0 = y0;
    y0 = t;
    t = x1;
  ...
  bool Break = false;
  for (int x = x0; ((x0 < x1) && (x <= x1) || (x0 > x1) && (x >= x1));
       x += xstep) {
    if (steep) {
...
void Math::Bresenham3(int x0, int y0, int z0, int x1, int y1, int z1,
                      Bresenham3PointFunc PointFunc, void* pContext) {
...
Vector Math::Random(const Vector& Min, const Vector& Max) {
  return Vector(Random(Min.x, Max.x), Random(Min.y, Max.y),
                Random(Min.z, Max.z));
}

Vector4 Math::Random(const Vector4& Min, const Vector4& Max) {
  return Vector4(Random(Min.x, Max.x), Random(Min.y, Max.y),
                 Random(Min.z, Max.z), Random(Min.w, Max.w));
}

Angles Math::Random(const Angles& Min, const Angles& Max) {
  return Angles(Random(Min.Pitch, Max.Pitch), Random(Min.Roll, Max.Roll),
                Random(Min.Yaw, Max.Yaw));
}

Color Math::Random(const Color& Min, const Color& Max) {
  return Color((byte)Random(Min.A, Max.A), (byte)Random(Min.R, Max.R),
               (byte)Random(Min.G, Max.G), (byte)Random(Min.B, Max.B));
}
...
void Math::SeedGenerator() { Generator.Seed(); }

void Math::SeedGenerator(uint Seed) { Generator.SetRandomSeed(Seed); }