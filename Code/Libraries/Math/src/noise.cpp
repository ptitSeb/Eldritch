...
int Hasher[] = {
    ,
};

int Hash(const int X) { return Hasher[X & 255]; }

float CubicValues[] = {
    0.301961f,  -0.482353f, -0.396078f, 0.286275f,  -0.286275f, -0.921569f,
    0.403922f,  0.513725f,  0.592157f,  -0.952941f, 0.419608f,  0.074510f,
    -0.600000f, -0.960784f, -0.129412f, -0.874510f, 0.694118f,  0.435294f,
    -0.694118f, 0.168627f,  -0.372549f, 0.113725f,  -0.803922f, -0.474510f,
    0.803922f,  -0.498039f, 0.992157f,  -0.866667f, 0.686275f,  0.121569f,
    0.231373f,  0.921569f,  0.050980f,  -0.709804f, -0.301961f, -0.239216f,
    -0.356863f, 0.066667f,  0.129412f,  -0.050980f, -0.035294f, -0.145098f,
    -0.011765f, 0.584314f,  -0.968627f, -0.027451f, 0.152941f,  -0.686275f,
    -0.349020f, -0.741176f, -0.607843f, -0.764706f, -0.490196f, 0.796078f,
    -0.811765f, -0.513725f, 0.247059f,  -0.725490f, 0.701961f,  -0.984314f,
    0.521569f,  -0.192157f, 0.858824f,  0.145098f,  0.560784f,  0.388235f,
    0.058824f,  0.929412f,  0.333333f,  0.176471f,  -0.176471f, -0.639216f,
    0.003922f,  -0.262745f, -0.466667f, 0.968627f,  -0.654902f, -0.882353f,
    0.270588f,  -0.184314f, 0.396078f,  0.458824f,  0.662745f,  0.866667f,
    -0.662745f, -0.168627f, -0.137255f, 0.913725f,  0.105882f,  -0.019608f,
    0.905882f,  0.262745f,  0.223529f,  -0.796078f, 0.741176f,  -0.066667f,
    -0.631373f, -0.537255f, -0.568627f, -0.835294f, 0.882353f,  -0.858824f,
    0.043137f,  -0.121569f, 0.647059f,  -0.788235f, 0.960784f,  -0.819608f,
    -0.309804f, -0.552941f, 0.498039f,  0.160784f,  -0.443137f, 0.717647f,
    -0.937255f, -0.529412f, 0.568627f,  0.341176f,  -0.231373f, -0.215686f,
    0.600000f,  0.952941f,  -0.929412f, -0.223529f, 0.749020f,  0.325490f,
    0.537255f,  -0.278431f, 0.278431f,  -0.058824f, -0.380392f, 0.443137f,
    0.945098f,  0.654902f,  -0.200000f, 0.035294f,  -0.913725f, -0.898039f,
    -0.160784f, -0.098039f, -0.905882f, -0.670588f, 0.349020f,  -0.450980f,
    0.364706f,  0.545098f,  0.317647f,  0.082353f,  -0.105882f, 0.937255f,
    0.184314f,  0.850980f,  0.098039f,  -0.247059f, 0.984314f,  0.725490f,
    0.678431f,  0.607843f,  -0.388235f, 0.709804f,  0.835294f,  -0.647059f,
    0.843137f,  1.000000f,  -0.207843f, -0.890196f, 0.294118f,  0.874510f,
    -0.545098f, -0.992157f, -0.678431f, 0.090196f,  0.772549f,  0.019608f,
    -0.152941f, -1.000000f, -0.270588f, -0.717647f, 0.756863f,  -0.043137f,
    0.137255f,  -0.325490f, -0.333333f, 0.615686f,  -0.945098f, 0.733333f,
    -0.294118f, -0.074510f, 0.639216f,  -0.521569f, -0.827451f, -0.584314f,
    0.788235f,  -0.505882f, -0.419608f, -0.113725f, 0.631373f,  -0.411765f,
    -0.701961f, 0.827451f,  0.490196f,  -0.254902f, -0.317647f, -0.623529f,
    -0.090196f, 0.670588f,  0.215686f,  -0.976471f, 0.356863f,  -0.615686f,
    0.819608f,  -0.560784f, 0.372549f,  0.309804f,  0.450980f,  -0.780392f,
    -0.576471f, -0.435294f, 0.427451f,  -0.592157f, 0.239216f,  -0.427451f,
    -0.364706f, -0.403922f, -0.733333f, -0.772549f, -0.850980f, -0.843137f,
    -0.756863f, 0.192157f,  0.623529f,  0.505882f,  0.890196f,  -0.003922f,
    0.529412f,  0.254902f,  -0.458824f, 0.482353f,  -0.082353f, 0.576471f,
    0.976471f,  -0.341176f, 0.200000f,  0.207843f,  0.466667f,  0.552941f,
    0.898039f,  -0.749020f, 0.764706f,  0.411765f,  0.380392f,  0.011765f,
    0.811765f,  0.474510f,  0.780392f,  0.027451f,
};
...
float Noise::CubicNoise2(float X, float Y) {
  ...
  return Lerp(Lerp(CubicValues[AA], CubicValues[AB], U),
              Lerp(CubicValues[BA], CubicValues[BB], U), V);
}
...
float Noise::CubicNoise3(float X, float Y, float Z) {
  ...
  return Lerp(Lerp(Lerp(CubicValues[AAA], CubicValues[AAB], U),
                   Lerp(CubicValues[ABA], CubicValues[ABB], U), V),
              Lerp(Lerp(CubicValues[BAA], CubicValues[BAB], U),
                   Lerp(CubicValues[BBA], CubicValues[BBB], U), V),
              W);
}
...
float Noise::CellNoise2(float X, float Y) {
  ...
  const Vector2 BB =
      GetOffsetAtCell2(UnitX + 1, UnitY + 1) + Vector2(1.0f, 1.0f);
  const Vector2 BC =
      GetOffsetAtCell2(UnitX - 1, UnitY + 1) + Vector2(-1.0f, 1.0f);
...
float Noise::CellNoise3(float X, float Y, float Z) {
  ...
  const Vector AAB =
      GetOffsetAtCell3(UnitX + 1, UnitY, UnitZ) + Vector(1.0f, 0.0f, 0.0f);
  const Vector AAC =
      GetOffsetAtCell3(UnitX - 1, UnitY, UnitZ) + Vector(-1.0f, 0.0f, 0.0f);
  const Vector ABA =
      GetOffsetAtCell3(UnitX, UnitY + 1, UnitZ) + Vector(0.0f, 1.0f, 0.0f);
  const Vector ABB =
      GetOffsetAtCell3(UnitX + 1, UnitY + 1, UnitZ) + Vector(1.0f, 1.0f, 0.0f);
  const Vector ABC =
      GetOffsetAtCell3(UnitX - 1, UnitY + 1, UnitZ) + Vector(-1.0f, 1.0f, 0.0f);
  const Vector ACA =
      GetOffsetAtCell3(UnitX, UnitY - 1, UnitZ) + Vector(0.0f, -1.0f, 0.0f);
  const Vector ACB =
      GetOffsetAtCell3(UnitX + 1, UnitY - 1, UnitZ) + Vector(1.0f, -1.0f, 0.0f);
  const Vector ACC = GetOffsetAtCell3(UnitX - 1, UnitY - 1, UnitZ) +
                     Vector(-1.0f, -1.0f, 0.0f);
  const Vector BAA =
      GetOffsetAtCell3(UnitX, UnitY, UnitZ + 1) + Vector(0.0f, 0.0f, 1.0f);
  const Vector BAB =
      GetOffsetAtCell3(UnitX + 1, UnitY, UnitZ + 1) + Vector(1.0f, 0.0f, 1.0f);
  const Vector BAC =
      GetOffsetAtCell3(UnitX - 1, UnitY, UnitZ + 1) + Vector(-1.0f, 0.0f, 1.0f);
  const Vector BBA =
      GetOffsetAtCell3(UnitX, UnitY + 1, UnitZ + 1) + Vector(0.0f, 1.0f, 1.0f);
  const Vector BBB = GetOffsetAtCell3(UnitX + 1, UnitY + 1, UnitZ + 1) +
                     Vector(1.0f, 1.0f, 1.0f);
  const Vector BBC = GetOffsetAtCell3(UnitX - 1, UnitY + 1, UnitZ + 1) +
                     Vector(-1.0f, 1.0f, 1.0f);
  const Vector BCA =
      GetOffsetAtCell3(UnitX, UnitY - 1, UnitZ + 1) + Vector(0.0f, -1.0f, 1.0f);
  const Vector BCB = GetOffsetAtCell3(UnitX + 1, UnitY - 1, UnitZ + 1) +
                     Vector(1.0f, -1.0f, 1.0f);
  const Vector BCC = GetOffsetAtCell3(UnitX - 1, UnitY - 1, UnitZ + 1) +
                     Vector(-1.0f, -1.0f, 1.0f);
  const Vector CAA =
      GetOffsetAtCell3(UnitX, UnitY, UnitZ - 1) + Vector(0.0f, 0.0f, -1.0f);
  const Vector CAB =
      GetOffsetAtCell3(UnitX + 1, UnitY, UnitZ - 1) + Vector(1.0f, 0.0f, -1.0f);
  const Vector CAC = GetOffsetAtCell3(UnitX - 1, UnitY, UnitZ - 1) +
                     Vector(-1.0f, 0.0f, -1.0f);
  const Vector CBA =
      GetOffsetAtCell3(UnitX, UnitY + 1, UnitZ - 1) + Vector(0.0f, 1.0f, -1.0f);
  const Vector CBB = GetOffsetAtCell3(UnitX + 1, UnitY + 1, UnitZ - 1) +
                     Vector(1.0f, 1.0f, -1.0f);
  const Vector CBC = GetOffsetAtCell3(UnitX - 1, UnitY + 1, UnitZ - 1) +
                     Vector(-1.0f, 1.0f, -1.0f);
  const Vector CCA = GetOffsetAtCell3(UnitX, UnitY - 1, UnitZ - 1) +
                     Vector(0.0f, -1.0f, -1.0f);
  const Vector CCB = GetOffsetAtCell3(UnitX + 1, UnitY - 1, UnitZ - 1) +
                     Vector(1.0f, -1.0f, -1.0f);
  const Vector CCC = GetOffsetAtCell3(UnitX - 1, UnitY - 1, UnitZ - 1) +
                     Vector(-1.0f, -1.0f, -1.0f);
...
float Noise::SumNoise3(float x, float y, float z, uint Octaves,
                       Noise3Func NoiseFunc) {
  float Sum = 0.0f;
  float NoiseScale = 1.0f;
...
float Noise::SumAbsNoise2(float x, float y, uint Octaves,
                          Noise2Func NoiseFunc) {
  float Sum = 0.0f;
  float NoiseScale = 1.0f;
...
float Noise::SumAbsNoise3(float x, float y, float z, uint Octaves,
                          Noise3Func NoiseFunc) {
  float Sum = 0.0f;
  float NoiseScale = 1.0f;
...
float Fade(float t) {
  // Improved Perlin noise: 6*t^5 - 15*t^4 + 10*t^3 (instead of the original
  // fixed Hermite curve 3*t^2 - 2*t^3)
  return t * t * t * (t * (t * 6.0f - 15.0f) + 10.0f);
}

// I didn't derive this one, it's based on
// http://webstaff.itn.liu.se/~stegu/aqsis/aqsis-newnoise/noise1234.cpp
...
static float Grad2(int hash, float x, float y) {
  // Convert low 3 bits of hash code into 8 Gradient directions (8 cardinal
  // directions rotated 22.5 degrees)
  int h = hash & 7;
  float u = h < 4 ? x : y;
  float v = h < 4 ? y : x;
...
// Improved Perlin noise: fixed Gradients instead of pseudorandom Gradients
// DLPNOTE: I've named this Grad3, because to do a proper 1D or 2D noise
// implementation,
// we'd need equivalent 1D/2D Gradients (similarly built not along axes and not
// along diagonals)
...
float Noise::Noise1(float x) {
  ...
  int A = Hash(X), AA = Hash(A), B = Hash(X + 1), BA = Hash(B);

  return Lerp(Grad1(Hash(AA), x), Grad1(Hash(BA), x - 1), u);
}
...
float Noise::Noise2(float x, float y) {
  ...
  int A = Hash(X) + Y, AA = Hash(A), AB = Hash(A + 1), B = Hash(X + 1) + Y,
      BA = Hash(B), BB = Hash(B + 1);

  return Lerp(Lerp(Grad2(Hash(AA), x, y), Grad2(Hash(BA), x - 1, y), u),
              Lerp(Grad2(Hash(AB), x, y - 1), Grad2(Hash(BB), x - 1, y - 1), u),
              v);
}
...
float Noise::Noise3(float x, float y, float z) {
  ...
  return Lerp(
      Lerp(Lerp(Grad3(Hash(AA), x, y, z), Grad3(Hash(BA), x - 1, y, z), u),
           Lerp(Grad3(Hash(AB), x, y - 1, z), Grad3(Hash(BB), x - 1, y - 1, z),
                u),
           v),
      Lerp(Lerp(Grad3(Hash(AA + 1), x, y, z - 1),
                Grad3(Hash(BA + 1), x - 1, y, z - 1), u),
           Lerp(Grad3(Hash(AB + 1), x, y - 1, z - 1),
                Grad3(Hash(BB + 1), x - 1, y - 1, z - 1), u),
           v),
      w);
}