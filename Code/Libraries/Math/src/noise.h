...
// Cubic noise functions return range -1..1 (easily changed to 0..1 in
// InitCubicNoise)
...
// For noise range -1..1, these approach range -2..2 as Octaves approaches
// infinity
...
// Helper function, returns the scale applied when summing noise with the given
// number of octaves.
// (E.g., 1 => 1.0f, 2 => 1.5f, 3 => 1.75f, etc.)
float GetOctaveScale(uint Octaves);

// For noise range -1..1, these approach range 0..2 as Octaves approaches
// infinity
...
float SumAbsNoise3(float x, float y, float z, uint Octaves,
