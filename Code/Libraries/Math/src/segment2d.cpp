...
Segment2D::Segment2D() : m_Start(), m_End() {}

Segment2D::Segment2D(const Vector2& InStart, const Vector2& InEnd)
    : m_Start(InStart), m_End(InEnd) {}

bool Segment2D::Intersects(const Box2D& Box,
  ...
  if (Abs(Offset.x) < SMALLER_EPSILON) {
    ...
    if (MinT == T1) {
      EnumHitNormal = (Offset.x > 0.0f) ? CollisionInfo2D::EHN_Left
                                        : CollisionInfo2D::EHN_Right;
  ...
  if (Abs(Offset.y) < SMALLER_EPSILON) {
    ...
    if (MinT == T1) {
      EnumHitNormal = (Offset.y > 0.0f) ? CollisionInfo2D::EHN_Up
                                        : CollisionInfo2D::EHN_Down;
...
bool Segment2D::Intersects(const Array<Triangle2D>& Tris,
                           CollisionInfo2D* const pInfo /*= NULL*/) const {
...
bool Segment2D::Intersects(const Triangle2D& Tri,
                           CollisionInfo2D* const pInfo /*= NULL*/) const {
  CollisionInfo2D Info;
  CollisionInfo2D MinInfo;
...
bool Segment2D::Intersects(const Segment2D& Other,
                           CollisionInfo2D* const pInfo /*= NULL*/) const {
  ...
  // if( Abs( NumerA ) < EPSILON && Abs( NumerB ) < EPSILON && Abs( Denom ) <
  // EPSILON )
  //{
  //	// TODO: Fill in pInfo
  //	return true;
