...
Segment::Segment() : m_Point1(), m_Point2() {}
...
Vector Segment::NearestPointTo(const Segment& Other,
                               float* pTValue /*= NULL*/) const {
  ...
  if (a >= SMALLER_EPSILON)  // Make sure first segment isn't degenerate
  {
    ...
    if (e < SMALLER_EPSILON) {
      ...
      // If segments are not parallel, compute closest point on this line to
      // other
      // line and clamp to this segment. Else pick arbitrary place on line for
      // now,
...
bool Segment::Intersects(const Triangle& t,
...
bool Segment::Intersects(const AABB& a) const {
  ...
  const Vector SegmentCenter =
      SegmentCenterWS -
      BoxCenter;  // Subtraction effectively translates objects to origin
  ...
  if (Abs(SegmentCenter.y * SegmentHalfLength.z -
          SegmentCenter.z * SegmentHalfLength.y) >
      BoxHalfLengths.y * AbsHalfZ + BoxHalfLengths.z * AbsHalfY) {
    return false;
  }

  if (Abs(SegmentCenter.z * SegmentHalfLength.x -
          SegmentCenter.x * SegmentHalfLength.z) >
      BoxHalfLengths.z * AbsHalfX + BoxHalfLengths.x * AbsHalfZ) {
    return false;
  }

  if (Abs(SegmentCenter.x * SegmentHalfLength.y -
          SegmentCenter.y * SegmentHalfLength.x) >
...
bool Segment::Intersects(const AABB& a, CollisionInfo* const pInfo) const {
  ...
  for (uint i = 0; i < 3; ++i) {
    if (Abs(dir.v[i]) < SMALLER_EPSILON) {
      // Segment is (close to) parallel to slab in ith dimension. No
      // intersection if origin is outside slab.
      if (m_Point1.v[i] < a.m_Min.v[i] || m_Point1.v[i] > a.m_Max.v[i]) {
        // But warn if there might be an intersection that we're ignoring (maybe
...
bool Segment::Intersects(const CollisionMesh& m,
                         CollisionInfo* const pInfo /*= NULL*/) const {
  return m.Intersects(*this, pInfo);
}

bool Segment::Intersects(const Sphere& s,
                         CollisionInfo* const pInfo /*= NULL*/) const {
  return s.Intersects(*this, pInfo);
}

bool Segment::Intersects(const Cylinder& c,
                         CollisionInfo* const pInfo /*= NULL*/) const {
  return c.Intersects(*this, pInfo);
}

// Works for either side of a plane; this wasn't always the case
bool Segment::Intersects(const Plane& p,
                         CollisionInfo* const pInfo /*=NULL*/) const {
...
bool Segment::Intersects(const Ellipsoid& e,
                         CollisionInfo* const pInfo /*= NULL*/) const {
