...
Quat::Quat(float scalar, const Vector& vector)
...
Quat Quat::operator*(float s) const { return Quat(s * w, s * x, s * y, s * z); }
...
Quat Quat::operator/(const Quat& q) const {
  ...
  return Quat((w * q.w + x * q.x + y * q.y + z * q.z) * rd,
...
bool Quat::operator==(const Quat& q) const {
  return ((Abs(w - q.w) < EPSILON) && (Abs(x - q.x) < EPSILON) &&
          (Abs(y - q.y) < EPSILON) && (Abs(z - q.z) < EPSILON));
}
...
float Quat::LengthSquared() const { return w * w + x * x + y * y + z * z; }
...
// Reciprocal (multiplicative inverse) of Q = Q* / |Q|^2 (conjugate divided by
// length-squared)
void Quat::Invert() {
...
Vector Quat::GetVectorPart() const { return Vector(x, y, z); }

float Quat::GetScalarPart() const { return w; }
...
Matrix Quat::ToMatrix() const {
  ...
  return Matrix(1.f - (yy + zz), xy + wz, xz - wy, 0.f, xy - wz,
                1.f - (xx + zz), yz + wx, 0.f, xz + wy, yz - wx,
                1.f - (xx + yy), 0.f, 0.f, 0.f, 0.f, 1.f);
}

Angles Quat::ToAngles() const {
  // This old version wasn't working, so I'm just falling back on always
  // converting through a matrix.
  // const float Pitch = ATan( ( 2.0f * ( w * x + y * z ) ) / ( 1.0f - ( 2.0f *
  // ( x * x + y * y ) ) ) );
  // const float Roll = ASin( 2.0f * ( w * y - x * z ) );
  // const float Yaw = ATan( ( 2.0f * ( w * z + x * y ) ) / ( 1.0f - ( 2.0f * (
  // y * y + z * z ) ) ) );
