...
Plane::Plane() : m_Normal(), m_Distance(0.0f) {}
...
bool Plane::Intersects(const Line& l) const { return l.Intersects(*this); }
...
Line Plane::GetIntersection(const Plane& p) const {
  ...
  return Line((c1 * m_Normal) + (c2 * p.m_Normal),
              m_Normal.Cross(p.m_Normal).GetNormalized());
}
...
bool Plane::IsCoplanarWith(const Plane& p) const {
  return (((m_Normal == p.m_Normal) &&
           (Abs(m_Distance - p.m_Distance) < EPSILON)) ||
          ((m_Normal == -p.m_Normal) &&
           (Abs(m_Distance + p.m_Distance) < EPSILON)));
}
