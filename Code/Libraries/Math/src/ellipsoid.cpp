...
Ellipsoid::Ellipsoid()
    : m_Center(Vector(0.0f, 0.0f, 0.0f)), m_Extents(Vector(0.0f, 0.0f, 0.0f)) {}
...
bool Ellipsoid::Intersects(const Ellipsoid& e) const {
  ...
  return Sphere(Sum.m_Center * InvExtents, 1.0f)
      .Intersects(m_Center * InvExtents);
}

bool Ellipsoid::Intersects(const Triangle& t,
                           CollisionInfo* const pInfo /*= NULL*/) const {
  Vector InvExtents = 1.0f / m_Extents;
  Triangle ScaledTriangle(t.m_Vec1 * InvExtents, t.m_Vec2 * InvExtents,
                          t.m_Vec3 * InvExtents);
...
bool Ellipsoid::SweepAgainst(const Ellipsoid& e, const Vector& v,
  ...
  if (Sphere(Sum.m_Center * InvExtents, 1.0f).Intersects(Velocity, pInfo)) {
    if (pInfo) {
      pInfo->m_Intersection *=
          Sum.m_Extents;  // This isn't really correct; it returns the center of
                          // this ellipsoid at time of intersection
      pInfo->m_Plane =
          Plane((pInfo->m_Plane.m_Normal * InvExtents).GetNormalized(),
                pInfo->m_Intersection);
    }
...
bool Ellipsoid::SweepAgainst(const AABB& a, const Vector& v) const {
  ...
  const Vector InvExtents = 1.0f / m_Extents;
  const AABB ESpaceAABB(a.m_Min * InvExtents, a.m_Max * InvExtents);
  return Sphere(m_Center * InvExtents, 1.0f)
      .SweepAgainst(ESpaceAABB, v * InvExtents);
}
...
bool Ellipsoid::SweepAgainst(const CollisionMesh& m, const Vector& v,
                             const AABB& SweepAABB,
                             CollisionInfo* const pInfo /*= NULL*/) const {
  CollisionInfo Info;
