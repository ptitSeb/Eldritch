...
Triangle::Triangle() : m_Vec1(), m_Vec2(), m_Vec3() {}

Triangle::Triangle(const Vector& Vec1, const Vector& Vec2, const Vector& Vec3)
    : m_Vec1(Vec1), m_Vec2(Vec2), m_Vec3(Vec3) {}
...
bool Triangle::IsInFrontOf(const Plane& P) const {
  return (P.OnFrontSide(m_Vec1) && P.OnFrontSide(m_Vec2) &&
          P.OnFrontSide(m_Vec3));
}
...
Vector Triangle::GetClosestPoint(const Vector& v,
                                 Vector* OutBarycentricCoords) const {
...
bool Triangle::Intersects(const Segment& s,
...
bool Triangle::Intersects(const Sphere& s,
                          CollisionInfo* const pInfo /*= NULL*/) const {
  return s.Intersects(*this, pInfo);
}

bool Triangle::Intersects(const Ellipsoid& e,
                          CollisionInfo* const pInfo /*= NULL*/) const {
