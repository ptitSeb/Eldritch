...
Vector4 Vector4::ProjectionOnto(const Vector4& v) const {
  return (Dot(v) / v.LengthSquared()) *
         v;  // I assume this is still valid for 4D vectors
}
...
uint Vector4::ToColor() const {
  ...
  return ARGB_TO_COLOR((byte)(A * 255.0f), (byte)(R * 255.0f),
                       (byte)(G * 255.0f), (byte)(B * 255.0f));
}
...
bool Vector4::IsValid() const {
  return FIsValid(x) && FIsValid(y) && FIsValid(z) && FIsValid(w);
}
#endif