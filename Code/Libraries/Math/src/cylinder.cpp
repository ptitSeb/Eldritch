...
Cylinder::Cylinder(const Vector& Point1, const Vector& Point2, float Radius)
    : m_Point1(Point1), m_Point2(Point2), m_Radius(Radius) {}

bool Cylinder::Intersects(const Segment& s,
                          CollisionInfo* const pInfo /*= NULL*/) const {
  Vector d = m_Point2 - m_Point1;  // Cylinder axis
  Vector m =
      s.m_Point1 - m_Point1;  // Vector from cylinder base to segment base?
  ...
  if (Abs(a) < SMALLER_EPSILON) {
    ...
    if (md < 0.0f) {
      ...
      // TODO: This seems to be problematic (or getting here is indicative of an
      // earlier problem)
    ...
    if (pInfo) {
      ...
      Vector PointOnLine = Line(m_Point1, m_Point2 - m_Point1)
                               .NearestPointTo(pInfo->m_Intersection);
  ...
  // Test endcaps--if we collide with them, count it as not colliding with
  // cylinder
  if (md + t * nd < 0.0f) {
    return false;
  ...
  if (pInfo) {
    Vector Intersection = s.m_Point1 + t * n;
    Vector PointOnLine =
        Line(m_Point1, m_Point2 - m_Point1).NearestPointTo(Intersection);
    Vector Normal = (Intersection - PointOnLine).GetNormalized();
