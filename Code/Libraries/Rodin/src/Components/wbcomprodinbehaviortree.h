...
class WBCompRodinBehaviorTree : public WBComponent {
 public:
  ...
  }  // Tick before motion. Maybe should tick after sensors and thinkers?
 ...
 private:
  struct SScheduledNode {
    SScheduledNode() : m_Node(NULL), m_ParentNode(NULL) {}
  ...
  uint m_TickIterateNodeIndex;  // This is a member so it can be modified when
                                // callbacks adjust the array order.

  bool m_Paused;  // Serialized

  // This just maps to definition name, since looked-up nodes are still
