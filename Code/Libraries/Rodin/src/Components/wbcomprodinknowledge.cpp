...
WBCompRodinKnowledge::WBCompRodinKnowledge()
    : m_KnowledgeMap(), m_ExpireTime(0.0f), m_StaleSeenTime(0.0f) {}

WBCompRodinKnowledge::~WBCompRodinKnowledge() {}

/*virtual*/ void WBCompRodinKnowledge::InitializeFromDefinition(
  ...
  m_ExpireTime =
      ConfigManager::GetInheritedFloat(sExpireTime, 0.0f, sDefinitionName);

  STATICHASH(StaleSeenTime);
  m_StaleSeenTime =
      ConfigManager::GetInheritedFloat(sStaleSeenTime, 0.0f, sDefinitionName);
}
...
WBCompRodinKnowledge::TKnowledge& WBCompRodinKnowledge::UpdateEntity(
    const WBEntityRef& Entity, const float UpdateTime /*= 0.0f*/,
    const float ExpireTimeBonus /*= 0.0f*/) {
  ...
  const bool FixTimes =
      (UpdateTime > CurrentTime) || (OldLastKnownTime > CurrentTime);
  ...
  if (FixTimes) {
    ...
    const float NewLastKnownTime =
        Max(OldLastKnownTime, (UpdateTime > 0.0f) ? UpdateTime : CurrentTime);
    const float NewTimeRemaining =
        Max(OldTimeRemaining, m_ExpireTime + ExpireTimeBonus);
    const float NewExpireTime = NewLastKnownTime + NewTimeRemaining;
...
bool WBCompRodinKnowledge::GetLastKnownLocationFor(
    const WBEntity* const pEntity, Vector& OutLocation) const {
  DEVASSERT(pEntity);
  ...
  const Vector LastKnownLocation =
      pKnowledgeEntry->GetVector(sLastKnownLocation);
  ...
  const Vector LastHeardLocation =
      pKnowledgeEntry->GetVector(sLastHeardLocation);

  if (LastSeenTime > 0.0f && LastHeardTime > 0.0f) {
    // Entity has been seen and heard; use seen location if it's more recent or
    // not too stale.
...
bool WBCompRodinKnowledge::IsCurrentlyVisible(
    const WBEntity* const pEntity) const {
  DEVASSERT(pEntity);
...
void WBCompRodinKnowledge::Load(const IDataStream& Stream) {
  ...
  if (Version >= VERSION_KNOWLEDGEMAP) {
    ...
    for (uint KnowledgeIndex = 0; KnowledgeIndex < NumKnowledge;
         ++KnowledgeIndex) {
      WBEntityRef Entity;
