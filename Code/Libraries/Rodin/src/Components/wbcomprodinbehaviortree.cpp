...
WBCompRodinBehaviorTree::~WBCompRodinBehaviorTree() { SafeDelete(m_RootNode); }

void WBCompRodinBehaviorTree::InitializeFromDefinition(
    const SimpleString& DefinitionName) {
  MAKEHASH(DefinitionName);

  STATICHASH(NumLookupNodes);
  const uint NumLookupNodes =
      ConfigManager::GetInheritedInt(sNumLookupNodes, 0, sDefinitionName);
  for (uint LookupNodeIndex = 0; LookupNodeIndex < NumLookupNodes;
       ++LookupNodeIndex) {
    const HashedString Key = ConfigManager::GetInheritedSequenceHash(
        "LookupNode%dKey", LookupNodeIndex, HashedString::NullString,
        sDefinitionName);
    const SimpleString Value = ConfigManager::GetInheritedSequenceString(
        "LookupNode%dValue", LookupNodeIndex, "", sDefinitionName);
  ...
  STATICHASH(RootNode);
  m_RootNode = RodinBTNodeFactory::Create(
      ConfigManager::GetInheritedString(sRootNode, "", sDefinitionName), this);
}

SimpleString WBCompRodinBehaviorTree::GetLookupNode(const HashedString& Key) {
  Map<HashedString, SimpleString>::Iterator LookupNodeIter =
      m_LookupNodeMap.Search(Key);
...
/*virtual*/ void WBCompRodinBehaviorTree::Tick(float DeltaTime) {
  ...
  for (m_TickIterateNodeIndex = 0;
       m_TickIterateNodeIndex < m_ScheduledNodes.Size();
       ++m_TickIterateNodeIndex) {
    ASSERT(m_TickIterateNodeIndex >= 0 &&
           m_TickIterateNodeIndex < m_ScheduledNodes.Size());
...
void WBCompRodinBehaviorTree::Wake(RodinBTNode* pNode) {
  ...
  uint NodeIndex = 0;
  if (FindScheduledNode(pNode, NodeIndex)) {
    // Make a copy, because if we expand the array, a reference into the array
...
void WBCompRodinBehaviorTree::Finish(uint NodeIndex,
                                     RodinBTNode::ETickStatus FinishStatus) {
  // Make a copy, because if we expand the array, a reference into the array
  // won't be valid anymore.
  ...
  if (ScheduledNode.m_ParentNode) {
    ScheduledNode.m_ParentNode->OnChildCompleted(ScheduledNode.m_Node,
                                                 FinishStatus);
  }
}

bool WBCompRodinBehaviorTree::FindScheduledNode(RodinBTNode* pNode,
                                                uint& OutIndex) {
