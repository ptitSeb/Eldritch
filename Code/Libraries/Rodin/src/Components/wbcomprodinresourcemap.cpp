...
WBCompRodinResourceMap::WBCompRodinResourceMap() : m_ResourceMap() {}
...
bool WBCompRodinResourceMap::ClaimResource(
    IRodinResourceUser* const pResourceUser, const HashedString& Resource,
    const bool ForceClaim) {
  ...
  if (ForceClaim) {
    ...
    // Get the resource stack again; it might have been destroyed by
    // OnResourceStolen releasing the resource.
    TResourceStack& NewResourceStack = m_ResourceMap[Resource];
    if (!RemainOnStack && NewResourceStack.Size() > 0 &&
        NewResourceStack.Last() == pCurrentClaimant) {
      // If the former user does not remain on stack but didn't already release
      // the resource, replace it
...
void WBCompRodinResourceMap::ReleaseResource(
    IRodinResourceUser* const pResourceUser, const HashedString& Resource) {
