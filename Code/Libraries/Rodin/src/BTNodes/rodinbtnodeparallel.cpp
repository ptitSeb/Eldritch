...
RodinBTNodeParallel::RodinBTNodeParallel()
    : m_ChildStatuses(), m_NumChildrenToSucceed(0), m_NumChildrenToFail(0) {}

RodinBTNodeParallel::~RodinBTNodeParallel() {}

void RodinBTNodeParallel::InitializeFromDefinition(
  ...
  m_NumChildrenToSucceed = ConfigManager::GetInt(
      sNumChildrenToSucceed, m_Children.Size(), sDefinitionName);
  m_NumChildrenToFail =
      ConfigManager::GetInt(sNumChildrenToFail, 1, sDefinitionName);
}
...
void RodinBTNodeParallel::OnChildCompleted(RodinBTNode* pChildNode,
  ...
  // Only wake if we were sleeping, because multiple things might finish
  // simultaneously and
  // the scheduler doesn't like trying to wake a woken node. When this wakes, it
  // will go back
