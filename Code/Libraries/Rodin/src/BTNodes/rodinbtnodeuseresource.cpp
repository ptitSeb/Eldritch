...
RodinBTNodeUseResource::RodinBTNodeUseResource()
    : m_Resource(), m_ForceClaim(false) {}

RodinBTNodeUseResource::~RodinBTNodeUseResource() {}

void RodinBTNodeUseResource::InitializeFromDefinition(
  ...
  STATICHASH(Resource);
  m_Resource = ConfigManager::GetHash(sResource, HashedString::NullString,
                                      sDefinitionName);
...
/*virtual*/ RodinBTNode::ETickStatus RodinBTNodeUseResource::Tick(
    float DeltaTime) {
  ...
  if (pResourceMap->ClaimResource(this, m_Resource, m_ForceClaim)) {
    // We got the resource, we're all good (or we're just continuing from being
    // woken).
    return RodinBTNodeDecorator::Tick(DeltaTime);
  } else {
...
/*virtual*/ bool RodinBTNodeUseResource::OnResourceStolen(
