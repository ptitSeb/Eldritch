...
RodinBTNodeTimeout::RodinBTNodeTimeout()
    : m_TimeoutPE(), m_NextCanExecuteTime(0.0f) {}

RodinBTNodeTimeout::~RodinBTNodeTimeout() {}

void RodinBTNodeTimeout::InitializeFromDefinition(
  ...
  m_TimeoutPE.InitializeFromDefinition(
      ConfigManager::GetString(sTimeoutPE, "", sDefinitionName));
}
...
/*virtual*/ RodinBTNode::ETickStatus RodinBTNodeTimeout::Tick(float DeltaTime) {
  ...
  if (Time < m_NextCanExecuteTime) {
    ...
    // If that ever changes, I can return failure and override the result with
    // another decorator.
    return ETS_Success;
  }

  // NOTE: In the old Couriers implementation, the timeout was only set if the
  // child ran successfully.
  // I can't find if there was a use case for that or if it was just a
  // theoretical benefit, but I'm
