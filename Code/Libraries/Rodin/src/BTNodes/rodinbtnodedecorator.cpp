...
void RodinBTNodeDecorator::InitializeFromDefinition(
    const SimpleString& DefinitionName) {
  MAKEHASH(DefinitionName);

  STATICHASH(Child);
  const SimpleString ChildDef =
      ConfigManager::GetString(sChild, "", sDefinitionName);
...
RodinBTNode::ETickStatus RodinBTNodeDecorator::Tick(float DeltaTime) {
  ...
  if (m_ChildStatus == ETS_Running) {
    WARN;  // This case shouldn't happen if we put this task to sleep, so make
           // sure!
    return ETS_Running;
  }
...
void RodinBTNodeDecorator::OnChildCompleted(RodinBTNode* pChildNode,
