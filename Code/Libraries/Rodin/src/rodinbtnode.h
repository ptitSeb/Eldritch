...
#define DEFINE_RODINBTNODE_FACTORY(type) \
  static class RodinBTNode* Factory() { return new RodinBTNode##type; }
typedef class RodinBTNode* (*RodinBTNodeFactoryFunc)(void);

class RodinBTNode {
 public:
  enum ETickStatus { ETS_None, ETS_Fail, ETS_Running, ETS_Success };
  ...
  WBCompRodinBehaviorTree* m_BehaviorTree;  // Provided for behavior
                                            // context--this is our scheduler,
                                            // and we can get back to the entity
                                            // through it
  bool m_IsSleeping;
