...
#define WB_DISPATCH_EVENT(mgr, event, rec) \
  (mgr)->DispatchEvent((event##AutoEvent), (rec))
#define WB_QUEUE_EVENT(mgr, event, rec) \
  (mgr)->QueueEvent((event##AutoEvent), (rec))
#define WB_QUEUE_EVENT_DELAY(mgr, event, rec, delay) \
  (mgr)->QueueEvent((event##AutoEvent), (rec), (delay))
...
class WBEventManager {
 public:
  ...
  void DispatchEvent(const WBEvent& Event,
                     IWBEventObserver* const Recipient) const;
  TEventUID QueueEvent(const WBEvent& Event, IWBEventObserver* const Recipient,
  ...
  void AddObserver(const HashedString& EventName,
                   IWBEventObserver* const Observer,
                   IWBEventObserver* const Recipient = NULL);
  void RemoveObserver(const HashedString& EventName,
                      IWBEventObserver* const Observer,
                      IWBEventObserver* const Recipient = NULL);
 ...
 private:
  ...
  void InternalDispatchEvent(const WBEvent& Event,
                             IWBEventObserver* const Recipient) const;
  ...
  struct SObserver {
    SObserver() : m_Observer(NULL), m_Recipient(NULL) {}
    ...
    IWBEventObserver*
        m_Recipient;  // Optional, only observe this recipient's events
  };
