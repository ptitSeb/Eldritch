...
/*virtual*/ void WBActionSendEvent::InitializeFromDefinition(
  ...
  const SimpleString DispatchDelayPEDef =
      ConfigManager::GetString(sDispatchDelayPE, "", sDefinitionName);
  m_DispatchDelayPE.InitializeFromDefinition(DispatchDelayPEDef);

  STATICHASH(EventOwner);
  const SimpleString EventOwnerDef =
      ConfigManager::GetString(sEventOwner, "", sDefinitionName);
  ...
  m_VariableMapTag = ConfigManager::GetHash(
      sVariableMapTag, HashedString::NullString, sDefinitionName);
}
...
/*virtual*/ void WBActionSendEvent::Execute() {
  ...
  WBEntity* const pEventOwnerEntity =
      pEvaluatedEventOwnerEntity ? pEvaluatedEventOwnerEntity : pContextEntity;
  ...
  if (m_QueueEvent) {
    ...
    const float DispatchDelay =
        (m_DispatchDelayPE.GetType() == WBParamEvaluator::EPT_Float)
            ? m_DispatchDelayPE.GetFloat()
            : m_DispatchDelay;
    const TEventUID EventUID =
        WBWorld::GetInstance()->GetEventManager()->QueueEvent(
            Event, pRecipientEntity, DispatchDelay);

    if (pEventOwnerEntity && m_VariableMapTag != HashedString::NullString) {
      WBCompVariableMap* const pVarMap =
          GET_WBCOMP(pEventOwnerEntity, VariableMap);
      ASSERT(pVarMap);
    ...
    WBWorld::GetInstance()->GetEventManager()->DispatchEvent(Event,
                                                             pRecipientEntity);
  }
}