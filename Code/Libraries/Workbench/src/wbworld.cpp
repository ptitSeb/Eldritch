...
#define BEGIN_ITERATING_ENTITIES \
  do {                           \
    m_IteratingEntities = true;  \
  } while (0)
#define END_ITERATING_ENTITIES   \
  do {                           \
    m_IteratingEntities = false; \
  } while (0)
#define CHECK_ITERATING_ENTITIES       \
  do {                                 \
    DEBUGASSERT(!m_IteratingEntities); \
  } while (0)

#define BEGIN_ITERATING_SCENES \
  do {                         \
    m_IteratingScenes = true;  \
  } while (0)
#define END_ITERATING_SCENES   \
  do {                         \
    m_IteratingScenes = false; \
  } while (0)
#define CHECK_ITERATING_SCENES       \
  do {                               \
    DEBUGASSERT(!m_IteratingScenes); \
  } while (0)
...
void WBWorld::Tick(const float DeltaTime, const bool DoClientTick,
                   const bool DoServerTick) {
  PROFILE_FUNCTION;
...
WBEntity* WBWorld::CreateEntity(const SimpleString& Archetype,
  ...
  // OnSpawned only fires when a thing is spawned, not when loading. (For that,
  // use OnLoaded.)
...
float WBWorld::GetTime() const { return m_Clock->GetGameCurrentTime(); }
...
void WBWorld::Load(const IDataStream& Stream) {
  ...
  // Load the event manager *after* everything else so the recipient UIDs are
  // valid.
