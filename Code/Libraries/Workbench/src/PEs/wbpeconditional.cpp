...
WBPEConditional::WBPEConditional() : m_ConditionalOp(ECO_None) {}
...
WBPEConditional::EConditionalOp WBPEConditional::GetConditionalOp(
    const HashedString& ConditionalOp) const {
  ...
  if (ConditionalOp == sEquals || ConditionalOp == sE) {
    return ECO_Equals;
  }
  if (ConditionalOp == sNotEquals || ConditionalOp == sNE) {
    return ECO_NotEquals;
  }
  if (ConditionalOp == sLessThan || ConditionalOp == sLT) {
    return ECO_LessThan;
  }
  if (ConditionalOp == sLessThanOrEqual || ConditionalOp == sLTE) {
    return ECO_LessThanOrEqual;
  }
  if (ConditionalOp == sGreaterThan || ConditionalOp == sGT) {
    return ECO_GreaterThan;
  }
  if (ConditionalOp == sGreaterThanOrEqual || ConditionalOp == sGTE) {
    return ECO_GreaterThanOrEqual;
  }
...
void WBPEConditional::InitializeFromDefinition(
  ...
  STATICHASH(Op);
  const HashedString ConditionalOp =
      ConfigManager::GetHash(sOp, HashedString::NullString, sDefinitionName);
  m_ConditionalOp = GetConditionalOp(ConditionalOp);
}

/*virtual*/ void WBPEConditional::Evaluate(
  ...
  if (m_ConditionalOp == ECO_Equals) {
    if (ValueA.m_Type == WBParamEvaluator::EPT_Int &&
        ValueB.m_Type == WBParamEvaluator::EPT_Int) {
      EvaluatedParam.m_Type = WBParamEvaluator::EPT_Bool;
      EvaluatedParam.m_Bool = ValueA.m_Int == ValueB.m_Int;
    } else if (ValueA.m_Type == WBParamEvaluator::EPT_Float ||
               ValueB.m_Type == WBParamEvaluator::EPT_Float) {
      EvaluatedParam.m_Type = WBParamEvaluator::EPT_Bool;
      EvaluatedParam.m_Bool = ValueA.GetFloat() == ValueB.GetFloat();
    } else if (ValueA.m_Type == WBParamEvaluator::EPT_String ||
               ValueA.m_Type == WBParamEvaluator::EPT_String) {
      EvaluatedParam.m_Type = WBParamEvaluator::EPT_Bool;
      EvaluatedParam.m_Bool = ValueA.GetString() == ValueB.GetString();
    } else if (ValueA.m_Type == WBParamEvaluator::EPT_Entity ||
               ValueA.m_Type == WBParamEvaluator::EPT_Entity) {
      EvaluatedParam.m_Type = WBParamEvaluator::EPT_Bool;
      EvaluatedParam.m_Bool = ValueA.GetEntity() == ValueB.GetEntity();
    } else if (ValueA.m_Type == WBParamEvaluator::EPT_Vector ||
               ValueA.m_Type == WBParamEvaluator::EPT_Vector) {
      EvaluatedParam.m_Type = WBParamEvaluator::EPT_Bool;
