...
WBPESelector::WBPESelector() : m_Selections() {}
...
/*virtual*/ void WBPESelector::InitializeFromDefinition(
    const SimpleString& DefinitionName) {
  MAKEHASH(DefinitionName);

  STATICHASH(NumSelections);
  const uint NumSelections =
      ConfigManager::GetInt(sNumSelections, 0, sDefinitionName);
  for (uint SelectionIndex = 0; SelectionIndex < NumSelections;
       ++SelectionIndex) {
    SSelection& Selection = m_Selections.PushBack();

    const SimpleString Condition = ConfigManager::GetSequenceString(
        "Selection%dCondition", SelectionIndex, "", sDefinitionName);
    Selection.m_ConditionPE = WBParamEvaluatorFactory::Create(Condition);
    ASSERT(Selection.m_ConditionPE);

    const SimpleString Value = ConfigManager::GetSequenceString(
        "Selection%dValue", SelectionIndex, "", sDefinitionName);
    Selection.m_ValuePE = WBParamEvaluatorFactory::Create(Value);
    ASSERT(Selection.m_ValuePE);
  }
}

/*virtual*/ void WBPESelector::Evaluate(
