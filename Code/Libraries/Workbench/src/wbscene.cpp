...
void WBScene::Tick() {
  ...
  for (uint HandleIndex = 0; HandleIndex < m_DeferredRemoveHandles.Size();
       ++HandleIndex) {
...
WBEntity* WBScene::GetEntity(const uint EntitySceneHandle) const {
  Map<uint, SEntityRef>::Iterator EntityIter =
      m_Entities.Search(EntitySceneHandle);
...
void WBScene::GetEntitiesByComponent(Array<WBEntity*>& OutEntities,
                                     const HashedString& ComponentName) const {
  BEGIN_ITERATING_ENTITIES;
  FOR_EACH_MAP(EntityIter, m_Entities, uint, SEntityRef) {
    const SEntityRef& EntityRef = EntityIter.GetValue();
...
void WBScene::Report() const {
  PRINTF(WBSCENE_REPORT_PREFIX
         "Scene 0x%08X (*0x%08X) has %d entities. Next entity scene handle is "
         "%d.\n",
