...
void WBActionFactory::RegisterFactory(const HashedString& TypeName,
                                      WBActionFactoryFunc Factory) {
...
void WBActionFactory::ShutDown() { sFactoryFuncMap.Clear(); }

WBAction* WBActionFactory::Create(const SimpleString& DefinitionName) {
  STATICHASH(ActionType);
  MAKEHASH(DefinitionName);
  HashedString ActionType =
      ConfigManager::GetHash(sActionType, "", sDefinitionName);

  Map<HashedString, WBActionFactoryFunc>::Iterator FactoryIter =
      sFactoryFuncMap.Search(ActionType);
...
void WBActionFactory::InitializeActionArray(const SimpleString& DefinitionName,
                                            Array<WBAction*>& OutActionArray) {
  MAKEHASH(DefinitionName);

  STATICHASH(NumActions);
  const uint NumActions =
      ConfigManager::GetInt(sNumActions, 0, sDefinitionName);
  for (uint ActionIndex = 0; ActionIndex < NumActions; ++ActionIndex) {
    const SimpleString Action = ConfigManager::GetSequenceString(
        "Action%d", ActionIndex, "", DefinitionName);
