...
WBCompStatMod::WBCompStatMod() : m_StatModMap() {}

WBCompStatMod::~WBCompStatMod() {}

void WBCompStatMod::InitializeFromDefinition(
  ...
  const int NumModifiers =
      ConfigManager::GetInheritedInt(sNumModifiers, 0, sDefinitionName);
  for (int ModifierIndex = 0; ModifierIndex < NumModifiers; ++ModifierIndex) {
    const HashedString Modifier = ConfigManager::GetInheritedSequenceHash(
        "Modifier%d", ModifierIndex, HashedString::NullString, sDefinitionName);

    SStatModifier NewStatMod;
    NewStatMod.m_Event =
        ConfigManager::GetHash(sEvent, HashedString::NullString, Modifier);
    NewStatMod.m_Stat =
        ConfigManager::GetHash(sStat, HashedString::NullString, Modifier);
    NewStatMod.m_Function = GetModifierFunctionFromString(
        ConfigManager::GetHash(sFunction, HashedString::NullString, Modifier));
...
/*static*/ WBCompStatMod::EModifierFunction
WBCompStatMod::GetModifierFunctionFromString(const HashedString& Function) {
...
float WBCompStatMod::ModifyFloat(const float Value,
                                 const HashedString& StatName) {
  PROFILE_FUNCTION;
  ...
  FOR_EACH_MULTIMAP_SEARCH(StatModIter, m_StatModMap, HashedString,
                           SStatModifier, StatName) {
    SStatModifier& StatModifier = *StatModIter;
