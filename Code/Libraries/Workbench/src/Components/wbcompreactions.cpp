...
WBCompReactions::WBCompReactions()
    : m_Rules(), m_Actions(), m_DoMultiCompare(false) {}

WBCompReactions::~WBCompReactions() {
  const uint NumReactions = m_Actions.Size();
  for (uint ReactionsIndex = 0; ReactionsIndex < NumReactions;
       ++ReactionsIndex) {
...
/*virtual*/ void WBCompReactions::InitializeFromDefinition(
  ...
  m_DoMultiCompare =
      ConfigManager::GetInheritedBool(sDoMultiCompare, false, sDefinitionName);

  STATICHASH(NumReactions);
  const uint NumReactions =
      ConfigManager::GetInheritedInt(sNumReactions, 0, sDefinitionName);

  for (uint ReactionIndex = 0; ReactionIndex < NumReactions; ++ReactionIndex) {
    const SimpleString ReactionDef = ConfigManager::GetInheritedSequenceString(
        "Reaction%d", ReactionIndex, "", sDefinitionName);
...
/*virtual*/ void WBCompReactions::HandleEvent(const WBEvent& Event) {
  ...
  if (m_DoMultiCompare) {
    ...
    if (WBPatternMatching::MultiCompare(m_Rules, Event, PEContext,
                                        MatchingIndices)) {
      const uint NumMatchingIndices = MatchingIndices.Size();
      for (uint MatchingIndexIndex = 0; MatchingIndexIndex < NumMatchingIndices;
           ++MatchingIndexIndex) {
    ...
    uint MatchingIndex;
    if (WBPatternMatching::SingleCompare(m_Rules, Event, PEContext,
                                         MatchingIndex)) {
      ExecuteActions(m_Actions[MatchingIndex], Event);
    }
  }
...
void WBCompReactions::ExecuteActions(const WBCompReactions::TActions& Actions,
                                     const WBEvent& ContextEvent) const {
  XTRACE_FUNCTION;
