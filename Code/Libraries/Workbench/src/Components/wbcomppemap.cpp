...
WBCompPEMap::WBCompPEMap() : m_PEMap() {}
...
/*virtual*/ void WBCompPEMap::InitializeFromDefinition(
  ...
  STATICHASH(NumPEs);
  const uint NumPEs =
      ConfigManager::GetInheritedInt(sNumPEs, 0, sDefinitionName);
  for (uint PEIndex = 0; PEIndex < NumPEs; ++PEIndex) {
    const HashedString PEKey = ConfigManager::GetInheritedSequenceHash(
        "PE%dKey", PEIndex, HashedString::NullString, sDefinitionName);
    const SimpleString PEName = ConfigManager::GetInheritedSequenceString(
        "PE%dDef", PEIndex, "", sDefinitionName);
