...
WBCompOwner::WBCompOwner() : m_Owner() {}

WBCompOwner::~WBCompOwner() {}

void WBCompOwner::SetOwner(WBEntity* const pNewOwner) { m_Owner = pNewOwner; }

WBEntity* WBCompOwner::GetOwner() const { return m_Owner.Get(); }
...
/*virtual*/ void WBCompOwner::HandleEvent(const WBEvent& Event) {
  ...
  // Bit of a hack; OnEquipped is sent from Eldritch code and this lives in
  // Workbench.
