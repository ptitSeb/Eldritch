...
void WBParamEvaluator::InitializeFromDefinition(
    const SimpleString& DefinitionName) {
  if (DefinitionName == "") {
    // Gracefully handle undefined PEs (used for pattern matching without a max,
    // for instance).
    return;
  }

  // Just forward this definition to the root, we don't actually need to
...
int WBParamEvaluator::SEvaluatedParam::GetInt() const {
  return m_Type == WBParamEvaluator::EPT_Int
             ? m_Int
             : m_Type == WBParamEvaluator::EPT_Float ? static_cast<int>(m_Float)
                                                     : 0;
}

float WBParamEvaluator::SEvaluatedParam::GetFloat() const {
  return m_Type == WBParamEvaluator::EPT_Float
             ? m_Float
             : m_Type == WBParamEvaluator::EPT_Int ? static_cast<float>(m_Int)
                                                   : 0.0f;
}
...
void WBParamEvaluator::SEvaluatedParam::Set(
    const WBEvent::SParameter* const pParameter) {
  if (!pParameter) {
    return;
  }
  ...
  switch (pParameter->GetType()) {
    case WBEvent::EWBEPT_Bool:
      m_Type = EPT_Bool;
      m_Bool = pParameter->GetBool();
      break;
    case WBEvent::EWBEPT_Int:
      m_Type = EPT_Int;
      m_Int = pParameter->GetInt();
      break;
    case WBEvent::EWBEPT_Float:
      m_Type = EPT_Float;
      m_Float = pParameter->GetFloat();
      break;
    case WBEvent::EWBEPT_Hash:
      m_Type = EPT_String;
      m_String = ReverseHash::ReversedHash(pParameter->GetHash());
      break;
    case WBEvent::EWBEPT_Vector:
      m_Type = EPT_Vector;
      m_Vector = pParameter->GetVector();
      break;
    case WBEvent::EWBEPT_Angles:
      WARN;
      break;
    case WBEvent::EWBEPT_Entity:
      m_Type = EPT_Entity;
      m_Entity = pParameter->GetEntity();
      break;
  }
}