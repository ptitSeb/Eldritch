...
#define DEFINE_WBCOMP_FACTORY(type) \
  static class WBComponent* Factory() { return new WBComp##type; }
typedef class WBComponent* (*WBCompFactoryFunc)(void);

#define DEFINE_WBCOMP_SUPER(parent) typedef parent Super;

#define DEFINE_WBCOMP_STATICNAME(type)         \
  static const HashedString& GetStaticName() { \
    static const HashedString sName(#type);    \
    return sName;                              \
  }
#define DEFINE_WBCOMP_NAME \
  virtual HashedString GetName() const { return GetStaticName(); }
#define DEFINE_WBCOMP_STATICREADABLENAME(type) \
  static const char* GetStaticReadableName() { return #type; }
#define DEFINE_WBCOMP_READABLENAME              \
  virtual const char* GetReadableName() const { \
    return GetStaticReadableName();             \
  }
...
#define DEFINE_WBCOMP(type, parent)                         \
  ...
  DEFINE_WBCOMP_NAME DEFINE_WBCOMP_STATICREADABLENAME(type) \
      DEFINE_WBCOMP_READABLENAME

// This sort of enforces one component per type, but shouldn't prevent manually
// adding a component with a unique name if I want to do that
#define GET_WBCOMP(entity, type) \
  (static_cast<WBComp##type*>(   \
      (entity)->GetComponent(WBComp##type::GetStaticName())))
#define SAFE_GET_WBCOMP(entity, type)                                     \
  ((entity) ? (static_cast<WBComp##type*>(                                \
                  (entity)->GetComponent(WBComp##type::GetStaticName()))) \
            : NULL)

#define CREATE_WBCOMP(type, def) \
  (static_cast<WBComp##type##*>( \
      WBComponent::Create(WBComp##type::GetStaticName(), def)))
...
class WBComponent : public IWBEventObserver {
 public:
  ...
  static void RegisterWBCompFactory(const HashedString& TypeName,
                                    WBCompFactoryFunc Factory);
  static const FactoryMap& GetWBCompFactories();

  static void InitializeBaseFactories();
  static void ShutDownBaseFactories();

  static WBComponent* Create(const HashedString& TypeName,
  ...
  // Constructor and destructor, effectively; but called when object is fully
  // constructed.
...
#define WBPROPERTY_REPORT_PREFIX "        "
 ...
 protected:
  ...
  void SetEntity(WBEntity* pEntity) {
    ASSERT(!m_Entity);
    DEVASSERT(pEntity);
    m_Entity = pEntity;
  }
