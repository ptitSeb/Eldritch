...
#if BUILD_DEBUG
#define BEGIN_ITERATING_COMPONENTS \
  do {                             \
    m_IteratingComponents = true;  \
  } while (0)
#define END_ITERATING_COMPONENTS   \
  do {                             \
    m_IteratingComponents = false; \
  } while (0)
#define CHECK_ITERATING_COMPONENTS       \
  do {                                   \
    DEBUGASSERT(!m_IteratingComponents); \
  } while (0)
#else
...
void WBEntity::InitializeFromDefinition(const SimpleString& DefinitionName) {
  ...
  m_ShouldDebugRender = ConfigManager::GetInheritedBool(sShouldDebugRender,
                                                        false, sDefinitionName);
#endif

  const WBComponent::FactoryMap WBCompFactoryMap =
      WBComponent::GetWBCompFactories();
  FOR_EACH_MAP(FactoryIter, WBCompFactoryMap, HashedString, WBCompFactoryFunc) {
    // NOTE: This GetInheritedString call will fail to compile with
    // PARANOID_HASH_CHECK. If I ever care
    // to test that, I can make the factory map value type be a struct which
    // includes the string.
...
void WBEntity::SendOnInitializedEvent() {
  // OnInitialized fires whenever the entity is constructed (both spawning and
  // loading).
  // At time of writing, it is invoked just before the OnSpawned or OnLoaded
  // event, so it
  // should work fine wherever those were previously being used. (This wasn't
  // always the
  // case, so watch for errors.) Also be aware that when loading, the object
  // will be
...
// NOTE: I'm using char* here instead of SimpleString because
// InitializeFromDefinition attempts to look up every
// type of component for every entity, and the added cost of hitting the
// allocator seemed like more than I wanted here.
// If and only if the definition is not null, then a SimpleString is made from
// the definition.
void WBEntity::AddComponent(const HashedString& ComponentType,
                            const char* ComponentDefinition) {
  PROFILE_FUNCTION;
  ...
  if (ComponentType && ComponentDefinition) {
    ...
    // string. Setting the config var to 0 works too.
    // (This is also handled in WBComponent::Create, but this saves the
    // conversion to a SimpleString)
    if (0 != strcmp(ComponentDefinition, "")) {
      ...
      WBComponent* pNewComponent =
          WBComponent::Create(ComponentType, ComponentDefinition, this);
      if (pNewComponent) {
...
void WBEntity::Render() const {
  ...
  for (uint RenderComponentIndex = 0;
       RenderComponentIndex < NumRenderComponents; ++RenderComponentIndex) {
...
void WBEntity::Destroy() {
  if (!m_Destroyed) {
    ...
    WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(), OnDestroyed,
                      this);
...
WBComponent* WBEntity::GetComponent(const HashedString& ComponentName) const {
  Map<HashedString, WBComponent*>::Iterator ComponentIter =
      m_Components.Search(ComponentName);
...
void WBEntity::Load(const IDataStream& Stream) {
  ...
  // WBHACK: If we have no components, destroy self (and skip whatever
  // components were defined, but that will happen anyway.)
  // That way, anything that was created in a modded game won't leak when
  // running unmodded.
  ...
  for (uint SerializeComponentIndex = 0;
       SerializeComponentIndex < NumSerializedComponents;
  ...
  if (!m_Destroyed) {
    ...
    WB_DISPATCH_EVENT(WBWorld::GetInstance()->GetEventManager(), OnLoaded,
                      this);
  }
}
