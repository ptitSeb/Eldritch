...
class WBEntity : public IWBEventObserver {
 public:
  WBEntity();
  ~WBEntity();  // Not virtual, because WorkbenchEntities aren't designed to be
                // inherited.
  ...
  void SetSceneHandle(const uint EntitySceneHandle) {
    m_SceneHandle = EntitySceneHandle;
  }
  ...
  template <class C>
  C* GetTransformComponent() const {
    return static_cast<C*>(GetTransformComponent());
  }
...
#define WBENTITY_REPORT_PREFIX "    "
  ...
  SimpleString GetUniqueName() const {
    return SimpleString::PrintF("%s_0x%08X", m_DefinitionName.CStr(), m_UID);
  }
 ...
 private:
  void AddComponent(const HashedString& ComponentType,
                    const char* ComponentDefinition);

  void ForwardEventToComponents(const WBEvent& Event) const;

  Map<HashedString, WBComponent*>
      m_Components;  // All components, mapped by their type name
  Multimap<int, WBComponent*>
      m_TickComponents;  // Only the tickable components in their tick order
  Array<WBComponent*> m_TickComponentsFlat;  // Flattened array version of
                                             // m_TickComponents, for better
                                             // cache performance
  Array<WBComponent*> m_RenderComponents;    // Only the renderable components
  WBComponent* m_TransformComponent;  // For optimization. Not guaranteed to
                                      // exist. Actual type depends on game.
