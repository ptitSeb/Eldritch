...
void WBParamEvaluatorFactory::RegisterFactory(const HashedString& TypeName,
                                              WBPEFactoryFunc Factory) {
...
void WBParamEvaluatorFactory::ShutDown() { sFactoryFuncMap.Clear(); }
...
WBPE* WBParamEvaluatorFactory::Create(const SimpleString& DefinitionName) {
  ...
  Map<HashedString, WBPEFactoryFunc>::Iterator FactoryIter =
      sFactoryFuncMap.Search(PEType);
