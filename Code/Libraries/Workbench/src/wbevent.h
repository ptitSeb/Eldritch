...
// NOTE: I've removed Strings from events, because managing their memory is
// tricky,
// they wouldn't transmit across a network properly without special
// consideration,
...
// I can turn this back on if needed, but I'm not really sure how useful it will
// be right now.
#define LOG_EVENTS 0
...
class WBPackedEvent {
 public:
  WBPackedEvent() : m_PackedEvent() { m_PackedEvent.SetDeflate(false); }

  WBPackedEvent(const void* const pData, const uint Size) : m_PackedEvent() {
...
class WBEvent {
 public:
  ...
  struct SParameter {
    SParameter() : m_Type(EWBEPT_None), m_Data1(0), m_Data2(0), m_Data3(0) {}
    ...
    void SetHash(const HashedString& Value) {
      m_Type = EWBEPT_Hash;
      *(HashedString*)(&m_Data1) = Value;
    }
    void SetVector(const Vector& Value) {
      m_Type = EWBEPT_Vector;
      *(Vector*)(&m_Data1) = Value;
    }
    void SetAngles(const Angles& Value) {
      m_Type = EWBEPT_Angles;
      *(Angles*)(&m_Data1) = Value;
    }
    void SetEntity(const WBEntityRef& Value) {
      m_Type = EWBEPT_Entity;
      *(WBEntityRef*)(&m_Data1) = Value;
    }
    void SetPointer(void* const Value) {
      m_Type = EWBEPT_Pointer;
      *(void**)(&m_Data1) = Value;
    }
    ...
    bool GetBool() const {
      DEVASSERT(m_Type == EWBEPT_Bool);
      return *(bool*)(&m_Data1);
    }
    int GetInt() const {
      DEVASSERT(m_Type == EWBEPT_Int);
      return *(int*)(&m_Data1);
    }
    float GetFloat() const {
      DEVASSERT(m_Type == EWBEPT_Float);
      return *(float*)(&m_Data1);
    }
    HashedString& GetHash() const {
      DEVASSERT(m_Type == EWBEPT_Hash);
      return *(HashedString*)(&m_Data1);
    }
    Vector& GetVector() const {
      DEVASSERT(m_Type == EWBEPT_Vector);
      return *(Vector*)(&m_Data1);
    }
    Angles& GetAngles() const {
      DEVASSERT(m_Type == EWBEPT_Angles);
      return *(Angles*)(&m_Data1);
    }
    WBEntityRef& GetEntity() const {
      DEVASSERT(m_Type == EWBEPT_Entity);
      return *(WBEntityRef*)(&m_Data1);
    }
    void* GetPointer() const {
      DEVASSERT(m_Type == EWBEPT_Pointer);
      return *(void**)(&m_Data1);
    }
  ...
  void SetBool(const HashedString& Name, const bool Value) {
    m_Parameters[Name].SetBool(Value);
  }
  void SetInt(const HashedString& Name, const int Value) {
    m_Parameters[Name].SetInt(Value);
  }
  void SetFloat(const HashedString& Name, const float Value) {
    m_Parameters[Name].SetFloat(Value);
  }
  void SetHash(const HashedString& Name, const HashedString& Value) {
    m_Parameters[Name].SetHash(Value);
  }
  void SetVector(const HashedString& Name, const Vector& Value) {
    m_Parameters[Name].SetVector(Value);
  }
  void SetAngles(const HashedString& Name, const Angles& Value) {
    m_Parameters[Name].SetAngles(Value);
  }
  void SetEntity(const HashedString& Name, const WBEntityRef& Value) {
    m_Parameters[Name].SetEntity(Value);
  }
  void SetPointer(const HashedString& Name, void* const Value) {
    m_Parameters[Name].SetPointer(Value);
  }
  ...
  template <class C>
  C* GetPointer(const HashedString& Name) const {
    return static_cast<C*>(GetPointer(Name));
  }

  // Even though string isn't a fully supported type, it's useful to coerce from
  // other types.
