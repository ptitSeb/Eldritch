...
#define BEGIN_ITERATING_QUEUE \
  do {                        \
    m_IteratingQueue = true;  \
  } while (0)
#define END_ITERATING_QUEUE   \
  do {                        \
    m_IteratingQueue = false; \
  } while (0)
#define CHECK_ITERATING_QUEUE       \
  do {                              \
    DEBUGASSERT(!m_IteratingQueue); \
  } while (0)

#define BEGIN_ITERATING_OBSERVERS \
  do {                            \
    m_IteratingObservers = true;  \
  } while (0)
#define END_ITERATING_OBSERVERS   \
  do {                            \
    m_IteratingObservers = false; \
  } while (0)
#define CHECK_ITERATING_OBSERVERS       \
  do {                                  \
    DEBUGASSERT(!m_IteratingObservers); \
  } while (0)
...
void WBEventManager::DispatchEvent(const WBEvent& Event,
                                   IWBEventObserver* const Recipient) const {
  InternalDispatchEvent(Event, Recipient);
}

TEventUID WBEventManager::QueueEvent(const WBEvent& Event,
                                     IWBEventObserver* const Recipient,
                                     const float DispatchDelay /*= 0.0f*/) {
...
void WBEventManager::UnqueueEvent(const TEventUID& EventUID) {
  for (uint QueuedEventIndex = 0; QueuedEventIndex < m_EventQueue.Size();
       ++QueuedEventIndex) {
  ...
  for (uint QueuedEventIndex = 0; QueuedEventIndex < m_EventQueueAdd.Size();
       ++QueuedEventIndex) {
...
void WBEventManager::AddObserver(const HashedString& EventName,
                                 IWBEventObserver* const Observer,
                                 IWBEventObserver* const Recipient /*= NULL*/) {
...
void WBEventManager::RemoveObserver(
    const HashedString& EventName, IWBEventObserver* const Observer,
    IWBEventObserver* const Recipient /*= NULL*/) {
  CHECK_ITERATING_OBSERVERS;
...
void WBEventManager::RemoveObserver(IWBEventObserver* const Observer) {
  ...
  FOR_EACH_MULTIMAP_NOINCR(ObserverIter, m_EventObservers, HashedString,
                           SObserver) {
...
void WBEventManager::Tick() {
  ...
  for (int QueuedEventIndex = m_EventQueue.Size() - 1; QueuedEventIndex >= 0;
       --QueuedEventIndex) {
...
void WBEventManager::InternalDispatchEvent(
    const WBEvent& Event, IWBEventObserver* const Recipient) const {
  XTRACE_FUNCTION;
  ...
  if (LogEvent) {
    const SimpleString EventName =
        ReverseHash::ReversedHash(Event.GetEventName());

    STATIC_HASHED_STRING(EventOwner);
    const WBEntity* const pEventOwner = Event.GetEntity(sEventOwner);
    const SimpleString EventOwnerName =
        pEventOwner ? pEventOwner->GetUniqueName() : "None";

    const WBEntityRef RecipientEntity =
        Recipient ? Recipient->GetEntityUID() : 0;
    const WBEntity* const pRecipient = RecipientEntity.Get();
    const SimpleString RecipientName =
        pRecipient ? pRecipient->GetUniqueName() : "None";

    const float CurrentTime = WBWorld::GetInstance()->GetTime();

    if (EventOwnerName == RecipientName) {
      PRINTF("WBE %.2f: %s Own: %s\n", CurrentTime, EventName.CStr(),
             EventOwnerName.CStr());
    } else {
      PRINTF("WBE %.2f: %s Own: %s Rec: %s\n", CurrentTime, EventName.CStr(),
             EventOwnerName.CStr(), RecipientName.CStr());
    }
  }
#endif
  ...
  FOR_EACH_MULTIMAP_SEARCH(ObserverIter, m_EventObservers, HashedString,
                           SObserver, Event.GetEventName()) {
    const SObserver& ObserverEntry = ObserverIter.GetValue();
    ...
    if (ObserverRecipient && ObserverRecipient != Recipient) {
      // Ignore if observer isn't listening for this recipient (and isn't a
      // global observer).
      continue;
    }
...
// Observers are not serialized. Registered observers will remain registered,
// and new observers should reregister.
...
void WBEventManager::Load(const IDataStream& Stream) {
  ...
  for (uint QueuedEventIndex = 0; QueuedEventIndex < NumQueuedEvents;
       ++QueuedEventIndex) {
    ...
    const TEventUID EventUID =
        (Version >= VERSION_EVENTUIDS) ? Stream.ReadUInt32() : 0;
...
void WBEventManager::Save(const IDataStream& Stream) const {
  ...
  for (uint QueuedEventIndex = 0; QueuedEventIndex < m_EventQueue.Size();
    ...
    Stream.WriteUInt32(
        QueuedEvent.m_Recipient ? QueuedEvent.m_Recipient->GetEntityUID() : 0);
