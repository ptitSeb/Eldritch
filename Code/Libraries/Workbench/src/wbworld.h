...
class WBWorld {
 public:
  ...
  void Tick(const float DeltaTime, const bool DoClientTick,
            const bool DoServerTick);

  // In case game ticks and renders at different rates. We always need to submit
  // a renderable even when we don't tick.
  void Render() const;
